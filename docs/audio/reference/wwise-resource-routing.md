...
Audience: Resource and audio maintainers
Summary: Defines implemented dry-output routing and the remaining Wwise bus-processing work.

## Current support

The `resource` layer exposes typed complete v150 Audio Bus, Auxiliary Bus, Fx
ShareSet, and Fx Custom bodies and retains every music node's qualified
NodeBase. The `audio` layer consumes the routing records only in its optional
...
qualified PLUG-to-impulse decoder in the resource layer, then a cancellation-safe
effect-media preparation seam in the audio layer before the shared Bus graph is
