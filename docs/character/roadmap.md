...
The [renderer adoption gates](reference/renderer-adoption-gates.md) are the
public parity worklist. A replacement must preserve or explicitly supersede
reviewed capabilities while moving inputs from filename rules, fixed
fallbacks, fixture policies, and hardcoded layer tables to retained domain
identities, typed relationships, decoded metadata, and explicit renderer
policy.
...
Continue verifying maintained `src/character/trinity` classes against current Carbon
...
When reviewed Incarna records require an identity absent from current Carbon,
add the smallest honest hydration contract under `src/character/incarna`.
Keep current Carbon identities in `src/character/trinity`, and do not recreate
unavailable lighting or rendering systems by guesswork.
