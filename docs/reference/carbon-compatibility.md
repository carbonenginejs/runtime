...
| Authored music Pause/Resume events | Portable data retained; scheduler fidelity barrier | The music graph retains EVE build 3453885's seven qualified actions in order: five element/game-object controls for dynamic, cemetery, and login music and two all-music/game-object controls. Target, mode, flags, Wwise curve, and 1/2/7/10-second transitions survive. The browser scheduler does not execute them: Wwise Resume continues the paused musical timeline, while the unrelated UI transport recreates sources at an entry cue. Execution remains blocked until layered clip offsets, the live playlist iterator, pending preparations, and active fade progress can be frozen and restored. |
