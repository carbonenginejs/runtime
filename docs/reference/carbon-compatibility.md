...
| Wwise Tremolo | Opt-in approximation | `wwiseModulation: "approximate-web-audio"` admits static, control-free Sine plus unsmoothed 50%-duty, zero-phase Square source-local EVE-v150 records and rejects other bank versions/shapes. The 38-byte layout is empirical: pinned wwiser identifies `0x00830003` and shows a corresponding modulation/phase sequence in Flanger, while the official authoring order and EVE corpus pin waveform IDs. Gain/Oscillator nodes map the unipolar authored depth to `[1-depth, 1]`; a custom `PeriodicWave` retains nonzero Sine global phase. Phase mode and spread remain portable metadata, but one all-channel browser carrier omits Wwise's per-channel Left-Right, Front-Rear, Circular, or Random distribution. Native oscillator/channel law, Center/LFE bypass, smoothed/variable-duty Square, other waveforms, and shared-Bus Tremolo remain unsupported; strict mode or missing primitives keeps the whole chain audible and dry. |
