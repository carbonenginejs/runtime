...
So `runtime Trinity layer` carries Carbon's layout directly, in
`src/engine/webgpu/core/rawData/CjsPerObjectLayouts.js`, and a packer is no longer
...
`runtime Trinity layer`'s layouts compile to.
...
is not required. Shared `cb1` through `cb4` need none, because `runtime Trinity layer`
...
duplicating an ABI `runtime Trinity layer` owns, so it moved to
...
the head of `runtime Trinity layer`'s `RawData`. A producer that owns the logical
...
`Tr2RenderBatch` shape without importing `runtime Trinity layer`. It accepts the
...
internal dispatcher shape but imports neither runtime Trinity layer nor a Trinity
graph. The duck-typed boundary is shaped for future
`runtime Trinity layer` `Tr2RenderBatch`,
...
do not load SOF, runtime Trinity layer, a Trinity graph, production textures, or
...
runtime Trinity layer nor a Trinity graph, and make no depth-policy claim.
...
textures/defaults, runtime Trinity layer, or a Trinity graph. It makes no
depth-policy claim.
...
There is no production or source dependency on `runtime core layer`,
`runtime resource layer`, or `runtime Trinity layer`; the latter two are development
