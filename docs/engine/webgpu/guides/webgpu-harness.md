...
Status: Experimental
Scope: `@carbonenginejs/runtime/engine/webgpu` browser harness
...
geometry. This tests
the engine crossing without importing `runtime Trinity layer`, loading a Trinity
...
fixtures are intentionally self-contained; runtime resource layer is not involved.
...
`test/engine/webgpu/wgsb-equivalence.test.js` compares, for the selected body, the
...
`test/engine/webgpu/fixtures/quadv5/manifest.json` pins the identity instead — per fixture and
...
geometry, per-object defaults, runtime Trinity layer, or a Trinity graph. It supplies
...
runtime Trinity layer, a Trinity graph, production textures, or authoritative
per-object defaults.
...
draws, or verify depth ordering. Neither command loads SOF, runtime Trinity layer, a
...
textures, authoritative parameter defaults, runtime Trinity layer, or a Trinity
graph.
...
uses the runtime resource layer WebGPU format reader but starts no resource manager
and loads no SOF, EVE mesh or texture, authoritative defaults, runtime core layer,
runtime Trinity layer, or Trinity graph. It attaches no depth target.
...
`CjsWebgpuPackage`. The runtime resource layer format reader participates, but no
resource manager, runtime core layer/runtime Trinity layer runtime, or Trinity graph
...
import the runtime resource layer WebGPU format reader, but do not start a resource
manager or load `runtime core layer`, `runtime Trinity layer`, or a Trinity graph.
