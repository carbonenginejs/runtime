...
@carbonenginejs/runtime/resource/formats/webgl <---- @carbonenginejs/runtime/resource/formats/dxbc
          ...
          |
          +---- resource layer ----- option selection + Tr2Shader
          +---- trinity layer ------ effect/material facade + parameters
...
- the `resource` layer owns `Tr2EffectRes`, the container bytes, option
  selection, per-permutation cache identity, canonical `Tr2Shader`, reflection
  records, and sampler setup records.
- the `trinity` layer owns the mutable `Tr2Effect`/`Tr2Material` facade,
