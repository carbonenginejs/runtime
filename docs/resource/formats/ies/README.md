...
Summary: Reads authored TILT=NONE photometric tables without creating GPU textures.
...
labels parse errors. `emit: "payload"` is the default; `emit: "lightProfile"`
returns the CPU-baked light-profile output.

The plain JSON-compatible `payload` result contains `headerText` (the text before TILT,
...
For the default `payload` output, `inspect()` validates the complete input and
returns the same metadata without
...
The optional `lightProfile` output performs the CPU profile bake; the default
`payload` output retains authored photometry. DDS handling and GPU uploads
belong to consuming resource and backend code. The format has no resource-class
or GPU dependency.
