...
- Foundations consumed: the combined runtime's event emitter, model contracts,
  neutral helpers, constants, and math values.
- Normal consumers: the `core` layer (configures and exposes a `CjsResMan`),
  the `trinity` and `sof` layers (request GPU-free objects), and engine
  layers (WebGPU and future WebGL engines) that realize prepared
  resources.
...
- Owned elsewhere: WebGL/WebGPU realization, device budgets, and device-loss
  recovery belong to engine layers. Concrete readers remain opt-in resource
  format subpaths.
