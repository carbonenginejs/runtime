...
**Current:** HLSL, DXBC, WebGL, and WebGPU are consolidated into the runtime
resource capability and exposed as independently importable
...
that do not need one. A consumer such as the `core` layer that must prove an output
