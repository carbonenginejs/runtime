...
The child-graph booster set: instanced booster geometry, the lensflare sprite set at each exhaust point, and the flickering point lights.
...
Legacy transformable volumetric-cloud child.
...
A child mesh whose distribution placements are converted into one logical current/previous transform record per instance.
...
A single animated turret living as a space-object child: it owns its target tracker, sysbone aiming, deploy/pack/fire animation state machine, firing effect and movement audio.
...
Deprecated compatibility identity retained for older serialized and Python type names.
...
A standalone spatial trigger that fires a script callback when a tracked position enters or exits its volumes.
...
Self-contained armour and hull damage state shared by a ship impact overlay and by independently rendered child meshes.
...
One object tracked by the tactical overlay, sampling a translation curve for its position and velocity and carrying the radius and flags the overlay presents it with.
