...
Cross-package character/format boundaries are maintained in the
organization's internal knowledge base; rendering experiments live in a
separate ccpwgl workspace and are not package contracts.
...
expanded internally before `CjsCharacterLibraryData` hydration. Consumers that
need the detached normalized record can call the camel-case static
`CjsCharacterLibrary.expandData(data)`; the supporting routines remain private
to the class module.
...
Detailed corpus, format, and rendering evidence belongs in the routed KBs, not
in this runtime package contract.
