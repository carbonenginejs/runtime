...
// "a builder that can only work against one service is not a builder, it is a
// client for that service". This is a client for tools-core: it resolves logical
// paths against one service and issues HTTP Range requests into its banks. So the
// consumer owns it and injects it through `CjsAudioMan.SetMediaProvider`, which
// asks only for `Read`/`ReadRange`.
//
// Source: tools-browser/src/audio/CjsAudioLibrary.js (donor, retired 2026-09-10).
// Renamed: the donor called it `CjsAudioLibrary`, which in this runtime is the
// immutable library DOCUMENT - a different thing entirely. Repointing the demo's
// import bound it to that class instead, and the failure surfaced as
// "Unsupported audio-library schema: undefined" - a validator complaining about a
// service config it had been handed.
//
// RANGE READING IS THE SUBSTANCE. EVE sound lives inside multi-hundred-megabyte
// .bnk banks, so playing one sound fetches an exact byte range rather than a bank.

// Source: tools-browser/src/audio/CjsAudioLibrary.js (donor, retired 2026-09-10)
//
// LEFT BEHIND BY THE CONSOLIDATION. tools-browser folded into `src/tools`, which
// may import only `global/utils`, so this reader had nowhere to land there and
// was simply dropped - `src/tools` has no audio at all. The audio demo needs it,
// so it lands in the audio layer instead, which may import what it uses.
//
// RENAMED FROM `CjsAudioLibrary`, which is already taken in this very folder by
// the immutable hydrated DOCUMENT. Two classes shared that name across the two
// packages, and when the demo's import was repointed it bound silently to the
// wrong one - the failure surfaced as "Unsupported audio-library schema:
// undefined", a validator complaining about a service config it was handed.
// This one READS; the other one IS the document.

import { installAudioLibraryDocument } from "../../../npm/dist/audio/library/index.js";
 ...
 */
export class CjsAudioMediaProvider
{
    #fetch;
