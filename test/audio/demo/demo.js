...
import { CjsAudioMediaProvider } from "./CjsAudioMediaProvider.js";
import { CjsSharedBusMixer } from "../../../npm/dist/audio/internal/busGraphMixer.js";
import { CjsBusGraphRuntime } from "../../../npm/dist/audio/internal/busGraphRuntime.js";
import { CjsBusDuckingController } from "../../../npm/dist/audio/internal/busDucking.js";
...
/**
 * How far this emitter can be heard, in world units.
 *
 * Authored curves are the real law wherever the library has an SFX graph; the
 * inverse-distance formula is only the backend's fallback for emitters without a
 * retained curve, and must not be drawn as though it applied to everything.
 *
 * @param {object} item A scene emitter item.
 * @param {boolean} authored Whether the library carries an authored SFX graph.
 * @returns {number} The audible radius in world units.
 */
function HearingRadius(item, authored)
{
...
class AudioLibrary
{
    ...
    static async Load()
    {
        ...
        const service = (params.get("audio-service") ?? "http://127.0.0.1:5510").replace(/\/$/u, "");
        // 3435006 is prepared but carries NO authored SFX graph, so the scene has
        // nothing to place and the demo comes up empty. Pinned to a build whose
        // prepared library has one; override with ?audioBuild=.
        const build = params.get("audioBuild") ?? "3498825";

        // The prepared library from tools-core first; the served copies remain as
        // fallbacks so another static server still works, but nothing here ships
        // a library of its own.
        const raw = await FetchLibraryJson(`${service}/eve/${build}/audio/library.json`)
            ?? await FetchLibraryJson("/audio-library.json.gz")
            ?? await FetchLibraryJson("/audio-library.json");

        if (!raw)
        {
            throw new Error(
                `No audio library available. Start tools-core (npm run service) so `
                + `${service}/eve/${build}/audio/library.json resolves, or pass `
                + `?audioBuild=<build> for a build it has prepared.`
            );
        }
...
class JukeboxLibrary
{
    ...
    static async Load()
    {
        // `?musicLibrary=` for anyone holding the optional catalog and its tracks;
        // the served path otherwise, and null when neither is there.
        const params = new URLSearchParams(globalThis.location?.search ?? "");
        const raw = await FetchLibraryJson(
            params.get("musicLibrary") ?? "/eve-online-music-library.json",
        );

        // ABSENCE IS NOT A FAILURE. The tracks are an optional download, so a
        // miss disables the jukebox panel and leaves every other part working.
        return raw ?? null;
    }
...
class MediaSource
{
    ...
    constructor(library)
    {
        ...
        this.#remote = new CjsAudioMediaProvider({
            fileIndex: {
                Resolve(logicalPath)
                {
                    const route = [
...
class SfxUi
{
    ...
    SetAudioEnabled(enabled)
    {
        ...
        {
            // `Initialize` bails before assigning this when the library carries no
            // authored SFX graph, so the panel may legitimately have no status
            // line to write. Throwing here aborted EnableAudio and took the
            // listener, the emitter ring and playback down with it.
...
class DemoApp
{
    ...
    constructor(library, jukeboxLibrary)
    {
        ...
        this.jukeboxUi = jukeboxLibrary ? new JukeboxUi(this) : null;

        if (!jukeboxLibrary)
        {
            const card = document.getElementById("jukebox");
...
/**
 * Builds the demo against its libraries.
 *
 * EXPORTED RATHER THAN RUN AT IMPORT, as the WebGPU demo is, because `node --test`
 * executes every .js file under `test/` - a module that builds a DemoApp on import
 * reaches for `document` and fails the suite. The page calls this instead.
 *
 * @returns {Promise<object>} The running demo application.
 */
export async function RunDemo()
{
    ...
    return new DemoApp(audioLibrary, jukeboxLibrary);
}
