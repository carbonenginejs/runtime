...
import {
    CjsFileIndex,
    CjsFileIndexLibrary,
} from "../../../src/tools/fileindex/index.js";
...
test("remote audio client keeps the donor name as an identity alias", () =>
{
    assert.equal(CjsAudioLibrary, CjsRemoteAudioLibrary);
    assert.throws(
        () => new CjsRemoteAudioLibrary({ fileIndex: { Resolve() {} } }),
        /CjsFileIndexLibrary/u
    );
});
...
test("remote audio client retains the browser Fetch receiver", async () =>
{
    ...
    const client = new CjsRemoteAudioLibrary({ fetch });
...
test("remote audio client returns caller-owned builder inputs", async () =>
{
    ...
    const calls = [];
    const client = new CjsRemoteAudioLibrary({
        fileIndex,
        fetch: CreateFetch({
