...
test("SFX construction retains a qualified static Wwise Peak Limiter override", async () =>
{
    const library = await CjsAudioLibraryBuilder.buildFromBanks({
        includeSfx: true,
        metadata: {
            Events: {
                refinery_l_play: {
                    eventID: 100,
                    soundbanks: [ "refinery.bnk" ],
                },
            },
            SoundBanks: {
                "refinery.bnk": {
                    name: "refinery",
                    path: "\\SoundBanks\\refinery.bnk",
                    shortId: 200,
                },
            },
            WemFileIDs: {},
        ...
        indexEntries: [ {
            logicalPath: "res:/audio/refinery.bnk",
            storagePath: "banks/refinery.bnk",
            byteLength: 256,
        } ],
        loadBank()
        {
                ...
                inspection: {
                    ...
                    hirc: [
                        ...
                        },
                        {
                            type: 17,
                            id: 754157063,
                            payload: wwisePeakLimiterEffectPayload(),
                        },
                    ],
                    media: [ {
                        id: 9001,
    ...
    assert.deepEqual(library.sfx.nodes["300"].sourceEffects, [ {
        ...
        slotIndex: 0,
        type: "peak-limiter",
        thresholdDb: Math.fround(-6.6),
        ratio: Math.fround(12.1),
...
test("SFX construction retains only complete empirical Wwise Tremolo overrides", async () =>
{
    ...
    const square = await Build([], 150, {
        waveform: 1,
        modulationDepthPercent: 50,
        modulationFrequencyHz: 0.05,
    });

    assert.deepEqual(square.sfx.nodes["300"].sourceEffects, [ {
        effectId: "2196086003",
        slotIndex: 0,
        type: "tremolo",
        modulationDepthPercent: 50,
...
function wwisePeakLimiterEffectPayload({
    thresholdDb = -6.6,
    ratio = 12.1,
