...
import { ClassifyIntent, IntentClass, PlanFrame } from "#engine/webgpu/core/framePlan";
...
test("PlanFrame folds a leading clear into the region's load operations", () =>
{
  ...
  assert.deepEqual(plan.regions[0].intents.map(intent => intent.source), [ "a", "b" ]);
...
test("PlanFrame cuts a region when a clear arrives after work", () =>
{
  ...
  assert.deepEqual(plan.regions[0].intents.map(intent => intent.source), [ "a" ]);
  assert.deepEqual(plan.regions[1].clear, { color: [ 1, 0, 0, 1 ] });
  assert.deepEqual(plan.regions[1].intents.map(intent => intent.source), [ "b" ]);
});
...
test("PlanFrame snapshots the live viewport shape for the render region", () =>
{
  ...
  assert.deepEqual(kinds(plan), [ IntentClass.RENDER ]);
  assert.deepEqual(plan.regions[0].intents.map(intent => intent.type), [ "render-texture" ]);
...
test("PlanFrame cuts a render region when the viewport changes after a draw", () =>
{
  ...
  assert.deepEqual(plan.regions.map(region => region.intents.map(intent => intent.source)), [ [ "a" ], [ "b" ] ]);
});
...
test("PlanFrame preserves order across segments and never merges over a boundary", () =>
{
  ...
  assert.deepEqual(plan.regions[2].intents.map(intent => intent.source), [ "b", "c" ]);
});
