...
test("Trinity step recorder captures immutable begin, execute, and end segments", () =>
{
  ...
  const step = new TestStep({
    ...
    execute(realTime, simTime, received)
    {
      ...
      received.DrawEffect({ id: "draw" }, { count: 3 });
      return 0;
    },
    end(received)
    {
  ...
  assert.deepEqual(segments.map((entry) => entry.intents[0].type), [ "clear", "draw-effect", "present-swap-chain" ]);
...
test("Trinity step recorder preserves nested intent order and exactly-once takes", () =>
{
  ...
  const parent = new TestStep({
    ...
    execute(_realTime, _simTime, received)
    {
      received.DrawEffect({ id: "parent-before" });
      ...
      received.DrawEffect({ id: "parent-after" });
      return 0;
    }
  });

  renderContext.DrawEffect({ id: "frame-setup" });
  ...
  assert.deepEqual(
    segments.flatMap((entry) => entry.intents.map((intent) => intent.effect.id)),
...
test("Trinity step recorder closes failed setup and enforces balanced ownership", () =>
{
  ...
  const broken = new TestStep({
    begin(received)
    {
      received.DrawEffect({ id: "before-error" });
  ...
  assert.equal(recorder.GetSegments()[0].intents[0].effect.id, "before-error");
