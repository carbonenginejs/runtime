...
import { EveUpdateContext } from "../../npm/dist/trinity/index.js";
...
test("EveCircle preserves Carbon clamps, partial arcs, and stale-point edge", () =>
{
  ...
  assertEquals(circle.Update(updateContext), true);
  assertEquals(circle.Update(updateContext), true);
});
...
{
  const files = [];
  for (const entry of await readdir("src/trinity/eve",
  {
    withFileTypes: true
  }))
  {
    ...
    {
      files.push(`src/trinity/eve/${entry.name}`);
    }
  }
