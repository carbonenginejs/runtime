...
export class Tr2GrannyStateRes extends CjsResource
{
  ...
  {
    const { format = null, ...values } = options || {};
    let document = data;
    ...
    if (!data?.stateMachine)
    {
      if (typeof format?.readGsf !== "function")
      {
        const error = new Error(
          "Tr2GrannyStateRes.DoLoad needs an already-projected GSF document, or "
          + "options.format carrying readGsf. The resource does not import gr2: "
          + "every format is a tree-shakeable subpath and importing one here "
          + "would pull it into every consumer of GState."
        );
