...
export function normalizeEmit(emit, inputType, readerName)
{
    ...
    if ([ OUTPUT_VIDEO, OUTPUT_RAW].includes(emit)) return emit;
...
function inspectWebM(bytes)
{
    ...
    const segment = roots.find((element) => element.id === 0x18538067) ||
        fallbackRoots.find((element) => element.id === 0x18538067);
    if (!segment)
    {
        return {
            sourceFormat: "webm",
...
{
    return readEbmlElements(bytes, tracksElement.dataStart, tracksElement.end)
...
{
    const track = readEbmlVint(bytes, element.dataStart, element.end, true);
    ...
    return {
        ...
        ...(frames.error ? { lacingError: frames.error } : {})
    };
}
...
function readWebmBlockFrames(bytes, payloadStart, payloadEnd, lacing)
{
    ...
    if (payloadStart > payloadEnd) return { frameCount: 0, frameSizes: [], payloadBytes: 0, error: "invalid-payload-range" };
    if (lacing === "none") return { frameCount: 1, frameSizes: [ payloadBytes ], payloadBytes };
    if (payloadStart >= payloadEnd) return { frameCount: 0, frameSizes: [], payloadBytes, error: "missing-lacing-header" };

    const frameCount = bytes[payloadStart] + 1;
    const dataStart = payloadStart + 1;
    if (frameCount < 2) return { frameCount, frameSizes: [], payloadBytes, error: "invalid-laced-frame-count" };
...
function readFixedLacedFrames(payloadBytes, frameCount)
{
    ...
    {
        return { frameCount, frameSizes: [], payloadBytes, error: "invalid-fixed-lacing-size" };
    }
    return { frameCount, frameSizes: Array.from({ length: frameCount }, () => dataBytes / frameCount), payloadBytes };
}
...
function readXiphLacedFrames(bytes, offset, end, payloadBytes, frameCount)
{
    ...
    for (let frame = 0; frame < frameCount - 1; frame++)
    {
        ...
        if (!closed) return { frameCount, frameSizes: [], payloadBytes, error: "unterminated-xiph-lacing-size" };
        frameSizes.push(size);
    }

    const remaining = end - offset - frameSizes.reduce((sum, size) => sum + size, 0);
    if (remaining < 0) return { frameCount, frameSizes: [], payloadBytes, error: "invalid-xiph-lacing-size" };
    frameSizes.push(remaining);
    return { frameCount, frameSizes, payloadBytes };
}

function readEbmlLacedFrames(bytes, offset, end, payloadBytes, frameCount)
{
    const first = readEbmlVint(bytes, offset, end, true);
    if (!first || first.value === null) return { frameCount, frameSizes: [], payloadBytes, error: "missing-ebml-lacing-size" };
    ...
    for (let frame = 1; frame < frameCount - 1; frame++)
    {
        ...
        if (!delta) return { frameCount, frameSizes: [], payloadBytes, error: "missing-ebml-lacing-delta" };
        const size = frameSizes[frame - 1] + delta.value;
        if (size < 0) return { frameCount, frameSizes: [], payloadBytes, error: "invalid-ebml-lacing-size" };
        frameSizes.push(size);
        offset += delta.length;
    }

    const remaining = end - offset - frameSizes.reduce((sum, size) => sum + size, 0);
    if (remaining < 0) return { frameCount, frameSizes: [], payloadBytes, error: "invalid-ebml-lacing-size" };
    frameSizes.push(remaining);
    return { frameCount, frameSizes, payloadBytes };
}
...
function readEbmlElements(bytes, start, end)
{
    ...
    while (offset < end)
    {
        ...
        if (!id) break;
        const size = readEbmlVint(bytes, offset + id.length, end, true);
        if (!size) break;
...
function readEbmlFloatValue(bytes, element)
{
    ...
    return element.end - element.dataStart === 4
        ? view.getFloat32(element.dataStart, false)
        : view.getFloat64(element.dataStart, false);
}
