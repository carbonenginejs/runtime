 ...
 */
export function isSoundbanksInfo(input)
{
    try
    {
 ...
 */
export function parseSoundbanksInfo(input)
{
...
{
    const media = Array.isArray(bank.Media)
        ? bank.Media
        : bank.IncludedMemoryFiles || [];
    const events = Array.isArray(bank.Events)
        ? bank.Events
        : bank.IncludedEvents || [];
    ...
    return {
        ...
        media: media.map((entry) => ({
            ...
            location: entry.Location
                || (Array.isArray(bank.IncludedMemoryFiles)
                    ? "Memory"
                    : "")
        })),
 ...
 */
export function buildSoundbanksCatalog(input)
{
 ...
 */
export function joinSoundbanksInfo(bankInfo, catalog)
{
    const bankId = String(bankInfo.bankId ?? "");
    const variants = catalog.bankVariantsById[bankId] || [];
    const bank = variants.find((variant) => wwiseIdFromName(variant.language) === bankInfo.languageId) ||
        catalog.banksById[bankId] ||
        null;
    ...
    const media = (bankInfo.media || []).map((entry) =>
    {
        ...
        const match = references.find((reference) => reference.bankId === bankId && reference.language === bankLanguage) ||
            references.find((reference) => reference.bankId === bankId) ||
            references[0] ||
            null;
        return {
            id: entry.id,
    ...
    const events = (bankInfo.hirc || [])
        .filter((entry) => entry.typeName === "event")
        .map((entry) =>
        {
