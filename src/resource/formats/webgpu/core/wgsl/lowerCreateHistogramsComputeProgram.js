...
const SYNC_NAMES = Object.freeze([
    "threads_in_group",
...
const mask = (typeName, registerIndex, selector) =>
    expectedOperand(typeName, registerIndex, "mask", selector);
const swizzle = (typeName, registerIndex, selector, modifierName = "none") =>
    expectedOperand(
        typeName, registerIndex, "swizzle", selector, [], modifierName);
const select = (typeName, registerIndex, selector, modifierName = "none") =>
    expectedOperand(
        typeName, registerIndex, "select1", selector, [], modifierName);
const none = (typeName, registerIndex, componentCount = 0) =>
    expectedOperand(
        typeName, registerIndex, "none", "", [], "none", componentCount);
const immediate = (...values) => expectedOperand(
    "immediate32",
    null,
...
const BODY_SM50 = Object.freeze([
    body("resinfo", [
        mask("temp", 0, "xy"),
        immediate(0),
        swizzle("resource", 0, "xyzw")
    ], { resinfoReturnTypeName: "uint" }),
    body("imad", [
        mask("temp", 0, "z"),
        select("input_thread_id_in_group", null, "y"),
        immediate(16),
        select("input_thread_id_in_group", null, "x")
    ]),
    body("ult", [
        mask("temp", 1, "xy"),
        swizzle("temp", 0, "zzzz"),
    ...
    body("if", [ select("temp", 1, "x") ], {
        testBoolean: "nonzero"
    }),
    body("store_structured", [
        mask("thread_group_shared_memory", 0, "x"),
        select("temp", 0, "z"),
        immediate(0),
        immediate(0)
    ]),
    ...
    body("ult", [
        mask("temp", 0, "xy"),
        swizzle("input_thread_id", null, "xyxx"),
        swizzle("temp", 0, "xyxx")
    ]),
    body("and", [
        mask("temp", 0, "x"),
    ...
    body("ld", [
        mask("temp", 0, "xyw"),
        swizzle("temp", 2, "xyzw"),
        swizzle("resource", 0, "xywz")
    ]),
    body("lt", [
        mask("temp", 1, "xzw"),
        swizzle("temp", 0, "xxyw"),
        immediate(1025879765, 0, 1025879765, 1025879765)
    ]),
    body("mul", [
        mask("temp", 2, "xyz"),
        swizzle("temp", 0, "xywx"),
        immediate(1033798545, 1033798545, 1033798545, 0)
    ]),
    body("add", [
        mask("temp", 0, "xyw"),
        swizzle("temp", 0, "xyxw"),
        immediate(1029785518, 1029785518, 0, 1029785518)
    ]),
    body("mul", [
        mask("temp", 0, "xyw"),
        swizzle("temp", 0, "xyxw"),
        immediate(1064478575, 1064478575, 0, 1064478575)
    ]),
    body("log", [
        mask("temp", 0, "xyw"),
        swizzle("temp", 0, "xyxw", "abs")
    ]),
    body("mul", [
        mask("temp", 0, "xyw"),
        swizzle("temp", 0, "xyxw"),
        immediate(1075419546, 1075419546, 0, 1075419546)
    ]),
    body("exp", [
        mask("temp", 0, "xyw"),
        swizzle("temp", 0, "xyxw")
    ...
    body("dp3", [
        mask("temp", 0, "x"),
        swizzle("temp", 0, "xywx"),
        immediate(1046059418, 1060578420, 1033087274, 0)
    ]),
    body("log", [
        mask("temp", 0, "x"),
        select("temp", 0, "x")
    ...
    body("mad", [
        mask("temp", 0, "x"),
        select("temp", 0, "x"),
        immediate(1060205080),
        select("constant_buffer", 0, "x", "neg")
    ]),
    body("add", [
        mask("temp", 0, "y"),
        select("constant_buffer", 0, "x", "neg"),
        select("constant_buffer", 0, "y")
    ]),
    body("div", [
        mask("temp", 0, "x"),
        select("temp", 0, "x"),
    ...
    body("atomic_iadd", [
        none("thread_group_shared_memory", 0),
        swizzle("temp", 0, "xyxx"),
        immediate(1)
    ]),
    body("endif"),
    body("sync"),
    body("if", [ select("temp", 1, "y") ], {
        testBoolean: "nonzero"
    }),
    body("ftou", [
        mask("temp", 0, "x"),
        select("constant_buffer", 0, "z")
    ]),
    body("imad", [
        mask("temp", 0, "x"),
        select("input_thread_group_id", null, "y"),
        select("temp", 0, "x"),
        select("input_thread_group_id", null, "x")
    ]),
    body("ishl", [
        mask("temp", 0, "x"),
        select("temp", 0, "x"),
    ...
    body("ld_structured", [
        ...
        select("temp", 0, "y"),
        immediate(0),
        swizzle("thread_group_shared_memory", 0, "xxxx")
    ]),
    body("iadd", [
        mask("temp", 0, "yw"),
        swizzle("temp", 0, "yyyy"),
...
const BODY_SM51 = Object.freeze([
    ...
    body("ld", [
        mask("temp", 1, "xzw"),
        swizzle("temp", 2, "xyzw"),
        swizzle("resource", 0, "xwyz")
    ]),
    body("lt", [
        mask("temp", 2, "xyz"),
        swizzle("temp", 1, "xzwx"),
        immediate(1025879765, 1025879765, 1025879765, 0)
    ]),
    body("mul", [
        mask("temp", 3, "xyz"),
        swizzle("temp", 1, "xzwx"),
    ...
    body("add", [
        mask("temp", 1, "xzw"),
        swizzle("temp", 1, "xxzw"),
        immediate(1029785518, 0, 1029785518, 1029785518)
    ]),
    body("mul", [
        mask("temp", 1, "xzw"),
        swizzle("temp", 1, "xxzw"),
        immediate(1064478575, 0, 1064478575, 1064478575)
    ]),
    body("log", [
        mask("temp", 1, "xzw"),
        swizzle("temp", 1, "xxzw", "abs")
    ]),
    body("mul", [
        mask("temp", 1, "xzw"),
        swizzle("temp", 1, "xxzw"),
        immediate(1075419546, 0, 1075419546, 1075419546)
    ]),
    body("exp", [
        mask("temp", 1, "xzw"),
        swizzle("temp", 1, "xxzw")
...
function validateAnalysisMetadata(program)
{
    ...
    if (JSON.stringify(originalSnapshot) !== JSON.stringify(rebuiltSnapshot))
    {
        ...
        throw new Error(
            "WGSL create-histograms compute CFG, SSA, or type metadata is inconsistent"
...
function validateCanonicalIndices(operand, expected, context)
{
    if (operand.indices?.length !== expected.length
        || expected.some((value, dimension) =>
            !isCanonicalIndex(operand.indices[dimension], dimension, value)))
    {
        throw new Error(
            `WGSL create-histograms ${context} has non-canonical index metadata`
        );
    }
}
...
function validateSm51Range(data, context)
{
    ...
    {
        throw new Error(
            `WGSL create-histograms ${context} requires one finite SM5.1 range`
        );
    }
}
...
function expectedIndices(typeName, registerIndex, minor, declaration)
{
    ...
    {
        return minor === 0
            ? [ registerIndex, declaration ? 1 : 0 ]
            : [ 0, registerIndex, declaration ? 0 : 0 ];
    }
...
function validateSm51Reference(operand, context)
{
    ...
    if ([ "resource", "uav" ].includes(operand.typeName))
    {
        ...
        {
            throw new Error(
                `WGSL create-histograms ${context} has an invalid SM5.1 resource reference`
            );
        }
    }
    else if (operand.typeName === "constant_buffer")
    {
        ...
        {
            throw new Error(
                `WGSL create-histograms ${context} has an invalid SM5.1 constant-buffer reference`
            );
        }
    }
    ...
    {
        throw new Error(
            `WGSL create-histograms ${context} has unexpected resource reference metadata`
        );
    }
}
...
{
    if (!operand
        || operand.typeName !== expected.typeName
        || operand.registerIndex !== expected.registerIndex
        ...
        || operand.modifierName !== expected.modifierName
        || operand.minPrecisionName !== "default"
        || operand.nonUniform !== false
        || operand.immediateValues?.length !== expected.immediateValues.length
        || expected.immediateValues.some((value, index) =>
            operand.immediateValues[index]?.uint32 !== value))
    {
        throw new Error(
            `WGSL create-histograms ${context} does not match the exact operand`
        );
    }
    validateCanonicalIndices(
        operand,
        expectedIndices(
            expected.typeName,
    ...
    if (declaration)
    {
        ...
        {
            throw new Error(
                `WGSL create-histograms ${context} has unexpected reference metadata`
            );
        }
    }
    else if (minor === 1)
    ...
    else if (operand.resourceReference !== undefined
        && operand.resourceReference !== null)
    {
        throw new Error(
            `WGSL create-histograms ${context} has unexpected SM5.1 reference metadata`
        );
    }
}
 ...
 */
export function isCreateHistogramsComputeProfile(program)
{
    return program?.stage === "compute"
        && program.declarations?.length === DECLARATION_OPCODES.length
        && program.instructions?.length === 49
...
)
{
    ...
    {
        throw new Error(
            `WGSL create-histograms compute requires exactly ${typeName}.xy`
        );
    }
    validateDeclarationOperand(
        declaration.operands[0],
        mask(typeName, null, "xy"),
        minor,
...
function validateDeclarations(program)
{
    ...
    {
        throw new TypeError(
            "WGSL create-histograms compute profile requires CJS SM5.0 or finite-range SM5.1 compute IR"
        );
    }
    if (!isCreateHistogramsComputeProfile(program))
    {
        throw new Error(
            "WGSL create-histograms compute declaration shape is not supported"
        );
    }
    if (program.declarations.some((declaration) =>
        declaration.tailTokens?.length))
    {
        throw new Error(
            "WGSL create-histograms compute declarations must not contain trailing payload words"
        );
    }
    ...
    {
        throw new Error(
            "WGSL create-histograms compute does not support signatures or constant tables"
        );
    }
    ...
    {
        throw new Error(
            "WGSL create-histograms compute requires only the refactoring-allowed global flag"
        );
    }
    ...
    {
        throw new Error(
            "WGSL create-histograms compute requires immediate one-row cb0"
        );
    }
    validateDeclarationOperand(
        constants.operands[0],
        swizzle("constant_buffer", 0, "xyzw"),
        minor,
        "cb0 declaration"
    );
    if (input.data?.registerIndex !== 0
        || input.data.resourceDimension !== 3
        || input.data.resourceDimensionName !== "texture2d"
        || input.data.sampleCount !== 0
        || input.data.returnType?.returnTypes?.length !== 4
        || input.data.returnType.returnTypes.some((entry) => entry !== 5)
        || input.data.returnType?.returnTypeNames?.length !== 4
        || input.data.returnType.returnTypeNames.some((entry) =>
            entry !== "float")
        || input.operands?.length !== 1)
    {
        throw new Error(
            "WGSL create-histograms compute requires float texture2d t0"
        );
    }
    validateDeclarationOperand(
        input.operands[0],
    ...
    if (output.data?.registerIndex !== 0
        || output.data.resourceDimension !== 1
        || output.data.resourceDimensionName !== "buffer"
        || output.data.globallyCoherent !== false
        || output.data.returnType?.returnTypes?.length !== 4
        || output.data.returnType.returnTypes.some((entry) => entry !== 4)
        || output.data.returnType?.returnTypeNames?.length !== 4
        || output.data.returnType.returnTypeNames.some((entry) =>
            entry !== "uint")
        || output.operands?.length !== 1)
    {
        throw new Error(
            "WGSL create-histograms compute requires non-coherent typed uint Buffer u0"
        );
    }
    validateDeclarationOperand(
        output.operands[0],
    ...
    validateBuiltinDeclaration(
        groupId, "input_thread_group_id", 33, minor);
    validateBuiltinDeclaration(
        localId, "input_thread_id_in_group", 34, minor);
    validateBuiltinDeclaration(
        dispatchId, "input_thread_id", 32, minor);
    if (temps.data?.tempCount !== (minor === 0 ? 3 : 4)
        || temps.operands?.length)
    {
        throw new Error(
            `WGSL create-histograms SM5.${minor} requires exactly ${minor === 0 ? "three" : "four"} temps`
        );
    }
    ...
    {
        throw new Error(
            "WGSL create-histograms compute requires g0 with stride 4 and 64 records"
        );
    }
    validateDeclarationOperand(
        shared.operands[0],
    ...
    if (group.data?.threadGroupX !== 16
        || group.data.threadGroupY !== 16
        || group.data.threadGroupZ !== 1
        || group.operands?.length)
    {
        throw new Error(
            "WGSL create-histograms compute requires dcl_thread_group 16,16,1"
        );
    }
    if (minor === 1)
    {
    ...
    {
        throw new Error(
            "WGSL create-histograms SM5.0 declarations must use register bindings"
        );
    }
}
...
function validTextureExtensions(program, instruction)
{
    ...
    return extensions.length === 2
        && extensions[0]?.token === 2147483842
        && extensions[0].type === 2
        && extensions[0].typeName === "resource_dimension"
        && extensions[0].resourceDimension === 3
        && extensions[0].resourceDimensionName === "texture2d"
        && extensions[0].structureStride === 0
        && extensions[1]?.token === 1398083
        && extensions[1].type === 3
        && extensions[1].typeName === "resource_return_type"
        && extensions[1].resourceReturnTypes?.length === 4
        && extensions[1].resourceReturnTypes.every((entry) => entry === 5);
}

function validateBody(program)
{
    const expectedBody = program.shaderModel.minor === 0
        ? BODY_SM50
        : BODY_SM51;
    if (program.instructions?.length !== expectedBody.length)
    {
        throw new Error(
            "WGSL create-histograms compute requires the exact 49-instruction body"
        );
    }
    for (const [ index, expected ] of expectedBody.entries())
    {
        const instruction = program.instructions[index];
        if (instruction?.opcodeName !== expected.opcodeName
            || instruction.operands?.length !== expected.operands.length)
        {
            throw new Error(
                "WGSL create-histograms compute requires the exact backend opcode and operand schedule"
            );
        }
        for (const [ operandIndex, expectedEntry ] of
            expected.operands.entries())
        {
            validateOperand(
                instruction.operands[operandIndex],
                expectedEntry,
                program.shaderModel.minor,
                `instruction ${index} operand ${operandIndex}`
            );
        }
        ...
        if (instruction.index !== index
            ...
            || instruction.saturate !== expected.saturate
            || instruction.preciseMask !== ""
            || instruction.tailTokens?.length
            || instruction.resinfoReturnTypeName
                !== expected.resinfoReturnTypeName
            || !validTextureExtensions(program, instruction)
            || (sync
                ? instruction.syncFlags !== 3
                    || JSON.stringify(instruction.syncFlagNames)
                        !== JSON.stringify(SYNC_NAMES)
                : instruction.syncFlags !== undefined
                    || instruction.syncFlagNames !== undefined))
        {
            throw new Error(
                `WGSL create-histograms instruction ${index} has inconsistent envelope metadata`
            );
        }
    }
    ...
    const minor = program.shaderModel.minor;
    if (program.blocks.length !== 10
        || program.controlFlow.edgeCount !== 12
        || program.controlFlow.regions.length !== 3
        || program.controlFlow.unreachableBlockIds.length
        || program.values.length !== 53
        || origins["instruction-write"] !== 37
        || origins["program-input"] !== 6
        || origins["undefined-register"] !== (minor === 0 ? 6 : 7)
        || origins["control-flow-merge"] !== (minor === 0 ? 4 : 3))
    {
        throw new Error(
            "WGSL create-histograms compute requires the canonical CFG and register-value shape"
        );
    }
    const values = new Map(program.values.map((value) => [ value.id, value ]));
    for (const instruction of program.instructions)
    {
        for (const ref of instruction.dataflow.reads.flatMap((read) =>
            read.refs))
        {
            if (values.get(ref.valueId)?.origin === "undefined-register")
            {
                throw new Error(
                    `WGSL create-histograms instruction ${instruction.index} reads undefined register data`
                );
            }
        }
    }
    ...
    {
        throw new Error(
            "WGSL create-histograms compute requires three canonical no-else selections without live merges"
        );
    }
}
...
{
    if (bindings.length !== 3) throw new Error(
        "WGSL create-histograms compute binding layout does not match cb0/t0/u0"
    );
    const [ constants, input, output ] = bindings;
    if (constants.resourceKind !== "uniform-buffer"
        || constants.generatedSymbol !== "cb0"
        || constants.registerIndex !== 0
        || constants.registerSpace !== 0
        || constants.declaration !== "var<uniform>"
        || constants.type !== "array<vec4<f32>, 1>"
        || constants.buffer?.type !== "uniform"
        || constants.buffer.hasDynamicOffset !== false
        || constants.buffer.minBindingSize !== 16
        || input.resourceKind !== "sampled-resource"
        || input.generatedSymbol !== "t0"
        ...
        || output.generatedSymbol !== "u0"
        || output.registerIndex !== 0
        || output.registerSpace !== 0
        || output.declaration !== "var<storage, read_write>"
        || output.type !== "array<atomic<u32>>"
        || output.buffer?.type !== "storage"
        || output.buffer.hasDynamicOffset !== false
        || output.buffer.minBindingSize !== 4)
    {
        throw new Error(
            "WGSL create-histograms compute binding layout does not match cb0/t0/u0"
        );
...
function lowerBody(program, bindings)
{
    ...
    const sourceRgb = [
        letStatement(
            program, 10, 11, "safe_pixel", "vec2<u32>",
            "min(dispatch_thread_id.xy, texture_size - vec2<u32>(1u))"
        ),
        letStatement(
            program, 12, 13, "source_rgba", "vec4<f32>",
            `select(vec4<f32>(), textureLoad(${input}, safe_pixel, 0), ${inBounds})`
        ),
        letStatement(
            program, 12, 13, "source_rgb", "vec3<f32>",
            "source_rgba.xyz"
        ),
        letStatement(
            program, 14, 15, "low_rgb", "vec3<f32>",
            "(source_rgb * vec3<f32>(bitcast<f32>(0x3d9e8391u)))"
        ),
        letStatement(
            program, 19, 20, "high_rgb", "vec3<f32>",
            "exp2(log2(abs((source_rgb + vec3<f32>(bitcast<f32>(0x3d6147aeu)))"
                + " * vec3<f32>(bitcast<f32>(0x3f72a76fu))))"
                + " * vec3<f32>(bitcast<f32>(0x4019999au)))"
        ),
        letStatement(
            program, 20, 21, "linear_rgb", "vec3<f32>",
            "select(high_rgb, low_rgb, source_rgb"
                + " < vec3<f32>(bitcast<f32>(0x3d25aed5u)))"
        ),
        letStatement(
            program, 21, 22, "luminance", "f32",
            "dot(linear_rgb, vec3<f32>("
                + "bitcast<f32>(0x3e59999au), "
                + "bitcast<f32>(0x3f372474u), "
                + "bitcast<f32>(0x3d93a92au)))"
        ),
        letStatement(
            program, 23, 24, "log_luminance", "f32",
            `(log2(luminance) * bitcast<f32>(0x3f317218u)) - ${constants}[0u].x`
        ),
        letStatement(
            program, 24, 25, "luminance_range", "f32",
            `${constants}[0u].y - ${constants}[0u].x`
        ),
        letStatement(
            program, 25, 26, "normalized_luminance", "f32",
            "clamp(log_luminance / luminance_range, 0.0, 1.0)"
        ),
        letStatement(
            program, 28, 29, "histogram_bin", "i32",
            "min(i32(normalized_luminance"
                + " * bitcast<f32>(0x42800000u)), 63)"
        ),
        {
            kind: "if",
            ...location(program, 30, 31),
            condition: expression(
                "histogram_bin >= 0 && histogram_bin < 64", "bool"),
            statements: [
                callStatement(
                    program,
                    30,
                    31,
                    "atomicAdd(&g0[u32(histogram_bin)], 1u)"
                )
            ]
        }
    ];
    const outputStores = [ "x", "y", "z", "w" ].map((component, word) =>
        callStatement(
            program,
            46,
            46,
            `atomicStore(&${output}[(output_element * 4u) + ${word}u], histogram_words.${component})`
        )
    );
    return [
        letStatement(
            program, 0, 0, "texture_size", "vec2<u32>", textureSize),
        letStatement(
            program,
            1,
            1,
        ...
        letStatement(
            program, 2, 3, "initialize_bin", "bool", "local_index < 64u"),
        letStatement(
            program, 2, 3, "write_histogram", "bool", "local_index < 16u"),
        {
            kind: "if",
            ...
            statements: [
                callStatement(
                    program, 4, 5, "atomicStore(&g0[local_index], 0u)")
            ]
        },
        ...
        callStatement(program, 32, 33, "workgroupBarrier()"),
        {
            ...
            statements: [
                letStatement(
                    ...
                    "group_linear",
                    "u32",
                    "(workgroup_id.y * screen_tiles_x) + workgroup_id.x"
                ),
                letStatement(
                    program,
                    39,
                ...
                letStatement(
                    ...
                    "vec4<u32>",
                    "vec4<u32>("
                        + "atomicLoad(&g0[histogram_base + 0u]), "
                        + "atomicLoad(&g0[histogram_base + 1u]), "
                        + "atomicLoad(&g0[histogram_base + 2u]), "
                        + "atomicLoad(&g0[histogram_base + 3u]))"
                ),
                {
                    kind: "if",
                    ...location(program, 46, 46),
                    condition: expression(
                        `output_element < (arrayLength(&${output}) / 4u)`,
                        "bool"
                    ),
 ...
 */
export function lowerCreateHistogramsComputeProgram(program, options = {})
{
