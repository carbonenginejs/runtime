...
function rejectTailTokens(entries, kind, context)
{
    ...
    {
        if (entry?.tailTokens !== undefined
            && (!Array.isArray(entry.tailTokens) || entry.tailTokens.length !== 0))
        {
            const location = kind === "declaration"
                ? `at DXBC offset ${entry?.dxbcOffset ?? "<unknown>"}`
                : `${entry?.index ?? "<unknown>"}`;
            fail(context,
                `has inconsistent envelope metadata: ${kind} ${location} has unconsumed tail tokens; exact compute programs must not contain trailing payload words`);
        }
    }
}
...
function finiteRange(data, binding, context)
{
    ...
    if (!data
        ...
        || !Number.isInteger(range.lowerBound) || range.lowerBound < 0
        || !Number.isInteger(range.upperBound) || range.upperBound < range.lowerBound
        || range.unbounded !== false
        || range.registerCount !== range.upperBound - range.lowerBound + 1
        || !Number.isInteger(range.registerSpace) || range.registerSpace < 0
        || data.registerIndex !== range.lowerBound)
    {
...
{
    if (!binding.range
        || RANGE_FIELDS.some((field) => binding.range[field] !== range[field])
        || binding.registerIndex !== range.lowerBound)
    {
        fail(context,
            `binding ${binding.id || "<unknown>"} has inconsistent declaration range identity; array or unbounded range metadata is not canonical`);
    }
}
...
function validateHandleIdentity(declaration, binding, descriptor, range, context)
{
    ...
    const handleRegister = ranged
        ? range.rangeId
        : range.lowerBound;
    const indices = operand?.indices;
    const validIndices = ranged
        ? indices?.length === 3
            && isCanonicalIndex(indices[0], 0, range.rangeId)
            && isCanonicalIndex(indices[1], 1, range.lowerBound)
            && isCanonicalIndex(indices[2], 2, range.upperBound)
        : descriptor.operandType === "constant_buffer"
            ? indices?.length === 2
                && isCanonicalFixedIndex(indices[0], 0, range.lowerBound)
                && isCanonicalFixedIndex(
                    indices[1], 1, declaration.data?.sizeInVec4)
            : indices?.length === 1
                && isCanonicalFixedIndex(indices[0], 0, range.lowerBound);
    if (declaration.operands?.length !== 1
        || operand?.typeName !== descriptor.operandType
        || binding.operandType !== descriptor.operandType
        || operand.registerIndex !== handleRegister
        || !validIndices)
    {
...
function validateBinding(program, binding, context)
{
    ...
    const expectedId =
        `${descriptor.resourceKind}:space${range.registerSpace}:range${range.rangeId ?? range.lowerBound}`;
 ...
 */
export function validateExactComputeEnvelope(program, context = "WGSL exact compute")
{
    if (!Array.isArray(program?.declarations)
 ...
 */
export function validateReturnTypeMirror(
    returnType,
    context = "WGSL exact compute return type")
{
    const numeric = returnType?.returnTypes;
    const names = returnType?.returnTypeNames;
    if (!isDenseArrayOfLength(numeric, 4)
        || !isDenseArrayOfLength(names, 4)
        || numeric.some((entry, index) =>
            !Number.isInteger(entry)
            || !Object.prototype.hasOwnProperty.call(RETURN_TYPE_NAMES, entry)
            || RETURN_TYPE_NAMES[entry] !== names[index]))
    {
        fail(context,
            "has inconsistent numeric/name return-type component mirrors");
    }
    return true;
}
 ...
 */
export function validateSm50ResourceExtensions(
    extensions,
    expected,
    context = "WGSL exact compute instruction")
{
    const resourceReturnTypes = expected?.resourceReturnTypes;
    if (!Number.isInteger(expected?.resourceDimension)
        || !Number.isInteger(expected?.structureStride)
        || !isDenseArrayOfLength(resourceReturnTypes, 4))
    {
        throw new TypeError(`${context} requires exact expected SM5.0 resource extensions`);
    }
    const dimensionToken = (
        0x80000000
        | (expected.structureStride << 11)
        | (expected.resourceDimension << 6)
        | 2
    ) >>> 0;
    const returnToken = (
        3
        | (resourceReturnTypes[0] << 6)
        | (resourceReturnTypes[1] << 10)
        | (resourceReturnTypes[2] << 14)
        | (resourceReturnTypes[3] << 18)
    ) >>> 0;
    const dimension = extensions?.[0];
    const returns = extensions?.[1];
    if (!Array.isArray(extensions) || extensions.length !== 2
        || dimension?.token !== dimensionToken
        || dimension.type !== 2
        || dimension.typeName !== "resource_dimension"
        || dimension.resourceDimension !== expected.resourceDimension
        || dimension.resourceDimensionName !== expected.resourceDimensionName
        || dimension.structureStride !== expected.structureStride
        || returns?.token !== returnToken
        || returns.type !== 3
        || returns.typeName !== "resource_return_type"
        || !isDenseArrayOfLength(returns.resourceReturnTypes, 4)
        || returns.resourceReturnTypes.some((entry, index) =>
            entry !== resourceReturnTypes[index]))
    {
        fail(context,
            "has inconsistent SM5.0 resource-extension semantic or numeric mirrors");
    }
    return true;
}
