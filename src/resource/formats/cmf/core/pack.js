...
function writeComponent(view, offset, type, value)
{
    switch (type)
    {
        case "Float32": view.setFloat32(offset, value || 0, true); break;
        case "Float16": view.setUint16(offset, floatToHalf(value || 0), true); break;
        case "UInt16Norm": view.setUint16(offset, clampRound(value * 65535, 0, 65535), true); break;
        case "UInt16": view.setUint16(offset, clampRound(value, 0, 65535), true); break;
        case "Int16Norm": view.setInt16(offset, clampRound(value * 32767, -32767, 32767), true); break;
        case "Int16": view.setInt16(offset, clampRound(value, -32768, 32767), true); break;
        case "UInt8Norm": view.setUint8(offset, clampRound(value * 255, 0, 255)); break;
        case "UInt8": view.setUint8(offset, clampRound(value, 0, 255)); break;
        case "Int8Norm": view.setInt8(offset, clampRound(value * 127, -127, 127)); break;
        case "Int8": view.setInt8(offset, clampRound(value, -128, 127)); break;
...
export function packIndexBuffer(groups)
{
    ...
    {
        if (wide) view.setUint32(i * stride, faces[i], true);
        else view.setUint16(i * stride, faces[i], true);
    }
    return { bytes, stride, count: faces.length };
}

function assertCmfNativeSkeleton(skeleton)
{
    const bonesAreNames = Array.isArray(skeleton?.bones) &&
        skeleton.bones.every((bone) => typeof bone === "string");
    if (!bonesAreNames)
    {
        throw packError(
            "skeletons must be CMF-native shaped (bones as name strings with parents/restTransforms); " +
            "GR2-shaped skeletons need conversion before writing"
        );
    }
}
 ...
 */
export function packGraphBuffers(graph)
{
    const buffers = [ null ];
    const allocate = (bytes) =>
    {
        const index = buffers.length;
        buffers.push({ index, data: bytes });
        return index;
    };
    ...
    const meshes = (graph.meshes || []).map((mesh) =>
    {
        ...
        const lods = (mesh.lods || []).map((lod) =>
        {
            ...
            const vb = packedVb.count
                ...
                : { index: 0, offset: 0, size: 0, stride: 0 };
            const ib = packedIb.count
                ? { index: allocate(packedIb.bytes), offset: 0, size: packedIb.bytes.byteLength, stride: packedIb.stride }
                : { index: 0, offset: 0, size: 0, stride: 0 };
            ...
            const morphTargets = (lod.morphTargets || []).map((target) =>
            {
                ...
                return {
                    vb: packed.count
                        ? { index: allocate(packed.bytes), offset: 0, size: packed.bytes.byteLength, stride: packed.stride }
                        : { index: 0, offset: 0, size: 0, stride: 0 }
                };
            });
