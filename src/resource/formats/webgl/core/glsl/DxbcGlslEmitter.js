...
const NON_COMPARISON_TEXTURE_OPCODES = new Set([
    "sample", "sample_l", "sample_b", "sample_d",
    "gather4", "gather4_po", "ld", "ld_ms", "lod"
]);
...
export class DxbcGlslEmitter
{
   ...
   */
    ...
    // here can ever reach 32 (GLSL shift-by-bit-width is undefined).
        this.helpers.define("hlslcc_ibfe", {
            source: [
                "int hlslcc_ibfe(int width, int offset, int value) {",
                "    int w = width & 31;",
                "    int o = offset & 31;",
                "    if (w == 0) return 0;",
                "    if (w + o < 32) {",
                "        return (value << (32 - w - o)) >> (32 - w);",
                "    }",
                "    return value >> o;",
                "}"
            ].join("\n")
        ...
        this.helpers.define("hlslcc_ubfe", {
            source: [
                "uint hlslcc_ubfe(uint width, uint offset, uint value) {",
                "    uint w = width & 31u;",
                "    uint o = offset & 31u;",
                "    if (w == 0u) return 0u;",
                "    if (w + o < 32u) {",
                "        return (value << (32u - w - o)) >> (32u - w);",
                "    }",
                "    return value >> o;",
                "}"
            ].join("\n")
        ...
        this.helpers.define("hlslcc_bfi", {
            source: [
                "uint hlslcc_bfi(uint width, uint offset, uint insert, uint base) {",
                "    uint w = width & 31u;",
                "    uint o = offset & 31u;",
                "    uint bits = ((1u << w) - 1u) << o;",
                "    return ((insert << o) & bits) | (base & ~bits);",
                "}"
            ].join("\n")
        });
        ...
        this.helpers.define("hlslcc_countbits", {
            source: [
                "uint hlslcc_countbits(uint value) {",
                "    uint v = value;",
        ...
        this.helpers.define("hlslcc_textureGather4Emulated", {
            source: [
                "vec4 hlslcc_textureGather4Emulated(sampler2D samp, vec2 uv, int channel) {",
                "    ivec2 size = textureSize(samp, 0);",
                "    ivec2 maxCoord = size - ivec2(1);",
                "    vec2 texel = uv * vec2(size) - vec2(0.5);",
                "    ivec2 base = ivec2(floor(texel));",
                "    ivec2 c0 = clamp(base + ivec2(0, 1), ivec2(0), maxCoord);",
                "    ivec2 c1 = clamp(base + ivec2(1, 1), ivec2(0), maxCoord);",
                "    ivec2 c2 = clamp(base + ivec2(1, 0), ivec2(0), maxCoord);",
                "    ivec2 c3 = clamp(base + ivec2(0, 0), ivec2(0), maxCoord);",
                "    return vec4(",
                "        texelFetch(samp, c0, 0)[channel],",
                "        texelFetch(samp, c1, 0)[channel],",
                "        texelFetch(samp, c2, 0)[channel],",
                "        texelFetch(samp, c3, 0)[channel]",
                "    );",
                "}"
            ].join("\n")
        });
        ...
        this.helpers.define("hlslcc_textureGather4ArrayEmulated", {
            source: [
                ...
                "    ivec2 size = textureSize(samp, 0).xy;",
                "    ivec2 maxCoord = size - ivec2(1);",
                "    int layer = int(uvw.z + 0.5);",
                "    vec2 texel = uvw.xy * vec2(size) - vec2(0.5);",
                "    ivec2 base = ivec2(floor(texel));",
                "    ivec2 c0 = clamp(base + ivec2(0, 1), ivec2(0), maxCoord);",
                "    ivec2 c1 = clamp(base + ivec2(1, 1), ivec2(0), maxCoord);",
                "    ivec2 c2 = clamp(base + ivec2(1, 0), ivec2(0), maxCoord);",
                "    ivec2 c3 = clamp(base + ivec2(0, 0), ivec2(0), maxCoord);",
                "    return vec4(",
                "        texelFetch(samp, ivec3(c0, layer), 0)[channel],",
                "        texelFetch(samp, ivec3(c1, layer), 0)[channel],",
                "        texelFetch(samp, ivec3(c2, layer), 0)[channel],",
                "        texelFetch(samp, ivec3(c3, layer), 0)[channel]",
                "    );",
                "}"
            ].join("\n")
        });
   ...
   */
    Emit(bytes, options = {})
    {
        ...
        const state = {
            ...
            computeInputsUsed: { threadId: false, groupId: false, tig: false, flattened: false }
        };
        this.helpers.reset();
        ...
        if (!state.isPixel && !state.isCompute && Array.isArray(options.pairVaryings))
        {
            for (const register of options.pairVaryings)
            {
                ...
                state.outputs.push({ register, name, width: 4, semanticName: null, semanticIndex: 0 });
        ...
        if (state.isCompute)
        {
            ...
            result.computeFragment = {
                ...
                uavOutputs: state.bindings
                    .filter((binding) => binding.kind === "uavTexture")
                    .map((binding) => ({
        ...
        for (const register of comparisonResources.keys())
        {
            if (ordinaryResources.has(register))
            {
                throw new WebglReadError(
                    "A resource sampled with comparison and non-comparison operations requires separate combined samplers",
                    { source: sourceName, registerIndex: register }
                );
            }
        }
   ...
   */
    _ensureLightConstantBuffer(state)
    {
        ...
        state.declarationLines.push(
            ...
            "    int local = element - listBase;",
            "    int node = local >> 1;",
            "    if (node < 0 || uint(node) >= count) return 0u;",
            "    if ((local & 1) == 0) return uint(node + 1);",
   ...
   */
    _assertMapStyleCompute(state)
    {
        for (const instruction of state.decoder.instructions)
        {
            ...
            const blocked = name.startsWith("dcl_thread_group_shared_memory")
        || name === "sync"
        || name.startsWith("atomic_")
        || name.startsWith("imm_atomic_")
        || name === "ld_uav_typed"
        || name === "store_raw"
        || name === "store_structured";
            ...
            {
                throw new WebglReadError(
                    "compute stage is not map-style; WebGL2 fragment lowering is not supported for this instruction",
                    { source: state.sourceName, opcodeName: name, offset: instruction.offset }
                );
            }
        }
    }
   ...
   */
    _analyzeUavStores(state)
    {
        ...
        for (const [ register, stores ] of storesByUav)
        {
            if (stores.length <= 1)
            {
                state.uavSlicePlan.set(register, { kind: "single" });
                continue;
            }
            const throwNotMapStyle = (shape) =>
            {
                throw new WebglReadError(
                    `compute stage is not map-style; ${shape} are not supported by the WebGL2 emitter's fragment-output lowering`,
                    { source: state.sourceName, registerIndex: register, storeCount: stores.length }
                );
            };
   ...
   */
    _trackTempConstants(instruction, constantTemps)
    {
        ...
        const isConstantMov = instruction.opcodeName === "mov"
      && !instruction.saturate
      && source?.type === 4
      && source.modifierName === "none";
        for (const operand of instruction.operands)
        {
            ...
            for (const component of operand.mask || "xyzw")
            {
                ...
                {
                    const value = source.componentCount === 1
                        ? source.immediateValues[0]
                        : source.immediateValues["xyzw".indexOf(component)];
   ...
   */
    _resolveSliceConstant(addressOperand, constantTemps)
    {
        ...
        {
            const value = addressOperand.componentCount === 1
                ? addressOperand.immediateValues[0]
                : addressOperand.immediateValues[2];
        ...
        const component = addressOperand.selectionModeName === "swizzle" && addressOperand.swizzle
            ? addressOperand.swizzle[2]
            : "z";
   ...
   */
    _allocateUavOutputLocation(state, register)
    {
        ...
        {
            throw new WebglReadError(
                "compute stage is not map-style; too many simultaneous UAV writes are not supported by the WebGL2 emitter's fragment-output lowering",
                { source: state.sourceName, registerIndex: register, uavCount: location + 1 }
            );
        }
    ...
    _emitDeclaration(state, instruction)
    {
        ...
        switch (instruction.opcodeName)
        {
            ...
            case "dcl_indexable_temp":
                state.declarationLines.push(
                    `vec${declaration.componentCount} x${declaration.registerIndex}[${declaration.registerCount}];`
                );
                break;
            case "dcl_constant_buffer": {
                ...
                {
                    state.declarationLines.push(
                        `layout(std140) uniform ConstantBuffer${mapped} {`,
                        `    vec4 data[${declaration.sizeInVec4}];`,
                        `} ${name};`
                    );
                }
                else
            ...
            case "dcl_resource": {
                ...
                const samplerType = comparisonSamplers
                    ? SHADOW_SAMPLER_TYPE_BY_DIMENSION[declaration.resourceDimension]
                    : SAMPLER_TYPE_BY_DIMENSION[declaration.resourceDimension];
                if (!samplerType)
                {
                    throw new WebglReadError(
                        comparisonSamplers
                            ? "Comparison sampling is not supported for this resource dimension by the WebGL2 emitter"
                            : "Resource dimension is not supported by the WebGL2 emitter",
                        {
                            source: state.sourceName,
            ...
            case "dcl_resource_structured": {
                ...
                {
                    this._ensureLightPackedTexture(state);
                    state.structuredBuffers.set(register, { kind: "lightIndexPacked", name: state.lightPackedTexture.name, strideDwords });
                    break;
                }
                ...
                {
                    this._ensureLightConstantBuffer(state);
                    state.structuredBuffers.set(register, { kind: "lightIndexCb", name: state.lightConstantBuffer.name, strideDwords });
                    break;
                }
                if (state.lightConstantBuffer?.dataRegister === register)
                {
                    this._ensureLightConstantBuffer(state);
                    state.structuredBuffers.set(register, { kind: "lightDataCb", name: state.lightConstantBuffer.name, strideDwords });
                    break;
                }
                if (state.stubbedResources.has(register))
                {
                    ...
                    state.structuredBuffers.set(register, { kind: "stub", name: `sb${register}`, strideDwords });
                    break;
                }
                if (state.isPixel)
                {
                    ...
                    state.structuredBuffers.set(register, { kind: "texture", name, strideDwords });
                ...
                else
                {
                    ...
                    state.declarationLines.push(
                        `layout(std140) uniform ${name}Block {`,
                        `    vec4 data[${rows}];`,
                        `} ${name};`
                    );
                    state.bindings.push({
    ...
    _declarePixelInput(state, instruction)
    {
        ...
        const name = `vs_r${register}`;
        const flat = element.componentType === 1 || element.componentType === 2
      || declaration.interpolationModeName === "constant";
        state.inputNames.set(register, name);
   ...
   */
    ...
    _declareOutput(state, instruction)
    {
        ...
        if (instruction.opcodeName === "dcl_output_siv")
        {
            if (declaration.systemValueName === "position")
            {
                ...
                state.outputs.push({ register, name: "gl_Position", semanticName: "SV_Position", semanticIndex: 0 });
                return;
            }
    ...
    _declareImmediateConstantBuffer(state, instruction)
    {
        ...
        const literals = rows.map((row) =>
            `vec4(${row.map((value) => `uintBitsToFloat(0x${(value.uint32 >>> 0).toString(16)}u)`).join(", ")})`
        );
        state.declarationLines.push(
            `const vec4 ${state.icbName}[${rows.length}] = vec4[${rows.length}](`,
            ...literals.map((literal, index) => `    ${literal}${index < literals.length - 1 ? "," : ""}`),
            ");"
        );
    }
   ...
   */
    ...
    {
        const expression = state.formatter.sourceExpression(operand, { destMask, as });
        if (destMask.length === 1)
        {
            return expression;
        }
        const ctor = { float: "vec", int: "ivec", uint: "uvec" }[as] + destMask.length;
    ...
    _destMask(state, instruction, operandIndex = 0)
    {
        ...
        return { target, mask: target?.registerMask || "xyzw" };
    }
    ...
    {
        return operand.selectionModeName === "select1"
      || (operand.type === 4 && operand.componentCount === 1)
      || operand.componentCount === 1;
    }
    ...
    _assign(state, instruction, valueExpression, options = {})
    {
        ...
        if (statement)
        {
            ...
            {
                const { target } = this._destMask(state, instruction);
    ...
    _infixBinary(state, instruction, operator)
    {
        ...
        const a = state.formatter.sourceExpression(instruction.operands[1], { destMask: mask });
        const b = state.formatter.sourceExpression(instruction.operands[2], { destMask: mask });
        const width = Math.max(
            state.formatter.expressionWidth(instruction.operands[1], mask),
            state.formatter.expressionWidth(instruction.operands[2], mask)
        );
    ...
    _assignWidened(state, instruction, mask, expression, rhsIsScalar)
    {
        ...
        this._assign(state, instruction, value, { saturate: instruction.saturate });
    }
    ...
    _helperUnary(state, instruction, glslName)
    {
        ...
        this._assignWidened(state, instruction, mask, `${glslName}(${src})`,
    ...
    _helperBinary(state, instruction, glslName)
    {
        ...
        this._assign(state, instruction, `${glslName}(${a}, ${b})`, { saturate: instruction.saturate });
    }
     ...
     * @private
     */
    _comparison(state, instruction, { intrinsic, operator, as })
    {
        const { mask } = this._destMask(state, instruction);
        if (mask.length === 1)
        {
            const a = state.formatter.sourceExpression(instruction.operands[1], { destMask: mask, as });
            const b = state.formatter.sourceExpression(instruction.operands[2], { destMask: mask, as });
        ...
        this._assign(
            state,
            instruction,
            `uintBitsToFloat(uvec${mask.length}(${intrinsic}(${a}, ${b})) * 0xFFFFFFFFu)`
        );
    }
    ...
    _movc(state, instruction)
    {
        ...
        const { target, mask } = this._destMask(state, instruction);
        if (!target) return;
    ...
    _sincos(state, instruction)
    {
        ...
        const emitOne = (destOperand, fn) =>
        {
            ...
            const src = state.formatter.sourceExpression(angle, { destMask: mask });
            const call = state.formatter.expressionWidth(angle, mask) === 1 && mask.length > 1
                ? `vec${mask.length}(${fn}(${src}))`
                : `${fn}(${src})`;
            const text = this._destText(target);
    ...
    _declareDetailArrayMap(state, register, declaration, comparisonSamplers)
    {
        ...
        {
            throw new WebglReadError(
                "Detail-map array merging requires plain 2D textures without comparison sampling",
                {
                    source: state.sourceName,
                    register,
                    dimensionName: declaration.resourceDimensionName
    ...
    } = {})
    {
        ...
        if (state.stubbedResources.has(texOperand.registerIndex))
        {
            ...
            const zeroSwizzle = [ ...mask ]
                .map((component) => texOperand.swizzle ? texOperand.swizzle["xyzw".indexOf(component)] : component)
                .join("");
            this._assign(state, instruction, `vec4(0.0).${zeroSwizzle}`, { saturate: instruction.saturate });
            return;
        }
        ...
        if (comparisonRefOperandIndex !== null)
        {
            const reference = state.formatter.sourceExpression(
                instruction.operands[comparisonRefOperandIndex],
                { destMask: "x" }
            );
            ...
            const value = mask.length === 1
                ? comparisonCall
                : `vec${mask.length}(${comparisonCall})`;
            this._assign(state, instruction, value, { saturate: instruction.saturate });
            return;
        }
        ...
        {
            const lod = state.formatter.sourceExpression(instruction.operands[lodOperandIndex], { destMask: "x" });
            call = `textureLod(${texName}, ${coord}, ${lod})`;
        }
        else if (biasOperandIndex !== null)
        {
            const bias = state.formatter.sourceExpression(instruction.operands[biasOperandIndex], { destMask: "x" });
    ...
    _texelFetch(state, instruction)
    {
        ...
        if (dimension === 1)
        {
            const index = state.formatter.sourceExpression(coordOperand, { destMask: "x", as: "int" });
        ...
        else
        {
            ...
            const mip = state.formatter.sourceExpression(coordOperand, { destMask: "w", as: "int" });
...
DxbcGlslEmitter.LOWERINGS = {
    ...
    mad(state, instruction)
    {
        ...
        const a = state.formatter.sourceExpression(instruction.operands[1], { destMask: mask });
        const b = state.formatter.sourceExpression(instruction.operands[2], { destMask: mask });
        const c = state.formatter.sourceExpression(instruction.operands[3], { destMask: mask });
        const width = Math.max(...[ 1, 2, 3 ].map((index) =>
            state.formatter.expressionWidth(instruction.operands[index], mask)));
        this._assignWidened(state, instruction, mask, `${a} * ${b} + ${c}`, width === 1);
    },
    mul(state, instruction) { this._infixBinary(state, instruction, "*"); },
    add(state, instruction) { this._infixBinary(state, instruction, "+"); },
    div(state, instruction) { this._infixBinary(state, instruction, "/"); },
    dp2(state, instruction) { this._dotProduct(state, instruction, "xy"); },
    dp3(state, instruction) { this._dotProduct(state, instruction, "xyz"); },
    dp4(state, instruction) { this._dotProduct(state, instruction, "xyzw"); },
    max(state, instruction) { this._helperBinary(state, instruction, "max"); },
    min(state, instruction) { this._helperBinary(state, instruction, "min"); },
    sqrt(state, instruction) { this._helperUnary(state, instruction, "sqrt"); },
    rsq(state, instruction) { this._helperUnary(state, instruction, "inversesqrt"); },
    exp(state, instruction) { this._helperUnary(state, instruction, "exp2"); },
    log(state, instruction) { this._helperUnary(state, instruction, "log2"); },
    frc(state, instruction) { this._helperUnary(state, instruction, "fract"); },
    round_ne(state, instruction) { this._helperUnary(state, instruction, "roundEven"); },
    round_ni(state, instruction) { this._helperUnary(state, instruction, "floor"); },
    round_pi(state, instruction) { this._helperUnary(state, instruction, "ceil"); },
    round_z(state, instruction) { this._helperUnary(state, instruction, "trunc"); },
    rcp(state, instruction)
    {
        ...
        this._assign(state, instruction, `${one} / ${src}`, { saturate: instruction.saturate });
    },
    sincos(state, instruction) { this._sincos(state, instruction); },

    // --- cmp-controlflow ---
    lt(state, instruction) { this._comparison(state, instruction, { intrinsic: "lessThan", operator: "<", as: "float" }); },
    ge(state, instruction) { this._comparison(state, instruction, { intrinsic: "greaterThanEqual", operator: ">=", as: "float" }); },
    eq(state, instruction) { this._comparison(state, instruction, { intrinsic: "equal", operator: "==", as: "float" }); },
    ne(state, instruction) { this._comparison(state, instruction, { intrinsic: "notEqual", operator: "!=", as: "float" }); },
    ilt(state, instruction) { this._comparison(state, instruction, { intrinsic: "lessThan", operator: "<", as: "int" }); },
    ige(state, instruction) { this._comparison(state, instruction, { intrinsic: "greaterThanEqual", operator: ">=", as: "int" }); },
    ieq(state, instruction) { this._comparison(state, instruction, { intrinsic: "equal", operator: "==", as: "int" }); },
    ine(state, instruction) { this._comparison(state, instruction, { intrinsic: "notEqual", operator: "!=", as: "int" }); },
    ult(state, instruction) { this._comparison(state, instruction, { intrinsic: "lessThan", operator: "<", as: "uint" }); },
    uge(state, instruction) { this._comparison(state, instruction, { intrinsic: "greaterThanEqual", operator: ">=", as: "uint" }); },
    and(state, instruction) { this._bitwiseBinary(state, instruction, "&"); },
    or(state, instruction) { this._bitwiseBinary(state, instruction, "|"); },
    xor(state, instruction) { this._bitwiseBinary(state, instruction, "^"); },
    ...
    movc(state, instruction) { this._movc(state, instruction); },
    ...
    {
        const selector = state.formatter.sourceExpression(instruction.operands[0], { destMask: "x", as: "int" });
    ...
    ret(state) { this._line(state, "return;"); },
    ...
    {
        const scalar = state.formatter.sourceExpression(instruction.operands[0], { destMask: "x", as: "int" });
        const test = instruction.testBoolean === "nonzero" ? "!=" : "==";
        this._line(state, `if((${scalar})${test}0){discard;}`);
    },

    // --- integer-conv (corpus-common subset) ---
    iadd(state, instruction) { this._intBinary(state, instruction, "+"); },
    ...
    imax(state, instruction) { this._intMinMax(state, instruction, "max", "int"); },
    imin(state, instruction) { this._intMinMax(state, instruction, "min", "int"); },
    umax(state, instruction) { this._intMinMax(state, instruction, "max", "uint"); },
    umin(state, instruction) { this._intMinMax(state, instruction, "min", "uint"); },
    ishl(state, instruction) { this._intBinary(state, instruction, "<<"); },
    ishr(state, instruction) { this._intBinary(state, instruction, ">>"); },
    ushr(state, instruction) { this._intBinary(state, instruction, ">>", "uint"); },
    ...
    itof(state, instruction)
    {
        ...
        this._assign(state, instruction, `${type}(${src})`, { saturate: instruction.saturate });
    },
    ...
    // --- texture-sample ---
    sample(state, instruction) { this._textureSample(state, instruction); },
    sample_c(state, instruction) { this._textureSample(state, instruction, { comparisonRefOperandIndex: 4 }); },
    sample_c_lz(state, instruction) { this._textureSample(state, instruction, { comparisonRefOperandIndex: 4, forceLodZero: true }); },
    sample_l(state, instruction) { this._textureSample(state, instruction, { lodOperandIndex: 4 }); },
    sample_b(state, instruction) { this._textureSample(state, instruction, { biasOperandIndex: 4 }); },
    sample_d(state, instruction) { this._textureSample(state, instruction, { gradOperandIndexes: [ 4, 5 ] }); },
    ...
    ld(state, instruction) { this._texelFetch(state, instruction); },
    deriv_rtx(state, instruction) { this._helperUnary(state, instruction, "dFdx"); },
    deriv_rtx_coarse(state, instruction) { this._helperUnary(state, instruction, "dFdx"); },
    deriv_rtx_fine(state, instruction) { this._helperUnary(state, instruction, "dFdx"); },
    deriv_rty(state, instruction) { this._helperUnary(state, instruction, "dFdy"); },
    deriv_rty_coarse(state, instruction) { this._helperUnary(state, instruction, "dFdy"); },
    deriv_rty_fine(state, instruction) { this._helperUnary(state, instruction, "dFdy"); },
    resinfo(state, instruction) { this._resinfo(state, instruction); },

    // --- memory-structured ---
    ld_structured(state, instruction) { this._ldStructured(state, instruction); },
    ...
    ibfe(state, instruction)
    {
        ...
        for (const component of mask)
        {
            const width = state.formatter.sourceExpression(instruction.operands[1], { destMask: component, as: "int" });
            const offset = state.formatter.sourceExpression(instruction.operands[2], { destMask: component, as: "int" });
            const src = state.formatter.sourceExpression(instruction.operands[3], { destMask: component, as: "int" });
            const lvalue = this._destComponentRef(state, instruction.operands[0], target, component);
    ...
    ubfe(state, instruction)
    {
        ...
        for (const component of mask)
        {
            const width = state.formatter.sourceExpression(instruction.operands[1], { destMask: component, as: "uint" });
            const offset = state.formatter.sourceExpression(instruction.operands[2], { destMask: component, as: "uint" });
            const src = state.formatter.sourceExpression(instruction.operands[3], { destMask: component, as: "uint" });
            const lvalue = this._destComponentRef(state, instruction.operands[0], target, component);
    ...
    bfi(state, instruction)
    {
        ...
        for (const component of mask)
        {
            const width = state.formatter.sourceExpression(instruction.operands[1], { destMask: component, as: "uint" });
            const offset = state.formatter.sourceExpression(instruction.operands[2], { destMask: component, as: "uint" });
            const insert = state.formatter.sourceExpression(instruction.operands[3], { destMask: component, as: "uint" });
            const base = state.formatter.sourceExpression(instruction.operands[4], { destMask: component, as: "uint" });
            const lvalue = this._destComponentRef(state, instruction.operands[0], target, component);
    ...
    gather4(state, instruction) { this._gather4(state, instruction); },
    ...
    // the slice entirely (the host attaches the target layer).
        ...
        {
            state.warnings.push(
                `store_uav_typed at offset ${instruction.offset} targets a non-float UAV (${returnTypes.join(",")}); output is treated as an RGBA float render target`
            );
        }
...
DxbcGlslEmitter.prototype._ldStructured = function _ldStructured(state, instruction)
{
    ...
    let element = state.formatter.sourceExpression(instruction.operands[1], { destMask: "x", as: "int" });
    ...
    const destOperand = instruction.operands[0];
    const aliasesAddress = addressOperand
        && destOperand
        && addressOperand.type === destOperand.type
        && addressOperand.registerIndex === destOperand.registerIndex;
...
DxbcGlslEmitter.prototype._gather4 = function _gather4(state, instruction)
{
    ...
    this._assign(state, instruction, mask === "xyzw" ? call : `${call}.${mask}`, { saturate: instruction.saturate });
};
