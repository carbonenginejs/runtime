...
export function buildEffectPackage(input, options = {})
{
    ...
    const info = {
        ...
        containerVersion,
        targetBackend: "webgl",
        // The producer identity names the current import surface. PACKAGE_VERSION
        // independently versions the translated artifact semantics.
    ...
    {
        throw new Error(
            `Carbon WebGL target is incomplete; output was not built. ${qualification.errors.join("; ")}`
        );
    }
...
function normalizeLightProfileMode(value)
{
    ...
    {
        throw new TypeError(
            `Carbon WebGL lightProfile must be one of ${LIGHT_PROFILE_MODES.join(", ")}; got "${mode}"`
        );
    }
    return mode;
}
...
function lightProfileEmitterOptions(plan, localLightMode, profileMode)
{
    ...
    return { neutralResourceRegisters: [ plan.profileRegister ] };
}
...
function normalizeLocalLightMode(value)
{
    ...
    {
        throw new TypeError(
            `Carbon WebGL localLights must be one of ${LOCAL_LIGHT_MODES.join(", ")}; got "${mode}"`
        );
    }
    return mode;
}
...
function translateStages(shaderMap, stageMap, values)
{
    const emitInto = (record, pairVaryings) =>
    {
        try
        {
            const profileNeutral = lightProfileEmitterOptions(
                record.localLights,
                values.localLights,
                values.lightProfile
            );
...
function recogniseEmulatedAddressing(samplers, resources)
{
    ...
    for (const entry of resources ?? [])
    {
        ...
        {
            textures.push({ registerIndex });
        }
    }

    if (!Object.keys(samplerModes).length && !textures.length) return null;

    return { samplerModes, ...(textures.length ? { textures } : {}) };
}
...
function normalizeSourceIdentity(values)
{
    ...
    {
        throw new Error(
            "Carbon WebGL sourceIdentity.sha256 does not match the exact effect input bytes"
        );
    }
