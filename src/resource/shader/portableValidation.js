 ...
 */
export function validatePortableEffectReflection(document)
{
  ...
  if (document.format !== EFFECT_BODY_REFLECTION_FORMAT
  ...
  {
    throw new Error(
      "Portable effect body reflection schema or coverage is unsupported"
    );
  }
  ...
  {
    throw new Error(
      "Portable reflection permutationIndex must fit uint32"
    );
  }
  ...
  {
    throw new Error(
      "Portable reflection sourceRecord offset must fit uint32"
    );
  }
  if (!isUint32(sourceRecord.byteLength))
  {
    throw new Error(
      "Portable reflection sourceRecord byteLength must fit uint32"
    );
  }
  ...
  {
    throw new Error(
      "Portable reflection sourceRecord is outside the source envelope"
    );
  }
...
export function isPortableEffectReflection(value)
{
  try
  {
 ...
 */
export function validatePortableEffectInput(
  input,
  context = "standalone input"
)
{
...
function validateSource(source)
{
  ...
  {
    throw new Error(
      "Portable reflection stringTableByteLength must fit uint32"
    );
  }
  if (!isUint32(source.byteLength))
  {
    throw new Error(
      "Portable reflection source byteLength must fit uint32"
    );
  }
  if (!source.byteLength
    || source.stringTableByteLength > source.byteLength
    || source.effectVersion !== PORTABLE_EFFECT_VERSION)
  {
  ...
  {
    throw new Error(
      "Portable reflection compilerVersion must fit uint32"
    );
  }
  if (!(source.nativeHash instanceof Uint8Array)
    || source.nativeHash.byteLength !== 32)
  {
...
{
  if (!Array.isArray(pass.renderStates)
    || !Array.isArray(pass.stages)
  ...
  validateRegisterIndexed(
    ...
    entry =>
    {
      ...
      {
        throw new Error(
          `pass ${pass.key} render-state value must fit uint32`
        );
      }
    }
  );
  ...
  for (const stage of pass.stages)
  {
    ...
    {
      throw new Error(
        `pass ${pass.key} stage type must fit uint32`
      );
    }
    if (stage.stageType >= STAGE_NAMES.length
      || stageTypes.has(stage.stageType))
    {
      throw new Error(
        `Portable reflection pass ${pass.key} stage type is invalid or duplicated`
      );
    }
...
function validateLibrary(library, source)
{
  ...
  {
    throw new Error(
      `library ${library.key} payloadSize must fit uint32`
    );
  }
  validateSourceProgram(
  ...
  for (const [ index, entry ] of library.exports.entries())
  {
    ...
    {
      throw new Error(
        `library ${library.key} export ${index} type must fit uint8`
      );
    }
...
)
{
  ...
  {
    throw new Error(
      "Portable reflection source program stage identity disagrees"
    );
  }
  if (!isUint32(program.shaderSize))
  {
    throw new Error(
      "Portable reflection source program shaderSize must fit uint32"
    );
  }
  if (!(program.bytes instanceof Uint8Array)
    || !program.bytes.byteLength
    || program.shaderSize !== program.bytes.byteLength)
  {
  ...
  {
    throw new Error(
      "Portable reflection source program stringTableOffset must fit uint32"
    );
  }
  if (program.stringTableOffset + program.shaderSize > stringTableByteLength)
  {
    throw new Error(
      "Portable reflection source program is outside the shared string table"
    );
  }
}
...
function validateInput(input, context)
{
  ...
  {
    throw new Error(
      `Portable reflection ${context} constant defaults must be an object`
    );
  }
  const defaults = input.constantDefaults;
  requireExactKeys(
    defaults,
    [ "declaredByteLength", "bytes" ],
    `Portable reflection ${context} constant defaults`
  );
  if (!isUint32(defaults.declaredByteLength))
  {
    throw new Error(
      `Portable reflection ${context} constant-default length must fit uint32`
    );
  }
  if (!(defaults.bytes instanceof Uint8Array)
    || defaults.declaredByteLength !== defaults.bytes.byteLength
    || defaults.bytes.byteLength > CONSTANT_BYTES_MAX)
  {
    throw new Error(
      `Portable reflection ${context} constant defaults are invalid`
    );
  }
  ...
  {
    throw new Error(
      `Portable reflection ${context} input collections are malformed`
    );
  }
  ...
  for (const [ index, constant ] of input.constants.entries())
  {
    ...
    {
      throw new Error(
        `${context} constant ${constant.name} offset must fit uint32`
      );
    }
    if (!isUint32(constant.size))
    {
      throw new Error(
        `${context} constant ${constant.name} size must fit uint32`
      );
    }
    if (!isUint8(constant.type))
    {
      throw new Error(
        `${context} constant ${constant.name} type must fit uint8`
      );
    }
    if (!isUint8(constant.dimension))
    {
      throw new Error(
        `${context} constant ${constant.name} dimension must fit uint8`
      );
    }
    if (!isUint32(constant.elements))
    {
      throw new Error(
        `${context} constant ${constant.name} elements must fit uint32`
      );
    }
    if (!constant.size
      || constant.type > 4
    ...
    {
      throw new Error(
        `Portable reflection ${context} constant ${constant.name} is invalid`
      );
    }
  }
...
function validateSamplerList(entries, context)
{
  validateRegisterIndexed(entries, "registerIndex", context, entry =>
  {
    ...
    if (typeof entry.isDynamic !== "boolean"
      || (entry.isDynamic && typeof entry.name !== "string")
...
function validateSignature(signature, context)
{
  ...
  for (const [ index, input ] of signature.pipelineInputs.entries())
  {
    ...
    ])
    {
      ...
      {
        throw new Error(
          `${context} pipeline input ${index} ${field} must fit uint8`
        );
      }
    }
  }
  ...
  for (const [ index, register ] of signature.registers.entries())
  {
    ...
    {
      throw new Error(
        `${context} register ${index} type must fit uint8`
      );
    }
    if (!isUint32(register.registerIndex))
    {
      throw new Error(
        `${context} register ${index} index must fit uint32`
      );
    }
    if (!isUint32(register.arrayCount))
    {
      throw new Error(
        `${context} register ${index} arrayCount must fit uint32`
      );
    }
    if (!isUint32(register.registerCount))
    {
      throw new Error(
        `${context} register ${index} registerCount must fit uint32`
      );
    }
    if (!isUint8(register.registerSpace))
    {
      throw new Error(
        `${context} register ${index} space must fit uint8`
      );
    }
    const classification = registerClass(register.registerType);
    const unbounded = register.arrayCount === 0 && register.registerCount === 0;
    const rangeEnd = unbounded
      ? UINT32_MAX + 1
    ...
    {
      throw new Error(
        `Portable reflection ${context} register ${index} is malformed`
      );
    }
    ...
    {
      throw new Error(
        `Portable reflection ${context} signature register range overlaps`
      );
    }
  ...
  validateRegisterIndexed(
    ...
    entry =>
    {
      ...
      {
        throw new Error(
          `${context} static sampler space must fit uint8`
        );
      }
  ...
  {
    throw new Error(
      `Portable reflection ${context} threadGroupSize must be an object`
    );
  }
  ...
  for (const field of [ "x", "y", "z" ])
  {
    ...
    {
      throw new Error(
        `${context} threadGroupSize.${field} must fit uint32`
      );
    }
  }
}
...
function validateMapSignatureReconciliation(input, context)
{
  validateMappedBindings(
    input.resources,
    input.signature.registers.filter(entry =>
  ...
  for (const sampler of input.samplers)
  {
    const matches = sampler.isDynamic
      ...
      : input.signature.staticSamplers.filter(entry =>
        entry.registerIndex === sampler.registerIndex);
    if (matches.length > 1
      || (matches.length === 1
        && sampler.isDynamic
...
function validateSamplerDescriptor(descriptor, context, staticSampler)
{
  ...
  {
    throw new Error(
      `Portable reflection ${context} sampler comparison is malformed`
    );
  }
  for (const field of [
    "minFilter",
  ...
  ])
  {
    ...
    {
      throw new Error(
        `${context} sampler ${field} must fit uint8`
      );
    }
  }
  for (const field of [ "mipLODBiasRaw", "minLODRaw", "maxLODRaw" ])
  {
    ...
    {
      throw new Error(
        `${context} sampler ${field} must fit uint32`
      );
    }
  }
  if (staticSampler)
  {
    ...
    {
      throw new Error(
        `${context} static sampler borderColor must fit uint8`
      );
    }
  }
  ...
  {
    throw new Error(
      `Portable reflection ${context} sampler borderColorRaw is malformed`
    );
  }
  else
  {
    for (const [ index, value ] of descriptor.borderColorRaw.entries())
    {
      ...
      {
        throw new Error(
          `${context} sampler borderColorRaw[${index}] must fit uint32`
        );
      }
    }
  }
...
function validateAnnotationGroups(groups, context)
{
  ...
  for (const [ index, group ] of groups.entries())
  {
    ...
    if (typeof group.parameterName !== "string"
...
function validateAnnotations(annotations, context)
{
  for (const [ index, annotation ] of annotations.entries())
  {
    ...
    {
      throw new Error(
        `Portable reflection ${context} annotation ${index} is malformed`
      );
    }
    if (!isUint8(annotation.type))
    {
      throw new Error(
        `${context} annotation ${index} type must fit uint8`
      );
    }
    if (annotation.type === 3)
    {
      ...
      {
        throw new Error(
          `Portable reflection ${context} string annotation is malformed`
        );
      }
    }
    else if (annotation.type <= 2)
    {
      ...
      {
        throw new Error(
          `${context} annotation ${index} rawValue must fit uint32`
        );
      }
    }
    else
    {
      throw new Error(
        `Portable reflection ${context} annotation type is unsupported`
      );
    }
  }
}

function validateRegisterIndexed(
  entries,
  field,
  context,
  validate,
...
)
{
  ...
  for (const [ index, entry ] of entries.entries())
  {
    ...
    {
      throw new Error(
        `Portable reflection ${context} entry ${index} must be an object`
      );
    }
    if (!isUint32(entry[field]))
    {
      throw new Error(
        `${context} entry ${index} ${field} must fit uint32`
      );
    }
    ...
    {
      throw new Error(
        `Portable reflection ${context} entry ${index} is duplicated`
      );
    }
    identities.add(identity);
    validate(entry);
...
function requireKey(value, expected, context, keys)
{
  ...
  {
    throw new Error(
      `Portable reflection ${context} key is malformed or duplicated`
    );
  }
  keys.add(value);
}
