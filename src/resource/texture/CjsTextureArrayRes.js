...
export class CjsTextureArrayRes extends CjsResource
{
  ...
  SetLayerResourcePaths(values, names = null)
  {
    ...
    const paths = values.map(value => normalizeResourcePath(
      value && typeof value === "object"
        ? value.resourcePath ?? value.path ?? value.GetPath?.() ?? ""
        : value
    ));
  ...
  Update(context = {})
  {
    ...
    return this.#updateHandler
      ? this.#updateHandler(this, request, context)
      : request;
  }
  ...
  Ready()
  {
    ...
    {
      this.#readyWaiters.push({ revision, resolve, reject });
    });
  }
  ...
  #CreateLayer(layer, name = "", path = "")
  {
    return {
      ...
      resource: null,
      proxy: new CjsTextureArrayResParameterProxy(this, layer, { name })
    };
  }
  ...
  #ResolveReadyWaiters(revision)
  {
    ...
    {
      if (waiter.revision <= revision) waiter.resolve(this);
