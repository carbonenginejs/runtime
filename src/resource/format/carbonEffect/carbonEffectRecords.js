 ...
 */
export const CARBON_SHADER_CONSTANTS_MAX = 4096;
 ...
 */
export function internArena(table)
{
    return {
        /**
 ...
 */
export function collectArena(table)
{
    return {
        /**
...
function readSignature(reader, version, stageType)
{
    ...
    return { registers, staticSamplers };
}
...
function readPass(reader, version, makeError, backend)
{
    ...
    return { stages, renderStates, ...(backendBlock ? { backendBlock } : {}) };
}
 ...
 */
export function readEffectDescription(reader, options = {})
{
    const makeError = options.makeError
        ?? ((message, details) => reader._error(message, details));
    const backend = options.backend === true;
    const version = Number.isInteger(options.version)
        ? options.version
        : CARBON_EFFECT_DATA_VERSION;
    if (version < CARBON_EFFECT_MIN_DATA_VERSION || version > CARBON_EFFECT_DATA_VERSION)
    {
        throw makeError(
            `Unsupported Carbon effect version ${version}; expected ${CARBON_EFFECT_MIN_DATA_VERSION}..${CARBON_EFFECT_DATA_VERSION}`,
            { version }
        );
    }
    ...
    for (let techniqueIndex = 0; techniqueIndex < techniqueCount; techniqueIndex += 1)
    {
        ...
        techniques.push({ name, passes, libraries });
    }
