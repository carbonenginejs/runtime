...
function ReadRenderJoints(values)
{
    ...
    return values.map((value, index) =>
    {
        ...
        return {
            name,
            inverseWorldTransform: ReadMatrix(
                value.inverseWorldTransform,
                `render joint "${name}" inverse world transform`
            )
        };
    });
}
...
function WritePaletteMatrix(palette, index, value)
{
    ...
    if (!Number.isFinite(value[0])
        ...
        || !Number.isFinite(value[8])
        || !Number.isFinite(value[9])
        || !Number.isFinite(value[10])
        || !Number.isFinite(value[12])
        || !Number.isFinite(value[13])
        || !Number.isFinite(value[14]))
    {
...
{
    return left.length === right.length && left.every((joint, index) =>
        joint.name === right[index].name
        && MatrixEquals(joint.inverseWorldTransform, right[index].inverseWorldTransform));
}
