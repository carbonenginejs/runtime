...
| `EveInstancedMeshManager.js` | Native scene-owned instancing manager with GPU buffers, allocator state, picking state, and nested C++ records; it has no `BLUE_CLASS` declaration or persisted graph identity. | Not ported: Carbon owns the instancing manager in the scene, fed by Trinity instance data. |
