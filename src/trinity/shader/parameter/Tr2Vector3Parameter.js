...
export class Tr2Vector3Parameter extends withITriReroutable(CjsVectorParameter)
{
  ...
  // Carbon returns `const Vector3&` - a reference the compiler forbids writing
  // through (Tr2Vector3Parameter.cpp). JavaScript has no const reference,
  ...
  /**
   * Writes the three components an upload should use into the caller's
   ...
   * gamma-to-linear on all three components.
   */
  #updateLinearValue()
  {
    if (!this.isSrgb)
