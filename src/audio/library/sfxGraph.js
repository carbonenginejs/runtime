...
const BUS_VOICE_VOLUME_ACTION_KINDS = new Set([
    "set-bus-voice-volume",
]);
const BUS_VOLUME_ACTION_KINDS = new Set([
    "reset-bus-volume",
    "set-bus-volume",
]);
 ...
 */
export function validateSfxGraph(
    graph,
    media = {},
    embeddedMedia = {},
)
{
    ...
    {
        throw new TypeError(
            `Unsupported audio SFX schema version: ${graph.schemaVersion}`,
        );
    }

    const events = RequireRecord(
        graph.events,
        "Audio library SFX events",
    );
    const nodes = RequireRecord(
        graph.nodes,
        "Audio library SFX nodes",
    );
    const programs = graph.programs === undefined
        ? {}
        : RequireRecord(
            graph.programs,
    ...
    for (const [ rawID, node ] of Object.entries(nodes))
    {
        ...
        {
            throw new TypeError(
                `Audio library SFX node ${id} has unsupported type ${node.type}`,
            );
        }

        ValidateGain(node, `Audio library SFX node ${id}`);
        ValidateNodePlaybackProperties(
            node,
            ...
            || node.type === "timed-silence")
        {
            ...
            const mediaID = NormalizePositiveID(
                node.mediaId,
                `Audio library SFX sound ${id} mediaId`,
            );

            if (!media[mediaID] && !embeddedMedia[mediaID])
            {
                throw new TypeError(
                    `Audio library SFX sound ${id} references missing source ${mediaID}`,
                );
            }
        ...
        if (node.type === "switch")
        {
            NormalizeName(
                node.group,
                `Audio library SFX switch ${id} group`,
            );

            if (!SWITCH_SCOPES.has(node.scope ?? "switch"))
            {
                throw new TypeError(
                    `Audio library SFX switch ${id} scope must be switch or state`,
                );
            }

            const cases = RequireRecord(
                node.cases,
                `Audio library SFX switch ${id} cases`,
            );

            if (!Object.keys(cases).length)
            {
                throw new TypeError(
                    `Audio library SFX switch ${id} must have at least one case`,
                );
            }
            ...
            for (const [ name, child ] of Object.entries(cases))
            {
                const normalizedName = NormalizeName(
                    name,
                    `Audio library SFX switch ${id} case`,
                );
                ...
                {
                    throw new TypeError(
                        `Audio library SFX switch ${id} has duplicate case ${name}`,
                    );
                }
                ...
                ValidateChild(
                    child,
                    nodes,
                    `Audio library SFX switch ${id} case ${name}`,
                );
            }
        ...
        {
            throw new TypeError(
                `Audio library SFX ${node.type} ${id} must have children`,
            );
        }
        ...
        for (let index = 0; index < node.children.length; index++)
        {
            ...
            ValidateChild(
                child,
                nodes,
                `Audio library SFX ${node.type} ${id} child ${index}`,
                node.type === "random",
            );
        ...
        if ((node.type === "random" || node.type === "sequence")
            && !CONTAINER_SCOPES.has(node.scope ?? "object"))
        {
            throw new TypeError(
                `Audio library SFX ${node.type} ${id} scope must be object or global`,
            );
        }
        if (node.type === "random"
            && !RANDOM_MODES.has(node.mode ?? "random"))
        {
            throw new TypeError(
                `Audio library SFX random ${id} mode must be random or shuffle`,
            );
        }
        if (node.type === "sequence"
            && node.loop !== undefined
            && typeof node.loop !== "boolean")
        {
            throw new TypeError(
                `Audio library SFX sequence ${id} loop must be boolean`,
            );
        }
        if ((node.type === "random" || node.type === "sequence")
            && node.continuous !== undefined)
        {
            ValidateContinuousContainer(
                node.continuous,
                `Audio library SFX ${node.type} ${id} continuous`,
                node.type,
            );
        }
    }
    ...
    for (const [ eventName, roots ] of Object.entries(events))
    {
        ...
        {
            throw new TypeError(
                `Audio library SFX event ${eventName} must have roots`,
            );
        }
        ...
        for (let index = 0; index < roots.length; index++)
        {
            ValidateChild(
                roots[index],
                nodes,
                `Audio library SFX event ${eventName} root ${index}`,
            );
        }
    }
    ...
    for (const [ eventName, actions ] of Object.entries(programs))
    {
        ...
        {
            throw new TypeError(
                `Audio library SFX program ${eventName} must have actions`,
            );
        }
        ...
        for (let index = 0; index < actions.length; index++)
        {
            const action = RequireRecord(
                actions[index],
                `Audio library SFX program ${eventName} action ${index}`,
            );
        ...
        const projected = actions
            ...
            .map(action => NormalizeChild(action.child));
        const roots = events[eventName] ?? [];

        if (!ChildrenEqual(projected, roots.map(NormalizeChild)))
        {
            throw new TypeError(
                `Audio library SFX event ${eventName} roots must equal its ordered Play projection`,
            );
        }
    }
...
function ValidateBusVoiceVolumeTargets(programs, nodes)
{
    const targets = new Set(
        Object.values(programs).flatMap(actions => actions
    ...
    for (const targetId of targets)
    {
        ...
        for (const node of Object.values(nodes))
        {
            ...
            {
                throw new TypeError(
                    `Audio library SFX Bus-target Voice Volume ${targetId} must be every affected Sound's output Bus`,
                );
            }
        }
        if (!used)
        {
            throw new TypeError(
                `Audio library SFX Bus-target Voice Volume ${targetId} has no affected Sound`,
            );
        }
    }
}
 ...
 */
export function normalizeSfxGraph(graph, media = {}, embeddedMedia = {})
{
    ...
    const result = {
        ...
        generator: String(
            graph.generator
            ?? "@carbonenginejs/runtime/audio/library-builder",
        ),
        events,
        nodes,
    };
...
function NormalizeNode(node)
{
    ...
    if ((node.type === "random" || node.type === "sequence")
        && node.continuous !== undefined)
    {
        result.continuous = NormalizeContinuousContainer(
            node.continuous,
            node.type,
        );
    }
...
function ValidateContinuousContainer(value, label, type)
{
    ...
    {
        throw new TypeError(
            `${label} loopCount must not exceed 32767`,
        );
    }
    ...
    {
        if (continuous.transitionMs !== undefined
            || continuous.transitionRangeMs !== undefined)
        {
            throw new TypeError(
                `${label} disabled transition cannot define timing`,
            );
        }
    }
    else
    ...
    if (type === "sequence"
        && continuous.resetPlaylistEachPlay !== undefined
        && typeof continuous.resetPlaylistEachPlay !== "boolean")
    {
        throw new TypeError(
            `${label} resetPlaylistEachPlay must be boolean`,
        );
    }
    if (type === "random"
        && continuous.resetPlaylistEachPlay !== undefined)
    {
        throw new TypeError(
            `${label} resetPlaylistEachPlay is sequence-only`,
        );
    }
}
...
function NormalizeGain(value)
{
    ...
    {
        result.gainDbRanges = NormalizeRandomRanges(
            value.gainDbRanges,
        );
    }
    ...
    return {
        stateProperties: value.stateProperties.map(property => ({
            ...
            cases: Object.fromEntries(
                Object.keys(property.cases).sort().map(name => [
                    name,
                    {
...
function ValidateBusVoiceVolumeAction(value, label)
{
    ...
    {
        throw new TypeError(
            `${label} kind must be set-bus-voice-volume`,
        );
    }
    ...
    if (action.volumeRangeDb !== undefined)
    {
        ...
        {
            throw new TypeError(
                `${label} volumeRangeDb must be zero`,
            );
        }
    }
    if (action.probability !== undefined)
    ...
    if (action.delayRangeMs !== undefined
        || action.transitionRangeMs !== undefined)
    {
...
function ValidateBusVolumeAction(value, label)
{
    ...
    {
        throw new TypeError(
            `${label} kind must be set-bus-volume or reset-bus-volume`,
        );
    }
    if (!PLAYBACK_CONTROL_SCOPES.has(action.scope))
    {
    ...
    if ((setting && action.mode !== "element")
        ...
        || (action.scope === "game-object" && action.mode !== "element"))
    {
    ...
    for (let index = 0; index < action.exceptions.length; index++)
    {
        ...
        {
            throw new TypeError(
                `${label} exception ${index} targetFlags must be 1`,
            );
        }
    }
    ...
    if (setting)
    {
        ...
        if (action.busVolumeRangeDb !== undefined)
        {
            const range = RequireRecord(
                ...
                `${label} busVolumeRangeDb`,
            );
            const min = ValidateVoiceVolumeDb(
                range.min,
                `${label} busVolumeRangeDb min`,
            );
            const max = ValidateVoiceVolumeDb(
                range.max,
                `${label} busVolumeRangeDb max`,
            );

            if (min > max)
            {
                throw new TypeError(
                    `${label} busVolumeRangeDb min must not exceed max`,
                );
            }
            ValidateVoiceVolumeDb(
                Number(action.busVolumeDb) + min,
                `${label} minimum randomized busVolumeDb`,
            );
            ValidateVoiceVolumeDb(
                Number(action.busVolumeDb) + max,
                `${label} maximum randomized busVolumeDb`,
            );
        }
    }
    else if (action.valueMode !== undefined
        || action.busVolumeDb !== undefined
        || action.busVolumeRangeDb !== undefined)
    {
...
function ValidatePlaybackControlAction(value, label)
{
    ...
    {
        throw new TypeError(
            `${label} element targetId must be greater than zero`,
        );
    }
    if (action.mode !== "element" && targetID !== "0")
    {
    ...
    if (action.targetFlags !== undefined)
    {
        ...
        {
            throw new TypeError(
                `${label} bus targets are unsupported`,
            );
        }
    }
    if (action.actionFlags !== undefined)
    {
        const actionFlags = NormalizeByte(
            action.actionFlags,
            `${label} actionFlags`,
        );
    ...
    for (let index = 0; index < action.exceptions.length; index++)
    {
        ...
        {
            throw new TypeError(
                `${label} has duplicate exception ${id}`,
            );
        }
        seen.add(id);
        if (exception.targetFlags !== undefined)
        {
            ...
            {
                throw new TypeError(
                    `${label} bus exceptions are unsupported`,
                );
            }
        }
    }
...
function ValidateMatchIds(value, nodeID, label)
{
    ...
    const normalized = value.map((entry, index) =>
        NormalizePositiveID(entry, `${label} ${index}`));
...
function ValidateBusPathIds(value, outputBusId, label)
{
    ...
    {
        throw new TypeError(
            `${label} must begin with output bus ${outputBusId}`,
        );
    }
    if (new Set(normalized).size !== normalized.length)
    {
...
function NormalizeBusVolumeAction(action)
{
    const result = {
        ...
        targetId: NormalizeUnsignedID(
            action.targetId,
            "Audio library SFX Bus Volume targetId",
        ),
...
function ValidateActionTiming(value, label)
{
    ...
    ])
    {
        if (value[field] !== undefined)
        {
            ...
            {
                throw new TypeError(
                    `${label} ${description} must be non-negative`,
                );
            }
        }
    }
    ...
    for (const field of [ "delayRangeMs", "fadeInRangeMs" ])
    {
        ...
        {
            throw new TypeError(
                `${label} ${field} max must be at least min`,
            );
        }
    }
    ...
    if (value.probability !== undefined)
    {
        ...
        {
            throw new TypeError(
                `${label} probability must be between 0 and 100`,
            );
        }
    }
    if (value.fadeCurve !== undefined
...
function ValidateGain(value, label)
{
    ...
    {
        const curve = RequireRecord(
            value.gainCurves[index],
            `${label} gain curve ${index}`,
        );
        ...
        {
            throw new TypeError(
                `${label} gain curve ${index} scope must be object or global`,
            );
        }
        if (!Array.isArray(curve.points) || !curve.points.length)
        {
            throw new TypeError(
                `${label} gain curve ${index} must have points`,
            );
        }
            ...
            pointIndex++)
        {
            ...
            if (hasGainDb === hasGain)
            {
                throw new TypeError(
                    `${label} gain curve ${index} point ${pointIndex}`
                    + " must have exactly one of gainDb or gain",
                );
            }
            ...
            {
                throw new TypeError(
                    `${label} gain curve ${index} points must use one gain unit`,
                );
            }
            ...
            if (point.interpolation !== undefined
                && (!Number.isSafeInteger(Number(point.interpolation))
                    || Number(point.interpolation) < 0
                    || Number(point.interpolation) > 9))
            {
                throw new TypeError(
                    `${label} gain curve ${index} point ${pointIndex}`
                    + " interpolation must be a Wwise curve value from 0 to 9",
                );
            }
            ...
            {
                throw new TypeError(
                    `${label} gain curve ${index} points must have non-decreasing x`,
                );
            }
            previous = x;
        }
...
function ValidateRtpcCurves(value, label)
{
    ...
    for (let index = 0; index < value.length; index++)
    {
        ...
        {
            throw new TypeError(
                `${label} ${index} scope must be object or global`,
            );
        }
        ...
        {
            throw new TypeError(
                `${label} ${index} scaling must be ${expectedScaling}`
                + ` for ${property}`,
            );
        }
        if (curve.defaultValue !== undefined)
            ...
            pointIndex++)
        {
            ...
            {
                throw new TypeError(
                    `${label} ${index} points must have non-decreasing x`,
                );
            }
            previous = x;
        }
...
function ValidateStateProperties(value, label)
{
    ...
    for (let index = 0; index < value.length; index++)
    {
        ...
        {
            throw new TypeError(
                `${label} ${index} cases must not be empty`,
            );
        }
        ...
        for (const [ name, rawCase ] of Object.entries(cases))
        {
            ...
            {
                throw new TypeError(
                    `${label} ${index} has duplicate case ${name}`,
                );
            }
            normalizedNames.add(normalizedName);
            const stateCase = RequireRecord(
                rawCase,
...
function ValidateRandomRanges(value, label)
{
    ...
    for (let index = 0; index < value.length; index++)
    {
        ...
        {
            throw new TypeError(
                `${label} ${index} max must be greater than or equal to min`,
            );
        }
    }
}
...
function ValidateAcyclic(events, nodes)
{
    ...
    const visit = (rawID) =>
    {
        ...
        {
            throw new TypeError(
                `Audio library SFX graph contains a cycle at node ${id}`,
            );
        }
        if (visited.has(id))
        {
...
function NormalizePositiveID(value, label)
{
    ...
    {
        throw new TypeError(
            `${label} must be an unsigned 32-bit integer greater than zero`,
        );
    }
