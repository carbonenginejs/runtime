...
import { CjsBnkFormat } from "#resource/formats/bnk";
import { normalizeBusGraphCatalog } from "../internal/busGraph.js";
import {
    createAudioResourceReader,
    decodeAudioResourceJson,
    decodeAudioResourceText,
...
const SFX_VOICE_FILTER_ACTION_FORMS = new Map([
    [ 0x02, { scope: "global", mode: "element" } ],
    [ 0x03, { scope: "game-object", mode: "element" } ],
    [ 0x04, { scope: "global", mode: "all" } ],
...
export class CjsAudioLibraryBuilder
{
    ...
    static async buildFromResources(options = {})
    {
        ...
        const registry = new fsdModule.CjsFsd64Reader()
        ...
        const metadata = options.metadata ?? await fsdModule.CjsFsdFormat.readJSON(
            await read(metadataPath, {
                kind: "audioMetadata",
        ...
        const includeMusic = options.music === undefined
            ? AUTO_MUSIC_BANK_NAMES.every(name => availableBankNames.has(name))
            : options.music;
        const values = await this.buildFromBanks({
            ...
            music: includeMusic,
            loadBank: options.loadBank ?? (async (bank, context = {}) =>
                read(bank.resPath, {
                    ...context,
                    kind: "audioBank",
                    bank,
     ...
     *
     * The resource layer owns HIRC decoding. This method only names and lowers
        ...
        for (const inspection of inspections)
        {
            for (const entry of inspection.hirc ?? [])
            {
                if (EFFECT_HIRC_TYPES.has(entry.type))
                {
                    effectVersions.set(
                        Number(entry.id) >>> 0,
                        Number(inspection.bankVersion) >>> 0,
                    );
                }
            }
        }
        const effects = new Map([ ...parsedEffects.effects ]
            .map(([ id, effect ]) => [
                id,
                {
                    ...effect,
    ...
    static async buildFromBanks(options = {})
    {
        ...
        const musicBusIds = includeMusic && busCatalog
            ...
            : new Set();
        const sfxBusIds = includeSfx && busCatalog
            ? CreateSfxRouteBusIds(graphInspections, busCatalog)
            : new Set();
        ...
        const busDucking = busCatalog
            ...
            : null;
        const busEffects = busCatalog
            ? CreateBusEffectCatalog(
                graphInspections,
                busCatalog,
                routedBusIds,
            )
            : null;
        const busGraph = busCatalog && routedBusIds.size
            ? CreateBusGraphCatalog(
                graphInspections,
                includeMusic
                    ? inspections.filter(inspection =>
                        MUSIC_BANK_NAMES.includes(
    ...
    static build(options = {})
    {
        ...
        if (busEffects !== null
            && Object.keys(busEffects.buses ?? {}).length)
        {
...
async function ResolveSoundbanksInfo(options, indexEntries, read, signal)
{
    ...
    return decodeAudioResourceJson(
        await read(entry.logicalPath, {
            kind: "soundbanksInfo",
            signal,
        }),
        entry.logicalPath,
...
class CjsAudioLibraryBuilderSfxEventLoweringSession
{
    ...
    #ApplyAuthoredActions(event, result)
    {
        for (const actionID of event.actionIds)
        {
            ...
            ].includes((action.actionType >> 8) & 0xff))
            {
                ...
                === SFX_SET_VOICE_PITCH_ACTION_FAMILY
                || ((action.actionType >> 8) & 0xff)
                === SFX_RESET_VOICE_PITCH_ACTION_FAMILY)
            {
                ...
                === SFX_SET_VOICE_VOLUME_ACTION_FAMILY
                || ((action.actionType >> 8) & 0xff)
                === SFX_RESET_VOICE_VOLUME_ACTION_FAMILY)
            {
                ...
                === SFX_RESET_BUS_VOLUME_ACTION_FAMILY)
            {
                ...
                SFX_SET_VOICE_HIGH_PASS_ACTION_FAMILY,
                SFX_RESET_VOICE_HIGH_PASS_ACTION_FAMILY,
            ].includes((action.actionType >> 8) & 0xff))
            {
                ...
                === SFX_SET_GAME_PARAMETER_ACTION_FAMILY
                || ((action.actionType >> 8) & 0xff)
    ...
    #ApplyPlayEvent(result, action)
    {
        ...
        if (hasTiming
            && nested.program.some(value => value.kind !== "play"))
        {
            throw new Error(
...
function ReadSfxVoiceVolumeAction(action, parsed, buses)
{
    ...
    if (!details
        || (family !== SFX_SET_VOICE_VOLUME_ACTION_FAMILY
            && family !== SFX_RESET_VOICE_VOLUME_ACTION_FAMILY))
    {
    ...
    if (details.actionMode !== "element"
        || (details.actionScope !== "game-object"
            && details.actionScope !== "global"))
    {
        throw new Error(
            `unsupported Voice Volume target mode ${action.id}`,
        );
    }
    if (!targetId)
    {
        throw new Error(
            `unresolved Voice Volume target ${targetId}`,
        );
    }
    ...
    if (targetsBus
        && (details.actionScope !== "game-object"
            || targetFlags !== 1
            || details.targetIsBus !== true
            || buses?.get(targetId)?.type !== "audio-bus"))
    {
    ...
    {
        throw new Error(
            `unsupported Voice Volume target flags ${targetFlags}`,
        );
    }
    ...
    if (targetsBus
        && (resetting
            || details.valueMode !== "absolute"
            || Number(details.volumeRangeDb?.min ?? 0) !== 0
            || Number(details.volumeRangeDb?.max ?? 0) !== 0
            || details.delayRangeMs !== undefined
            || details.transitionRangeMs !== undefined
            || details.probability !== undefined
            || (details.exceptions?.length ?? 0) !== 0
            || !BusVoiceTargetIsOutputOnly(parsed, buses, targetId)))
    {
        throw new Error(`unsupported bus Voice Volume action ${action.id}`);
    }
    const result = {
    ...
    if (!resetting)
    {
        ...
        {
            throw new Error(
                `unsupported Voice Volume value mode ${action.id}`,
            );
        }
...
function BusVoiceTargetIsOutputOnly(parsed, buses, targetId)
{
    ...
    for (const [ nodeId, node ] of parsed.nodes ?? [])
    {
        ...
        for (const busId of path)
        {
            ...
            if (aux?.hasAux
                || aux?.auxIds?.some(id => Number(id) !== 0)
                || Number(aux?.reflectionsAuxBusId ?? 0) !== 0)
            {
                return false;
            }
        }
...
function ReadSfxVoicePitchAction(action, parsed)
{
    ...
    if (!details
        || (family !== SFX_SET_VOICE_PITCH_ACTION_FAMILY
            && family !== SFX_RESET_VOICE_PITCH_ACTION_FAMILY))
    {
    ...
    {
        throw new Error(
            `unsupported Voice Pitch target mode ${action.id}`,
        );
    }
    if (!targetId)
    {
        throw new Error(
            `unresolved Voice Pitch target ${targetId}`,
        );
    }
    ...
    {
        throw new Error(
            `unsupported Voice Pitch target flags ${targetFlags}`,
        );
    }
    ...
    if (!resetting)
    {
        ...
        {
            throw new Error(
                `unsupported Voice Pitch value mode ${action.id}`,
            );
        }
        result.valueMode = details.valueMode;
...
function ReadSfxVoiceFilterAction(action, parsed)
{
    ...
    const actionType = Number(action.actionType) >>> 0;
    const family = (actionType >> 8) & 0xff;
    const lowPass = family === SFX_SET_VOICE_LOW_PASS_ACTION_FAMILY
        || family === SFX_RESET_VOICE_LOW_PASS_ACTION_FAMILY;
    const setting = family === SFX_SET_VOICE_LOW_PASS_ACTION_FAMILY
        || family === SFX_SET_VOICE_HIGH_PASS_ACTION_FAMILY;
    const resetting = family === SFX_RESET_VOICE_LOW_PASS_ACTION_FAMILY
        || family === SFX_RESET_VOICE_HIGH_PASS_ACTION_FAMILY;
    const property = lowPass ? "lowPass" : "highPass";
    const rangeField = `${property}Range`;
    const kind = `${setting ? "set" : "reset"}-voice-${
        lowPass ? "low-pass" : "high-pass"
    }`;

    if (!details || (!setting && !resetting)
        || details.actionName !== kind)
    {
        throw new Error(`untyped Voice Filter action ${action.id}`);
    }
    const actionForm = SFX_VOICE_FILTER_ACTION_FORMS.get(
        actionType & 0xff,
    );

    if (!actionForm
        || (setting && actionForm.mode !== "element"))
    {
    ...
    if (details[wrongProperty] !== undefined
        || details[`${wrongProperty}Range`] !== undefined)
    {
        throw new Error(
            `Voice Filter action ${action.id} carries ${wrongProperty}`,
        );
    }
    if (details.probability !== undefined)
    {
    ...
    for (const field of [ "properties", "ranges" ])
    {
        ...
        {
            throw new Error(
                `invalid Voice Filter ${field} ${action.id}`,
            );
        }
    }
    if (!Array.isArray(details.exceptions))
    ...
    const targetId = NormalizeWwiseUint32(
        details.targetId,
        `Voice Filter action ${action.id} targetId`,
    );
    const shallowTargetId = NormalizeWwiseUint32(
        action.targetId,
        `Voice Filter action ${action.id} shallow targetId`,
    );
    const targetFlags = Number(details.targetFlags ?? 0);
    ...
    {
        throw new Error(
            `Voice Filter target mismatch ${action.id}`,
        );
    }
    if (mode !== "element" && targetId !== 0)
    {
        throw new Error(
            `Voice Filter ${mode} target must be 0 ${action.id}`,
        );
    }
    if (details.targetIsBus || (targetFlags & 0x01))
    {
    ...
    {
        throw new Error(
            `unsupported Voice Filter target flags ${targetFlags}`,
        );
    }
    if (mode !== "all-except" && exceptions.length)
    {
        throw new Error(
            `unexpected Voice Filter exceptions ${action.id}`,
        );
    }
    ...
    const normalizedExceptions = exceptions.map(exception =>
    {
        ...
        const exceptionId = NormalizeWwiseUint32(
            exception.targetId,
            `Voice Filter action ${action.id} exception targetId`,
        );
        ...
        {
            throw new Error(
                `invalid Voice Filter exception ${action.id}`,
            );
        }
        exceptionIds.add(exceptionId);
    ...
    if (setting)
    {
        ...
        {
            throw new Error(
                `unsupported Voice Filter value mode ${action.id}`,
            );
        }
        result.valueMode = details.valueMode;
        ...
        || details.lowPass !== undefined
        || details.lowPassRange !== undefined
        || details.highPass !== undefined
        || details.highPassRange !== undefined)
    {
...
function ReadSfxGameParameterAction(action, names)
{
    ...
    const expectedName = resetting
        ? "reset-game-parameter"
        : "set-game-parameter";

    if (!details
        || (family !== SFX_SET_GAME_PARAMETER_ACTION_FAMILY
            && family !== SFX_RESET_GAME_PARAMETER_ACTION_FAMILY)
    ...
    {
        throw new Error(
            `unsupported Game Parameter target mode ${action.id}`,
        );
    }
    if (!targetId
        || targetId !== (Number(action.targetId) >>> 0))
    {
        throw new Error(
            `unresolved Game Parameter target ${targetId}`,
        );
    }
    if (details.targetIsBus || targetFlags !== 0)
    {
        throw new Error(
            `unsupported Game Parameter target flags ${targetFlags}`,
        );
    }
    if (!Array.isArray(details.exceptions)
        || details.exceptions.length)
    {
        throw new Error(
            `unsupported Game Parameter exceptions ${action.id}`,
        );
    }
    if (!HasExactSfxGameParameterProperties(details.properties)
        || !HasExactSfxGameParameterProperties(details.ranges)
        || details.probability !== undefined)
    {
        throw new Error(
            `unsupported Game Parameter properties ${action.id}`,
        );
    }
    if (typeof details.bypassTransition !== "boolean")
    {
        throw new Error(
            `invalid Game Parameter bypass flag ${action.id}`,
        );
    }
    ...
    {
        throw new Error(
            `missing Game Parameter default ${targetId}`,
        );
    }
    else
    {
    ...
    if (!resetting)
    {
        ...
        {
            throw new Error(
                `unsupported Game Parameter value mode ${action.id}`,
            );
        }
        ...
        {
            throw new Error(
                `invalid Game Parameter value ${action.id}`,
            );
        }
        result.valueMode = details.valueMode;
        result.gameParameterValue = value;
        result.gameParameterRange = { min, max };
    }
    else if (details.valueMode !== undefined
        || details.gameParameterValue !== undefined
        || details.gameParameterRange !== undefined)
    {
        throw new Error(
            `invalid Reset Game Parameter value ${action.id}`,
        );
    }
    ...
    for (const [ source, target ] of [
        ...
        [ "transitionRangeMs", "transitionRangeMs" ],
    ])
    {
        if (details[source] !== undefined)
        {
...
function CreateTypedBusCatalog(inspections)
{
    ...
    {
        const details = result.diagnostics.failed
            .map(failure => `${failure.bank}:${failure.type}:${failure.id}`)
            .join(", ");
...
function CreateBusEffectCatalog(inspections, buses, routedBusIds)
{
    ...
    if (parsed.diagnostics.failed.length
        || parsed.diagnostics.unsupportedVersions.length)
    {
        ...
        .sort((left, right) => left - right))
    {
            ...
            .sort((left, right) => left.index - right.index))
        {
            ...
            if ((Number(slot.flags) & ~0x03) !== 0)
            {
                throw new Error(
                    `Audio Bus ${rawBusId} effect ${slot.fxId} has unsupported slot flags`,
                );
            }
            ...
            {
                throw new Error(
                    `Audio Bus ${rawBusId} effect ${slot.fxId} is missing`,
                );
            }
...
class CjsAudioLibraryBuilderBusGraphConstructionSession
{
    ...
    #InternRoutes()
    {
        ...
        const routeIndices = new Map(signatures.map((signature, index) => [
            signature,
            index,
        ]));
    ...
    #CollectReachableBuses()
    {
        ...
        while (pending.length)
        {
            ...
            pending.push(
                ...
                ...(busAux.userAuxSends ?? [])
                    .map(send => Number(send.targetBusId)),
                Number(busAux.reflectionsAuxSend?.targetBusId) >>> 0,
            );
        }
    }
            ...
            .sort((left, right) => left - right))
        {
            ...
            this.#graphBuses[String(busId)] = {
                ...
                ...(Number(bus.overrideBusId) >>> 0
                    ...
                    : {}),
                channelConfig: { ...bus.channelConfig },
                ...
                positioning: {
                    ...
                    overrideParent: Boolean(
                        bus.positioning?.overrideParent,
                    ),
                    listenerRelative: Boolean(
                        bus.positioning?.listenerRelative,
                    ),
                ...
                hdr: {
                    ...
                    exponentialRelease: Boolean(
                        bus.hdr?.exponentialRelease,
                    ),
                },
                ...
                ...(bus.outputBusVolume === null
                    || bus.outputBusVolume === undefined
                    ? {}
    ...
    #ProjectEffectSlots(busId, bus)
    {
        return [ ...(bus.fx?.slots ?? []) ]
            ...
            .map(slot =>
            {
                if ((Number(slot.flags) & ~0x07) !== 0)
                {
                    throw new Error(
                        `Audio Bus ${busId} effect ${slot.fxId}`
                ...
                {
                    throw new Error(
                        `Audio Bus ${busId} effect ${slot.fxId} is missing`,
                    );
                }
                const expectsShareSet = effect.type === "effect-share-set";
    ...
    #CreateProcessingReasons(bus, aux, slots)
    {
        ...
        if (aux.userAuxSends?.some(send => send.dynamic)
            || aux.reflectionsAuxSend?.dynamic)
        {
...
function CreateEffectiveNodeAuxRouting(rawId, ancestry)
{
    ...
    for (const { id, nodeBase, parentId } of trace.entries)
    {
        ...
        if (!reflectionsResolved
            && (nodeBase.aux?.overrideReflectionsAux || root))
        {
    ...
    return {
        ...(userAuxSends?.length ? { userAuxSends } : {}),
        ...(reflectionsAuxSend ? { reflectionsAuxSend } : {}),
    };
}
...
function CreateAuxSends(value, label)
{
    ...
    for (let slotIndex = 0; slotIndex < 4; slotIndex++)
    {
        ...
        result.push({
            ...
            gainDb: ReadAuxProperty(
                value.properties,
                WWISE_USER_AUX_VOLUME_PROPERTY + slotIndex,
                0,
                `${label} User Aux Send ${slotIndex} Volume`,
            ),
            lowPass: ReadAuxProperty(
                value.properties,
                WWISE_USER_AUX_LOW_PASS_PROPERTY + slotIndex,
                0,
                `${label} User Aux Send ${slotIndex} LPF`,
                0,
                100,
            ),
            highPass: ReadAuxProperty(
                value.properties,
                WWISE_USER_AUX_HIGH_PASS_PROPERTY + slotIndex,
                0,
                `${label} User Aux Send ${slotIndex} HPF`,
                0,
                100,
            ),
...
function CreateReflectionsAuxSend(value, label)
{
    ...
    return {
        ...
        gainDb: ReadAuxProperty(
            value.properties,
            WWISE_REFLECTIONS_VOLUME_PROPERTY,
            0,
            `${label} Reflections Volume`,
        ),
...
function AuxSlotHasDynamicControls(value, slotIndex)
{
    ...
    return (value.ranges ?? []).some(range => propertyIds.has(Number(range.id)))
        || (value.rtpcs ?? []).some(control =>
            parameterIds.has(Number(control.parameterId)))
        || (value.state?.properties ?? []).some(property =>
            propertyIds.has(Number(property.propertyId)));
}

function ReflectionsHaveDynamicControls(value)
{
    return (value.ranges ?? []).some(range => Number(range.id) === 0x1a)
        || (value.rtpcs ?? []).some(control =>
            Number(control.parameterId) === 0x2f)
        || (value.state?.properties ?? []).some(property =>
            Number(property.propertyId) === 0x1a);
}

function ReadAuxProperty(
    properties,
    propertyId,
    fallback,
    label,
...
function ParseDynamicParametricEq(ownerLabel, slot, effect, names)
{
    ...
    {
        throw new Error(
            `Wwise Parametric EQ ${effect.id} on ${ownerLabel} has unsupported controls`,
        );
    }
    ...
    const rtpcCurves = effect.rtpcs.map(rtpc =>
    {
        ...
        const target = definition
            ? `${definition.bandIndex}:${definition.property}`
            : "";

        if (Number(rtpc.controlType) !== 0
            || !definition
            || Number(rtpc.accumulation) !== definition.accumulation
            || !definition.scalings.has(Number(rtpc.scaling))
            || !enabledBands.has(definition.bandIndex)
            || targets.has(target)
            || !parameter
            || !rtpc.points?.length)
        {
            throw new Error(
                `Wwise Parametric EQ ${effect.id} on ${ownerLabel} has unsupported RTPC ${rtpc.curveId}`,
            );
        }
        targets.add(target);
        const defaultValue = names.parameterDefaults.get(controlID);
...
function ParseStaticWwiseCompressor(ownerLabel, slot, effect)
{
    ...
    {
        throw new Error(
            `Wwise Compressor ${effect.id} on ${ownerLabel} is not static`,
        );
    }
...
function ParseStaticWwiseTremolo(ownerLabel, slot, effect)
{
    ...
    {
        throw new Error(
            `Wwise Tremolo ${effect.id} on ${ownerLabel} is not static`,
        );
    }
...
function ParseDynamicWwiseGuitarDistortion(ownerLabel, slot, effect, names)
{
    ...
    {
        throw new Error(
            `Wwise Guitar Distortion ${effect.id} on ${ownerLabel} has unsupported controls`,
        );
    }
    const parsed = parseStaticWwiseGuitarDistortionBytes(
        effect.parameterBlock,
        {
            effectId: effect.id,
            slotIndex: slot.index,
    ...
    if (Number(rtpc.controlType) !== 0
        || Number(rtpc.parameterId) !== 61
        || Number(rtpc.accumulation) !== SFX_ADDITIVE_ACCUMULATION
        || Number(rtpc.scaling) !== 0
        || Number(property.propertyId) !== 61
        || Number(property.accumulation) !== SFX_ADDITIVE_ACCUMULATION
        || Number(property.value) !== parsed.drivePercent
        || !parameter
        || !rtpc.points?.length)
    {
        throw new Error(
            `Wwise Guitar Distortion ${effect.id} on ${ownerLabel} has unsupported Drive RTPC`,
        );
    }
    const defaultValue = names.parameterDefaults.get(controlID);
    const controlTransition = ProjectSourceEffectControlTransition(
...
function ParseStaticWwiseSilenceDuration(effects, source, rawId)
{
    ...
    if (!effect
        ...
        || effect.pluginId !== WWISE_SILENCE_SOURCE_PLUGIN_ID
        || effect.pluginId !== source.pluginId
        || effect.media?.length
        || effect.rtpcs?.length
        || effect.state?.properties?.length
        || effect.state?.groups?.length
    ...
    {
        throw new Error(
            `Wwise Silence source ${soundId} requires 12 static parameter bytes`,
        );
    }
    const view = new DataView(
        bytes.buffer,
    ...
    {
        throw new Error(
            `Wwise Silence source ${soundId} is not a static positive duration`,
        );
    }
        ...
        for (const slot of slots)
        {
            ...
            if (effect.pluginId === PARAMETRIC_EQ_PLUGIN_ID)
            {
                ...
                chain.push(effect.rtpcs?.length
                    ? ParseDynamicParametricEq(
                        ownerLabel,
                        slot,
                        effect,
                        names,
                    )
                    : ParseStaticParametricEq(
                        ownerLabel,
                        slot,
                        effect,
                        { allowIndependentLfe: true },
            ...
            else if (effect.pluginId === WWISE_GUITAR_DISTORTION_PLUGIN_ID)
            {
                ...
                chain.push(effect.rtpcs?.length
                    ? ParseDynamicWwiseGuitarDistortion(
                        ownerLabel,
                        slot,
                        effect,
                        names,
                    )
                    : ParseStaticWwiseGuitarDistortion(
                        ownerLabel,
                        slot,
                        effect,
                    ));
...
{
    const properties = (nodeBase.properties ?? []).filter(property =>
        Number(property.id) === WWISE_OUTPUT_BUS_VOLUME_PROPERTY);

    if (properties.length > 1)
    {
        throw new Error(
            `Wwise NodeBase ${nodeID} has duplicate Output Bus Volume properties`,
        );
    }
    if (!properties.length)
    {
    ...
    {
        throw new Error(
            `Wwise NodeBase ${nodeID} has invalid Output Bus Volume`,
        );
    }
    return value;
}

function CreateBusRouting(
    rawOutputBusId,
    buses,
    authoredOutputBusVolumeDb = undefined,
)
{
    const outputBusId = String(Number(rawOutputBusId) >>> 0);

    if (authoredOutputBusVolumeDb !== undefined
        && !Number.isFinite(Number(authoredOutputBusVolumeDb)))
    {
    ...
    const resolve = (rawID) =>
    {
        ...
        try
        {
            if (nodeBase.positioning?.overrideParent)
            {
                ...
                const attenuationProjection = attenuationId
                    ? GetSfxAttenuationProjection(parsed, attenuationId)
                    : null;
                ...
                result = {
                    ...
                    ...(attenuationProjection === null
                        ? {}
                        : attenuationProjection),
                };
            }
            else
            {
...
class CjsAudioLibraryBuilderSfxNameCatalogAccumulator
{
    ...
    #AddGroup(scope, entry, valuesField)
    {
        ...
        if (entryName && group.name && group.name !== entryName)
        {
            throw new TypeError(
                ...
                + ` name conflicts between ${group.name}`
                + ` and ${entryName}`,
            );
        }
        if (entryName)
        {
            for (const [ otherKey, otherGroup ] of this.#groups)
            {
                if (otherKey !== key
                    ...
                    && otherGroup.name?.toLowerCase()
                        === entryName.toLowerCase())
                {
                    throw new TypeError(
                        `Audio ${scope} group ${entryName}`
                        + ` conflicts between ${otherKey} and ${key}`,
                    );
                }
            }
    ...
    #AddGlobalSettings(globalSettings)
    {
        ...
        if (!globalSettings
            ...
            || !Array.isArray(globalSettings.rtpcParameters)
            || !Array.isArray(globalSettings.stateGroups))
        {
            throw new TypeError(
                "Audio STMG globalSettings must contain"
                + " stateGroups and rtpcParameters",
            );
        }
    ...
    #AddStateTransitionSettings(rawGroup)
    {
        ...
        {
            throw new TypeError(
                "Audio STMG State group must contain transitions",
            );
        }
        const id = NormalizeWwiseUint32(
            rawGroup.id,
            "Audio STMG State group id",
        );
        const defaultTransitionMs = NormalizeWwiseUint32(
            rawGroup.defaultTransitionTimeMs,
            `Audio STMG State group ${id} defaultTransitionTimeMs`,
        );
        ...
        const transitions = rawGroup.transitions.map((raw, index) =>
        {
                ...
                || Array.isArray(raw))
            {
                throw new TypeError(
                    `Audio STMG State group ${id}`
                    + ` transition ${index} must be an object`,
                );
            }
            const fromId = NormalizeWwiseUint32(
                raw.fromId,
                `Audio STMG State group ${id} transition ${index} fromId`,
            );
            const toId = NormalizeWwiseUint32(
                raw.toId,
                `Audio STMG State group ${id} transition ${index} toId`,
            );
            ...
            if (routes.has(route))
            {
                throw new TypeError(
                    `Audio STMG State group ${id}`
                    + ` has duplicate transition ${route}`,
                );
            }
            routes.add(route);
            return {
                fromId,
                toId,
                transitionMs: NormalizeWwiseUint32(
                    raw.transitionTimeMs,
                    `Audio STMG State group ${id}`
                    + ` transition ${index} transitionTimeMs`,
                ),
            };
        }).sort((left, right) =>
    ...
    #AddParameterDefault(parameter)
    {
        ...
        {
            throw new TypeError(
                "Audio STMG RTPC parameter must be an object",
            );
        }
    ...
    #AddValueName(values, id, rawName, label)
    {
        ...
        {
            throw new TypeError(
                `${label} value ${id} name conflicts between`
                + ` ${existing} and ${name}`,
            );
        }
        for (const [ otherId, otherName ] of values)
        {
            ...
            {
                throw new TypeError(
                    `${label} value ${name} conflicts between`
                    + ` ${otherId} and ${id}`,
                );
            }
        }
...
{
    return new CjsAudioLibraryBuilderSfxNameCatalogAccumulator()
        .AddSoundbanksInfo(soundbanksInfo)
        .AddInspections(inspections)
        .AddEnrichment(enrichment)
        .Build();
...
function NormalizeGameParameterID(value)
{
    ...
    {
        throw new TypeError(
            `Audio game parameter ID ${text} must be uint32`,
        );
    }
...
function compactBankInspection(value, source, bank)
{
        ...
        && value.globalSettings !== undefined)
    {
        ...
        {
            throw new TypeError(
                `Audio bank ${bank.resPath} contains invalid globalSettings`,
            );
        }
    ...
    for (const inspection of inspections)
    {
        ...
        for (const [ eventID, event ] of eventsByID)
        {
            ...
            for (const actionID of actionIDs)
            {
                ...
                {
                    const playbackControl = ReadMusicPlaybackControlAction(
                        action,
...
function ReadMusicPlaybackControlAction(action, kind, nodes)
{
    ...
    const unsupported = [
        ...
        "transitionRangeMs",
        "probability",
    ].find(field => result[field] !== undefined);

    if (unsupported !== undefined)
    {
        throw new Error(
            `unsupported Music ${kind} ${unsupported} ${action.id}`,
        );
    }
    if (result.scope !== "game-object"
        || (result.mode !== "element" && result.mode !== "all")
        || result.targetFlags !== 0
    ...
    if (!Number.isSafeInteger(result.curve)
        || result.curve < 0
        || result.curve > 9)
    {
...
function ReadSetterActionIDs(action, rawPayload, label)
{
    ...
    if (hasTypedGroup)
    {
        ...
        if (raw
            && (raw.groupID !== typed.groupID
                || raw.valueID !== typed.valueID))
        {
    ...
    for (const [ name, actions ] of Object.entries(programs))
    {
        ...
        for (const action of actions)
        {
            ...
            if (!action || ![ "pause", "resume" ].includes(action.kind)
                ...
                || action.exceptions.length
                || !Number.isSafeInteger(Number(action.curve))
            ...
            {
                throw new TypeError(
                    `Audio library music programs.${name} has an invalid target`,
                );
            }
        }
    }
...
function ValidateMusicBusRouting(node, id)
{
    ...
    {
        throw new TypeError(
            `Audio library music node ${id} outputBusId must be a positive id`,
        );
    }
    ...
    if (path.some(value => !Number.isSafeInteger(value) || value <= 0)
        || String(path[0]) !== String(outputBusId)
