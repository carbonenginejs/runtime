...
export function indexBusDuckingCatalog(value)
{
    ...
    {
        throw new TypeError(
            `Unsupported audio bus ducking schema version: ${value.schemaVersion}`,
        );
    }
    const sources = RequireRecord(
        value.sources,
        "Audio Bus ducking sources",
    );
    const result = new Map();

    for (const [ rawSourceBusId, rawSource ] of Object.entries(sources))
    {
        const sourceBusId = CanonicalPositiveId(
            rawSourceBusId,
            `Audio Bus ducking source ${rawSourceBusId}`,
        );
        ...
        const maxDuckVolumeDb = FiniteDb(
            source.maxDuckVolumeDb,
            `Audio Bus ducking source ${sourceBusId} maxDuckVolumeDb`,
        );

        if (maxDuckVolumeDb > 0)
        {
            throw new TypeError(
                `Audio Bus ducking source ${sourceBusId} maxDuckVolumeDb must be nonpositive`,
            );
        }
        if (!Array.isArray(source.targets) || !source.targets.length)
        {
            throw new TypeError(
                `Audio Bus ducking source ${sourceBusId} must have targets`,
            );
        }
        ...
        const targets = source.targets.map((rawTarget, index) =>
        {
            ...
            const targetBusId = CanonicalPositiveId(
                target.targetBusId,
                `${label} bus id`,
            );
            ...
            {
                throw new TypeError(
                    `${label} volumeDb must be from ${maxDuckVolumeDb} to 0`,
                );
            }
            ...
            if (targetProperty !== "voice-volume"
...
export class CjsBusDuckingController
{
    ...
    CanSplitTargetProperties(busPathIds)
    {
        ...
        for (const source of this.#catalog.values())
        {
            ...
            const busTargets = matching.filter(target =>
                target.targetProperty === "bus-volume");

            if (busTargets.length
                && matching.some(target =>
                    target.targetProperty === "voice-volume"))
            {
                return false;
            }
        }
    ...
    ScheduleActivity(busPathIds, startContextTime, endContextTime = Infinity)
    {
        ...
        {
            throw new TypeError(
                "Audio Bus ducking activity end must be at or after its start",
            );
        }
    ...
    Prune(at)
    {
        ...
        for (const source of this.#catalog.values())
        {
            ...
            const fadeInSeconds = Math.max(
                0,
                ...source.targets.map(target => target.fadeInMs / 1000),
            );
            const complete = Math.max(...values.map(record => record.end))
                + source.recoveryMs / 1000
                + fadeInSeconds;
    ...
    #EvaluateTarget(source, target, at)
    {
        ...
        for (const record of this.#activities.get(source.sourceBusId).values())
        {
            ...
            {
                events.push({ time: record.end, starts: 0, ends: 1, release: false });
                events.push({
        ...
        while (index < events.length && events[index].time <= time)
        {
            ...
            if (starts > 0)
            {
                if (wasInactive && releaseAt === null)
                {
                    segment = TransitionSegment(
                        current,
                        DbToGain(target.volumeDb),
                        eventTime,
                        target.fadeOutMs / 1000,
                        target.curve,
                    );
                }
                releaseAt = null;
            ...
            if (release && starts === 0 && count === 0
                && releaseAt === eventTime)
            {
                segment = TransitionSegment(
                    EvaluateSegment(segment, eventTime),
                    1,
                    eventTime,
                    target.fadeInMs / 1000,
                    target.curve,
                );
                releaseAt = null;
            }
    ...
    #Settle(record, rawAt, cancel)
    {
        ...
        else
        {
            ...
            const nextEnd = Number.isFinite(record.end)
                ? Math.min(record.end, requestedEnd)
                : requestedEnd;
...
{
    return { from, to, start, duration, curve };
}
...
function EvaluateSegment(segment, at)
{
    ...
    return segment.from + (segment.to - segment.from)
        * evaluateWwiseInterpolation(segment.curve, progress);
}
