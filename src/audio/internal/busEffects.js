...
function NormalizeStaticWwiseEffectChain(value, ownerLabel, allowSourceEffects)
{
    ...
    const effects = value.map((rawEffect, effectIndex) =>
    {
        ...
        if (allowSourceEffects && effect.type === "flanger")
        {
            ...
            const wetDryMixRtpcCurve = effect.wetDryMixRtpcCurve === undefined
                ? undefined
                : NormalizeFlangerWetDryMixRtpcCurve(
                    effect.wetDryMixRtpcCurve,
        ...
        if (allowSourceEffects && effect.type === "tremolo")
        {
            ...
            const smoothingPercent = BoundedFinite(
                effect.smoothingPercent ?? 0,
                MODULATION_PERCENT_MIN,
                MODULATION_PERCENT_MAX,
                `${label} smoothingPercent`,
            );
            const pwmPercent = BoundedFinite(
                effect.pwmPercent ?? 50,
                MODULATION_PERCENT_MIN,
                MODULATION_PERCENT_MAX,
                `${label} pwmPercent`,
            );
        ...
        if (allowSourceEffects && effect.type === "guitar-distortion")
        {
            ...
            const driveRtpcCurve = effect.driveRtpcCurve === undefined
                ? undefined
                : NormalizeGuitarDistortionDriveRtpcCurve(
...
)
{
    ...
    if ((sourceEqualizers.some(effect => effect.rtpcCurves?.length)
        || sourceFlangers.some(effect => effect.wetDryMixRtpcCurve)
...
function CreateWwiseGuitarDistortionApproximation(context, effect)
{
    ...
    const maximumDrive = dynamicDrive
        ? 1 + 100 / driveDivisor
        : 1 + effect.drivePercent / driveDivisor;
...
function CreateWwiseTremoloApproximation(context, effect)
{
    ...
    const pulseDuty = effect.waveform === "square"
        && effect.pwmPercent !== undefined
        ? effect.pwmPercent / 100
        : null;
    ...
    if (depth > 0 || effect.rtpcCurves?.length)
    {
        ...
        if (effect.waveform === "square"
            && effect.pwmPercent !== undefined)
        {
            oscillator.setPeriodicWave(CreatePulsePeriodicWave(
                context,
                effect.pwmPercent,
            ));
        }
...
})
{
    ...
    return smoothingPercent === EVE_OSSE_SQUARE_SMOOTHING_PERCENT
        && pwmPercent === EVE_OSSE_SQUARE_PWM_PERCENT
        && phaseMode === "circular"
        && phaseSpreadDegrees === EVE_OSSE_SQUARE_PHASE_SPREAD_DEGREES;
}
...
function CreatePulsePeriodicWave(context, pwmPercent)
{
    ...
    for (let harmonic = 1; harmonic < real.length; harmonic++)
    {
        ...
        imaginary[harmonic] = 2 * (1 - Math.cos(angle))
            / (Math.PI * harmonic);
    }
    return context.createPeriodicWave(
        real,
