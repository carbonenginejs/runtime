...
export class CjsCharacterDocumentLibrary
{
...
export default CjsCharacterDocumentLibrary;
...
function isCharacterLibraryDocument(value)
{
    return IsPlainObject(value)
        && value.schema === CHARACTER_LIBRARY_SCHEMA
        && Number(value.schemaVersion) === CHARACTER_LIBRARY_SCHEMA_VERSION
        && IsPlainObject(value.documents);
}
...
function validateCharacterLibraryDocument(value)
{
    ...
    if (value.schema !== CHARACTER_LIBRARY_SCHEMA
        || Number(value.schemaVersion) !== CHARACTER_LIBRARY_SCHEMA_VERSION)
    {
...
function NormalizeDocuments(input)
{
    ...
    const orderedNames = [
        ...
        ...[ ...values.keys() ]
            .filter(name => !REQUIRED_DOCUMENT_NAMES.includes(name))
            .sort(CompareText)
    ];
    const result = {};
...
function AddDocument(documents, value, data)
{
    ...
    for (const key of Object.keys(data).sort(CompareIdentities))
    {
        ...
        {
            throw new TypeError(
                `Character document ${name} record ${key} already defines reserved _id`
            );
        }
        ...
        for (const [ recordID, record ] of Object.entries(records))
        {
            VisitRelationshipField(
                ...
                (owner, field, label) =>
                {
                    ...
                    const id = validate
                        ? ReadReference(value, label)
                        : ReadSourceIdentity(value, label);
    ...
    for (const [ name, records ] of Object.entries(documents))
    {
        ...
        for (const [ key, record ] of Object.entries(records))
        {
            ...
            if (expected !== actual)
            {
                throw new TypeError(
                    expected
                        ? `Referenced character record ${name}.${key} is missing _id`
                        : `Unreferenced character record ${name}.${key} must not define _id`
                );
            }
