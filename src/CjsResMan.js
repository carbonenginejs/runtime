...
const BUILD_MATERIAL_OPTION_KEYS = Object.freeze([
  "buildKey",
  "buildVersion",
  "requirement",
  "payload",
...
export class CjsResMan extends CjsEventEmitter
{
  ...
  GetResource(path, options = {}) {
    ...
    if (!existing && buildOptions.reload === true)
    {
      this.InvalidateReadCache(key, { source: buildOptions.source || this.source });
    }
    ...
    const resource = this.CreateResource(
      ...
      ext,
      buildOptions,
      existing && buildOptions.reload === true ? existing : null
    );
    if (existing && buildOptions.reload === true)
    {
      this.#BindResourceLifecycle(cacheKey, existing);
      ...
      const loaderOptions = Object.freeze(GetResourceLoaderOptions(
        buildOptions,
        buildOptions.source || this.source
      ));
      ...
      {
        const reloadOptions = { ...buildOptions, reload: true };
  ...
  #FinalizeCommittedReload(candidate)
  {
    ...
    try
    {
      ...
      {
        candidate.resource.SetObjectLoader(loadOptions => this.GetObject(
          candidate.resource.GetPath(),
          { ...candidate.loaderOptions, ...loadOptions }
        ));
      }
    }
    catch (error)
  ...
  async ReadResourceObjectPayload(resource, bytes, options = {}) {
    const buildPlan = GetResolvedBuildPlan(options, this)
      || this.#resourceBuildPlans.get(resource)
      || null;
    const explicitLoader = buildPlan
      ? buildPlan.objectLoader
      : this.GetObjectLoader(resource.GetExt());
    if (explicitLoader) {
      return explicitLoader(bytes, CreatePrepareContext(this, resource, bytes, options, "read"));
    }

    const descriptor = buildPlan
      ? ResolveFormatDescriptorCandidates(
        buildPlan.formatDescriptors,
        buildPlan.ext,
        { ...options, bytes }
      )
  ...
  async ReadFormat(descriptor, bytes, options = {})
  {
    ...
    const readAsync = Object.prototype.hasOwnProperty.call(descriptor, "readAsync")
      ? descriptor.readAsync
      : Format.readAsync;
    const read = Object.prototype.hasOwnProperty.call(descriptor, "read")
      ? descriptor.read
      : Format.read;
    const ReadAsync = Object.prototype.hasOwnProperty.call(descriptor, "ReadAsync")
      ? descriptor.ReadAsync
      : Format.prototype?.ReadAsync;
    const Read = Object.prototype.hasOwnProperty.call(descriptor, "Read")
      ? descriptor.Read
      : Format.prototype?.Read;
    const formatOptions = {
      ...defaults,
  ...
  #ResolveResourceBuildOptions(path, options)
  {
    ...
    const requestedPipeline = materialOptions.preparePipeline
      ?? materialOptions.pipeline
      ?? this.defaultPreparePipeline;
    const preparePipeline = NormalizePipelineName(requestedPipeline);
    const pipelineDescriptor = preparePipeline
      ? this.preparePipelines.get(preparePipeline) || null
      : null;
    ...
    const prepareStages = Object.freeze([
      ...(pipelineDescriptor?.stages || []),
      ...NormalizePrepareStages(materialOptions.prepareStages || [])
    ]);
    const objectLoader = this.GetObjectLoader(ext);
    const formatDescriptors = Object.freeze(objectLoader
      ? []
      : FilterFormatDescriptors(this.GetFormatDescriptors(ext), materialOptions));
  ...
  {
    const plan = this.#resourceBuildPlans.get(resource)
      || GetResolvedBuildPlan(options, this);
    return plan
      ? ApplyResourceBuildPlan(options, plan)
      : this.#ResolveResourceBuildOptions(resource.GetPath(), options);
  }
...
{
  const plan = options && typeof options === "object"
    ? options[RESOLVED_BUILD_PLAN]
    : null;
...
function CreateResourceBuildIdentity(build)
{
  ...
  if (build.Constructor !== CjsResource) identity.resourceConstructor = build.Constructor;
...
function SerializeBuildIdentity(value, stack = new WeakSet())
{
  ...
  try
  {
    ...
    return `object:{${Object.keys(value)
      .sort()
      .map(key => `${JSON.stringify(key)}:${SerializeBuildIdentity(value[key], stack)}`)
      .join(",")}}`;
  }
  finally
  {
...
function GetResourceLoaderOptions(options = {}, effectiveSource = null)
{
  ...
  const loaderOptions = plan
    ? { ...plan.materialOptions, [RESOLVED_BUILD_PLAN]: plan }
...
function ResolveFormatDescriptorCandidates(descriptors, ext, options)
{
  ...
  if (candidates.length > 1 && options.bytes !== undefined)
  {
    ...
    {
      const { Format, defaults } = descriptor;
      const isSupported = Object.prototype.hasOwnProperty.call(descriptor, "isSupported")
...
function NormalizePrepareStages(value)
{
  ...
  return entries.map((entry, index) => {
    ...
    const id = typeof entry === "function"
      ? name
      : entry.id ?? name;
    const version = typeof entry === "function"
      ? 1
      : NormalizeBuildVersion(entry.version ?? 1, `prepare stage ${String(id)} version`);
    return Object.freeze({ id: String(id), name: String(name), version, prepare });
  });
}
