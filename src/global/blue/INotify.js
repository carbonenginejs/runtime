...
export class INotify
{
   ...
   *
   * @param {object} _value Which member changed - see the note above the class.
 ...
 * take, so the port identifies the member by NAME instead - which is why the
 * argument is an options object carrying `property` rather than a bare pointer.
 ...
 *
 * @param {object} value The argument `OnModified` received.
 * @param {string} name The member being tested for.
 * @returns {boolean} True when that member is the one that changed.
 */
export function IsMatch(value, name)
{
  return value !== null && value !== undefined && value.property === name;
}
