import { ensureRuntimeState, getRuntimeState } from "../../compose/runtimeState.js";
...
export class CjsLifecycleState
{
    ...
    /** Reports whether a target currently has the requested lifecycle state. */
    Is(status)
    {
        return this.status === status;
    }

    /** Reports whether a target is in an alive lifecycle state. */
    IsAlive()
    {
        return this.Is(CJS_LIFECYCLE.ALIVE);
    }

    /** Reports whether a target is alive and available for runtime use. */
    IsAvailable()
    {
        return this.IsAlive();
    }
