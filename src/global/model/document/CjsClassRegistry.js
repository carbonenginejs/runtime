...
{
    /**
     * Creates an isolated class registry and optionally registers initial
     * entries.
     */
    constructor(options = {})
    {
    ...
    /** Returns the number of canonical class entries in the registry. */
    get size()
    {
        return this.entries.size;
    }

    /** Registers constructors and source-shape metadata from named maps. */
    ...
    /** Registers an iterable of class entries in order. */
    ...
    /**
     * Registers one canonical class identity and its optional aliases and
     * provenance.
     */
    Register(entry)
    {
    ...
    /** Registers multiple aliases for one canonical class identity. */
    ...
    /** Registers one alias while reporting conflicting existing ownership. */
    ...
    /** Resolves an alias to its canonical registered class name. */
    ResolveName(name)
    {
    ...
    /** Returns the complete registry entry for a class name or alias. */
    GetEntry(name)
    {
        return this.entries.get(this.ResolveName(name)) || null;
    }

    /** Reports whether a class name or alias resolves in the registry. */
    Has(name)
    {
        return Boolean(this.GetEntry(name));
    }

    /** Returns the registered constructor for a class name or alias. */
    GetConstructor(name)
    {
        return this.GetEntry(name)?.constructor || null;
    }

    /** Resolves a serialized class identity to its registered constructor. */
    ResolveClass(name)
    {
        return this.GetConstructor(name);
    }

    /** Returns the registered source-shape metadata for a class identity. */
    GetSourceShape(name)
    {
        return this.GetEntry(name)?.sourceShape || null;
    }

    /** Returns the provenance module path recorded for a class identity. */
    GetModulePath(name)
    {
        return this.GetEntry(name)?.modulePath || null;
    }

    /** Constructs an instance of a registered class identity. */
    Create(name, values)
    {
    ...
    /** Returns detached diagnostics accumulated during registry construction. */
    GetReports()
    {
        return this.reports.map(report => ({ ...report }));
    }

    /** Exports canonical registry entries and diagnostics as plain JSON data. */
    ToJSON()
    {
        return {
            name: this.name,
    ...
    /**
     * Records diagnostics for conflicting names across incoming registry
     * entries.
     */
    ...
    /** Creates a registry from an iterable or registry-shaped value. */
    static from(options = {})
    {
        return options instanceof CjsClassRegistry ? options : new CjsClassRegistry(options);
    }

    /** Creates a registry from constructor and metadata maps. */
