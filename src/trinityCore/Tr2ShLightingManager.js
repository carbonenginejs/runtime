 ...
 */
const L1_NORMALIZATION = Object.freeze([
...
const L2_NORMALIZATION = Object.freeze([
  2 * SQRT_PI * 0.282094791773878140 * Math.sqrt(0.3141593e1),
  2 / 3 * Math.sqrt(3 * Math.PI) * -0.488602511902919920 * (Math.sqrt(3) * Math.sqrt(0.3141593e1) / 2),
  2 / 3 * Math.sqrt(3 * Math.PI) * 0.488602511902919920 * (Math.sqrt(3) * Math.sqrt(0.3141593e1) / 2),
  2 / 3 * Math.sqrt(3 * Math.PI) * -0.488602511902919920 * (Math.sqrt(3) * Math.sqrt(0.3141593e1) / 2),
...
export class Tr2ShLightingManager extends CjsModel
{
  ...
  #CalculateSecondaryLighting(position, intensity, cutoffRadius, out, order)
  {
    ...
    const direction = Tr2ShLightingManager.#directionScratch;
    ...
    for (const source of this.#sourceData)
    {
      ...
      if (order === 3)
      {
        Tr2ShLightingManager.#EvalSphericalLightL2(direction, distance, source.radius, basis);
      }
      else
      {
        Tr2ShLightingManager.#EvalSphericalLightL1(direction, distance, source.radius, basis);
      }
