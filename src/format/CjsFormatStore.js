  ...
  Register(Format)
  {
    ...
    if (!Array.isArray(declared) || declared.length === 0)
    {
      ...
      const error = new Error(
        `${name} declares no extensions, so it cannot be reached by file suffix. `
        + "Formats whose inputs are logical names rather than file extensions - "
        + "webgl, webgpu, dxbc - are used directly instead of through a store."
      );
