...
const PARAMETRIC_EQ_FILTER_TYPES = Object.freeze([
    "lowpass",
    "highpass",
    "bandpass",
    "notch",
...
function ParseStaticParametricEq(busId, slot, effect)
{
    ...
    {
        throw new Error(
            `Wwise Parametric EQ ${effect.id} on bus ${busId} has an unsupported parameter block`,
        );
    }
    ...
    for (let index = 0; index < 3; index++)
    {
        ...
        {
            throw new Error(
                `Wwise Parametric EQ ${effect.id} on bus ${busId} has invalid band ${index}`,
            );
        }
        if (enabled)
        {
    ...
    {
        throw new Error(
            `Wwise Parametric EQ ${effect.id} on bus ${busId} has invalid output gain`,
        );
    }
    ...
    {
        throw new Error(
            `Wwise Parametric EQ ${effect.id} on bus ${busId} requires unsupported independent LFE routing`,
        );
    }
    return {
        effectId: String(effect.id),
