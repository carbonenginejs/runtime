    ...
    const lowerEvent = (rawID) =>
    {
        ...
        try
        {
            for (const actionID of event.actionIds)
            {
                ...
                {
                    result.program.push(
                        ReadSfxGameParameterAction(action, names),
                    );
                }
                else if (((action.actionType >> 8) & 0xff)
            ...
            if (IsMusicEventName(eventNames.get(eventID)))
            {
                ...
                {
                    throw new Error(
                        `music Bus Voice Volume event ${eventID}`,
                    );
                }
...
function CreateBusGraphCatalog(inspections, musicInspections, buses)
{
    ...
    const musicResult = musicInspections.length
        ? CjsBnkFormat.wwise.musicNodesFromBanks(musicInspections)
        : { nodes: new Map(), diagnostics: { failed: [], unsupportedVersions: [] } };

    if (effectsResult.diagnostics.failed.length
        || effectsResult.diagnostics.unsupportedVersions.length
        || sfxResult.diagnostics.failed.length
        || sfxResult.diagnostics.unsupportedVersions?.length
        || musicResult.diagnostics.failed.length
        || musicResult.diagnostics.unsupportedVersions?.length)
    {
    ...
    while (pending.length)
    {
        ...
        pending.push(
            ...
            ...(busAux.userAuxSends ?? []).map(send => Number(send.targetBusId)),
    ...
    for (const busId of [ ...reachable ].sort((left, right) => left - right))
    {
        ...
        const slots = [ ...(bus.fx?.slots ?? []) ]
            ...
            .map(slot =>
            {
                ...
                {
                    throw new Error(
                        `Audio Bus ${busId} effect ${slot.fxId} has unsupported slot flags`,
                    );
                }
                ...
                {
                    throw new Error(
                        `Audio Bus ${busId} effect ${slot.fxId} has a mismatched ShareSet flag`,
                    );
                }
    ...
    return {
        schemaVersion: 1,
        buses: graphBuses,
        effects: Object.fromEntries(Object.entries(graphEffects)
            .sort(([ left ], [ right ]) => Number(left) - Number(right))),
        routes,
        sfxRoutes,
        musicRoutes,
...
function CreateSfxNameCatalog(soundbanksInfo, enrichment, inspections)
{
        ...
        for (const bank of parsed.banks)
        {
            ...
            ])
            {
                for (const entry of entries)
                {
                    ...
                    const entryName = NormalizeOptionalCatalogName(
                        entry.name,
                    );
    ...
    const stateTransitions = [ ...stateTransitionSettings.values() ]
        .sort((left, right) => left.id - right.id)
        .map(settings =>
        {
