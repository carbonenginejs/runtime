...
export function decodeJpegToRgba(bytes, metadata = {})
{
    ...
    return {
        ...
        colorSpace: "srgb",
        alphaMode: "opaque",
        metadata: { ...metadata, decoder: "software-baseline" }
    };
}
...
export function canDecodeJpeg(metadata = {})
{
    return metadata.sourceFormat === "jpeg" &&
        ...
        metadata.components >= 1 && metadata.components <= 3 &&
        (metadata.marker === 0xc0 || metadata.marker === 0xc1) &&
...
class BaselineJpegDecoder
{
    ...
    decode()
    {
        ...
        while (this.#offset < this.#bytes.length)
        {
            ...
            if (marker === 0xdb) this.readQuantizationTables();
            else if (marker === 0xc0 || marker === 0xc1 || marker === 0xc2) this.readFrame(marker);
            else if (marker === 0xc4) this.readHuffmanTables();
            else if (marker === 0xdd) this.readRestartInterval();
            else if (marker >= 0xe0 && marker <= 0xef || marker === 0xfe) this.skipSegment();
            else if (marker === 0xd8 || marker === 0x01 || marker >= 0xd0 && marker <= 0xd7) continue;
    ...
    decodeBlock(reader, component)
    {
        ...
        while (index < 64)
        {
            ...
            if (value === 0) break;
            if (value === 0xf0) { index += 16; continue; }
            index += value >>> 4;
