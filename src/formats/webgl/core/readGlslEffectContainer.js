...
/**
 * Names a Carbon stage type from the enum rather than a local table.
 *
 * Naming an unsupported stage honestly matters here: `inspectGlslContainerIntegrity`
 * reports `unsupported_stage` for anything outside vertex/pixel/compute, and a
 * geometry stage has to arrive called "geometry" for that report to mean
 * anything.
 *
...
export const NO_PROGRAM_REASON = "no program was stored";
...
export function readGlslEffectContainer(input, values = {})
{
    ...
    const shaderKeyBySource = new Map();
    ...
    for (let index = 0; index < reader.records.length; index += 1)
    {
        ...
        for (const technique of description.techniques)
        {
            ...
            for (const [ passIndex, pass ] of technique.passes.entries())
            {
                ...
                {
                    const name = stageName(stage.type);
                    ...
                    if (stage.shaderData?.size)
                    {
                        ...
                        const pooled = shaderKeyBySource.get(code);
                        if (pooled)
                        {
                            shaderKey = pooled;
                        }
                        ...
                        {
                            shaderKey = `shader_${shaderKeyBySource.size}`;
                            shaderKeyBySource.set(code, shaderKey);
                            shaders.push({
                                key: shaderKey,
                                stageName: name,
                                source: code,
