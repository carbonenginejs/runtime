...
export function fixPackedLightFlagMask(source, record)
{
  ...
  return source.replace(
    ...
    (_match, indent, maskTarget, flagsSource) =>
      `${indent}${maskTarget} = uintBitsToFloat(((floatBitsToUint(${flagsSource}) & 65536u) != 0u) ? 1u : 0u);`
  );
}
...
export function fixPackedLightAcceptedMask(source, record)
{
  ...
  const directAccepted = source.replace(
    ...
    (_match, indent, radiusCondition, flagsSource) => {
      const flagTest = buildPackedLightFlagUintTest(source, flagsSource);
      return `${indent}if ((${radiusCondition}) && (${flagTest})) {`;
    }
  );

  const directMaskBranch = directAccepted.replace(
    /(\s*)([A-Za-z_][A-Za-z0-9_]*(?:\.[xyzw])?) = uintBitsToFloat\(floatBitsToUint\(([^)]+)\) & floatBitsToUint\(([^)]+)\)\);\r?\n\s*if \(floatBitsToUint\(\2\) != 0u\) \{/g,
    (_match, indent, _target, leftMask, rightMask) =>
      `${indent}if ((floatBitsToUint(${leftMask}) & floatBitsToUint(${rightMask})) != 0u) {`
  );

  return directMaskBranch.replace(
    /(\s*)[A-Za-z_][A-Za-z0-9_]*(?:\.[xyzw])? = uintBitsToFloat\(\(([^;\r\n]+?)\) \? 0xFFFFFFFFu : 0u\);\r?\n\s*[A-Za-z_][A-Za-z0-9_]*(?:\.[xyzw])? = uintBitsToFloat\(\(\(floatBitsToUint\(([^;\r\n]+?)\) & 65536u\) != 0u\) \? 1u : 0u\);\r?\n\s*if \(\(floatBitsToUint\([^)]+\) & floatBitsToUint\([^)]+\)\) != 0u\) \{/g,
    (_match, indent, radiusCondition, flagsSource) => {
      const flagTest = buildPackedLightFlagUintTest(source, flagsSource);
      return `${indent}if ((${radiusCondition}) && (${flagTest})) {`;
    }
...
function buildPackedLightFlagUintTest(source, flagsSource)
{
  ...
  return match
    ? `((${match[1]}) & 65536u) != 0u`
    : `(floatBitsToUint(${flagsSource}) & 65536u) != 0u`;
}
 ...
 */
export function applyPackedLightFixups(source, stageName)
{
    const record = { stageName };
