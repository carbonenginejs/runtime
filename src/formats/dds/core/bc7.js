...
const PARTITIONS_3 = new Uint32Array([
...
const ANCHOR_3_SECOND = new Uint8Array([
...
const ANCHOR_3_THIRD = new Uint8Array([
...
export function decodeBc7Block(block)
{
    ...
    const endpoints = Array.from({ length: endpointCount }, () => [ 0, 0, 0, 255 ]);
    ...
    for (let endpoint = 0; endpoint < endpointCount; endpoint++)
    {
        for (let channel = 0; channel < 3; channel++)
        {
            const value = info.endpointPBits || info.sharedPBits
                ? (endpoints[endpoint][channel] << 1) | pBits[endpoint]
                : endpoints[endpoint][channel];
        ...
        {
            const value = info.endpointPBits || info.sharedPBits
                ? (endpoints[endpoint][3] << 1) | pBits[endpoint]
    ...
    const secondary = info.secondaryIndexBits
        ? readIndices(reader, info.secondaryIndexBits, info.subsets, partition)
        : primary;
