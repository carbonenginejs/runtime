...
| A shipped 2048x1024 head-tattoo DDS with sparse authored alpha is composited as one complete head atlas by the GLES reference path; sending the same pixels through the retained mode-1 mesh projection performs a second placement operation. | **Reference parity plus shader-math diagnosis; structurally tested in the GLES adapter, with full-corpus visual qualification still open.** The schema can represent a typed projection profile, but the currently reviewed delivered library retains this projection only as a lossless `characterDefinitions` value and retains selected colours separately in the appearance plan; its optional typed material/projection catalogs are empty. No typed link currently declares either value to be the placement operation for this already-authored atlas. An engine may use projection parameters only after proving that its input is a raw projector decal rather than the published atlas. |
