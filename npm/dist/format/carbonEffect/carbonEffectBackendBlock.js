 ...
 */
const CARBON_BACKEND_RESOURCE_KIND = Object.freeze(["uniform-buffer", "sampled-resource", "sampler", "storage-resource"]);

/** Shader stages, as a bit position in the visibility mask. */
const CARBON_BACKEND_VISIBILITY = Object.freeze(["vertex", "fragment", "compute"]);
...
function packVisibility(visibility) {
  ...
  for (const stage of visibility ?? []) {
    ...
    if (bit < 0) {
      throw new CjsFormatWriteError(`Unknown binding visibility "${stage}"`, {
        stage
      });
    }
    mask |= 1 << bit;
 ...
 */
function readBackendBlock(bytes, options = {}) {
...
export { CARBON_BACKEND_RESOURCE_KIND, CARBON_BACKEND_TRANSFORM_FAMILY, CARBON_BACKEND_VISIBILITY, CARBON_EFFECT_BACKEND_BLOCK_VERSION, DETAIL_MAP_ARRAY_DEFAULTS, readBackendBlock, writeBackendBlock };
