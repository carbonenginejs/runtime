 ...
 */
const CARBON_BACKEND_TRANSFORM_FAMILY = Object.freeze(["detail-map-array", "local-light-profile-neutral"]);
 ...
 */
const LOCAL_LIGHT_PROFILE_NEUTRAL_DEFAULTS = Object.freeze({
  version: 1,
  kind: "constant",
  stage: "fragment",
...
function assertRestoredFieldsAgree(transform, family, defaults) {
  const restored = [["version", transform.version, defaults.version], ["kind", transform.kind, defaults.kind], ["stage", transform.stage, defaults.stage], ["representation", transform.representation, defaults.representation], ["missingLayer", transform.missingLayer, defaults.missingLayer], ["output.name", transform.output?.name, defaults.outputName], ["output.viewDimension", transform.output?.viewDimension, defaults.viewDimension]];
  for (const [field, actual, expected] of restored) {
    if (actual === undefined || actual === null) continue;
    if (actual === expected) continue;
    throw new CjsFormatWriteError(`Resource transform "${transform.id}" sets ${field} to ${JSON.stringify(actual)}, but the` + ` "${family}" family restores ${JSON.stringify(expected)} on read. The value is not` + ` stored, so writing this would silently change the document.`, {
  ...
  if (layerCount !== undefined && layerCount !== null && layerCount !== transform.inputs.length) {
    throw new CjsFormatWriteError(`Resource transform "${transform.id}" declares ${layerCount} output layers but carries` + ` ${transform.inputs.length} inputs; the reader derives the count from the inputs.`, {
      transform: transform.id,
      family,
      declared: layerCount,
      inputs: transform.inputs.length
    });
  }
}
...
export { CARBON_BACKEND_TRANSFORM_FAMILY, DETAIL_MAP_ARRAY_DEFAULTS, LOCAL_LIGHT_PROFILE_NEUTRAL_DEFAULTS, readInlineString, readTransformSection, writeInlineString, writeTransformSection };
