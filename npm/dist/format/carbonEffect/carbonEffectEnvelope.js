...
const CARBON_EFFECT_ENVELOPE_BYTES = 12;
 ...
 */
const CARBON_EFFECT_PAYLOAD_KIND = Object.freeze({
  DXBC: 0,
  WGSL: 1,
  GLSL: 2,
  AIR: 3
 ...
 */
function writeCarbonEffectEnvelope(writer, envelope) {
  ...
  for (const byte of magic) {
    if (byte < 0x20 || byte > 0x7e) {
      throw new CjsFormatWriteError("Container magic must be printable ASCII so it cannot collide with a Carbon version dword", {
        magic: envelope.magic
      });
    }
  }
 ...
 */
function readCarbonEffectEnvelope(reader, expected) {
  ...
  if (expected.containerVersion !== undefined && containerVersion !== expected.containerVersion) {
    throw new CjsFormatReadError(`Unsupported container version ${containerVersion}; expected ${expected.containerVersion}`, {
      source: reader.source,
      containerVersion,
      expected: expected.containerVersion
    });
  }
  return {
    magic,
    containerVersion,
    payloadKind: reader.readUint32()
  };
}
 ...
 */
function looksLikeBareCarbonEffect(bytes) {
  if (bytes.length < 4) return false;
  return bytes[1] === 0 && bytes[2] === 0 && bytes[3] === 0 && bytes[0] >= 2 && bytes[0] <= CARBON_EFFECT_DATA_VERSION;
}

export { CARBON_EFFECT_ENVELOPE_BYTES, CARBON_EFFECT_PAYLOAD_KIND, looksLikeBareCarbonEffect, readCarbonEffectEnvelope, writeCarbonEffectEnvelope };
//# sourceMappingURL=carbonEffectEnvelope.js.map
