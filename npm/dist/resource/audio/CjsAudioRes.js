...
class CjsAudioRes extends _CjsResource {
  ...
  SetBackingResource(backing, {
    ...
    if (!(backing instanceof CjsAudioBufferRes)) {
      throw new TypeError("CjsAudioRes backing must be a CjsAudioBufferRes");
    }
    const normalizedOffset = CjsAudioRes.normalizeNonNegativeInteger(offset, "CjsAudioRes offset");
    const normalizedByteLength = byteLength === null || byteLength === undefined ? null : CjsAudioRes.normalizeNonNegativeInteger(byteLength, "CjsAudioRes byteLength");
    if (this.#backing && this.#backing !== backing) {
      throw new CjsError("CJS_AUDIO_RESOURCE_CONFLICT", `Audio resource backing changed: ${this.GetPath()}`, {
        details: {
          path: this.GetPath()
        }
      });
    }
    if (this.#backing && (this.#offset !== normalizedOffset || this.#byteLength !== normalizedByteLength)) {
      throw new CjsError("CJS_AUDIO_RESOURCE_CONFLICT", `Audio resource window changed: ${this.GetPath()}`, {
        details: {
          path: this.GetPath()
        }
      });
  ...
  async GetBytes({
    ...
    if (!this.#backing) {
      throw new CjsError("CJS_AUDIO_BACKING_UNAVAILABLE", `Audio resource has no backing source: ${this.GetPath()}`, {
        details: {
          path: this.GetPath()
        }
      });
    ...
    try {
      ...
      if (available < 0 || totalByteLength > available) {
        throw new CjsError("CJS_AUDIO_SOURCE_WINDOW_INVALID", `Audio source window exceeds its backing bytes: ${this.GetPath()}`, {
          details: {
            path: this.GetPath(),
      ...
      const range = CjsAudioRes.normalizeRange(offset, byteLength, totalByteLength);
  ...
  static normalizeRange(offsetValue, byteLengthValue, totalByteLength) {
    const offset = this.normalizeNonNegativeInteger(offsetValue, "Audio byte offset");
    const byteLength = byteLengthValue === null || byteLengthValue === undefined ? totalByteLength - offset : this.normalizeNonNegativeInteger(byteLengthValue, "Audio byte length");
    if (offset > totalByteLength || byteLength > totalByteLength - offset) {
      throw new RangeError(`Audio byte range exceeds ${totalByteLength} bytes`);
    }
    return {
      offset,
      byteLength
...
export { CjsAudioRes };
//# sourceMappingURL=CjsAudioRes.js.map
