 ...
 */
function resourceFormatRequiredError(className, extension, output = null) {
  const source = extension ? `".${String(extension).replace(/^\./u, "")}"` : "this source";
  const error = new Error(`Nothing is registered to read ${source}${output ? ` as "${output}"` : ""} ` + `for ${className}. Register a route for it on the format store, or pass ` + "options.format. The resource imports no formats itself, so that it does " + "not drag every reader of its family into anything that touches it.");
...
export { assertResourcePayloadArray, assertResourcePayloadObject, resourceBoundaryError, resourceFormatRequiredError, resourcePayloadError, validateResourcePayload };
