...
import { recordBytes, recordRawBits, toRecordBlob, toRecordFloat } from './carbonRecordFields.js';
...
class Tr2EffectStageInput extends CjsModel {
  ...
  static from(values = {}, options = {}) {
    ...
    const maps = new Map();
    for (const [field, Constructor] of [["resources", Tr2EffectResource], ["uavs", Tr2EffectResource], ["samplers", Tr2SamplerSetup]]) {
  ...
  static fromCarbonBinaryInput(record, stageType = -1) {
    ...
    input.exists = true;
    input.constants = record.constants.map(entry => Tr2EffectConstant.fromCarbonBinary(entry));
    ...
    input.annotation = record.annotations.map(entry => Tr2EffectParameterAnnotation.fromCarbonBinary(entry));
    input.signature = {
  ...
  toCarbonBinary() {
    ...
    return {
      ...
      threadGroupSize: [signature.threadGroupSize?.x ?? 0, signature.threadGroupSize?.y ?? 0, signature.threadGroupSize?.z ?? 0],
  ...
  toCarbonBinaryInput() {
    ...
    return {
      ...
      defaultValues: toRecordBlob(this.constantValues, this.constantValueSize, this.constantValuesUnsetOffset),
      textures: Array.from(this.resources, ([register, resource]) => resource.toCarbonBinary(register)).sort(byRegister),
      samplers: Array.from(this.samplers, ([register, sampler]) => sampler.toCarbonBinary(register)).sort(byRegister),
      uavs: Array.from(this.uavs, ([register, uav]) => uav.toCarbonBinary(register, true)).sort(byRegister),
      annotations: this.annotation.map(entry => entry.toCarbonBinary()).sort((left, right) => compareUtf8(left.name.value, right.name.value))
    };
  }
  ...
  static readCarbonBinaryResourceMap(records) {
    ...
    for (const record of records) {
      if (result.has(record.registerIndex)) {
        throw new Error(`Carbon effect resource register ${record.registerIndex} is duplicated`);
      }
      result.set(record.registerIndex, Tr2EffectResource.fromCarbonBinary(record));
    }
    return result;
  }
  ...
  static readCarbonBinarySamplerMap(records) {
    ...
    for (const record of records) {
      if (result.has(record.registerIndex)) {
        throw new Error(`Carbon effect sampler register ${record.registerIndex} is duplicated`);
      }
      result.set(record.registerIndex, Tr2SamplerSetup.fromCarbonBinary(record));
    }
    return result;
  }
