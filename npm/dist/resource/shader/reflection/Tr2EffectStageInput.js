...
class Tr2EffectStageInput extends CjsModel {
  ...
  static fromPortableInput(value, stageType = -1, sourceProgram = null) {
    ...
    input.annotation = value.annotations.map(entry => Tr2EffectParameterAnnotation.fromPortable(entry));
  ...
  static readPortableResourceMap(values) {
    ...
    for (const value of values) {
      ...
      result.set(registerIndex, Tr2EffectResource.fromPortable(value));
    }
    return result;
  }
  ...
  static readPortableSamplerMap(values) {
    ...
    for (const value of values) {
      ...
      result.set(registerIndex, Tr2SamplerSetup.fromPortable(value));
    }
    return result;
  }
...
CjsSchema.define(Tr2EffectStageInput, {
  ...
  methods: [{
    ...
    name: "fromPortableInput",
    impl: {
      custom: true,
      status: "custom",
      reason: "Carbon reads stage-input bytes in place; CarbonEngineJS also supports the portable stage-input record used by effect packages."
    }
  }, {
    name: "createEmpty",
    impl: {
      custom: true,
      status: "custom",
      reason: "Carbon stores six fixed stage slots in each pass; CarbonEngineJS constructs explicit absent canonical slots before portable stages are assigned."
    }
  }, {
    name: "readPortableResourceMap",
    impl: {
      custom: true,
      status: "custom",
      reason: "Carbon reads a native register map; CarbonEngineJS indexes validated portable resource records into the canonical numeric map."
    }
  }, {
    name: "readPortableSamplerMap",
    impl: {
      custom: true,
      status: "custom",
      reason: "Carbon reads a native sampler map; CarbonEngineJS indexes validated portable sampler records into the canonical numeric map."
    }
  }, {
    name: "readCanonicalRegisterMap",
    impl: {
      custom: true,
      status: "custom",
      reason: "Carbon has fixed native map types; CarbonEngineJS restores numeric register keys and canonical child identity from JSON-compatible maps."
    }
  }]
});
