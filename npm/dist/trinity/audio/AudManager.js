...
class AudManager extends CjsModel {
  ...
  Enable(soundBanksToLoad = []) {
    ...
    if (this.GetSpatialAudioGeometryEnabled() && !this.#InitSpatialAudioGeometry(backend)) {
      return;
    }
    if (this.#state === "uninitialized") {
      this.#state = "disabled";
  ...
  SetEmitterLineOfSightBlockage(emitterID, blockage) {
    return this.#obstructionOcclusion.SetEmitterLineOfSightBlockage(emitterID, blockage);
  }
  ...
  SetSpatialAudioGeometryEnabled(enabled) {
    ...
    if (!this.#InitSpatialAudioGeometry(_AudGameObjResource.backend)) {
      return;
    }
  ...
  UnregisterGameObject(gameObjID) {
    ...
    for (const info of this.#soundBankInfoMap.values()) {
      info.waitingEventsAfterLoad = info.waitingEventsAfterLoad.filter(([emitter]) => emitter?.ID !== gameObjID);
    }
  }
  ...
  Process() {
    ...
    if (this.#state === "enabled") {
      ...
      this.#obstructionOcclusion.Update(_AudGameObjResource.backend);
