import { CjsCharacterBlendshapeLimits as _CjsCharacterBlendsha } from '../deformation/CjsCharacterBlendshapeLimits.js';
import { CjsCharacterControlLayer as _CjsCharacterControlL } from './CjsCharacterControlLayer.js';
import { CjsCharacterControlState as _CjsCharacterControlS } from './CjsCharacterControlState.js';
import { CjsCharacterGraph as _CjsCharacterGraph } from '../library/CjsCharacterGraph.js';
...
class CjsCharacterControlApplicator {
  ...
  Compose(graph, layers, {
    limits = null
  } = {}) {
    if (!(graph instanceof _CjsCharacterGraph)) {
    ...
    return _CjsCharacterControlS.from({
      morphs,
      parameters,
      boneOffsets,
      activePose,
...
function PrepareLayers(values) {
  ...
  return values.map((value, index) => {
    const layer = value instanceof _CjsCharacterControlL ? value : _CjsCharacterControlL.from(value || {});
...
function ComposeScalar(target, name, value, layer, kind) {
  ...
  const next = layer.blendMode === "add" ? current + value * layer.influence : current + (value - current) * layer.influence;
...
function ComposeVector(target, name, value, layer) {
  ...
  for (let index = 0; index < 3; index++) {
    ...
    result[index] = layer.blendMode === "add" ? currentValue + value[index] * layer.influence : currentValue + (value[index] - currentValue) * layer.influence;
...
function ApplyLimits(morphs, value) {
  ...
  const limits = value instanceof _CjsCharacterBlendsha ? value : _CjsCharacterBlendsha.from(value);
...
export { CjsCharacterControlApplicator };
//# sourceMappingURL=CjsCharacterControlApplicator.js.map
