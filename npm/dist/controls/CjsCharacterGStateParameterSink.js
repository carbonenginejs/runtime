...
class CjsCharacterGStateParameterSink {
  ...
  static findParameter(gStateAnimation, nodeName, parameterName) {
    const node = CjsCharacterGStateParameterSink.normalizeName(nodeName, "node");
    const parameter = CjsCharacterGStateParameterSink.normalizeName(parameterName, "parameter");
    const matches = CjsCharacterGStateParameterSink.getParameters(gStateAnimation).filter(value => CjsCharacterGStateParameterSink.getNodeName(value) === node && CjsCharacterGStateParameterSink.getParameterName(value) === parameter);
    if (matches.length > 1) {
  ...
  static getNodeName(parameter) {
    const value = typeof parameter?.GetNodeName === "function" ? parameter.GetNodeName() : parameter?.nodename ?? parameter?.nodeName;
  ...
  static getParameterName(parameter) {
    const value = typeof parameter?.GetName === "function" ? parameter.GetName() : parameter?.name;
  ...
  static getParameterValue(parameter) {
    const value = Number(typeof parameter?.GetValue === "function" ? parameter.GetValue() : parameter?.value);
  ...
  HasParameter(controlName) {
    const {
      nodeName,
      parameterName
    } = CjsCharacterGStateParameterSink.parseParameterName(controlName);
    return CjsCharacterGStateParameterSink.findParameter(this.#gStateAnimation, nodeName, parameterName) !== null;
  }
  ...
  Reset() {
    let changed = false;
    for (const name of [...this.#defaults.keys()].sort((left, right) => String(left).localeCompare(String(right), "en", {
      numeric: true
    }))) {
  ...
  #resolve(controlName) {
    ...
    const parameter = CjsCharacterGStateParameterSink.findParameter(this.#gStateAnimation, nodeName, parameterName);
    if (!parameter) {
...
export { CjsCharacterGStateParameterSink };
//# sourceMappingURL=CjsCharacterGStateParameterSink.js.map
