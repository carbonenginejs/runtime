...
function DiagnoseCharacterRules(plan, resource, selection, origin) {
  const hasUnresolvedCoverageRules = [resource.clothingAlsoCoversCategory, resource.clothingAlsoCoversCategory2].some(Boolean);
...
function DiagnosePartMetadata(library, plan, metadata, partSource, origin) {
  ...
  for (let index = 0; index < metadata.occludesModifiers.length; index++) {
    ...
    if (relation instanceof _CjsCharacterModifier$1 && relation.authoredValue === value && (relation.modifierLocation instanceof _CjsCharacterModifier || DecodeUtilityOcclusion(relation, value))) {
      continue;
    }
...
function ResolveSelectionSuppressions(plan, modifierPolicies, utilityRequests, utilityOcclusions) {
  const suppressions = new Map();
  for (const owner of modifierPolicies) {
    const targets = [];
    for (const value of [owner.resource?.clothingRemovesCategory, owner.resource?.clothingRemovesCategory2]) {
      if (value instanceof _CjsCharacterModifier) {
        targets.push({
          location: value,
          rule: "typed-clothing-removal"
        });
      }
    }
    for (let index = 0; index < (owner.metadata?.occlusions?.length ?? 0); index++) {
      const relation = owner.metadata.occlusions[index];
      if (!(relation instanceof _CjsCharacterModifier$1) || !(relation.modifierLocation instanceof _CjsCharacterModifier) || relation.authoredValue !== owner.metadata.occludesModifiers[index]) {
        continue;
      }
      targets.push({
    ...
    for (const target of targets) {
      for (const candidate of modifierPolicies) {
        ...
        const current = suppressions.get(candidate).find(value => value.owner === owner);
        if (current) {
