...
import { CjsCharacterPartSource as _CjsCharacterPartSour } from '../catalog/CjsCharacterPartSource.js';
import { CjsCharacterPartSourceVersion as _CjsCharacterPartSour$1 } from '../catalog/CjsCharacterPartSourceVersion.js';
...
class CjsCharacterAppearanceResolver {
  ...
  static resolvePaperdoll(library, paperdoll) {
    if (!library || library.schema !== "carbonenginejs.characterLibrary" || library.schemaVersion !== 6 || typeof library.Get !== "function" || typeof library.GetDocument !== "function") {
    ...
    for (let modifierIndex = 0; modifierIndex < paperdoll.modifiers.length; modifierIndex++) {
      ResolveModifier(plan, paperdoll, paperdoll.modifiers[modifierIndex], modifierIndex, groupIDs);
    }
...
function ResolveModifier(plan, paperdoll, modifier, modifierIndex, groupIDs) {
  ...
  if (!(partSource instanceof _CjsCharacterPartSour)) {
    AddDiagnostic(plan, "PART_SOURCE_UNRESOLVED", `Part type ${JSON.stringify(partType.recordID)} has no exact part-source relationship.`, "warning", selectionOrigin);
    return;
  }
  ...
  const matches = partSource.versions.map((version, index) => ({
    ...
    version
  }) => version instanceof _CjsCharacterPartSour$1 && version.resourceVersion === partType.resourceVersion);
  if (matches.length !== 1) {
...
function DiagnosePartMetadata(plan, metadata, partSource, origin) {
  ...
  const hasRules = metadata.dependentModifiers.length || metadata.occludesModifiers.length || metadata.forcesLooseTop !== null || metadata.hidesBootShin !== null || metadata.lod1Replacement !== null || metadata.lod2Replacement !== null || metadata.swapTops !== null || metadata.swapBottom !== null || metadata.swapSocks !== null || metadata.wap !== null;
  if (hasRules) {
    AddDiagnostic(plan, "PART_METADATA_RULES_UNRESOLVED", `Part source ${JSON.stringify(partSource.recordID)} has authored dependency, occlusion, replacement, or compatibility rules.`, "warning", origin);
  }
}
