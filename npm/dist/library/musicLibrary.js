 ...
 * mean.
 */
function validateMusicLibrary(value) {
  RequireRecord(value, "Music library");
  if (value.schema !== MUSIC_LIBRARY_SCHEMA) {
    throw new TypeError(`Unsupported music-library schema: ${value.schema}`);
  }
  if (value.schemaVersion !== MUSIC_LIBRARY_VERSION) {
    throw new TypeError(`Unsupported music-library schema version: ${value.schemaVersion}`);
  }
  ...
  if (!Array.isArray(value.playlists) || !value.playlists.length) {
    throw new TypeError("Music library playlists must be a non-empty array");
  }
  ...
  for (const [playlistIndex, playlist] of value.playlists.entries()) {
    ...
    if (playlistIDs.has(playlistID)) {
      throw new TypeError(`Music library has duplicate playlist id ${playlistID}`);
    }
    ...
    for (const [songIndex, song] of playlist.songs.entries()) {
      ...
      if (songIDs.has(songID)) {
        throw new TypeError(`Music library playlist ${playlistID} has duplicate song id ${songID}`);
      }
      songIDs.add(songID);
      ...
      if (!path && !url) {
        throw new TypeError(`${songLabel} must provide a path or url`);
      }
      if (song.durationMs !== undefined && (!Number.isFinite(song.durationMs) || song.durationMs <= 0)) {
        throw new TypeError(`${songLabel} durationMs must be a positive finite number`);
      }
    }
  }
  return true;
 ...
 */
function installMusicLibrary(value) {
  validateMusicLibrary(value);
  let clone;
  try {
    clone = JSON.parse(JSON.stringify(value));
  } catch (error) {
    throw new TypeError(`Music library must be JSON-compatible: ${error.message}`);
  }
...
export { installMusicLibrary, validateMusicLibrary };
