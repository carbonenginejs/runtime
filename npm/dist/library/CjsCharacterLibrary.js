...
import { CjsCharacterDependency as _CjsCharacterDependen } from './CjsCharacterDependency.js';
import { CjsCharacterGraph as _CjsCharacterGraph } from './CjsCharacterGraph.js';
import { CjsCharacterLibraryData as _CjsCharacterLibraryD } from './CjsCharacterLibraryData.js';
import { CjsCharacterLodBundle as _CjsCharacterLodBundl } from '../parts/CjsCharacterLodBundle.js';
import { CjsCharacterRecipe as _CjsCharacterRecipe } from '../recipes/CjsCharacterRecipe.js';
import { CjsCharacterRecipeResolution as _CjsCharacterRecipeRe } from '../recipes/CjsCharacterRecipeResolution.js';
import { CjsCharacterResolutionIssue as _CjsCharacterResoluti } from '../recipes/CjsCharacterResolutionIssue.js';
import { CjsCharacterResolvedPart as _CjsCharacterResolved } from '../parts/CjsCharacterResolvedPart.js';
import { CjsCharacterResolvedRule as _CjsCharacterResolved$1 } from '../recipes/CjsCharacterResolvedRule.js';
import { CjsCharacterVisemeSet as _CjsCharacterVisemeSe } from '../face/CjsCharacterVisemeSet.js';

let _initClass, _init_data, _init_extra_data;
const CATALOG_KEYS = [["sources", "ref"], ["partMetadata", "id"], ["parts", "id"], ["materials", "id"], ["projections", "id"], ["poses", "id"], ["presets", "id"], ["sculptFields", "id"], ["blendshapeLimits", "id"], ["uniqueCharacters", "id"], ["visemeSets", "id"]];
const KEYED_CATALOGS = ["materials", "projections", "poses", "presets", "sculptFields", "blendshapeLimits", "uniqueCharacters", "visemeSets"];
let _CjsCharacterLibrary;
class CjsCharacterLibrary extends _CjsCharacterNode {
  static {
    ({
      ...
      family: "character"
    })], [[[void 0, type.objectRef("CjsCharacterLibraryData"), io, io.read], 16, "data"]], 0, void 0, _CjsCharacterNode));
  }
  data = _init_data(this, null);
  #indexes = (_init_extra_data(this), new Map());
  ...
  static expandData(data) {
    ...
    for (const [sourceID, source] of Object.entries(data.partSources)) {
      ...
      for (const [versionID, version] of Object.entries(source.versions || {})) {
        ...
        const resourcePaths = [...(resources.configPaths || []), ...(resources.geometryPaths || []), ...(resources.texturePaths || [])];
  ...
  SetData(value) {
    const prepared = value instanceof _CjsCharacterLibraryD ? value : _CjsCharacterLibrary.expandData(value || {});
    const data = prepared instanceof _CjsCharacterLibraryD ? prepared : _CjsCharacterLibraryD.from(prepared);
    ...
    ValidatePartCatalogReferences(data.parts, indexes.get("materials"), indexes.get("projections"));
  ...
  ResolvePartLodBundle(selection, requestedLod) {
    ...
    return _CjsCharacterLodBundl.resolve(part.lodBundles, requestedLod);
  }
  ...
  ResolveGraphPart(selection, {
    ...
    if (!lodBundle && HasGeometryResources(part)) {
      throw new Error(`Character part "${part.id}" has model resources but no complete LOD bundle`);
    }
    ...
    const bundledPaths = new Set((part.lodBundles || []).flatMap(bundle => [bundle.configurationPath, bundle.geometryPath].filter(Boolean)));
    const resourcePaths = UniquePaths(part.resourcePaths).filter(path => !bundledPaths.has(path));
    const dependencies = BuildPartDependencies(part, lodBundle, materials.map(id => this.GetMaterial(id)).filter(Boolean), projection, resourcePaths);
    return _CjsCharacterResolved.from({
      partID: part.id,
      typeID: part.typeID,
      name: part.name,
      sex: part.sex,
  ...
  GetRecipeLinks(selection) {
    ...
    return this.data?.recipeLinks instanceof Map ? this.data.recipeLinks.get(id) ?? null : this.data?.recipeLinks?.[id] ?? null;
  }
  ...
  ResolveRecipe(selection, {
    ...
    for (let entryIndex = 0; entryIndex < recipe.entries.length; entryIndex++) {
      ...
      if (!link) {
        issues.push(CreateResolutionIssue(entryIndex, "missing-link", `Recipe entry ${entryIndex} has no prepared link`));
        continue;
      }
      if (link.status !== "resolved") {
        issues.push(CreateResolutionIssue(entryIndex, link.issueCode || link.status || "unresolved", `Recipe entry ${entryIndex} is ${link.status || "unresolved"}`, link.candidatePartIDs));
        continue;
      }
      if (link.kind === "part") {
        try {
          const part = this.ResolveGraphPart(link.partID, {
            lod,
            weight: entry.weight
          });
          if (!part) {
            issues.push(CreateResolutionIssue(entryIndex, "missing-part", `Prepared part "${link.partID}" is not in the library`));
            continue;
          }
          if (!HasMatchingSex(recipe.sex, part.sex)) {
            issues.push(CreateResolutionIssue(entryIndex, "part-sex-mismatch", `Prepared part "${part.partID}" has sex "${part.sex}", expected "${recipe.sex}"`));
            continue;
          }
          part.recipeEntryIndex = entryIndex;
          parts.push(part);
        } catch (error) {
          issues.push(CreateResolutionIssue(entryIndex, "part-resolution-failed", error?.message || `Recipe entry ${entryIndex} could not resolve its part`));
        }
        continue;
      }
      if (link.kind === "morph") {
        if (link.morphName) {
          ...
          issues.push(CreateResolutionIssue(entryIndex, "missing-morph-name", `Recipe entry ${entryIndex} has no prepared morph name`));
        }
        continue;
      }
      if (link.kind === "rule") {
        ...
        if (!metadata) {
          issues.push(CreateResolutionIssue(entryIndex, "missing-rule-metadata", `Prepared metadata "${link.metadataID}" is not in the library`));
          continue;
        }
        rules.push(_CjsCharacterResolved$1.from({
      ...
      if (link.kind === "material") {
        if (!this.GetMaterial(link.materialID)) {
          issues.push(CreateResolutionIssue(entryIndex, "missing-material", `Prepared material "${link.materialID}" is not in the library`));
          continue;
        }
        materialIDs.add(link.materialID);
        continue;
      }
      issues.push(CreateResolutionIssue(entryIndex, "unsupported-link-kind", `Recipe entry ${entryIndex} has unsupported prepared kind "${link.kind}"`));
    }
    if (!links) {
      issues.unshift(CreateResolutionIssue(-1, "missing-link-set", `Recipe "${recipe.id}" has no prepared link set`));
    }
    return _CjsCharacterRecipeRe.from({
  ...
  BuildGraphFromResolution(value, options = {}) {
    const resolution = value instanceof _CjsCharacterRecipeRe ? value : _CjsCharacterRecipeRe.from(value || {});
    ...
    const materialIDs = new Set([...resolution.materialIDs, ...parts.flatMap(part => part.materialIDs)]);
    ...
    return _CjsCharacterGraph.from({
      ...
      materials: [...materialIDs].sort(Compare).map(id => RequireCatalogValue(this, "material", id).GetValues()),
      projections: [...projectionIDs].sort(Compare).map(id => RequireCatalogValue(this, "projection", id).GetValues()),
...
function CreateResolutionIssue(entryIndex, code, message, candidatePartIDs = []) {
  return _CjsCharacterResoluti.from({
    entryIndex,
    code,
    message,
    blocking: true,
...
function ResolveNameCandidates(candidates, name) {
  ...
  if (candidates.length > 1) {
    throw new Error(`Character name "${name}" is ambiguous (${candidates.length} identities)`);
  }
  return candidates[0];
}
...
function NormalizeSearchName(value) {
  return NormalizeName(value).normalize("NFKC").replace(/[^\p{L}\p{N}]+/gu, " ").trim().replace(/\s+/gu, " ");
}
  ...
  for (const part of parts || []) {
    for (const materialID of part.colorIds || []) {
      if (!materials.has(materialID)) {
        throw new Error(`CjsCharacterLibrary part "${part.id}" references unknown material "${materialID}"`);
      }
    }
    if (part.projectionId && !projections.has(part.projectionId)) {
      throw new Error(`CjsCharacterLibrary part "${part.id}" references unknown projection "${part.projectionId}"`);
    }
  }
}
function ValidateRecipeLinkSets(recipeLinks, presets) {
  const values = recipeLinks instanceof Map ? recipeLinks.entries() : Object.entries(recipeLinks || {});
  for (const [presetID, linkSet] of values) {
    ...
    if (!HasMatchingSex(preset.sex, linkSet.sex)) {
      throw new Error(`CjsCharacterLibrary recipe "${presetID}" link sex "${linkSet.sex}" does not match preset sex "${preset.sex}"`);
    }
    const indexes = new Set();
    for (const link of linkSet.entries || []) {
      if (!Number.isInteger(link.entryIndex) || link.entryIndex < 0 || link.entryIndex >= preset.entries.length) {
...
function PrepareVisemeSets(visemeSets) {
  return (visemeSets || []).map(value => _CjsCharacterVisemeSe.prepare(value));
}
function NormalizeGraphSelection(value, defaultLod) {
  ...
  return {
    selection: value,
    lod: value && typeof value === "object" ? value.lod ?? value.requestedLod ?? defaultLod ?? null : defaultLod ?? null,
...
function ResolveGraphRecipe(library, value) {
  ...
  if (value instanceof _CjsCharacterRecipe) {
    return value;
  }
  ...
  return _CjsCharacterRecipe.from(value);
}
...
function ResolveRequestedGraphSex(recipe, requestedSex) {
  ...
  if (requestedSex !== null && requestedSex !== undefined) {
    ...
    if (!HasMatchingSex(recipeSex, requestedSex)) {
      throw new Error(`Character graph sex "${requestedSex}" does not match recipe sex "${recipeSex}"`);
    }
    return requestedSex;
  }
  return recipeSex;
...
function ResolveGraphSex(recipe, requestedSex, parts) {
  ...
  if (mismatch) {
    throw new Error(`Character part "${mismatch.partID}" has sex "${mismatch.sex}", expected "${sex}"`);
  }
  return sex;
}
function AppendPartSexIssues(issues, parts, sex) {
  ...
  const existing = new Set(issues.filter(issue => issue.code === "part-sex-mismatch").map(issue => issue.entryIndex));
  for (const part of parts) {
    if (HasMatchingSex(sex, part.sex) || existing.has(part.recipeEntryIndex)) {
      continue;
    }
    issues.push(CreateResolutionIssue(part.recipeEntryIndex, "part-sex-mismatch", `Prepared part "${part.partID}" has sex "${part.sex}", expected "${sex}"`));
  }
}
...
function BuildPartDependencies(part, lodBundle, materials, projection, resourcePaths) {
  ...
  if (projection?.maskPath) {
    values.push(CreateDependency(part.id, projection.maskPath, "texture", `projection-mask:${projection.id}`, projection.maskPathEnabled));
  }
  return UniqueDependencies(values);
}
function CreateDependency(partID, path, kind, role, required = true) {
  return _CjsCharacterDependen.from({
    id: `${kind}:${path}`,
    path,
    kind,
    required,
...
function UniqueDependencies(values) {
  const result = new Map();
  for (const value of values) {
    const dependency = value instanceof _CjsCharacterDependen ? value : _CjsCharacterDependen.from(value);
...
function HasGeometryResources(part) {
  return (part.lodBundles || []).length > 0 || (part.resourcePaths || []).some(path => {
...
function ResolveResources(inherited, overrides) {
  const result = Object.fromEntries(["configPaths", "geometryPaths", "texturePaths"].map(key => [key, Object.hasOwn(overrides, key) ? overrides[key] : inherited[key] || []]));
  const modelsOverridden = Object.hasOwn(overrides, "configPaths") || Object.hasOwn(overrides, "geometryPaths");
  result.lodBundles = Object.hasOwn(overrides, "lodBundles") ? overrides.lodBundles : !modelsOverridden && Object.hasOwn(inherited, "lodBundles") ? inherited.lodBundles : [];
  return result;
}
function ResolveLodBundles(resources) {
  ...
  return _CjsCharacterLodBundl.fromResources(resources.configPaths, resources.geometryPaths).map(value => value.GetValues());
}
