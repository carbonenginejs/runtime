...
function validateAudioLibraryDocument(value) {
  ...
  const usesBusVoiceVolume = Object.values(normalizedSfx?.programs ?? {}).some(actions => actions.some(action => action.kind === "set-bus-voice-volume"));
  if (usesBusVoiceVolume && value.busGraph === undefined) {
    throw new TypeError("Audio library Bus-target Voice Volume requires a Bus graph");
  }
  if (value.busGraph !== undefined) {
    ...
    ValidateBusGraphVoiceVolumeActionRoutes(busGraph, normalizedSfx);
    ValidateBusGraphVolumeActionRisk(busGraph, normalizedSfx);
  }
  return true;
}
function ValidateBusGraphVoiceVolumeActionRoutes(busGraph, sfx) {
  const targets = new Set(Object.values(sfx?.programs ?? {}).flatMap(actions => actions.filter(action => action.kind === "set-bus-voice-volume").map(action => String(action.targetId))));
  for (const targetId of targets) {
    if (busGraph.buses[targetId]?.type !== "audio-bus") {
      throw new TypeError(`Audio library Bus-target Voice Volume ${targetId} must target an Audio Bus`);
    }
    for (const [nodeId, node] of Object.entries(sfx?.nodes ?? {})) {
      if (node?.type !== "sound" || !node.busPathIds?.map(String).includes(targetId)) {
        continue;
      }
      const route = busGraph.routes[busGraph.sfxRoutes[nodeId]];
      const wetRoute = route.userAuxSends.length || route.reflectionsAuxSend || route.busPathIds.some(busId => {
        const bus = busGraph.buses[busId];
        return bus.userAuxSends.length || bus.reflectionsAuxSend;
      });
      if (wetRoute) {
        throw new TypeError(`Audio library Bus-target Voice Volume ${targetId} cannot use an Aux route`);
      }
    }
    for (const routeIndex of Object.values(busGraph.musicRoutes)) {
      if (busGraph.routes[routeIndex].busPathIds.includes(targetId)) {
        throw new TypeError(`Audio library Bus-target Voice Volume ${targetId} cannot affect music`);
      }
    }
  }
}
...
function ValidateBusStates(value) {
  ...
  if (legacy && (catalog.property !== "bus-volume" || catalog.accumulation !== "additive" || catalog.unit !== "db")) {
  ...
  for (const [rawBusId, groups] of Object.entries(buses)) {
    ...
    for (const [index, groupValue] of groups.entries()) {
      ...
      for (const [stateIndex, stateValue] of group.states.entries()) {
        ...
        const fields = [["gainDb", -200, 200], ["pitchCents", -2400, 2400], ["lowPass", -100, 100], ["highPass", -100, 100]].filter(([field]) => state[field] !== undefined);
        if (legacy && (fields.length !== 1 || fields[0][0] !== "gainDb")) {
          throw new TypeError(`${stateLabel} must contain one Bus Volume value`);
        }
        if (!legacy && !fields.length) {
          throw new TypeError(`${stateLabel} must contain a Bus State property`);
        }
        for (const [field, minimum, maximum] of fields) {
          ...
          if (number < minimum || number > maximum) {
            throw new TypeError(`${stateLabel} ${field} must be from ${minimum} to ${maximum}`);
          }
  ...
  if (!legacy && catalog.filterBehavior !== undefined && catalog.filterBehavior !== "additive") {
    throw new TypeError("Audio library busStates has unsupported filter behavior");
  }
  if (!legacy && usesFilters && catalog.filterBehavior !== "additive") {
    throw new TypeError("Audio library busStates filters must use additive behavior");
  }
}
function ValidateBusRtpcs(value) {
  ...
  for (const [rawBusId, curves] of Object.entries(buses)) {
    ...
    for (const [index, curveValue] of curves.entries()) {
      ...
      if (catalog.schemaVersion === 1 && curve.property !== undefined) {
        throw new TypeError(`${label} version 1 cannot declare a property`);
      }
      if (catalog.schemaVersion === 2 && curve.property !== "voice-volume" && curve.property !== "bus-volume") {
        throw new TypeError(`${label} property must be voice-volume or bus-volume`);
      }
...
function ValidateMusic(music, media, embeddedMedia) {
  ...
  for (const [id, node] of Object.entries(nodes)) {
    ...
    if (node.type === "music-track") {
      ...
      throw new TypeError(`Audio library music node ${id} RTPC curves are track-only`);
    }
  }
...
function ValidateMusicBusRouting(node, id) {
  ...
  const outputBusId = NormalizePositiveID(node.outputBusId, `Audio library music node ${id} outputBusId`);
  ...
  const path = node.busPathIds.map((value, index) => NormalizePositiveID(value, `Audio library music node ${id} busPathIds ${index}`));
...
function ValidateMusicTrackRtpcCurves(curves, id) {
  ...
  if (!Array.isArray(curves) || !curves.length) {
    throw new TypeError(`Audio library music track ${id} rtpcCurves must be non-empty`);
  }
  for (let index = 0; index < curves.length; index++) {
    ...
    if (curve.defaultValue !== undefined && !Number.isFinite(Number(curve.defaultValue))) {
      throw new TypeError(`${label} defaultValue must be finite`);
    }
    if (!Array.isArray(curve.points) || !curve.points.length) {
      throw new TypeError(`${label} must have points`);
    ...
    for (let pointIndex = 0; pointIndex < curve.points.length; pointIndex++) {
      ...
      if (!Number.isFinite(x) || x < previous) {
        throw new TypeError(`${pointLabel} x must be finite and non-decreasing`);
      }
      if (!Number.isFinite(value) || value < -1 || value > 1) {
        throw new TypeError(`${pointLabel} value must be between -1 and 1`);
      }
      if (!Number.isSafeInteger(interpolation) || interpolation < 0 || interpolation > 9) {
        throw new TypeError(`${pointLabel} interpolation must be from 0 to 9`);
      }
      previous = x;
    }
  }
