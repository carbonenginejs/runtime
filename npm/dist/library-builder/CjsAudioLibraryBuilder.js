...
class CjsAudioLibraryBuilder {
  ...
  static createMusicGraph({
    ...
    const ancestry = new CjsAudioLibraryBuilderWwiseNodeBaseAncestry(id => parsed.nodes.get(id)?.nodeBase);
    const nodes = {};
    for (const [id, value] of [...parsed.nodes.entries()].sort(([left], [right]) => left - right)) {
      ...
      const routing = node.type === "music-track" ? CreateMusicBusRouting(ancestry, id, buses) : null;
...
class CjsAudioLibraryBuilderSfxNodeLoweringSession {
  ...
  constructor({
    ...
    this.#usedIDs = new Set([...parsed.nodes.keys()].map(value => String(value >>> 0)));
  }
  ...
  Lower(rawID) {
    ...
    try {
      ...
      const lowerChild = childID => {
        ...
        summary.childContainsContinuous ||= child.containsContinuous;
        summary.childContainsNonSwitchContinuous ||= child.containsNonSwitchContinuous;
        return child.nodeId;
      };
      let lowered;
      if (source.type === "sound") {
        ...
        lowered = this.#LowerRandomSequence(id, source, lowerChild, summary);
      } else if (source.type === "switch") {
        lowered = this.#LowerSwitch(id, source, lowerChild, summary);
      } else if (source.type === "layer") {
        lowered = this.#LowerLayer(id, source, lowerChild);
      } else {
      ...
      Object.assign(node, CreateSfxNodeBasePlaybackProjection(this.#ancestry, this.#parsed.nodes, id, this.#names), source.type === "sound" ? CreateSfxSoundVoiceLimitProjection(this.#ancestry, id, this.#buses) : {});
      this.#nodes[id] = node;
      const projection = {
        nodeId: id,
        leaves: summary.leaves,
        containsContinuous: summary.childContainsContinuous || node.continuous !== undefined || source.type === "layer" && source.continuousValidation && neverCompletes,
        containsNonSwitchContinuous: summary.childContainsNonSwitchContinuous || node.continuous !== undefined && node.type !== "switch" || source.type === "layer" && source.continuousValidation && neverCompletes,
        neverCompletes
      };
  ...
  #LowerSound(id, source, summary) {
    ...
    const loopCount = this.#parsed.nodeBases?.get(Number(id))?.loopCount;
    const matchIds = CreateSfxMatchIds(this.#ancestry, id);
    const routing = CreateSfxBusRouting(this.#ancestry, id, this.#buses);
    if (source.pluginType === 1 && !this.#media[mediaID] && !this.#embeddedMedia[mediaID]) {
    ...
    if (source.pluginType === 1) {
      ...
      node = {
        type: "timed-silence",
        durationMs: ParseStaticWwiseSilenceDuration(this.#effects, source, id),
        ...identity
      };
    } else {
  ...
  #LowerRandomSequence(id, source, lowerChild, summary) {
    ...
    if (source.continuous && (source.transitionMode === 1 || source.transitionMode === 2)) {
      this.#QualifyCrossfadeChildren(source.playlist.length ? source.playlist.map(item => item.playId) : source.children, id);
    }
    ...
    const absorbsInfiniteChildren = source.type === "random" && source.continuous && source.loopCount === 0 && source.transitionMode === 0 && summary.childContainsContinuous && children.every(child => this.#projections.get(String(child.nodeId))?.neverCompletes === true);
    const nestedChild = children.length === 1 ? this.#nodes[String(children[0].nodeId)] : null;
    ...
    if (source.continuous && summary.childContainsContinuous && !absorbsInfiniteChildren && !supportsNestedTriggerRateDelay && !supportsNestedCrossfadeDelay) {
      throw new Error(`nested continuous container ${id}`);
    }
    const neverCompletes = absorbsInfiniteChildren || source.continuous && source.loopCount === 0;
    const node = {
      type: source.type,
  ...
  #LowerSwitch(id, source, lowerChild, summary) {
    const empty = source.groupType === 0 && source.groupId === 0 && source.defaultValueId === 0 && !source.continuousValidation && !source.children.length && !source.assignments.length;
    if (empty) {
    ...
    const group = this.#names.groups.get(`${scope}:${source.groupId}`);
    if (!group?.name) {
    ...
    if (source.continuousValidation && summary.childContainsNonSwitchContinuous) {
  ...
  #LowerLayer(id, source, lowerChild) {
    ...
    const associatedContinuous = source.continuousValidation && source.layers.some(layer => layer.associations.length);
    const children = source.children.map(nodeId => ({
      nodeId: lowerChild(nodeId)
    }));
    const associatedChildIDs = new Set(source.layers.flatMap(layer => layer.associations.filter(association => association.points.length).map(association => Number(association.childId) >>> 0)));
    if (associatedContinuous && (source.layers.some(layer => layer.associations.some(association => !association.points.length)) || source.children.some(childID => !associatedChildIDs.has(Number(childID) >>> 0)))) {
      throw new Error(`continuous layer ${id} has incomplete associations`);
    }
    const finiteChildren = associatedContinuous ? source.children.filter(childID => this.#projections.get(String(Number(childID) >>> 0))?.neverCompletes !== true) : [];
    if (finiteChildren.length) {
      throw new Error(`continuous layer ${id} has finite child ` + finiteChildren.map(value => Number(value) >>> 0).join(","));
    }
    ...
    for (const layer of source.layers) {
      ...
      const parameter = associations.some(association => association.points.length) ? this.#names.parameters.get(Number(layer.controlId) >>> 0) : null;
      ...
      for (const association of associations) {
        ...
        for (const rtpc of layer.initialRtpcs) {
          if (associatedContinuous && Number(rtpc.parameterId) === SFX_INITIAL_DELAY_PROPERTY) {
            throw new Error("continuous layer InitialDelay RTPC");
          }
          const curve = CreateSfxRtpcCurve(rtpc, this.#names);
          if (!curve) {
  ...
  #Allocate(node) {
    while (this.#syntheticID > 0 && this.#usedIDs.has(String(this.#syntheticID))) {
      this.#syntheticID--;
    }
  ...
  #QualifyCrossfadeChildren(rawIDs, containerID) {
    ...
    while (pending.length) {
      ...
      if (source.type === "sound" && this.#parsed.nodeBases?.get(id)?.loopCount === 0) {
    ...
    for (const key of qualifiedSounds) {
      ...
      if (this.#nodes[key]?.type === "sound" && this.#nodes[key].loop === undefined) {
...
class CjsAudioLibraryBuilderSfxEventLoweringSession {
  ...
  #FinalizeEvent(eventID, result) {
    if (IsMusicEventName(this.#eventNames.get(eventID))) {
      ...
      result.program = result.program.filter(IsRetainedMusicSfxAction);
      result.leaves.clear();
  ...
  #PublishNamedEvent(eventID, name, result) {
    ...
    const retainedProgram = music ? program.filter(IsRetainedMusicSfxAction) : program;
...
function IsBoundedCrossfadeLayerSiblingFallback(parsed, event, blockedActionID, action) {
  ...
  return IsPlainSfxPlayAction(sibling) && siblingSource?.type === "sound" && siblingSource.pluginType === 1 && siblingNodeBase && siblingNodeBase.loopCount !== 0 && !layer.children.some(childID => Number(childID) >>> 0 === Number(sibling.targetId) >>> 0);
}
...
function BusVoiceTargetIsOutputOnly(parsed, buses, targetId) {
  ...
  for (const [nodeId, node] of parsed.nodes ?? []) {
    ...
    const path = CreateSfxBusRouting(ancestry, nodeId, buses)?.busPathIds ?? [];
    if (!path.includes(String(targetId))) continue;
    used = true;
    if (path[0] !== String(targetId)) return false;
    const nodeAux = CreateEffectiveNodeAuxRouting(nodeId, ancestry);
...
function ReadSfxSetterAction(action, names) {
  ...
  const {
    groupID,
    valueID
  } = ReadSetterActionIDs(details, action.payload, `${scope} setter action ${action.id}`);
...
function ReadExactSetterDelay(details, {
  ...
  const expectedName = family === SFX_SET_SWITCH_ACTION_FAMILY ? "set-switch" : "set-state";
  const unsupported = details.delayRangeMs !== undefined || details.probability !== undefined || details.transitionTimeMs !== undefined || details.transitionRangeMs !== undefined || ranges !== undefined && (!Array.isArray(ranges) || ranges.length !== 0) || properties !== undefined && !Array.isArray(properties) || Array.isArray(properties) && properties.some(property => Number(property?.id) !== 0x39);
  if (unsupported) {
  ...
  if (!Number.isFinite(delayMs) || delayMs < 0 || properties?.length !== 1 || !Number.isFinite(propertyValue) || !Number.isFinite(rawPropertyValue) || propertyValue !== delayMs || rawPropertyValue !== delayMs || Number(details.actionType) >>> 0 !== Number(actionType) >>> 0 || details.actionName !== expectedName || Number(details.actionFamily) !== family || details.groupId === undefined || details.valueId === undefined || details.targetId === undefined || targetID === undefined || Number(details.targetId) >>> 0 !== Number(targetID) >>> 0) {
...
class CjsAudioLibraryBuilderBusGraphConstructionSession {
  ...
  Build() {
    ...
    return {
      ...
      effects: Object.fromEntries(Object.entries(this.#graphEffects).sort(([left], [right]) => Number(left) - Number(right))),
      routes: this.#routes,
  ...
  #ParseSources() {
    this.#effectsResult = CjsBnkFormat.wwise.effectNodesFromBanks(this.#inspections);
    this.#sfxResult = CjsBnkFormat.wwise.sfxNodesFromBanks(this.#inspections);
    this.#musicResult = this.#musicInspections.length ? CjsBnkFormat.wwise.musicNodesFromBanks(this.#musicInspections) : {
      nodes: new Map(),
      diagnostics: {
        failed: [],
        unsupportedVersions: []
    ...
    if (this.#effectsResult.diagnostics.failed.length || this.#effectsResult.diagnostics.unsupportedVersions.length || this.#sfxResult.diagnostics.failed.length || this.#sfxResult.diagnostics.unsupportedVersions?.length || this.#musicResult.diagnostics.failed.length || this.#musicResult.diagnostics.unsupportedVersions?.length) {
      throw new Error("Audio Bus graph qualification failed");
    }
    this.#sfxAncestry = new CjsAudioLibraryBuilderWwiseNodeBaseAncestry(id => this.#sfxResult.nodeBases.get(id));
    this.#musicAncestry = new CjsAudioLibraryBuilderWwiseNodeBaseAncestry(id => this.#musicResult.nodes.get(id)?.nodeBase);
  }
  ...
  #CollectCandidates() {
    for (const [id, node] of this.#sfxResult.nodes) {
      if (node.type !== "sound") continue;
      const routing = CreateSfxBusRouting(this.#sfxAncestry, id, this.#buses);
      if (!routing) continue;
    ...
    for (const [id, node] of this.#musicResult.nodes) {
      ...
      const routing = CreateMusicBusRouting(this.#musicAncestry, id, this.#buses);
      if (!routing) continue;
      this.#candidates.push({
        kind: "music",
        nodeId: String(id),
  ...
  #InternRoutes() {
    ...
    this.#routes = signatures.map(signature => routesBySignature.get(signature));
    for (const candidate of this.#candidates.sort((left, right) => Number(left.nodeId) - Number(right.nodeId))) {
      const target = candidate.kind === "sfx" ? this.#sfxRoutes : this.#musicRoutes;
  ...
  #CollectReachableBuses() {
    const pending = this.#routes.flatMap(route => [...route.busPathIds, ...(route.userAuxSends ?? []).map(send => send.targetBusId), route.reflectionsAuxSend?.targetBusId].filter(Boolean).map(Number));
    while (pending.length) {
  ...
  #ProjectBuses() {
    for (const busId of [...this.#reachable].sort((left, right) => left - right)) {
      const bus = this.#buses.get(busId);
      ...
      this.#graphBuses[String(busId)] = {
        ...
        ...aux,
        effects: slots,
        requiresProcessing: this.#CreateProcessingReasons(bus, aux, slots)
      };
    }
  }
  ...
  #ProjectEffectSlots(busId, bus) {
    return [...(bus.fx?.slots ?? [])].sort((left, right) => left.index - right.index).map(slot => {
      ...
      this.#graphEffects[String(effect.id)] = CreatePortableEffect(effect);
      return {
...
function CreateBusGraphCatalog(inspections, musicInspections, buses) {
  return new CjsAudioLibraryBuilderBusGraphConstructionSession({
    inspections,
    musicInspections,
    buses
  }).Build();
...
function CreateEffectiveNodeAuxRouting(rawId, ancestry) {
  ...
  for (const {
    ...
    if (!reflectionsResolved && (nodeBase.aux?.overrideReflectionsAux || root)) {
      reflectionsAuxSend = CreateReflectionsAuxSend(nodeBase, `Wwise NodeBase ${id}`);
      reflectionsResolved = true;
    }
    if (userResolved && reflectionsResolved) break;
  }
  if ((!userResolved || !reflectionsResolved) && trace.terminal === "cycle") {
    throw new Error(`Wwise NodeBase aux ancestry cycle at ${trace.terminalId}`);
  }
  return {
    ...(userAuxSends?.length ? {
      userAuxSends
...
function ParseStaticWwiseCompressor(ownerLabel, slot, effect) {
  if (effect.media?.length || effect.rtpcs?.length || effect.state?.properties?.length || effect.state?.groups?.length || effect.propertyValues?.length) {
    throw new Error(`Wwise Compressor ${effect.id} on ${ownerLabel} is not static`);
  }
  ...
  // Keep host policy out of the portable library. The base record proves
  // that the explicit browser approximation could realize this complete
  // source chain; strict runtimes still omit the chain and play dry.
  return {
    ...approximation,
    type: "compressor"
  };
...
function ParseDynamicWwiseFlanger(ownerLabel, slot, effect, names) {
  if (Number(effect.bankVersion) !== 150 || effect.media?.length || effect.rtpcs?.length !== 1 || effect.state?.properties?.length || effect.state?.groups?.length || effect.propertyValues?.length !== 1) {
    throw new Error(`Wwise Flanger ${effect.id} on ${ownerLabel} has unsupported controls`);
  }
  ...
  if (parameter !== "ship_Distance" || defaultValue !== 0 || transition?.rampType !== 0 || transition.rampUpSeconds !== 0 || transition.rampDownSeconds !== 0 || builtInParameter !== 1 || Number(rtpc.controlType) !== 0 || Number(rtpc.parameterId) !== 1 || Number(rtpc.accumulation) !== SFX_ADDITIVE_ACCUMULATION || Number(rtpc.scaling) !== 0 || !rtpc.points?.length || Number(property.propertyId) !== 1 || Number(property.accumulation) !== SFX_ADDITIVE_ACCUMULATION || Number(property.value) !== parsed.wetDryMixPercent) {
    throw new Error(`Wwise Flanger ${effect.id} on ${ownerLabel} has unsupported distance control`);
  }
  let previous = -Infinity;
  const points = rtpc.points.map((point, index) => {
    const x = Number(point.from);
    const value = Number(point.to);
    const interpolation = Number(point.interpolation);
    if (!Number.isFinite(x) || !Number.isFinite(value) || x < previous || !Number.isSafeInteger(interpolation) || interpolation < 0 || interpolation > 9) {
      throw new Error(`Wwise Flanger ${effect.id} on ${ownerLabel} has invalid distance point ${index}`);
    }
    previous = x;
    return {
      x,
...
function ParseDynamicWwiseTremolo(ownerLabel, slot, effect, names) {
  ...
  const property = effect.propertyValues[0];
  const controlIds = new Set(effect.rtpcs.map(rtpc => Number(rtpc.controlId) >>> 0));
  ...
  const rtpcCurves = effect.rtpcs.map(rtpc => {
    ...
    if (!target || seen.has(target.property) || Number(rtpc.controlType) !== 0 || Number(rtpc.accumulation) !== target.accumulation || Number(rtpc.scaling) !== target.scaling || !rtpc.points?.length) {
      throw new Error(`Wwise Tremolo ${effect.id} on ${ownerLabel} has unsupported RTPC`);
    }
    ...
    return {
      ...
      accumulation: target.accumulation === SFX_ADDITIVE_ACCUMULATION ? "additive" : "exclusive",
...
function ParseStaticWwiseGuitarDistortion(ownerLabel, slot, effect) {
  if (effect.media?.length || effect.rtpcs?.length || effect.state?.properties?.length || effect.state?.groups?.length || effect.propertyValues?.length) {
    throw new Error(`Wwise Guitar Distortion ${effect.id} on ${ownerLabel} is not static`);
  }
...
function ParseStaticWwiseMatrixReverb(ownerLabel, slot, effect) {
  if (effect.media?.length || effect.rtpcs?.length || effect.state?.properties?.length || effect.state?.groups?.length || effect.propertyValues?.length) {
    throw new Error(`Wwise Matrix Reverb ${effect.id} on ${ownerLabel} is not static`);
  }
...
function CreateSfxSoundEffectProjection(ancestry, effects, names, rawId) {
  ...
  for (const {
    ...
    nodeBase,
    parentId
  } of ancestry.Trace(soundId).entries) {
    const candidate = nodeBase.fx;
    if (!candidate || candidate.overrideParentRaw !== 0 && candidate.overrideParentRaw !== 1 || candidate.bypassAllRaw !== 0 && candidate.bypassAllRaw !== 1 || candidate.bypassAll) {
      return {};
    }
  ...
  try {
    for (const slot of slots) {
      ...
      if (!effect || slot.shareSet !== (effect.type === "effect-share-set")) {
        return {};
      }
      if (effect.pluginId === PARAMETRIC_EQ_PLUGIN_ID) {
        ...
        const ownerLabel = `NodeBase ${ownerId} inherited by Sound ${soundId}`;
        chain.push(effect.rtpcs?.length ? ParseDynamicWwiseFlanger(ownerLabel, slot, effect, names) : ParseStaticWwiseFlanger(ownerLabel, slot, effect));
      } else if (effect.pluginId === WWISE_TREMOLO_PLUGIN_ID) {
        const ownerLabel = `NodeBase ${ownerId} inherited by Sound ${soundId}`;
        chain.push(effect.rtpcs?.length ? ParseDynamicWwiseTremolo(ownerLabel, slot, effect, names) : ParseStaticWwiseTremolo(ownerLabel, slot, effect));
...
function CreateSfxBusRouting(ancestry, rawID, buses) {
  for (const {
    id,
    nodeBase
  } of ancestry.Trace(rawID).entries) {
    const outputBusId = Number(nodeBase.overrideBusId) >>> 0;
    if (outputBusId) {
      return CreateBusRouting(outputBusId, buses, ReadOutputBusVolume(nodeBase, id));
    }
  }
  return null;
}
...
function CreateSfxStopRelationships(ancestry, leavesByEvent, stopTargetsByEvent) {
  const events = {};
  const projected = [];
  const unresolved = [];
...
function CreateSfxSoundVoiceLimitProjection(ancestry, rawID, buses) {
  ...
  if (!nodeBase || advanced?.flags !== 0x09 || advanced.maxInstances !== 1 || !(buses instanceof Map) || trace.terminal !== "root") {
    return {};
  }
...
function CreateSfxNodeBasePlaybackProjection(ancestry, playableNodes, rawID, names) {
  const chain = [];
  const trace = ancestry.Trace(rawID);
  for (const {
    nodeBase,
...
class CjsAudioLibraryBuilderSfxNameCatalogAccumulator {
  ...
  #AddParameterDefault(parameter) {
    ...
    const builtInParameter = parameter.builtInParameter === undefined ? undefined : Number(parameter.builtInParameter);
    if (builtInParameter !== undefined && (!Number.isSafeInteger(builtInParameter) || builtInParameter < 0 || builtInParameter > 255)) {
      throw new TypeError(`Audio STMG game parameter ${id}` + " builtInParameter is invalid");
    }
    const existingBuiltIn = this.#parameterBuiltIns.get(id);
    if (builtInParameter !== undefined && existingBuiltIn !== undefined && existingBuiltIn !== builtInParameter) {
      throw new TypeError(`Audio STMG game parameter ${id}` + ` builtInParameter conflicts with ${existingBuiltIn}`);
    }
    if (builtInParameter !== undefined) {
      this.#parameterBuiltIns.set(id, builtInParameter);
    }
    const hasTransition = parameter.rampType !== undefined || parameter.rampUp !== undefined || parameter.rampDown !== undefined;
    ...
    if (!Number.isSafeInteger(rampType) || rampType < 0 || rampType > WWISE_FILTERING_OVER_TIME_RAMP || !Number.isFinite(rampUpSeconds) || rampUpSeconds < 0 || !Number.isFinite(rampDownSeconds) || rampDownSeconds < 0) {
      throw new TypeError(`Audio STMG game parameter ${id}` + " transition settings are invalid");
    }
    const transition = {
      rampType,
      rampUpSeconds,
    ...
    if (existingTransition && JSON.stringify(existingTransition) !== JSON.stringify(transition)) {
      throw new TypeError(`Audio STMG game parameter ${id}` + " transition settings conflict between banks");
    }
...
function IsRetainedMusicSfxAction(action) {
  return action.kind === "set-bus-volume" || action.kind === "reset-bus-volume" || (action.kind === "pause" || action.kind === "resume") && action.scope === "game-object" && action.mode === "all" && action.targetId === "0" && Number(action.targetFlags) === 0 && !action.exceptions?.length;
}
...
function createMusicEventProjection(inspections, metadata, nodes) {
    ...
    for (const [eventID, event] of eventsByID) {
      ...
      for (const actionID of actionIDs) {
        ...
        if (family === 0x04 && nodes[fields.targetID]) {
          ...
          const identity = ReadSetterActionIDs(fields.action, fields.payload, `Music setter action ${actionID}`);
  ...
  for (const [name, actions] of Object.entries(programs)) {
    ...
    } else if (eventTargets[name]?.length || eventStops[name]?.length || switchSetters[name]?.length) {
      throw new Error(`unsupported ordered Music action mix ${name}`);
    }
  }
  return {
    ...
    for (const setter of table[name]) {
      for (const field of ["delayRangeMs", "probability", "transitionMs", "transitionTimeMs", "transitionRangeMs", "properties", "ranges"]) {
        if (setter[field] !== undefined) {
          throw new TypeError(`Music setter ${name} has unsupported ${field}`);
        }
      }
      const normalized = {
        kind: setter.kind,
      ...
      unique.set(`${normalized.kind}:${normalized.groupId}:` + `${normalized.targetId}:` + `${Number(normalized.delayMs) || 0}`, normalized);
    }
...
function validateMusicGraph(music, media, embeddedMedia) {
  ...
  for (const [name, setters] of Object.entries(music.switchSetters)) {
    ...
    const keys = setters.map(setter => {
      ...
      if (!Number.isFinite(delayMs) || delayMs < 0) {
        throw new TypeError(`Audio library music switchSetters.${name} has an invalid delay`);
      }
  ...
  ValidateMusicPrograms(music.programs, music.nodes, music.eventTargets, music.eventStops, music.switchSetters);
}
function ValidateMusicPrograms(programs, nodes, eventTargets, eventStops, switchSetters) {
  ...
  for (const [name, actions] of Object.entries(programs)) {
    ...
    for (const action of actions) {
      ...
      const targetId = normalizeUnsignedID(action?.targetId, `Audio library music programs.${name} targetId`);
      ...
      if (!action || !["pause", "resume"].includes(action.kind) || action.scope !== "game-object" || !["element", "all"].includes(action.mode) || Number(action.targetFlags) !== 0 || Number(action.actionFlags) !== expectedFlags || !Array.isArray(action.exceptions) || action.exceptions.length || !Number.isSafeInteger(Number(action.curve)) || Number(action.curve) < 0 || Number(action.curve) > 9 || !Number.isFinite(transitionMs) || transitionMs < 0 || unsupported) {
