...
class CjsAudioLibraryBuilder {
  ...
  static build(options = {}) {
    ...
    if (busGraph !== null && Object.keys(busGraph.buses ?? {}).length) {
      library.busGraph = normalizeBusGraphCatalog(MarkBusGraphVolumeIncreaseRisk(busGraph, library.sfx), library.embeddedMedia ?? {});
    }
    if (source) {
...
function CreateBusGraphCatalog(inspections, musicInspections, buses) {
  ...
  for (const busId of [...reachable].sort((left, right) => left - right)) {
    ...
    if (bus.rtpcs?.length) {
      reasons.push(bus.rtpcs.every(rtpc => BUS_RTPC_PROPERTIES.has(Number(rtpc.parameterId))) ? "rtpc" : "unsupported-rtpc");
    }
