...
class CjsAudioLibraryBuilderSfxNodeLoweringSession {
  ...
  Lower(rawID) {
    ...
    try {
      ...
      Object.assign(node, CreateSfxNodeBasePlaybackProjection(this.#parsed, id, this.#names), source.type === "sound" ? CreateSfxSoundVoiceLimitProjection(this.#parsed, id, this.#buses) : {});
      this.#nodes[id] = node;
      const projection = {
        nodeId: id,
  ...
  #LowerSound(id, source, summary) {
    ...
    const routing = CreateSfxBusRouting(this.#parsed, id, this.#buses);
    ...
    if (source.pluginType === 1) {
      node = {
        ...
        mediaId: mediaID,
        ...identity,
        ...CreateSfxSoundEffectProjection(this.#parsed, this.#effects, id),
        ...(loopCount === 0 ? {
          loop: true
