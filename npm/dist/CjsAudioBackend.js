...
import { indexBusEffectCatalog, normalizeWwiseDynamicsMode, normalizeWwiseDistortionMode, normalizeWwiseModulationMode, normalizeWwiseMeterFeedbackMode, createBusEffectChain, createWwiseEffectChain, normalizeStaticSourceEffectChain } from './internal/busEffects.js';
...
class CjsAudioBackend {
  ...
  #voiceLimitLedger = new CjsAudioBackendSfxVoiceLimitLedger({
    isOwnerActive: owner => this.#playing.get(owner.playingID) === owner
  });
  #scheduledSfxActions = [];
  ...
  constructor({
    ...
    this.#stateTransitionCatalog = IndexStateTransitionCatalog(stateTransitions);
    ...
    try {
      if (this.#resolveSfxProgram) {
        ...
        if (resolvedProgram !== null && resolvedProgram !== undefined) {
          resolvedProgram = this.#InstallSfxProgram(playingID, record, resolvedProgram);
        }
      }
    } catch {
    ...
    Promise.resolve().then(() => {
      ...
      for (const descriptor of descriptors) {
        ...
        const voice = this.#CreateVoice(selectionMetadata ? {
          ...
          voiceLimitReservationId: selectionMetadata.voiceLimitReservationId,
      ...
      this.#ReleasePendingSfxVoiceLimitReservations(record, resolvedProgram);
    });
    return playingID;
  }
  ...
  ExecuteActionOnPlayingID(action, playingID, fadeOutDuration = 1000) {
    ...
    if (!breaking) {
      ...
      this.#scheduledSfxActions = this.#scheduledSfxActions.filter(value => value.ownerPlayingID !== playingID);
    }
  ...
  GetSourcePlayPosition(playingID) {
    ...
    const voice = record.voices?.find(value => !value.ended && (value.source || value.paused || value.pausing));
    ...
    if (!voice.paused && voice.positionAnchorContextTime !== null) {
      ...
      const to = Number(this.#context.currentTime);
      seconds += UsesVoicePitchAutomation(voice) ? IntegrateVoicePitchPlaybackRate(voice, from, to) : Math.max(0, to - from) * voice.playbackRate;
    }
    const duration = Number(voice.buffer?.duration);
    if (Number.isFinite(duration) && duration > 0) {
      seconds = voice.source?.loop || voice.loop || voice.playCount > 1 ? seconds % duration : Math.min(seconds, duration);
    }
  ...
  SetGlobalState(stateGroup, stateName) {
    ...
    if (changed) {
      ...
      const duration = this.#StateTransitionDuration(group, previous, state) / 1000;
      if (duration > 0 && !StateWeightsEqualTarget(fromWeights, state)) {
        this.#statePropertyTransitions.set(NormalizeStateIdentity(group), {
          fromWeights,
          toState: state,
          startTime: now,
          duration
        ...
        this.#statePropertyTransitions.delete(NormalizeStateIdentity(group));
      }
    }
    this.#globalStateValues.set(group, state);
    const transition = this.#statePropertyTransitions.get(NormalizeStateIdentity(group));
    this.#RefreshSfxControls(null, transition ? transition.startTime + transition.duration : null);
    if (changed) {
  ...
  #StateTransitionDuration(stateGroup, fromState, toState) {
    const group = this.#stateTransitionCatalog.get(NormalizeStateIdentity(stateGroup));
    if (!group) {
      return 0;
    }
    const custom = group.transitions.find(transition => StateTransitionEndpointMatches(transition.from, transition.fromId, fromState ?? group.noneState) && StateTransitionEndpointMatches(transition.to, transition.toId, toState));
  ...
  GetGlobalRTPCValue(rtpcName, at = undefined) {
    const name = String(rtpcName);
    return this.#ReadRtpcValue("global", name, undefined, at === undefined ? Number(this.#context?.currentTime) || 0 : Number(at) || 0);
  }
  ...
  #ProcessCrossfadeSlots() {
    ...
    for (const [playingID, record] of this.#playing) {
      ...
      for (const slot of record.programSlots?.values?.() ?? []) {
        if (slot.advanceMode !== "crossfade" || slot.state === "ended" || slot.state === "cancelled" || slot.broken || !slot.preparedBatch || !Number.isFinite(slot.nextTriggerContextTime) || slot.nextTriggerContextTime > now) {
          continue;
        }
        ...
        this.#UpdateOverlappingSlotState(slot);
        if (this.#MaybeAdvanceNestedCompletionBarrier(playingID, record, slot)) {
          this.#MaybeFinishSfxProgram(playingID, record);
          continue;
        }
        if (!slot.exhausted && slot.continuation && !slot.completionBarrier && batch.state !== "cancelled") {
          this.#PrepareCrossfadeSuccessor(playingID, record, slot);
        }
        this.#MaybeFinishSfxProgram(playingID, record);
      }
    }
  ...
  #BreakContinuousSlots(record) {
    ...
    for (const slot of record.programSlots?.values?.() ?? []) {
      ...
      if (slot.preparedBatch) {
        this.#DiscardTriggerRateBatch(record, slot, slot.preparedBatch);
      }
      slot.preparedBatch = null;
      for (const batch of slot.batches?.values?.() ?? []) {
        if (batch.state === "loading" || batch.state === "pending") {
  ...
  #IsRestrictedContinuousProgramVoice(record, voice) {
    const mode = voice.programSlotId === undefined ? null : record.programSlots?.get(voice.programSlotId)?.advanceMode;
  ...
  #Seek(playingID, seek) {
    ...
    const hasRestrictedContinuous = [...(record.programSlots?.values?.() ?? [])].some(slot => slot.advanceMode === "trigger-rate" || slot.advanceMode === "crossfade");
    const canSeekSfx = !record.sfxProgram || [...(record.programSlots?.values?.() ?? [])].some(slot => slot.advanceMode !== "trigger-rate" && slot.advanceMode !== "crossfade");
    if (!canSeekSfx) {
      return handled;
    }
    if (record.loaded) {
      ...
      const started = record.voices.filter(voice => voice.source && !voice.ended && (!hasRestrictedContinuous || !this.#IsRestrictedContinuousProgramVoice(record, voice)) && voice.startContextTime <= now + renderQuantum);
      if (!started.length) {
        return handled;
      }
    ...
    try {
      return program.map(operation => {
        ...
        for (const selection of originalSelections) {
          ...
          if (!limit || typeof limit !== "object" || Array.isArray(limit) || limit.scope !== "game-object" || Number(limit.maxInstances) !== 1 || limit.behavior !== "reject-newest" || !Number.isSafeInteger(counterNumber) || counterNumber <= 0 || counterNumber > 0xffffffff || String(counterNumber) !== String(limit.counterId) || Number(selection.delayMs) > 0) {
            throw new TypeError("Resolved SFX voice limit is unsupported");
          }
          const reservationID = this.#voiceLimitLedger.Reserve(record, String(counterNumber));
      ...
      for (const reservationID of created) {
        this.#voiceLimitLedger.Release(record, reservationID);
      }
      throw error;
    }
  }
  ...
  #ReleasePendingSfxVoiceLimitReservations(record, program) {
    for (const operation of Array.isArray(program) ? program : []) {
      for (const selection of operation.kind === "play" ? operation.selections ?? [] : []) {
        this.#voiceLimitLedger.ReleasePending(record, [selection]);
      }
    }
  }
  ...
  #AbortSfxProgramSlot(record, slot) {
    this.#voiceLimitLedger.ReleasePending(record, slot?.selections);
    for (const batch of slot?.batches?.values?.() ?? []) {
      this.#voiceLimitLedger.ReleasePending(record, batch.selections);
    }
    slot.Abort();
  }
  ...
  #InstallSfxProgram(playingID, record, program) {
    ...
    try {
      for (const rawOperation of program) {
        const operation = rawOperation.kind === "play" ? this.#ReserveSfxProgram(record, [rawOperation])[0] : rawOperation;
  ...
  #ProcessScheduledSfxActions() {
    const now = Number(this.#context?.currentTime) || 0;
    while (this.#scheduledSfxActions.length && this.#scheduledSfxActions[0].actionTime <= now) {
      const action = this.#scheduledSfxActions.shift();
      const owner = this.#playing.get(action.ownerPlayingID);
      if (!owner || owner.stopped) {
        continue;
      }
      owner.pendingProgramActions = Math.max(0, owner.pendingProgramActions - 1);
      this.#ApplySfxProgramAction(action, now);
      this.#MaybeFinishSfxProgram(action.ownerPlayingID, owner);
    }
  }
  ...
  #ApplySfxVoiceVolume(action) {
    ...
    const apply = nodes => {
      ApplyVoiceVolumeAction(nodes.voiceVolumes, targetId, action);
    };
  ...
  #ApplySfxBusVoiceVolume(action) {
    if (action.scope !== "game-object" || this.#emitterNodes.get(action.gameObjID) !== action.emitterNodes) {
      return;
    }
    ApplyVoiceVolumeAction(action.emitterNodes.busVoiceVolumes, String(action.targetId), action);
  ...
  #ApplySfxVoicePitch(action, now) {
    const targetId = String(action.targetId);
    const apply = nodes => {
      this.#AdvanceMatchingSfxVoices(nodes, targetId, now);
      ApplyVoicePitchAction(nodes.voicePitches, targetId, action);
    };
    if (action.scope === "global") {
      for (const nodes of this.#emitterNodes.values()) {
        apply(nodes);
  ...
  #RtpcTransitionEndForRecord(record, from) {
    return this.#ControlTransitionBoundariesForRecord(record, from).at(-1) ?? (Number(from) || 0);
  }
  ...
  #ControlTransitionBoundariesForRecord(record, from) {
    ...
    const add = transition => {
      const end = Number(transition?.startTime) + Math.max(0, Number(transition?.duration) || 0);
  ...
  #WriteGlobalRtpcValue(name, value) {
    ...
    this.#RefreshSfxControls();
    const authoredBusControl = busRtpcCatalogUsesControl(this.#busRtpcCatalog, name);
  ...
  #AdvanceMatchingSfxVoices(nodes, targetId, at) {
    for (const record of this.#playing.values()) {
      ...
      for (const voice of record.voices ?? []) {
        if (!voice.ended && voice.matchIds?.map(String).includes(targetId)) {
          this.#AdvanceSfxVoiceTransport(voice, at);
        }
      }
    }
  ...
  #ApplySfxPlaybackControl(action, now) {
    ...
    for (const [playingID, record] of this.#playing) {
      if (!record.sfx || action.scope === "game-object" && record.gameObjID !== action.gameObjID) {
        continue;
      }
      if (!record.sfxProgram) {
        if (CompareFallbackOrder(record, playingID, action) > 0) {
          continue;
        }
        for (const voice of record.voices ?? []) {
          if (!voice.ended && PlaybackControlMatchesValue(action, voice)) {
            this.#ApplySfxVoicePauseDepth(playingID, record, voice, action, now, pausing);
          }
        }
        continue;
      }
      for (const slot of record.programSlots.values()) {
        ...
        for (const selection of ProgramSlotSelections(slot)) {
          if (CompareProgramOrder(selection, slot, action) > 0 || !PlaybackControlMatchesValue(action, selection)) {
            continue;
          }
          AdjustPauseDepth(slot.pauseDepths, ProgramSelectionKey(selection), pausing);
        }
        for (const voice of slot.voices) {
          if (voice.ended || CompareProgramOrder(voice, slot, action) > 0 || !PlaybackControlMatchesValue(action, voice)) {
            continue;
          }
          this.#ApplySfxVoicePauseDepth(playingID, record, voice, action, now, pausing);
        }
  ...
  #ApplySfxVoicePauseDepth(playingID, record, voice, action, now, pausing) {
    const previous = Math.max(0, Number(voice.pauseDepth) || 0);
    const next = pausing ? previous + 1 : Math.max(0, previous - 1);
    voice.pauseDepth = next;
    if (previous === 0 && next === 1) {
      this.#PauseSfxVoice(voice, action, now);
    } else if (previous === 1 && next === 0) {
      this.#ResumeSfxVoice(playingID, record, voice, action, now);
    }
  }
  ...
  #PauseSfxVoice(voice, action, now) {
    ...
    const seconds = Math.max(0, Number(action.transitionMs) || 0) / 1000;
    ...
    if (!voice.source || pauseTime <= currentTime) {
      this.#FinalizeSfxVoicePause(voice, Math.min(pauseTime, currentTime));
      return;
    }
    const progress = seconds > 0 ? Math.max(0, Math.min(1, (currentTime - actionTime) / seconds)) : 1;
    const remaining = Math.max(0, pauseTime - currentTime);
    const param = voice.stopGain.gain;
    param?.cancelScheduledValues?.(currentTime);
    ScheduleWwiseFade(param, 1, 0, currentTime, remaining, Number(action.curve ?? LINEAR_FADE_CURVE), progress);
    voice.pausing = true;
  ...
  #ResumeSfxVoice(playingID, record, voice, action, now) {
    ...
    ClearPauseFadeForResume(voice.stopGain.gain, currentTime);
    ...
    voice.pauseContextTime = null;
    voice.pauseSource = null;
    const startTime = currentTime + RenderQuantumSeconds(this.#context);
    this.#StartVoice(playingID, record, voice, {
      kind: "resume",
    ...
    const seconds = Math.max(0, Number(action.transitionMs) || 0) / 1000;
    if (seconds > 0) {
      ScheduleWwiseFade(voice.stopGain.gain, 0, 1, startTime, seconds, Number(action.curve ?? LINEAR_FADE_CURVE));
    } else {
  ...
  #FinalizeDueSfxPauses() {
    ...
    for (const record of this.#playing.values()) {
      for (const voice of record.voices ?? []) {
        if (voice.pausing && voice.pauseSource === voice.source && Number(voice.pauseContextTime) <= now) {
          this.#FinalizeSfxVoicePause(voice, voice.pauseContextTime);
        }
      }
    }
  }
  ...
  #AdvanceContinuousSwitchSlot(playingID, record, slot) {
    ...
    let continuation = play?.continuations?.find(value => value.programSlotId === slot.id);
    ...
    continuation = play.continuations.find(value => value.programSlotId === slot.id);
    const now = Number(this.#context.currentTime) || 0;
    ...
    slot.cancelledSelectionKeys = CreateProgramCancelledSelectionKeys(selectionMetadata);
  ...
  #ApplySfxStop(stop, now) {
    ...
    for (const [playingID, record] of this.#playing) {
      ...
      for (const slot of record.programSlots.values()) {
        ...
        if (IsOverlappingAdvanceMode(slot.advanceMode)) {
          this.#ApplyTriggerRateStop(record, slot, stop, actionTime);
          continue;
        }
        const selections = slot.selections ?? [];
        const matchingSelections = selections.filter(selection => CompareProgramOrder(selection, slot, stop) <= 0 && PlaybackControlMatchesValue(stop, selection));
  ...
  #ApplyTriggerRateStop(record, slot, stop, actionTime) {
    ...
    for (const voice of matchingVoices) {
      this.#StopSfxProgramVoice(voice, stop.actionTime, stop.transitionMs, stop.curve, actionTime);
      if (voice.ended) {
        this.#SetSfxProgramSlotEnded(slot.playingID, record, voice);
      }
    }
    if (stopsWholeSlot) {
      slot.continuation = null;
  ...
  #MaybeFinishSfxProgram(playingID, record) {
    if (!record?.sfxProgram || record.posting || record.planningProgram || record.pendingProgramActions > 0) {
      return;
    }
  ...
  #CreateSfxControls(gameObjID, signal = null, playingID = 0, record = null) {
    return Object.freeze({
      ...
      getStatePropertyWeights: (group, at = undefined) => this.#ReadStatePropertyWeights(group, at ?? (Number(this.#context?.currentTime) || 0)),
  ...
  #CreateVoice(descriptor, emitterNodes, gameObjID) {
    ...
    const gain = this.#context.createGain();
    const busVoiceActionGain = descriptor.busVoiceVolumeActionControlled ? this.#context.createGain() : null;
    ...
    const hasBuiltInDistanceEffect = descriptor.sourceEffects?.some(effect => effect.wetDryMixRtpcCurve?.controlSource === "built-in-distance") === true;
    const needsExternalEffectControl = descriptor.sourceEffects?.some(effect => effect.rtpcCurves?.length || effect.driveRtpcCurve) === true;
    const canReadAllSourceEffects = !needsExternalEffectControl || typeof descriptor.getSourceEffectRtpc === "function";
    const readSourceEffectRtpc = hasBuiltInDistanceEffect && canReadAllSourceEffects ? (curve, at = undefined, readControl = false) => {
      if (curve.controlSource === "built-in-distance") {
        const distance = EvaluateSfxBuiltInDistance(emitterNodes?.position, this.#listenerPosition, emitterNodes?.scalingFactor);
        return readControl ? distance : evaluateWwiseRtpcCurve(curve.points, distance);
      }
      return descriptor.getSourceEffectRtpc?.(curve, at, readControl);
    ...
    gain.connect(busVoiceActionGain ?? busVoiceGain ?? transitionGain ?? busGain ?? busEffectInput);
    busVoiceActionGain?.connect(busVoiceGain ?? transitionGain ?? busGain ?? busEffectInput);
    ...
    const voice = new CjsAudioBackendSfxVoice({
      ...
      controlTransitionBoundaries: this.#ControlTransitionBoundariesForRecord({
        gameObjID,
        emitterNodes
      }, Number(this.#context?.currentTime) || 0),
      nodes: {
  ...
  #StartVoices(playingID, record, selectedVoices = null, batchStartContextTime = null) {
    ...
    const hasOrdinaryVoice = voices.some(voice => !this.#IsRestrictedContinuousProgramVoice(record, voice));
    ...
    for (const voice of voices) {
      ...
      if (voice.stopping || voice.pauseDepth > 0 || voice.paused || voice.pausing) {
        continue;
      }
      ...
      this.#StartVoice(playingID, record, voice, this.#IsRestrictedContinuousProgramVoice(record, voice) ? null : seek, voiceStartContextTime);
      if (voice.ended) {
        this.#SetSfxProgramSlotEnded(playingID, record, voice);
      }
    }
    if (batchStartContextTime === null) {
      const crossfadeSlots = new Set(voices.map(voice => record.programSlots?.get(voice.programSlotId)).filter(slot => slot?.advanceMode === "crossfade"));
  ...
  #StartVoice(playingID, record, voice, seek, startContextTime) {
    ...
    source.onended = () => {
      if (voice.source === source) {
        ...
        } else if (voice.pausing && voice.pauseSource === source) {
          this.#FinalizeSfxVoicePause(voice, voice.pauseContextTime);
        } else {
          this.#VoiceEnded(playingID, record, voice);
        }
  ...
  #AdvanceSfxProgramSlot(playingID, record, slot, boundaryContextTime, forceCompletionDelay = false) {
    ...
    const continuationDelayMs = Math.max(0, Number(continuation.delayMs) || 0);
    const pendingTransitionDelayMs = forceCompletionDelay ? 0 : continuationDelayMs;
    const pendingSelections = (play.selections ?? []).filter(selection => selection.programSlotId === slot.id);
    if (pendingTransitionDelayMs > 0 && pendingSelections.some(selection => selection.voiceLimit !== undefined)) {
      this.#ExhaustSfxProgramSlot(slot);
      return;
    }
    try {
    ...
    slot.advanceMode = continuation.advance === "trigger-rate" ? "trigger-rate" : continuation.advance === "crossfade" ? "crossfade" : "completion";
    ...
    const batchStartContextTime = boundaryContextTime + pendingTransitionDelayMs / 1000;
    ...
    Promise.resolve().then(() => this.#loadBuffer(record.eventID, record.eventName, record.sfxControls, program)).then(result => {
      ...
      if (overlappingBatch) {
        overlappingBatch.state = voices.length ? "voice" : "ended";
      }
      slot.voice = voices[0] ?? null;
      if (!voices.length) {
      ...
      if (slot.advanceMode === "crossfade" && !slot.exhausted && !slot.completionBarrier && slot.continuation) {
        this.#PrepareCrossfadeSuccessor(playingID, record, slot);
      }
    }).catch(() => {
  ...
  #AdvanceTriggerRateSlot(playingID, record, slot, boundaryContextTime) {
    ...
    if (!selectionMetadata.length) {
      ...
      batch.state = "ended";
      this.#UpdateOverlappingSlotState(slot);
      this.#MaybeAdvanceNestedCompletionBarrier(playingID, record, slot);
      this.#MaybeFinishSfxProgram(playingID, record);
      return;
    }
    Promise.resolve().then(() => this.#loadBuffer(record.eventID, record.eventName, record.sfxControls, program)).then(result => {
  ...
  #PrepareCrossfadeSuccessor(playingID, record, slot) {
    if (slot.advanceMode !== "crossfade" || slot.preparingCrossfade || slot.preparedBatch || slot.completionBarrier || !slot.continuation || slot.broken || slot.exhausted || record.stopped || this.#playing.get(playingID) !== record) {
      this.#UpdateOverlappingSlotState(slot);
      return;
    }
    const outgoingBatch = slot.currentBatch;
    const outgoingVoices = [...(outgoingBatch?.voices ?? [])];
    ...
    const timing = outgoingVoice.ended ? {
      boundary: now + RenderQuantumSeconds(this.#context),
      duration: 0,
      naturalEnd: now + RenderQuantumSeconds(this.#context)
    } : CrossfadeTiming(outgoingVoice, slot.transitionDelayMs);
    if (!timing) {
      this.#FailOverlappingSlot(slot);
      return;
    }
    ...
    try {
      ...
      transaction = this.#prepareSfxProgram(slot.continuation, record.sfxControls);
      if (!transaction || typeof transaction.commit !== "function" || typeof transaction.rollback !== "function") {
        throw new TypeError("Crossfade preparation must return a transaction");
      }
    ...
    let play = program.find(operation => operation.kind === "play");
    let continuation = play?.continuations?.find(value => value.programSlotId === slot.id);
    if (!play || !continuation || continuation.advance !== "crossfade" || continuation.crossfadeMode !== "crossfade-amplitude" && continuation.crossfadeMode !== "crossfade-power") {
      transaction?.rollback?.();
      this.#FailOverlappingSlot(slot);
      return;
    }
    let batchSelections = (play.selections ?? []).filter(selection => selection.programSlotId === slot.id);
    ...
    play = program.find(operation => operation.kind === "play");
    continuation = play.continuations.find(value => value.programSlotId === slot.id);
    batchSelections = (play.selections ?? []).filter(selection => selection.programSlotId === slot.id);
    const selectionMetadata = batchSelections.map(selection => CreateProgramSelectionMetadata(selection, timing.boundary));
    const selectionControllers = CreateProgramSelectionControllers(selectionMetadata);
    const cancelledSelectionKeys = CreateProgramCancelledSelectionKeys(selectionMetadata);
    const controller = new AbortController();
    const batch = slot.CreateBatch({
    ...
    Promise.resolve().then(() => this.#loadBuffer(record.eventID, record.eventName, record.sfxControls, program)).then(result => {
      ...
      const now = Number(this.#context.currentTime) || 0;
      const boundary = outgoingVoice.ended ? now + RenderQuantumSeconds(this.#context) : Math.max(timing.boundary, now + RenderQuantumSeconds(this.#context));
      const selection = batch.selections[0];
      const actionTime = boundary + Math.max(0, Number(descriptors[0].delayMs) || 0) / 1000;
      const voice = this.#CreateVoice({
        ...descriptors[0],
        actionIndex: selection.actionIndex,
        leafIndex: selection.leafIndex,
      ...
      voice.programBatchId = batch.id;
      ApplySlotPauseDepth(voice, slot);
      SetAudioParam(voice.transitionGain?.gain, 0, this.#context);
      ...
      slot.voice = outgoingVoice.ended ? voice : outgoingVoice;
      this.#StartVoices(playingID, record, [voice], boundary);
      const overlap = outgoingVoice.ended ? 0 : Math.max(0, Math.min(timing.duration, timing.naturalEnd - voice.startContextTime));
      if (overlap > 0) {
        this.#ScheduleVoiceCrossfade(outgoingVoice, 1, 0, voice.startContextTime, overlap, slot.crossfadeMode);
        this.#ScheduleVoiceCrossfade(voice, 0, 1, voice.startContextTime, overlap, slot.crossfadeMode);
      } else {
        SetAudioParam(voice.transitionGain?.gain, 1, this.#context);
      }
      this.#DisposeEndedSlotVoices(record, slot);
      this.#UpdateOverlappingSlotState(slot);
    }).catch(() => {
  ...
  #MaybeAdvanceNestedCompletionBarrier(playingID, record, slot) {
    if (!IsOverlappingAdvanceMode(slot.advanceMode) || !slot.completionBarrier || !slot.continuation || slot.broken || slot.exhausted || record.stopped || this.#playing.get(playingID) !== record || [...slot.voices].some(voice => !voice.ended) || [...(slot.batches?.values?.() ?? [])].some(batch => batch.state === "loading" || batch.state === "pending")) {
      return false;
    }
    const forceCompletionDelay = slot.advanceMode === "crossfade";
    slot.completionBarrier = false;
    this.#AdvanceSfxProgramSlot(playingID, record, slot, Number(this.#context.currentTime) || 0, forceCompletionDelay);
    return true;
  }
  ...
  #CommitHeardCrossfadeTransactions() {
    ...
    for (const record of this.#playing.values()) {
      for (const slot of record.programSlots?.values?.() ?? []) {
        this.#SettleCrossfadeBatchTransaction(slot.preparedBatch, now, false);
      }
    }
  }

  /** Commits a heard Crossfade choice or rolls back an unheard discard. */
  #SettleCrossfadeBatchTransaction(batch, now = Number(this.#context?.currentTime) || 0, rollbackUnheard = true) {
    if (!batch?.transaction) {
      return;
    }
    const heard = [...batch.voices].some(voice => voice.sourceStarted === true && voice.cancelledBeforeStart !== true && Number.isFinite(voice.startContextTime) && voice.startContextTime <= now);
    if (heard) {
  ...
  #DisposeEndedSlotVoices(record, slot) {
    ...
    for (const [id, batch] of slot.batches ?? []) {
      if (batch !== slot.currentBatch && batch !== slot.preparedBatch && (batch.state === "ended" || batch.state === "cancelled") && [...batch.voices].every(voice => voice.ended)) {
  ...
  #RefreshSfxControls(gameObjID = null, transitionEnd = null) {
    ...
    for (const record of this.#playing.values()) {
      ...
      for (const voice of record.voices ?? []) {
        if (!voice.ended) {
          const boundaries = this.#ControlTransitionBoundariesForRecord(record, now);
          const explicit = Number(transitionEnd);
          if (Number.isFinite(explicit) && explicit > now) {
            boundaries.push(explicit);
            boundaries.sort((left, right) => left - right);
          }
          voice.controlTransitionBoundaries = [...new Set(boundaries)];
          voice.rtpcTransitionEnd = voice.controlTransitionBoundaries.at(-1) ?? now;
          this.#ApplyVoiceGain(voice);
          this.#ApplyVoiceBusRtpcGain(voice);
          this.#ApplyVoiceBusGain(voice);
          this.#ApplyVoiceFilters(voice);
  ...
  #ApplyVoiceBusActionGain(voice) {
    ...
    ScheduleBusVoiceActionGain(voice.busVoiceActionGain.gain, voice, this.#context);
  }
  ...
  #ApplyVoiceBusRtpcGain(voice) {
    ...
    ScheduleBusVoiceRtpcGain(voice.busVoiceGain.gain, voice, this.#context, this.#busRtpcCatalog, (name, at) => this.#ReadRtpcValue("global", name, undefined, at));
  }
  ...
  #ApplyVoiceSourceEffects(voice, smooth = false) {
    voice.sourceEffectRtpcLane?.Apply?.(voice.controlTransitionBoundaries ?? [], smooth);
  }
  ...
  #AdvanceSfxVoiceTransport(voice, contextTime) {
    ...
    if (!voice.source || voice.positionAnchorContextTime === null || !Number.isFinite(anchor) || !Number.isFinite(time) || !variablePitch && (!Number.isFinite(rate) || rate <= 0) || time <= anchor) {
      return;
    }
    const elapsedMedia = variablePitch ? IntegrateVoicePitchPlaybackRate(voice, anchor, time) : (time - anchor) * rate;
    const duration = Number(voice.buffer?.duration);
    let offset = Math.max(0, Number(voice.offsetSeconds) || 0) + elapsedMedia;
    if (Number.isFinite(duration) && duration > 0) {
    ...
    if (variablePitch) {
      voice.playbackRate = EvaluateVoicePitchPlaybackRate(voice, time);
    }
    if (voice.repeatRemainingSeconds !== null) {
      voice.repeatRemainingSeconds = Math.max(0, voice.repeatRemainingSeconds - elapsedMedia);
  ...
  #ApplyVoicePlaybackRate(voice) {
    ...
    if (variablePitch) {
      ...
      ScheduleVoicePitchPlaybackRate(source?.playbackRate, voice, this.#context);
      if (source && !voice.stopping && voice.repeatRemainingSeconds !== null && voice.repeatAnchorContextTime !== null) {
        voice.scheduledEndContextTime = SolveVoicePitchRepeatEnd(voice, now);
        try {
          source.stop(voice.scheduledEndContextTime);
        } catch {
          // already stopped
  ...
  #HoldVoiceTransitionFade(voice, actionTime) {
    ...
    if (!param || !voice.transitionFadeScheduled || !Number.isFinite(start) || !Number.isFinite(duration) || duration <= 0 || actionTime < start || actionTime >= start + duration) {
      return;
    }
    if (typeof param.cancelAndHoldAtTime === "function") {
      param.cancelAndHoldAtTime(actionTime);
    ...
    const progress = (actionTime - start) / duration;
    const value = EvaluateCrossfadeGain(voice.transitionFadeFrom, voice.transitionFadeTo, progress, voice.transitionFadeMode);
    param.cancelScheduledValues?.(actionTime);
    param.setValueAtTime?.(value, actionTime);
    if ("value" in param) {
      param.value = value;
  ...
  #ScheduleVoiceCrossfade(voice, from, to, when, duration, mode) {
    ...
    ScheduleCrossfadeGain(voice.transitionGain?.gain, from, to, when, duration, mode);
  }
...
function EvaluateSfxBuiltInDistance(emitterPosition, listenerPosition, scalingFactor) {
  if (!Array.isArray(emitterPosition) || !Array.isArray(listenerPosition)) {
    return 0;
  }
  const distance = Math.hypot(Number(emitterPosition[0]) - Number(listenerPosition[0]), Number(emitterPosition[1]) - Number(listenerPosition[1]), Number(emitterPosition[2]) - Number(listenerPosition[2]));
...
function SetSpatialAudioParam(param, value, context, smooth) {
  ...
  if (!smooth || !Number.isFinite(now)) {
    param.value = value;
    return;
  }
  if (typeof param.setTargetAtTime === "function") {
    param.setTargetAtTime(value, now, SPATIAL_POSE_TIME_CONSTANT_SECONDS);
    return;
  }
  ...
  param.linearRampToValueAtTime(value, now + SPATIAL_POSE_TIME_CONSTANT_SECONDS * 3);
}
function IndexStateTransitionCatalog(value) {
  ...
  for (const rawGroup of value) {
    if (!rawGroup || typeof rawGroup !== "object" || Array.isArray(rawGroup) || !Array.isArray(rawGroup.transitions)) {
      throw new TypeError("State transition group must contain transitions");
    }
    const defaultTransitionMs = Number(rawGroup.defaultTransitionMs);
    if (!Number.isSafeInteger(defaultTransitionMs) || defaultTransitionMs < 0) {
      throw new TypeError("State transition defaultTransitionMs" + " must be a non-negative integer");
    }
    const group = {
      ...
      transitions: rawGroup.transitions.map(raw => {
        ...
        if (!raw || typeof raw !== "object" || Array.isArray(raw) || !Number.isSafeInteger(transitionMs) || transitionMs < 0) {
          throw new TypeError("State transition must define a non-negative" + " integer transitionMs");
        }
        return {
    ...
    if (rawGroup.states !== undefined) {
      if (!Array.isArray(rawGroup.states)) {
        throw new TypeError("State transition states must be an array");
      }
      for (const state of rawGroup.states) {
        AddRuntimeStateAlias(group.stateAliases, stateIdentities, state?.stateId, state?.state);
      }
    }
    for (const transition of group.transitions) {
      ReserveRuntimeStateIdentity(stateIdentities, transition.fromId, transition.fromId);
      ReserveRuntimeStateIdentity(stateIdentities, transition.toId, transition.toId);
      if (transition.from !== undefined) {
        AddRuntimeStateAlias(group.stateAliases, stateIdentities, transition.fromId, transition.from);
      }
      if (transition.to !== undefined) {
        AddRuntimeStateAlias(group.stateAliases, stateIdentities, transition.toId, transition.to);
      }
    }
    for (const transition of group.transitions) {
      for (const [nameField, idField] of [["from", "fromId"], ["to", "toId"]]) {
        if (transition[nameField] !== undefined) {
          continue;
        }
        const alias = group.stateAliases.get(NormalizeStateIdentity(transition[idField]));
        if (alias !== undefined) {
    ...
    for (const identity of [group.groupId, group.group]) {
      ...
      if (result.has(key) && result.get(key) !== group) {
        throw new TypeError(`Duplicate State transition group ${identity}`);
      }
...
function AddRuntimeStateAlias(aliases, identities, id, name) {
  ...
  for (const alias of [stateId, state]) {
    ...
    if (existing !== undefined && existing !== canonical) {
      throw new TypeError(`Conflicting State transition alias ${alias}`);
    }
...
function ReserveRuntimeStateIdentity(identities, alias, stateId) {
  ...
  if (existing !== undefined && existing !== canonical) {
    throw new TypeError(`Conflicting State transition identity ${alias}`);
  }
...
function CanonicalStateValue(group, fallback) {
  return group?.stateAliases.get(NormalizeStateIdentity(fallback)) ?? String(fallback);
}
function StateTransitionEndpointMatches(name, id, value) {
  if (name !== undefined) {
    return value !== undefined && value !== null && (NormalizeStateIdentity(name) === NormalizeStateIdentity(value) || String(id) === String(value));
  }
  return value !== undefined && value !== null && String(id) === String(value);
}
...
function StateWeightsEqualTarget(weights, target) {
  return weights.length === 1 && NormalizeStateIdentity(weights[0].state) === NormalizeStateIdentity(target) && Math.abs(Number(weights[0].weight) - 1) < 1e-12;
}
...
function ApplyVoiceFilter(node, readPercent, readAdditionalPercent, highPass, context, transitionBoundaries = []) {
  ...
  const boundaries = FutureAutomationBoundaries(transitionBoundaries, now);
  if (typeof param?.cancelAndHoldAtTime === "function") {
    param.cancelAndHoldAtTime(now);
  } else {
  ...
  for (const segmentEnd of boundaries) {
    if (typeof param?.setValueCurveAtTime === "function") {
      ...
      for (let index = 0; index < values.length; index++) {
        const at = segmentStart + (segmentEnd - segmentStart) * index / (values.length - 1);
        values[index] = cutoffAt(at) ?? cutoff;
      }
      param.setValueCurveAtTime(values, segmentStart, segmentEnd - segmentStart);
    } else {
      param?.linearRampToValueAtTime?.(cutoffAt(segmentEnd) ?? cutoff, segmentEnd);
    }
...
function FutureAutomationBoundaries(value, from) {
  const start = Number(from) || 0;
  return [...new Set((Array.isArray(value) ? value : [value]).map(Number).filter(boundary => Number.isFinite(boundary) && boundary > start))].sort((left, right) => left - right);
}
...
function SfxVoiceNaturalEndContextTime(voice) {
  ...
  if (!Number.isFinite(duration) || duration <= 0 || !Number.isFinite(rate) || rate <= 0 || !Number.isFinite(start)) {
    return Infinity;
  }
...
function PlaybackControlMatchesValue(action, value) {
  ...
  const protectedByException = action.exceptions.some(exception => matchIds.has(String(exception.targetId)));
  ...
  return action.mode === "element" && matchIds.has(String(action.targetId));
}
function ProgramSlotSelections(slot) {
  if (!IsOverlappingAdvanceMode(slot.advanceMode)) {
    return slot.selections ?? [];
  }
  return [...(slot.batches?.values?.() ?? [])].flatMap(batch => batch.selections ?? []);
}
...
function ApplySlotPauseDepth(voice, slot) {
  const depth = Math.max(0, Number(slot.pauseDepths?.get(ProgramSelectionKey(voice))) || 0);
...
function CreateProgramSelectionMetadata(selection, baseContextTime) {
  return Object.freeze({
    ...
    ...(selection.voiceLimitReservationId === undefined ? {} : {
      voiceLimitReservationId: Number(selection.voiceLimitReservationId)
    }),
...
function NormalizeVoiceDescriptors(result, eventLoop) {
  ...
  return values.map((value, index) => {
    ...
    const voiceLimitReservationId = value.voiceLimitReservationId === undefined ? undefined : Number(value.voiceLimitReservationId);
    ...
    if (voiceLimitReservationId !== undefined && (!Number.isSafeInteger(voiceLimitReservationId) || voiceLimitReservationId <= 0)) {
      throw new TypeError(`Audio voice ${index} voiceLimitReservationId must be a positive integer`);
    }
    ...
    if (value.busPathIds !== undefined && (!Array.isArray(value.busPathIds) || !value.busPathIds.length || value.busPathIds.some(busID => {
      ...
      throw new TypeError(`Audio voice ${index} busPathIds must be a non-empty array of positive ids`);
    }
    const busPathIds = (value.busPathIds ?? []).map(busID => String(Number(busID) >>> 0));
    if (value.busVoiceVolumeActionControlled !== undefined && typeof value.busVoiceVolumeActionControlled !== "boolean") {
      throw new TypeError(`Audio voice ${index} busVoiceVolumeActionControlled must be boolean`);
    }
    if (value.busVoiceVolumeActionControlled === true && !busPathIds.length) {
      throw new TypeError(`Audio voice ${index} busVoiceVolumeActionControlled requires a bus route`);
    }
    const hasAuthoredBusVolume = value.authoredBusVolumeDb !== undefined;
    ...
    return {
      ...
      getGainAtVoiceVolumeDb: typeof value.getGainAtVoiceVolumeDb === "function" ? value.getGainAtVoiceVolumeDb : null,
      getPlaybackRate: typeof value.getPlaybackRate === "function" ? value.getPlaybackRate : null,
      getPlaybackRateAtVoicePitchCents: typeof value.getPlaybackRateAtVoicePitchCents === "function" ? value.getPlaybackRateAtVoicePitchCents : null,
...
function ApplyVoiceVolumeAction(states, targetId, action) {
  const actionTime = Number(action.actionTime) || 0;
  const fromDb = EvaluateVoiceVolumeState(states.get(targetId), actionTime);
  const requested = action.kind === "reset-voice-volume" ? 0 : action.valueMode === "relative" ? fromDb + Number(action.volumeDb) : Number(action.volumeDb);
  const toDb = Math.max(-200, Math.min(200, Number.isFinite(requested) ? requested : 0));
  states.set(targetId, {
    fromDb,
    toDb,
...
function ApplyBusVolumeAction(states, action) {
  ...
  for (const targetId of targets) {
    const actionTime = Number(action.actionTime) || 0;
    const fromDb = EvaluateVoiceVolumeState(states.get(targetId), actionTime);
    const requested = action.kind === "reset-bus-volume" ? 0 : action.valueMode === "relative" ? fromDb + Number(action.busVolumeDb) : Number(action.busVolumeDb);
    const toDb = Math.max(-200, Math.min(200, Number.isFinite(requested) ? requested : 0));
    states.set(targetId, {
      fromDb,
      toDb,
...
function ScheduleVoiceVolumeGain(param, voice, context, smoothDistance = false) {
  ...
  const rawBoundaries = [...(voice.controlTransitionBoundaries ?? [])];
  for (const value of new Set(matchIds.map(String))) {
    const state = states.get(value);
  ...
  const boundaries = FutureAutomationBoundaries(rawBoundaries, now);
  const evaluate = at => {
    let value = 1;
    try {
...
function ScheduleBusVoiceRtpcGain(param, voice, context, busRtpcCatalog, readGlobalRtpc) {
  if (!param) {
    return;
  }
  const now = Number(context?.currentTime) || 0;
  const busPathIds = Array.isArray(voice.busPathIds) ? voice.busPathIds : [];
  const boundaries = [...new Set((voice.controlTransitionBoundaries ?? []).map(Number).filter(value => Number.isFinite(value) && value > now))].sort((left, right) => left - right);
  const evaluate = at => 10 ** (evaluateBusVoiceRtpcGainDb(busRtpcCatalog, busPathIds, readGlobalRtpc, at) / 20);
  const startValue = evaluate(now);
  if (typeof param.cancelAndHoldAtTime === "function") {
    param.cancelAndHoldAtTime(now);
  } else {
...
function ScheduleBusVoiceActionGain(param, voice, context) {
  ...
  const evaluate = at => 10 ** (EvaluateVoiceVolumeTargets(states, busPathIds, at) / 20);
  const startValue = evaluate(now);
  if (typeof param.cancelAndHoldAtTime === "function") {
    param.cancelAndHoldAtTime(now);
  } else {
...
function EvaluateVoiceVolumeTargets(states, matchIds, at) {
  ...
  for (const value of matchIds) {
    ...
    if (!seen.has(targetId)) {
      seen.add(targetId);
      result += EvaluateVoiceVolumeState(states.get(targetId), at);
    }
  }
  return result;
}
...
function ApplyVoicePitchAction(states, targetId, action) {
  const actionTime = Number(action.actionTime) || 0;
  const fromCents = EvaluateVoicePitchState(states.get(targetId), actionTime);
  const requested = action.kind === "reset-voice-pitch" ? 0 : action.valueMode === "relative" ? fromCents + Number(action.pitchCents) : Number(action.pitchCents);
  const toCents = Math.max(-2400, Math.min(2400, Number.isFinite(requested) ? requested : 0));
  states.set(targetId, {
    fromCents,
    toCents,
    startTime: actionTime,
...
function EvaluateVoicePitchTargets(states, matchIds, at) {
  ...
  for (const value of matchIds) {
    ...
    if (!seen.has(targetId)) {
      seen.add(targetId);
      result += EvaluateVoicePitchState(states.get(targetId), at);
    }
  }
  return result;
}
function EvaluateVoicePitchState(state, at) {
  ...
  return state.fromCents + (state.toCents - state.fromCents) * evaluateWwiseInterpolation(state.curve, progress);
}
function VoicePitchTransitionEnd(voice, from) {
  return VoicePitchTransitionBoundaries(voice, from).at(-1) ?? (Number(from) || 0);
}
function VoicePitchTransitionBoundaries(voice, from) {
  const states = voice.voicePitchStates;
  const matchIds = Array.isArray(voice.matchIds) ? voice.matchIds : [];
  const boundaries = [...(voice.controlTransitionBoundaries ?? [])];
  for (const value of new Set(matchIds.map(String))) {
    const state = states.get(value);
    const start = Number(state?.startTime);
    const end = Number(state?.startTime) + Math.max(0, Number(state?.duration) || 0);
...
function IntegrateVoicePitchPlaybackRate(voice, from, to) {
  ...
  for (const value of new Set((voice.matchIds ?? []).map(String))) {
    ...
    const transitionEnd = transitionStart + Math.max(0, Number(state?.duration) || 0);
    if (Number.isFinite(transitionStart) && transitionStart > start && transitionStart < end) {
      boundaries.add(transitionStart);
    }
    if (Number.isFinite(transitionEnd) && transitionEnd > start && transitionEnd < end) {
  ...
  for (let segment = 1; segment < ordered.length; segment++) {
    ...
    for (let index = 0; index < FADE_CURVE_SAMPLES; index++) {
      result += EvaluateVoicePitchPlaybackRate(voice, segmentStart + (index + 0.5) * step) * step;
    }
  }
  return result;
}
function ScheduleVoicePitchPlaybackRate(param, voice, context) {
  ...
  for (const segmentEnd of boundaries) {
    if (typeof param.setValueCurveAtTime === "function") {
      ...
      for (let index = 0; index < values.length; index++) {
        const ratio = index / (values.length - 1);
        values[index] = EvaluateVoicePitchPlaybackRate(voice, segmentStart + (segmentEnd - segmentStart) * ratio);
      }
      param.setValueCurveAtTime(values, segmentStart, segmentEnd - segmentStart);
    } else {
      param.linearRampToValueAtTime?.(EvaluateVoicePitchPlaybackRate(voice, segmentEnd), segmentEnd);
    }
    segmentStart = segmentEnd;
  }
}
function SolveVoicePitchRepeatEnd(voice, now) {
  const start = Math.max(Number(now) || 0, Number(voice.repeatAnchorContextTime) || 0);
  const remaining = Math.max(0, Number(voice.repeatRemainingSeconds) || 0);
  const transitionEnd = VoicePitchTransitionEnd(voice, start);
  const duringTransition = IntegrateVoicePitchPlaybackRate(voice, start, transitionEnd);
  if (remaining <= duringTransition && transitionEnd > start) {
    let low = start;
    let high = transitionEnd;
    for (let index = 0; index < 32; index++) {
      const middle = (low + high) * 0.5;
      if (IntegrateVoicePitchPlaybackRate(voice, start, middle) < remaining) {
        low = middle;
      } else {
        high = middle;
      }
  ...
  const finalRate = EvaluateVoicePitchPlaybackRate(voice, transitionEnd);
  return transitionEnd + Math.max(0, remaining - duringTransition) / finalRate;
}
...
function CrossfadeTiming(voice, transitionMs) {
  ...
  const start = Number(voice?.startContextTime);
  const offset = Math.max(0, Number(voice?.offsetSeconds) || 0);
  const playCount = Math.max(1, Number(voice?.playCount) || 1);
  if (voice?.loop || !Number.isFinite(duration) || duration <= 0 || !Number.isFinite(rate) || rate <= 0 || !Number.isFinite(start)) {
    return null;
  }
  const remaining = Math.max(0, duration - offset) + duration * (playCount - 1);
  const naturalEnd = start + remaining / rate;
  const authored = Math.max(0, Number(transitionMs) || 0) / 1000;
  const crossfadeDuration = Math.min(authored, duration / rate / 2, remaining / rate);
  return {
...
function ScheduleCrossfadeGain(param, from, to, when, duration, mode) {
  if (!param) {
    return;
  }
  if (!(duration > 0)) {
  ...
  if (mode !== "crossfade-power" || typeof param.setValueCurveAtTime !== "function") {
  ...
  for (let index = 0; index < values.length; index++) {
    const ratio = index / (values.length - 1);
    values[index] = incoming ? Math.sin(ratio * Math.PI / 2) : Math.cos(ratio * Math.PI / 2);
  }
  if ("value" in param) {
    param.value = from;
  }
...
function EvaluateCrossfadeGain(from, to, progress, mode) {
  ...
  if (mode === "crossfade-power") {
    return to > from ? Math.sin(ratio * Math.PI / 2) : Math.cos(ratio * Math.PI / 2);
  }
