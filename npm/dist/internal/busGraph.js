...
const PROCESSING_REASONS = new Set(["auxiliary-bus", "aux-sends", "ducking", "dynamic-aux", "effects", "hdr", "positioning", "rtpc", "state"]);
...
function normalizeBusGraphCatalog(value, embeddedMedia = {}) {
  const catalog = RequireRecord(value, "Audio Bus graph");
  if (catalog.schemaVersion !== 1) {
    throw new TypeError(`Unsupported audio bus graph schema version: ${catalog.schemaVersion}`);
  }
  const rawEffects = RequireRecord(catalog.effects, "Audio Bus graph effects");
  const effects = {};
  for (const rawEffectId of Object.keys(rawEffects).sort(CompareIds)) {
    const effectId = CanonicalPositiveId(rawEffectId, `Audio Bus graph effect ${rawEffectId}`);
    const raw = RequireRecord(rawEffects[rawEffectId], `Audio Bus graph effect ${effectId}`);
    if (!EFFECT_TYPES.has(raw.type)) {
      throw new TypeError(`Audio Bus graph effect ${effectId} has unsupported type ${raw.type}`);
    }
    const parametersBase64 = CanonicalBase64(raw.parametersBase64, `Audio Bus graph effect ${effectId} parametersBase64`);
    const parameterByteLength = NonNegativeInteger(raw.parameterByteLength, `Audio Bus graph effect ${effectId} parameterByteLength`);
    if (Base64ByteLength(parametersBase64) !== parameterByteLength) {
      throw new TypeError(`Audio Bus graph effect ${effectId} parameter length does not match`);
    }
    const media = NormalizeEffectMedia(raw.media ?? [], effectId, embeddedMedia);
    const controls = RequireRecord(raw.controls, `Audio Bus graph effect ${effectId} controls`);
    const pluginId = UnsignedInteger(raw.pluginId, `Audio Bus graph effect ${effectId} pluginId`);
    const pluginType = BoundedInteger(raw.pluginType, 0, 0x0f, `Audio Bus graph effect ${effectId} pluginType`);
    const companyId = BoundedInteger(raw.companyId, 0, 0x0fff, `Audio Bus graph effect ${effectId} companyId`);
    const pluginClassId = BoundedInteger(raw.pluginClassId, 0, 0xffff, `Audio Bus graph effect ${effectId} pluginClassId`);
    const packedPluginId = (pluginClassId << 16 | companyId << 4 | pluginType) >>> 0;
    if (pluginId !== packedPluginId) {
      throw new TypeError(`Audio Bus graph effect ${effectId} has inconsistent plugin identity`);
    }
    effects[effectId] = {
      type: raw.type,
      pluginId,
      ...
      media,
      controls: {
        rtpcCount: NonNegativeInteger(controls.rtpcCount, `Audio Bus graph effect ${effectId} controls.rtpcCount`),
        statePropertyCount: NonNegativeInteger(controls.statePropertyCount, `Audio Bus graph effect ${effectId} controls.statePropertyCount`),
        stateGroupCount: NonNegativeInteger(controls.stateGroupCount, `Audio Bus graph effect ${effectId} controls.stateGroupCount`),
        propertyValueCount: NonNegativeInteger(controls.propertyValueCount, `Audio Bus graph effect ${effectId} controls.propertyValueCount`)
      }
    };
  }
  ...
  for (const rawBusId of Object.keys(rawBuses).sort(CompareIds)) {
    const busId = CanonicalPositiveId(rawBusId, `Audio Bus graph bus ${rawBusId}`);
    const raw = RequireRecord(rawBuses[rawBusId], `Audio Bus graph bus ${busId}`);
    if (!BUS_TYPES.has(raw.type)) {
      throw new TypeError(`Audio Bus graph bus ${busId} has unsupported type ${raw.type}`);
    }
    const parentBusId = raw.parentBusId === undefined ? undefined : CanonicalPositiveId(raw.parentBusId, `Audio Bus graph bus ${busId} parentBusId`);
    if (parentBusId === busId) {
      throw new TypeError(`Audio Bus graph bus ${busId} parents itself`);
    }
    const channelConfig = RequireRecord(raw.channelConfig, `Audio Bus graph bus ${busId} channelConfig`);
    const bus = {
      type: raw.type,
      ...(parentBusId === undefined ? {} : {
        parentBusId
      }),
      channelConfig: {
        raw: UnsignedInteger(channelConfig.raw, `Audio Bus graph bus ${busId} channelConfig.raw`),
        channelCount: BoundedInteger(channelConfig.channelCount, 0, 0xff, `Audio Bus graph bus ${busId} channelConfig.channelCount`),
        configType: BoundedInteger(channelConfig.configType, 0, 0x0f, `Audio Bus graph bus ${busId} channelConfig.configType`),
        channelMask: BoundedInteger(channelConfig.channelMask, 0, 0xfffff, `Audio Bus graph bus ${busId} channelConfig.channelMask`)
      },
      ...
      auxFlags: BoundedInteger(raw.auxFlags, 0, 0xff, `Audio Bus graph bus ${busId} auxFlags`),
    ...
    if (bus.positioning.overrideParent !== Boolean(bus.positioning.flags & 0x01) || bus.positioning.listenerRelative !== Boolean(bus.positioning.flags & 0x02) || bus.positioning.pannerType !== (bus.positioning.flags >>> 2 & 0x03) || bus.positioning.positionType !== (bus.positioning.flags >>> 5 & 0x03)) {
      throw new TypeError(`Audio Bus graph bus ${busId} has inconsistent positioning flags`);
    }
    if (bus.hdr.enabled !== Boolean(bus.hdr.flags & 0x01) || bus.hdr.exponentialRelease !== Boolean(bus.hdr.flags & 0x02)) {
    ...
    for (const field of ["busVolumeDb", "makeUpGainDb", "outputBusVolumeDb"]) {
      if (raw[field] !== undefined) {
        bus[field] = FiniteNumber(raw[field], `Audio Bus graph bus ${busId} ${field}`);
      }
    }
    ValidateGainProperties(bus, busId);
    if (raw.reflectionsAuxSend !== undefined) {
      bus.reflectionsAuxSend = NormalizeReflectionSend(raw.reflectionsAuxSend, `Audio Bus graph bus ${busId}`);
    }
    ...
    if (bus.userAuxSends.length && bus.parentBusId !== undefined && !(bus.auxFlags & 0x04)) {
      throw new TypeError(`Audio Bus graph bus ${busId} does not override its inherited user sends`);
    }
    if (bus.reflectionsAuxSend && bus.parentBusId !== undefined && !(bus.auxFlags & 0x10)) {
  ...
  const sfxRoutes = NormalizeRouteReferences(catalog.sfxRoutes, routes.length, "Audio Bus graph sfxRoutes");
  const musicRoutes = NormalizeRouteReferences(catalog.musicRoutes, routes.length, "Audio Bus graph musicRoutes");
...
function assertBusGraphRouteProjection(route, projection, label) {
  const expected = RequireRecord(route, `${label} route`);
  const actual = RequireRecord(projection, `${label} projection`);
  const outputBusId = actual.outputBusId === undefined ? undefined : CanonicalPositiveId(actual.outputBusId, `${label} outputBusId`);
  const busPathIds = Array.isArray(actual.busPathIds) ? actual.busPathIds.map((id, index) => CanonicalPositiveId(id, `${label} busPathIds ${index}`)) : [];
  if (outputBusId !== expected.outputBusId || busPathIds.length !== expected.busPathIds.length || busPathIds.some((id, index) => id !== expected.busPathIds[index])) {
    throw new TypeError(`${label} dry route disagrees with the Audio Bus graph`);
  }
  for (const field of ["authoredBusVolumeDb", "authoredBusMakeUpGainDb", "authoredOutputBusVolumeDb"]) {
    const expectedHas = Object.hasOwn(expected, field);
    const actualHas = actual[field] !== undefined;
    if (expectedHas !== actualHas || expectedHas && expected[field] !== Number(actual[field])) {
...
function NormalizePositioning(value, busId) {
  const raw = RequireRecord(value, `Audio Bus graph bus ${busId} positioning`);
  return {
...
function NormalizeReflectionSend(raw, label) {
  ...
  return {
    targetBusId: CanonicalPositiveId(value.targetBusId, `${label} reflectionsAuxSend targetBusId`),
...
function ValidateRequiredProcessing(bus, busId) {
  ...
  if (bus.userAuxSends.some(send => send.dynamic) || bus.reflectionsAuxSend?.dynamic) {
...
function NormalizeRoutes(values, buses) {
  ...
  return values.map((raw, index) => {
    ...
    const busPathIds = value.busPathIds.map((id, offset) => CanonicalPositiveId(id, `${label} busPathIds ${offset}`));
    ...
    for (let offset = 0; offset < busPathIds.length; offset++) {
      ...
      const expected = offset + 1 < busPathIds.length ? busPathIds[offset + 1] : undefined;
...
function ValidateReferencedRoutes(routes, sfxRoutes, musicRoutes) {
  const referenced = new Set([...Object.values(sfxRoutes), ...Object.values(musicRoutes)]);
...
function ValidateBusReferences(buses) {
  for (const [busId, bus] of Object.entries(buses)) {
    for (const targetId of [bus.parentBusId, ...bus.userAuxSends.map(send => send.targetBusId), bus.reflectionsAuxSend?.targetBusId].filter(Boolean)) {
      if (!buses[targetId]) {
...
function ValidateReachableClosure(buses, routes) {
  ...
  const pending = routes.flatMap(route => [route.outputBusId, ...route.userAuxSends.map(send => send.targetBusId), route.reflectionsAuxSend?.targetBusId].filter(Boolean));
  while (pending.length) {
    ...
    reachable.add(busId);
    pending.push(...[bus.parentBusId, ...bus.userAuxSends.map(send => send.targetBusId), bus.reflectionsAuxSend?.targetBusId].filter(Boolean));
  }
...
function ValidateReferencedEffects(buses, effects) {
  const referenced = new Set(Object.values(buses).flatMap(bus => bus.effects.map(slot => slot.effectId)));
...
function CanonicalBase64(value, label) {
  ...
  if (text.endsWith("==") && (alphabet.indexOf(text.at(-3)) & 0x0f) !== 0 || text.endsWith("=") && !text.endsWith("==") && (alphabet.indexOf(text.at(-2)) & 0x03) !== 0) {
    throw new TypeError(`${label} must be canonical base64`);
  }
  return text;
}
...
export { assertBusGraphRouteProjection, normalizeBusGraphCatalog };
//# sourceMappingURL=busGraph.js.map
