...
function parseGraphStaticParametricEq(effect, effectId, slotIndex) {
  ...
  if (!effect || effect.type !== "effect-custom" && effect.type !== "effect-share-set" || effect.pluginId !== PARAMETRIC_EQ_PLUGIN_ID || effect.parameterByteLength !== 56 || !Array.isArray(effect.media) || effect.media.length !== 0 || !controls || controls.rtpcCount !== 0 || controls.statePropertyCount !== 0 || controls.stateGroupCount !== 0 || controls.propertyValueCount !== 0) {
    throw new TypeError(`${label} is not a static Wwise Parametric EQ`);
  }
  return parseStaticParametricEqBytes(DecodeBase64(effect.parametersBase64, label), {
    effectId,
    slotIndex,
    label
  });
...
export { PARAMETRIC_EQ_PLUGIN_ID, createBusEffectChain, indexBusEffectCatalog, parseGraphStaticParametricEq, parseStaticParametricEqBytes };
