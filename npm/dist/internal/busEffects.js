...
const WWISE_MATRIX_REVERB_PLUGIN_ID = 0x00730003;
...
const MODULATION_PHASE_MODES = Object.freeze(["left-right", "front-rear", "circular", "random"]);
...
function normalizeWwiseDynamicsMode(value = "strict") {
  ...
  if (!WWISE_DYNAMICS_MODES.has(mode)) {
    throw new TypeError(`Unsupported Wwise dynamics realization mode: ${mode}`);
  }
  return mode;
}

/** Validates the host policy for authored Wwise modulation realization. */
function normalizeWwiseModulationMode(value = "strict") {
  const mode = String(value);
...
function normalizeWwiseMeterFeedbackMode(value = "strict") {
  const mode = String(value);
  if (!WWISE_METER_FEEDBACK_MODES.has(mode)) {
    throw new TypeError(`Unsupported Wwise Meter feedback mode: ${mode}`);
  }
  return mode;
}

/** Validates host policy for the dynamic MaxNumInstances RTPC barrier. */
function normalizeWwiseVoiceLimitMode(value = "strict") {
  const mode = String(value);
  if (!WWISE_VOICE_LIMIT_MODES.has(mode)) {
    throw new TypeError(`Unsupported Wwise voice-limit mode: ${mode}`);
  }
  return mode;
}

/** Validates and indexes one portable static Wwise bus-effect catalog. */
function indexBusEffectCatalog(value) {
  ...
  if (value.schemaVersion !== 1) {
    throw new TypeError(`Unsupported audio bus effect schema version: ${value.schemaVersion}`);
  }
  const buses = RequireRecord(value.buses, "Audio Bus effect buses");
  const result = new Map();
  for (const [rawBusId, rawEffects] of Object.entries(buses)) {
    const busId = CanonicalPositiveId(rawBusId, `Audio Bus effect bus ${rawBusId}`);
    if (!Array.isArray(rawEffects) || !rawEffects.length) {
      throw new TypeError(`Audio Bus effect bus ${busId} must have effects`);
    }
    const effects = normalizeStaticParametricEqChain(rawEffects, `Audio Bus effect bus ${busId}`);
...
function NormalizeParametricEqRtpcCurves(value, bands, ownerLabel) {
  ...
  return Object.freeze(value.map((rawCurve, index) => {
    ...
    const bandIndex = BoundedInteger(curve.bandIndex, 0, 2, `${label} bandIndex`);
    ...
    if (property === "gainDb" && scaling !== 0 && scaling !== 2 || property === "frequencyHz" && scaling !== 3) {
    ...
    const points = curve.points.map((rawPoint, pointIndex) => {
      ...
      if (!Number.isFinite(x) || !Number.isFinite(output) || x < previous) {
    ...
    const controlTransition = NormalizeControlTransition(curve.controlTransition, `${label} controlTransition`);
    return Object.freeze({
      rtpc,
      scope,
      bandIndex,
...
function NormalizeGuitarDistortionDriveRtpcCurve(value, ownerLabel) {
  ...
  const controlTransition = NormalizeControlTransition(curve.controlTransition, `${ownerLabel} controlTransition`);
  return Object.freeze({
    rtpc,
    scope,
    accumulation,
...
function NormalizeControlTransition(value, ownerLabel, required = false) {
  ...
  if (transition.type !== "filtering-over-time" || !Number.isFinite(rampUpSeconds) || !Number.isFinite(rampDownSeconds) || rampUpSeconds <= 0 || rampDownSeconds <= 0) {
...
function NormalizeTremoloRtpcCurves(value, ownerLabel) {
  ...
  const curves = value.map((rawCurve, index) => {
    ...
    if (property === "modulationDepthPercent" && (accumulation !== "additive" || scaling !== 0) || property === "modulationFrequencyHz" && (accumulation !== "exclusive" || scaling !== 3) || property !== "modulationDepthPercent" && property !== "modulationFrequencyHz" || targets.has(property)) {
    ...
    const transition = NormalizeControlTransition(curve.controlTransition, `${label} controlTransition`, true);
    return Object.freeze({
      rtpc,
      scope: "object",
      property,
...
function NormalizeStaticWwiseEffectChain(value, ownerLabel, allowSourceEffects) {
  ...
  const effects = value.map((rawEffect, effectIndex) => {
    ...
    const effectId = CanonicalPositiveId(effect.effectId, `${label} id`);
    const slotIndex = BoundedInteger(effect.slotIndex, 0, 3, `${label} slotIndex`);
    ...
    if (allowSourceEffects && effect.type === "flanger") {
      ...
      return Object.freeze({
        ...
        slotIndex,
        type: "flanger",
        delayTimeSeconds: BoundedFinite(effect.delayTimeSeconds, FLANGER_DELAY_SECONDS_MIN, FLANGER_DELAY_SECONDS_MAX, `${label} delayTimeSeconds`),
        blend: BoundedFinite(effect.blend, 0, 1, `${label} blend`),
        feedforward: BoundedFinite(effect.feedforward, FLANGER_LEVEL_MIN, FLANGER_LEVEL_MAX, `${label} feedforward`),
        feedback: BoundedFinite(effect.feedback, FLANGER_LEVEL_MIN, FLANGER_LEVEL_MAX, `${label} feedback`),
        modulationDepthPercent: BoundedFinite(effect.modulationDepthPercent, MODULATION_PERCENT_MIN, MODULATION_PERCENT_MAX, `${label} modulationDepthPercent`),
        modulationFrequencyHz: BoundedFinite(effect.modulationFrequencyHz, MODULATION_FREQUENCY_MIN, MODULATION_FREQUENCY_MAX, `${label} modulationFrequencyHz`),
        outputGainDb: BoundedFinite(effect.outputGainDb, DYNAMICS_OUTPUT_GAIN_MIN, DYNAMICS_OUTPUT_GAIN_MAX, `${label} outputGainDb`),
    ...
    if (allowSourceEffects && effect.type === "tremolo") {
      ...
      const waveform = Enumerated(effect.waveform ?? "sine", TREMOLO_WAVEFORM_SET, `${label} waveform`);
      ...
      return Object.freeze({
        ...
        ...(effect.rtpcCurves === undefined ? {} : {
          rtpcCurves: NormalizeTremoloRtpcCurves(effect.rtpcCurves, `${label} rtpcCurves`)
        })
      });
    }
    ...
    if (allowSourceEffects && effect.type === "matrix-reverb") {
      if (effect.processLfe !== true || effect.delayLengthsMode !== "default" || !WWISE_MATRIX_REVERB_DELAY_COUNTS.has(Number(effect.numberOfDelays))) {
        throw new TypeError(`${label} has unsupported Matrix Reverb routing`);
      }
      return Object.freeze({
        effectId,
        slotIndex,
        type: "matrix-reverb",
        reverbTimeSeconds: BoundedFinite(effect.reverbTimeSeconds, MATRIX_REVERB_TIME_MIN, MATRIX_REVERB_TIME_MAX, `${label} reverbTimeSeconds`),
        hfRatio: BoundedFinite(effect.hfRatio, MATRIX_REVERB_HF_RATIO_MIN, MATRIX_REVERB_HF_RATIO_MAX, `${label} hfRatio`),
        numberOfDelays: Number(effect.numberOfDelays),
        dryLevelDb: BoundedFinite(effect.dryLevelDb, MATRIX_REVERB_LEVEL_MIN, MATRIX_REVERB_LEVEL_MAX, `${label} dryLevelDb`),
        wetLevelDb: BoundedFinite(effect.wetLevelDb, MATRIX_REVERB_LEVEL_MIN, MATRIX_REVERB_LEVEL_MAX, `${label} wetLevelDb`),
        preDelaySeconds: BoundedFinite(effect.preDelaySeconds, MATRIX_REVERB_PRE_DELAY_MIN, MATRIX_REVERB_PRE_DELAY_MAX, `${label} preDelaySeconds`),
        processLfe: true,
    ...
    if (effect.processLfe !== true && (!allowSourceEffects || effect.processLfe !== false)) {
      throw new TypeError(`${label} processLfe must be true until independent LFE routing is supported`);
    }
    if (!allowSourceEffects && effect.rtpcCurves !== undefined) {
      throw new TypeError(`${label} rtpcCurves require a source effect chain`);
    }
    const rtpcCurves = effect.rtpcCurves === undefined ? undefined : NormalizeParametricEqRtpcCurves(effect.rtpcCurves, bands, label);
    return Object.freeze({
      effectId,
      slotIndex,
      type: "parametric-eq",
      bands: Object.freeze(bands),
      outputGainDb: FiniteGain(effect.outputGainDb, `${label} outputGainDb`),
      processLfe: effect.processLfe,
      ...(rtpcCurves === undefined ? {} : {
        rtpcCurves
      })
    });
...
function createBusEffectChain(context, indexedCatalog, busPathIds) {
  if (!(indexedCatalog instanceof Map) || !indexedCatalog.size) {
    return null;
  }
  const effects = (busPathIds ?? []).flatMap(busId => indexedCatalog.get(String(busId)) ?? []);
...
function createWwiseEffectChain(context, effects, {
  ...
  const dynamicsEffects = effects.filter(effect => effect.type === "compressor" || effect.type === "compressor-approximation" || effect.type === "peak-limiter" || effect.type === "peak-limiter-approximation");
  const sourceDelays = effects.filter(effect => effect.type === "delay");
  const sourceEqualizers = effects.filter(effect => effect.type === "parametric-eq");
  ...
  const flangerEffects = effects.filter(effect => effect.type === "flanger" || effect.type === "flanger-approximation");
  ...
  if (tremoloEffects.some(effect => !effect.processCenter || !effect.processLfe)) {
    return null;
  }
  if (needsOscillator && typeof context?.createOscillator !== "function") {
    return null;
  }
  if (needsPeriodicWave && typeof context?.createPeriodicWave !== "function") {
    return null;
  }
  ...
  for (const effect of realizedEffects) {
    ...
    for (const band of effect.bands) {
      ...
      for (const curve of effect.rtpcCurves ?? []) {
        ...
        sourceEffectRtpcBindings.push({
          curve,
          baseValue: curve.property === "gainDb" ? band.gainDb : band.frequencyHz,
          param: curve.property === "gainDb" ? filter.gain : filter.frequency
        });
      }
    }
    if (effect.outputGainDb !== 0) {
      if (typeof context?.createGain !== "function") {
        throw new TypeError("AudioContext.createGain is required for Wwise Parametric EQ output gain");
      }
  ...
  return input ? {
    ...
    output,
    nodes,
    sourceEffectRtpcLane: createWwiseSourceEffectRtpcLane(context, sourceEffectRtpcBindings, readSourceEffectRtpc)
  } : null;
}
...
function CreateWwiseFlangerApproximation(context, effect) {
  ...
  const delayRange = effect.delayTimeSeconds * effect.modulationDepthPercent / 100;
  ...
  const nodes = [input, delay, dry, blend, feedforward, feedback, wet, output];
  ...
  SetParam(feedback.gain, Math.max(-0.999, Math.min(0.999, effect.feedback)));
...
function CreateWwiseTremoloApproximation(context, effect) {
  ...
  if (depth > 0 || effect.rtpcCurves?.length) {
    ...
    if (effect.waveform === "square" && effect.pwmPercent !== undefined) {
      ...
      oscillator.setPeriodicWave(CreateSinePeriodicWave(context, effect.phaseOffsetDegrees));
    }
    SetParam(oscillator.frequency, effect.modulationFrequencyHz);
...
function CreateWwiseDynamicsApproximation(context, effect) {
  if (typeof context?.createDynamicsCompressor !== "function" || typeof context?.createGain !== "function") {
    throw new TypeError("DynamicsCompressorNode and GainNode support is required for approximate Wwise dynamics");
  }
  ...
  SetParam(dynamics.attack, effect.type === "peak-limiter-approximation" ? 0 : effect.attackSeconds);
  ...
  nodes.push(gain);
  let output = gain;
  if (effect.type === "peak-limiter-approximation" && effect.lookaheadSeconds > WEB_AUDIO_DYNAMICS_LOOKAHEAD) {
    if (typeof context?.createDelay !== "function") {
      throw new TypeError("DelayNode support is required to approximate Wwise Peak Limiter lookahead");
    }
    const delaySeconds = effect.lookaheadSeconds - WEB_AUDIO_DYNAMICS_LOOKAHEAD;
...
function parseStaticParametricEqBytes(bytes, {
  effectId,
  slotIndex,
  ...
  for (let index = 0; index < 3; index++) {
    ...
    if (!filterType || !Number.isFinite(gainDb) || gainDb < MIN_GAIN_DB || gainDb > MAX_GAIN_DB || !Number.isFinite(frequencyHz) || frequencyHz <= 0 || !Number.isFinite(q) || q <= 0 || enabledRaw > 1) {
  ...
  if (!Number.isFinite(outputGainDb) || outputGainDb < MIN_GAIN_DB || outputGainDb > MAX_GAIN_DB) {
    throw new TypeError(`${label} has invalid output gain`);
  }
  const processLfeRaw = view.getUint8(at + 4);
  if (processLfeRaw > 1 || processLfeRaw !== 1 && !allowIndependentLfe) {
...
function parseGraphStaticParametricEq(effect, effectId, slotIndex) {
  const label = `Audio Bus graph effect ${effectId}`;
  return parseStaticParametricEqBytes(RequireStaticGraphEffect(effect, PARAMETRIC_EQ_PLUGIN_ID, 56, label, "Wwise Parametric EQ"), {
    effectId,
    slotIndex,
    label
  });
...
function parseStaticWwiseFlangerBytes(bytes, {
  effectId,
  slotIndex,
  ...
  if (!Number.isFinite(delayTimeMs) || delayTimeMs < FLANGER_DELAY_SECONDS_MIN * 1000 || delayTimeMs > FLANGER_DELAY_SECONDS_MAX * 1000 || !Number.isFinite(blend) || blend < 0 || blend > 1 || !Number.isFinite(feedforward) || feedforward < FLANGER_LEVEL_MIN || feedforward > FLANGER_LEVEL_MAX || !Number.isFinite(feedback) || feedback < FLANGER_LEVEL_MIN || feedback > FLANGER_LEVEL_MAX || !Number.isFinite(modulationDepthPercent) || modulationDepthPercent < MODULATION_PERCENT_MIN || modulationDepthPercent > MODULATION_PERCENT_MAX || !Number.isFinite(modulationFrequencyHz) || modulationFrequencyHz < MODULATION_FREQUENCY_MIN || modulationFrequencyHz > MODULATION_FREQUENCY_MAX || waveform !== 0 || !Number.isFinite(smoothingPercent) || smoothingPercent < MODULATION_PERCENT_MIN || smoothingPercent > MODULATION_PERCENT_MAX || !Number.isFinite(pwmPercent) || pwmPercent < MODULATION_PERCENT_MIN || pwmPercent > MODULATION_PERCENT_MAX || phaseOffsetDegrees !== 0 || phaseMode !== 0 || phaseSpreadDegrees !== 0 || !Number.isFinite(outputGainDb) || outputGainDb < DYNAMICS_OUTPUT_GAIN_MIN || outputGainDb > DYNAMICS_OUTPUT_GAIN_MAX || !Number.isFinite(wetDryMixPercent) || wetDryMixPercent < MODULATION_PERCENT_MIN || wetDryMixPercent > MODULATION_PERCENT_MAX || lfoEnabledRaw > 1 || processCenterRaw > 1 || processLfeRaw > 1) {
 ...
 */
function parseStaticWwiseTremoloBytes(bytes, {
  effectId,
  slotIndex,
  label = `Wwise Tremolo ${effectId}`,
  bankVersion = WWISE_TREMOLO_BANK_VERSION
} = {}) {
  if (Number(bankVersion) !== WWISE_TREMOLO_BANK_VERSION || !(bytes instanceof Uint8Array) || bytes.byteLength !== 38) {
    throw new TypeError(`${label} has an unsupported parameter block`);
  }
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
...
function parseStaticWwiseMatrixReverbBytes(bytes, {
  effectId,
  slotIndex,
  label = `Wwise Matrix Reverb ${effectId}`,
  bankVersion = WWISE_MATRIX_REVERB_BANK_VERSION
} = {}) {
  if (Number(bankVersion) !== WWISE_MATRIX_REVERB_BANK_VERSION || !(bytes instanceof Uint8Array) || bytes.byteLength !== 29) {
    throw new TypeError(`${label} has an unsupported parameter block`);
  }
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  ...
  if (!Number.isFinite(reverbTimeSeconds) || reverbTimeSeconds < MATRIX_REVERB_TIME_MIN || reverbTimeSeconds > MATRIX_REVERB_TIME_MAX || !Number.isFinite(hfRatio) || hfRatio < MATRIX_REVERB_HF_RATIO_MIN || hfRatio > MATRIX_REVERB_HF_RATIO_MAX || !WWISE_MATRIX_REVERB_DELAY_COUNTS.has(numberOfDelays) || !Number.isFinite(dryLevelDb) || dryLevelDb < MATRIX_REVERB_LEVEL_MIN || dryLevelDb > MATRIX_REVERB_LEVEL_MAX || !Number.isFinite(wetLevelDb) || wetLevelDb < MATRIX_REVERB_LEVEL_MIN || wetLevelDb > MATRIX_REVERB_LEVEL_MAX || !Number.isFinite(preDelaySeconds) || preDelaySeconds < MATRIX_REVERB_PRE_DELAY_MIN || preDelaySeconds > MATRIX_REVERB_PRE_DELAY_MAX || processLfeRaw !== 1 || delayLengthsModeRaw !== 0) {
    throw new TypeError(`${label} has unsupported Wwise Matrix Reverb parameters`);
  }
  return {
    effectId: String(effectId),
    slotIndex: Number(slotIndex),
...
function parseGraphStaticWwisePeakLimiter(effect, effectId, slotIndex) {
  const label = `Audio Bus graph effect ${effectId}`;
  const bytes = RequireStaticGraphEffect(effect, WWISE_PEAK_LIMITER_PLUGIN_ID, 22, label, "Wwise Peak Limiter");
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  ...
  if (!Number.isFinite(thresholdDb) || thresholdDb < DYNAMICS_THRESHOLD_MIN || thresholdDb > DYNAMICS_THRESHOLD_MAX || !Number.isFinite(ratio) || ratio < DYNAMICS_RATIO_MIN || ratio > DYNAMICS_RATIO_MAX || !Number.isFinite(lookaheadSeconds) || lookaheadSeconds < PEAK_LIMITER_LOOKAHEAD_MIN || lookaheadSeconds > PEAK_LIMITER_LOOKAHEAD_MAX || !Number.isFinite(releaseSeconds) || releaseSeconds < PEAK_LIMITER_RELEASE_MIN || releaseSeconds > PEAK_LIMITER_RELEASE_MAX || !Number.isFinite(outputGainDb) || outputGainDb < DYNAMICS_OUTPUT_GAIN_MIN || outputGainDb > DYNAMICS_OUTPUT_GAIN_MAX || processLfeRaw > 1 || channelLinkRaw > 1) {
    throw new TypeError(`${label} has invalid Wwise Peak Limiter parameters`);
  }
  return {
    effectId: String(effectId),
    slotIndex: Number(slotIndex),
 ...
 */
function parseGraphStaticWwiseCompressor(effect, effectId, slotIndex) {
  const label = `Audio Bus graph effect ${effectId}`;
  const bytes = RequireStaticGraphEffect(effect, WWISE_COMPRESSOR_PLUGIN_ID, 22, label, "Wwise Compressor");
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  const thresholdDb = view.getFloat32(0, true);
  const ratio = view.getFloat32(4, true);
  ...
  const processLfeRaw = view.getUint8(20);
  const channelLinkRaw = view.getUint8(21);
  if (!Number.isFinite(thresholdDb) || thresholdDb < DYNAMICS_THRESHOLD_MIN || thresholdDb > DYNAMICS_THRESHOLD_MAX || !Number.isFinite(ratio) || ratio < DYNAMICS_RATIO_MIN || ratio > DYNAMICS_RATIO_MAX || !Number.isFinite(attackSeconds) || attackSeconds < COMPRESSOR_TIME_MIN || attackSeconds > COMPRESSOR_TIME_MAX || !Number.isFinite(releaseSeconds) || releaseSeconds < COMPRESSOR_TIME_MIN || releaseSeconds > COMPRESSOR_TIME_MAX || !Number.isFinite(outputGainDb) || outputGainDb < DYNAMICS_OUTPUT_GAIN_MIN || outputGainDb > DYNAMICS_OUTPUT_GAIN_MAX || processLfeRaw > 1 || channelLinkRaw > 1) {
    throw new TypeError(`${label} has invalid Wwise Compressor parameters`);
  }
  return {
    effectId: String(effectId),
    slotIndex: Number(slotIndex),
...
function parseGraphSharedBusEffect(effect, effectId, slotIndex, {
  wwiseDynamics = "strict",
  wwiseMeterFeedback = "strict"
} = {}) {
  const dynamicsMode = normalizeWwiseDynamicsMode(wwiseDynamics);
  ...
  switch (effect?.pluginId) {
    ...
    case WWISE_COMPRESSOR_PLUGIN_ID:
      if (dynamicsMode === "approximate-web-audio") {
        return RequireApproximateDynamics(parseGraphStaticWwiseCompressor(effect, effectId, slotIndex), "compressor-approximation");
      }
      break;
    case WWISE_PEAK_LIMITER_PLUGIN_ID:
      if (dynamicsMode === "approximate-web-audio") {
        return RequireApproximateDynamics(parseGraphStaticWwisePeakLimiter(effect, effectId, slotIndex), "peak-limiter-approximation");
      }
      break;
  }
...
function RequireApproximateDynamics(effect, type) {
  if (effect.processLfe !== true || effect.channelLink !== true) {
    throw new TypeError(`Audio Bus graph effect ${effect.effectId} requires unsupported independent dynamics channels`);
  }
  if (type === "compressor-approximation" && (effect.attackSeconds === 0 || effect.attackSeconds > WEB_AUDIO_DYNAMICS_TIME_MAX) || effect.releaseSeconds > WEB_AUDIO_DYNAMICS_TIME_MAX) {
    throw new TypeError(`Audio Bus graph effect ${effect.effectId} exceeds Web Audio dynamics timing limits`);
  }
  return {
    ...effect,
    type
...
function RequireStaticGraphEffect(effect, pluginId, byteLength, label, kind) {
  ...
  if (!effect || effect.type !== "effect-custom" && effect.type !== "effect-share-set" || effect.pluginId !== pluginId || effect.parameterByteLength !== byteLength || !Array.isArray(effect.media) || effect.media.length !== 0 || !controls || controls.rtpcCount !== 0 || controls.statePropertyCount !== 0 || controls.stateGroupCount !== 0 || controls.propertyValueCount !== 0) {
...
function DecodeBase64(value, label) {
  ...
  const cleanLength = text.endsWith("==") ? text.length - 2 : text.endsWith("=") ? text.length - 1 : text.length;
  ...
  if (text.endsWith("==") && (alphabet.indexOf(text[cleanLength - 1]) & 0x0f) !== 0 || text.endsWith("=") && (alphabet.indexOf(text[cleanLength - 1]) & 0x03) !== 0) {
...
function FiniteGain(value, label) {
  const number = Number(value);
  if (!Number.isFinite(number) || number < MIN_GAIN_DB || number > MAX_GAIN_DB) {
    throw new TypeError(`${label} must be from ${MIN_GAIN_DB} to ${MAX_GAIN_DB} dB`);
  }
  return number;
}
