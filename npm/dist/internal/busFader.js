...
function scheduleSharedBusFader({
  param,
  busId,
  staticGainDb = 0,
  context,
  ...
  const evaluate = at => 10 ** (((Number(staticGainDb) || 0) + evaluateBusRtpcGainDb(busRtpcs, [id], readGlobalRtpc, at) + evaluateBusStateGainDb(busStates, [id], readGlobalStateWeights, at)) / 20);
  const boundaries = [...(readGlobalRtpcTransitionBoundaries?.(now) ?? []), ...(readGlobalStateTransitionBoundaries?.(now) ?? [])].map(Number).filter(value => Number.isFinite(value) && value > now);
  const uniqueBoundaries = [...new Set(boundaries)].sort((left, right) => left - right);
...
export { scheduleSharedBusFader };
//# sourceMappingURL=busFader.js.map
