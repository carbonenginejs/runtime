import { RawDataType, RawData } from './RawData.js';
...
function catalogDef(layout) {
  ...
  for (const field of layout.fields.values()) {
    let value = null;
    if (field.default) {
      value = field.count > 1 ? Array.from({
        length: field.count
...
class TriPoolAllocator {
  ...
  Register(structs) {
    for (const name of Object.keys(structs)) {
      ...
      if (Array.isArray(entry)) {
        ...
        this.RegisterStruct(name, entry.def, {
          stages: entry.stages
        });
      }
    }
  ...
  RegisterStruct(name, def, options = {}) {
    if (def === undefined) {
      ...
      if (!layout) {
        throw new Error(`TriPoolAllocator: struct "${name}" is not in CjsPerObjectLayouts - add it there, or pass a def explicitly`);
      }
      def = catalogDef(layout);
      options = {
        ...options,
        stages: options.stages ?? layout.stages
      };
    }
...
export { TriPoolAllocator };
//# sourceMappingURL=TriPoolAllocator.js.map
