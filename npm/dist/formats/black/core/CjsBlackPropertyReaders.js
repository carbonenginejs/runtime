  ...
  static skipBlackValue(reader, black, descriptor) {
    switch (black.beType) {
      ...
      case "IROOTWEAKREF":
        ...
        return;
      default:
        CjsBlackPropertyReaders.skipValue(reader, {
          jsType: descriptor
        });
    }
  }
  ...
  static readArray(reader, descriptor) {
    ...
    for (let i = 0; i < count; i++) {
      if (!descriptor.elementType || elementType.kind === CARBON_TYPE.OBJECT_REF || elementType.kind === CARBON_TYPE.UNKNOWN) {
        ...
        result[i] = CjsBlackPropertyReaders.readValue(reader, {
          jsType: elementType
        });
      }
    }
...
export { CjsBlackPropertyReaders };
//# sourceMappingURL=CjsBlackPropertyReaders.js.map
