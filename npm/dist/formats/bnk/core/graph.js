 ...
 */
function eventMediaFromBanks(inspections, {
  knownWemIds = []
} = {}) {
  ...
  for (const entry of objects.values()) {
    ...
    while (stack.length) {
      ...
      if (node.type === HIRC_SOUND && node.pluginType === 1 && node.sourceId !== undefined && wemIdSet.has(node.sourceId)) {
