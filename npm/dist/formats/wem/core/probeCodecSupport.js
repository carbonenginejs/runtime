...
function validatePtadpcm(metadata) {
  ...
  return channels >= 1 && frameSize >= 6 && metadata.blockAlign === frameSize * channels && metadata.dataBytes >= metadata.blockAlign && metadata.dataBytes % metadata.blockAlign === 0;
}
function validatePcm(metadata) {
  return metadata.bitsPerSample === 16 && metadata.channels >= 1 && metadata.blockAlign === metadata.channels * 2 && metadata.byteRate === metadata.sampleRate * metadata.blockAlign && metadata.dataBytes > 0 && metadata.dataBytes % metadata.blockAlign === 0;
}
