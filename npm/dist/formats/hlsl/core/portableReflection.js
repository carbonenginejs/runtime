 ...
 */
function readEffectBodyReflection(input, options = {}) {
  const source = String(options.source ?? "memory").trim() || "memory";
  const effectRes = readRaw(input, {
    source
  });
  return buildEffectBodyReflection(effectRes, options.permutationIndex ?? 0);
}
 ...
 */
function buildEffectBodyReflection(effectRes, permutationIndex) {
  if (!effectRes || typeof effectRes !== "object" || typeof effectRes.GetShaderByIndex !== "function") {
    throw new TypeError("Portable effect reflection requires a loaded HlslEffectRes with GetShaderByIndex");
  }
  assertUint(permutationIndex, "Portable effect reflection permutationIndex");
  if (permutationIndex >= effectRes.m_offsetCount) {
    throw new RangeError(`Portable effect reflection body index ${permutationIndex} is unavailable`);
  }
  if (effectRes.m_version !== PORTABLE_EFFECT_VERSION) {
    throw new Error(`Portable effect reflection requires source effect version ${PORTABLE_EFFECT_VERSION}`);
  }
  const sourceRecord = normalizeBodySourceRecord(effectRes.m_offsets?.[permutationIndex], permutationIndex, effectRes.m_data?.byteLength);
  ...
  if (!description || description.readError) {
    throw description?.readError || new Error(`Effect body ${permutationIndex} could not be decoded`);
  }
  const document = {
...
function enumerateUniqueEffectBodies(effectRes) {
  if (!effectRes || typeof effectRes !== "object" || effectRes.m_version !== PORTABLE_EFFECT_VERSION || !(effectRes.m_data instanceof Uint8Array) || !Array.isArray(effectRes.m_offsets) || effectRes.m_offsetCount !== effectRes.m_offsets.length || !effectRes.m_offsets.length) {
    throw new TypeError("Portable effect body inventory requires a loaded version-15 HlslEffectRes");
  }
  if (effectRes.m_offsets.length > EFFECT_BODY_COUNT_MAX) {
    throw new RangeError(`Portable effect body inventory exceeds ${EFFECT_BODY_COUNT_MAX} records`);
  }
 ...
 */
function validateEffectBodyReflection(document) {
  ...
  if (!isRecord(document) || document.format !== EFFECT_BODY_REFLECTION_FORMAT || document.formatVersion !== EFFECT_BODY_REFLECTION_VERSION || document.mode !== "single-body" || document.keyScope !== "body-local" || !isRecord(document.coverage) || document.coverage.bodies !== "single" || document.coverage.reflection !== "complete" || document.coverage.sourcePrograms !== "complete" || document.coverage.constantDefaults !== "exact") {
  ...
  const sourceRecord = requireRecord(document.sourceRecord, "Portable reflection sourceRecord");
  ...
  assertUint(sourceRecord.byteLength, "Portable reflection sourceRecord byteLength");
...
function emitEffect(description) {
  return {
    ...
    techniques: (description.techniques || []).map((technique, techniqueIndex) => emitTechnique(technique, techniqueIndex))
  };
}
function emitTechnique(technique, techniqueIndex) {
  ...
  return {
    ...
    passes: (technique.passes || []).map((pass, passIndex) => emitPass(pass, key, passIndex)),
    libraries: (technique.libraries || []).map((library, libraryIndex) => emitLibrary(library, key, libraryIndex))
  };
}
...
function emitSamplerMap(map) {
  return Array.from(map || [], ([registerIndex, setup]) => ({
    registerIndex,
    name: setup.name === null || setup.name === undefined ? null : String(setup.name),
...
function validateSource(source) {
  ...
  assertUint(source.stringTableByteLength, "Portable reflection stringTableByteLength");
  assertUint(source.byteLength, "Portable reflection source byteLength");
  if (!source.byteLength || source.stringTableByteLength > source.byteLength) {
...
function validatePass(pass, counts, source) {
  ...
  for (const stage of pass.stages) {
    ...
    if (stage.stageType >= HlslRenderContextEnum.SHADER_TYPE_COUNT || stageTypes.has(stage.stageType)) {
...
function validateLibrary(library, counts, source) {
  ...
  for (const [index, entry] of library.exports.entries()) {
    ...
    if (entry.type > 4 || typeof entry.name !== "string" || !entry.name || exportIdentities.has(identity)) {
...
function validateSourceProgram(program, expectedKind, stageType, stageName, stringTableByteLength) {
  ...
  assertUint(program.stringTableOffset, "Portable reflection source program stringTableOffset");
...
function validateInput(input, context) {
  ...
  const defaults = requireRecord(input.constantDefaults, `Portable reflection ${context} constant defaults`);
  requireExactKeys(defaults, ["declaredByteLength", "bytes"], `Portable reflection ${context} constant defaults`);
  assertUint(defaults.declaredByteLength, `Portable reflection ${context} constant-default length`);
  const defaultBytes = requireBytes(defaults.bytes, `Portable reflection ${context} constant-default bytes`);
  if (defaults.declaredByteLength !== defaultBytes.byteLength || defaultBytes.byteLength > CONSTANT_BYTES_MAX) {
  ...
  for (const [index, constant] of input.constants.entries()) {
    ...
    if (!isRecord(constant) || typeof constant.name !== "string" || !constant.name || constantNames.has(constant.name)) {
...
function validateResourceList(entries, context) {
  validateRegisterIndexed(entries, "registerIndex", context, entry => {
    ...
    if (typeof entry.isSRGB !== "boolean" || typeof entry.isAutoregister !== "boolean") {
...
function validateSamplerList(entries, context) {
  validateRegisterIndexed(entries, "registerIndex", context, entry => {
    ...
    assertUint8(entry.registerIndex, `${context} registerIndex`);
    if (entry.isDynamic && typeof entry.name !== "string" || !entry.isDynamic && entry.name !== null || typeof entry.isDynamic !== "boolean") {
      throw new Error(`Portable reflection ${context} entry is malformed`);
    }
...
function validateSignature(signature, context) {
  ...
  for (const [index, register] of signature.registers.entries()) {
    ...
    const isUnbounded = register.arrayCount === 0 && register.registerCount === 0;
    const rangeEnd = isUnbounded ? UINT32_MAX + 1 : register.registerIndex + register.registerCount;
    if (register.registerCount !== register.arrayCount || isUnbounded && classification !== "sampler" && classification !== "resource" && classification !== "uav" || rangeEnd > UINT32_MAX + 1) {
  ...
  const group = requireRecord(signature.threadGroupSize, `Portable reflection ${context} threadGroupSize`);
...
function validateMapSignatureReconciliation(input, context) {
  ...
  validateMappedBindings(input.resources, signature.registers.filter(entry => registerClass(entry.registerType) === "resource"), `${context} resources`);
  validateMappedBindings(input.uavs, signature.registers.filter(entry => registerClass(entry.registerType) === "uav"), `${context} UAVs`);
  for (const sampler of input.samplers) {
    const matches = sampler.isDynamic ? signature.registers.filter(entry => registerClass(entry.registerType) === "sampler" && entry.registerIndex === sampler.registerIndex) : signature.staticSamplers.filter(entry => entry.registerIndex === sampler.registerIndex);
...
function validateSamplerDescriptor(descriptor, context, staticSampler) {
  ...
  if (staticSampler) {
    ...
    descriptor.borderColorRaw.forEach((value, index) => assertUint(value, `${context} sampler borderColorRaw[${index}]`));
  }
}
function validateAnnotationGroups(groups, context) {
  ...
  for (const [index, group] of groups.entries()) {
    ...
    if (!isRecord(group) || typeof group.parameterName !== "string" || names.has(group.parameterName) || !Array.isArray(group.annotations)) {
...
function validateAnnotations(annotations, context) {
  for (const [index, annotation] of annotations.entries()) {
    ...
    if (!isRecord(annotation) || typeof annotation.name !== "string" || !annotation.name) {
    ...
    if (annotation.type === 3) {
      if (typeof annotation.stringValue !== "string" || Object.prototype.hasOwnProperty.call(annotation, "rawValue")) {
...
function decodeEffectBodyFresh(effectRes, sourceRecord) {
  ...
  const buffer = effectRes.m_data.subarray(sourceRecord.offset, sourceRecord.offset + sourceRecord.byteLength);
  const ok = shader.GetEffect().Read(buffer, sourceRecord.byteLength, effectRes.m_version, effectRes.m_stringTable, effectRes.m_stringTableSize, effectRes.sourcePath, {
...
function normalizeBodySourceRecord(record, permutationIndex, sourceByteLength) {
  ...
  if (!record || record.index !== permutationIndex || !Number.isSafeInteger(record.offset) || record.offset < 0 || record.offset > UINT32_MAX || !Number.isSafeInteger(record.size) || record.size < 1 || record.size > UINT32_MAX || !Number.isSafeInteger(end) || record.end !== end || !Number.isSafeInteger(sourceByteLength) || end > sourceByteLength) {
    throw new Error(`Portable effect reflection body index ${permutationIndex} ` + "disagrees with its source record");
  }
  return Object.freeze({
...
function validateDisjointBodySourceRecords(sourceRecords) {
  ...
  for (let index = 1; index < ordered.length; index += 1) {
    ...
    if (current.offset < previous.offset + previous.byteLength) {
      throw new Error("Portable effect reflection body source records partially overlap");
    }
  }
}
