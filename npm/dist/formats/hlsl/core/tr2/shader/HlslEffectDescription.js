...
import { HlslRenderContextEnum, hlslShaderStageName, HlslUsageCodeNames } from '../HlslRenderContextEnum.js';
...
function readPass(stream, version, effectName, context) {
  ...
  for (let stageIndex = 0; stageIndex < stageCount; stageIndex += 1) {
    ...
    if (stageType >= HlslRenderContextEnum.SHADER_TYPE_COUNT || pass.stageInputs[stageType]?.m_exists) {
    ...
    readInput(stageInput, stream, version);
...
function readLibrary(stream, version, context) {
  ...
  readInput(library.globalInput, stream, version);
  ...
  readInput(library.localInput, stream, version);
  return library;
}
...
function readInput(input, stream, version, stageType, context) {
  ...
  if (version < 5) {
    sourceConstantValues = constantValueSize ? stream.readRaw(constantValueSize) : new Uint8Array(0);
  } else {
  ...
  input.m_constantValueSize = Math.min(constantValueSize, HlslEffectStageInput.SHADER_CONSTANTS_MAX);
  ...
  for (const constant of input.constants) {
    patchSamplerHeapIndexConstant(input, constant);
  }
}
...
function readConstant(stream, version) {
  ...
  if (version < 11) {
    ...
    if (oldType === 0) constant.type = HlslEffectConstant.Type.FLOAT;else if (oldType === 1) constant.type = HlslEffectConstant.Type.INT;else if (oldType === 2) constant.type = HlslEffectConstant.Type.BOOL;else constant.type = HlslEffectConstant.Type.OTHER;
  } else {
...
export { ANY_TECHNIQUE, DEFAULT_TECHNIQUE, HlslEffectDescription };
