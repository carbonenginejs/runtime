...
function decodeCodec(stream) {
  ...
  if (matches(packet, [1, 0x76, 0x6f, 0x72, 0x62, 0x69, 0x73])) {
    ...
    stream.frameRate = stream.frameRateDenominator ? stream.frameRateNumerator / stream.frameRateDenominator : 0;
    stream.pixelAspectNumerator = readU24BE(packet, 30);
    stream.pixelAspectDenominator = readU24BE(packet, 33);
    stream.pixelAspectRatio = stream.pixelAspectDenominator ? stream.pixelAspectNumerator / stream.pixelAspectDenominator : 0;
...
function addAudioTiming(stream, preSkip) {
  ...
  const granule = typeof stream.lastGranulePosition === "number" ? stream.lastGranulePosition : typeof stream.lastGranulePosition === "string" && /^\d+$/u.test(stream.lastGranulePosition) ? Number(stream.lastGranulePosition) : null;
