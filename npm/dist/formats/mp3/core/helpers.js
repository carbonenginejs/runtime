...
function isSupportedWithValues(input, values = DEFAULT_VALUES, expectedType = "") {
  try {
    ...
    const variants = [{
      ...
      codec: "pcm",
      supported: false,
      reason: "MP3 PCM decode/output is not implemented yet."
    }];
    return {
...
function readVbrHeader(bytes, frame) {
  const sideInfoLength = frame.version === "mpeg1" ? frame.channels === 1 ? 17 : 32 : frame.channels === 1 ? 9 : 17;
...
function readMp3FrameHeader(bytes, offset) {
  ...
  const bitrateTable = version === "mpeg1" ? [0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320] : [0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160];
  const sampleRateTable = version === "mpeg1" ? [44100, 48000, 32000] : version === "mpeg2" ? [22050, 24000, 16000] : [11025, 12000, 8000];
