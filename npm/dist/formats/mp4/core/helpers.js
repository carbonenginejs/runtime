...
function inspectMP4(bytes) {
  ...
  const movie = moov ? inspectMp4Movie(bytes, moov) : {
    duration: 0,
    durationTimescale: 1,
    tracks: []
  };
...
function inspectMp4Movie(bytes, moov) {
  ...
  const movieTiming = mvhd ? readMp4Timing(bytes, mvhd) : {
    duration: 0,
    durationTimescale: 1
  };
...
function inspectMp4Track(bytes, trak) {
  ...
  const timing = mdhd ? readMp4Timing(bytes, mdhd) : {
    duration: 0,
    durationTimescale: 1
  };
...
function readMp4SampleTable(bytes, stbl) {
  ...
  const timing = stts ? readMp4TimeToSample(bytes, stts) : {
    entryCount: 0,
    sampleCount: 0,
    duration: 0
  };
  const composition = ctts ? readMp4CompositionOffsets(bytes, ctts) : {
    entryCount: 0,
    sampleCount: 0,
    minOffset: 0,
    maxOffset: 0
  };
  const sizes = stsz ? readMp4SampleSizes(bytes, stsz) : {
    sampleCount: 0,
    bytes: 0,
    fixedSize: 0
  };
...
function readMp4Language(bytes, box) {
  ...
  if (!value) return "und";
  return String.fromCharCode((value >>> 10 & 0x1f) + 0x60, (value >>> 5 & 0x1f) + 0x60, (value & 0x1f) + 0x60);
}
function readMp4SampleEntry(bytes, stsd) {
  ...
  const result = {
    codec: fourCc(bytes, entry.start + 4)
  };
