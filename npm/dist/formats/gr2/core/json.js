...
const VERTEX_CHANNELS = Object.freeze([["position", "Position", 3], ["blendIndice", "BoneIndices", 4], ["tangent", "Tangent", 4], ["normal", "Normal", 3], ["texcoord0", "TextureCoordinates0", 2], ["texcoord1", "TextureCoordinates1", 2], ["binormal", "Binormal", 4], ["blendWeight", "BoneWeights", 4]]);
 ...
 */
function farr(a) {
  return scalarArray(a).map(x => sf(fr(x)));
}
...
function mappedAnnotationRows(set, vertexCount) {
  const annotations = set.VertexAnnotations || [],
...
function emitMesh(mesh, classes = {}) {
  ...
  const vd = mesh.PrimaryVertexData,
    verts = vd && vd.Vertices || [];
  o.vertex = emitVertexChannels(verts);
  o.morphTargets = (mesh.MorphTargets || []).map(mt => emitMorphTarget(mt, classes));
  o.morphTargets.push(...(vd?.VertexAnnotationSets || []).map(set => emitVertexAnnotationTarget(set, verts.length, classes)).filter(Boolean));
  const topo = mesh.PrimaryTopology || {},
  ...
  if (indices) {
    for (const g of groups) {
      const faces = new Array(g.TriCount * 3),
        start = g.TriFirst * 3;
...
function emitBone(bone, classes = {}) {
  const t = bone.LocalTransform || bone.Transform || {
      flags: 0,
      position: [0, 0, 0],
      orientation: [0, 0, 0, 1],
 ...
 */
function emitJson(fileInfo, version, options = {}) {
  const {
    classes = {},
    ...hydrationOptions
  } = options;
