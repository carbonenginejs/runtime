...
function canonicalMorphVertex(baseVertex, target, specs, vertexCount) {
  ...
  for (const spec of specs) {
    const source = sourceVertex[spec.name] ?? [],
    ...
    for (let row = 0; row < rowCount; row++) {
      ...
      for (let component = 0; component < count; component++) {
        const sourceValue = source[row * sourceWidth + component],
          baseValue = base[vertexIndex * baseWidth + component] ?? 0;
        output[vertexIndex * spec.elementCount + component] = target.dataIsDeltas === false ? sourceValue - baseValue : sourceValue;
      }
    }
...
function buildDecl(vertex, vertexCount = vertexCountOf(vertex)) {
  const specs = CHANNELS.filter(spec => Array.isArray(vertex[spec.name]) && vertex[spec.name].length > 0).map(spec => ({
    ...spec,
    elementCount: channelWidth(spec, vertex, vertexCount)
  }));
...
function maxDisplacement(deltaPositions) {
  ...
  for (let i = 0; i + 2 < deltaPositions.length; i += 3) {
    max = Math.max(max, Math.hypot(deltaPositions[i], deltaPositions[i + 1], deltaPositions[i + 2]));
  }
  return max;
}
