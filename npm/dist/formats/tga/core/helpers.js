...
function canDecodeTgaToRgba(metadata) {
  ...
  return hasMinimumTgaDataForRgba(metadata) && (direct && metadata.colorMapType === 0 && [8, 15, 16, 24, 32].includes(metadata.bitsPerPixel) || indexed && metadata.colorMapType === 1 && metadata.colorMapLength > 0 && [8, 15, 16, 24, 32].includes(metadata.colorMapEntryBits) && [8, 16].includes(metadata.bitsPerPixel));
}
...
function readTgaPixelBytes(bytes, offset, bytesPerPixel, metadata) {
  ...
  if (bytesPerPixel === 2) {
    ...
    const a = metadata.bitsPerPixel === 15 || (metadata.descriptor & 0x0f) === 0 ? 255 : value & 0x8000 ? 255 : 0;
    return [r, g, b, a];
  }
  return [bytes[offset + 2], bytes[offset + 1], bytes[offset], bytesPerPixel === 4 ? bytes[offset + 3] : 255];
}
...
function readTgaColor(bytes, offset, entryBytes, entryBits) {
  if (entryBytes === 2) {
    const value = readU16LE(bytes, offset);
    return [Math.round((value >>> 10 & 0x1f) * 255 / 31), Math.round((value >>> 5 & 0x1f) * 255 / 31), Math.round((value & 0x1f) * 255 / 31), entryBits === 16 && value & 0x8000 ? 255 : entryBits === 16 ? 0 : 255];
  }
  return [bytes[offset + 2], bytes[offset + 1], bytes[offset], entryBits === 32 ? bytes[offset + 3] : 255];
}
