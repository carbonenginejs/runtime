 ...
 */
function normalizeEffectPermutation(value) {
  if (value === undefined || value === null) {
    return Object.freeze([]);
  }
  const entries = value instanceof Map ? Array.from(value, ([name, option]) => ({
    name,
    value: option
  })) : value;
  ...
  return Object.freeze(entries.map(entry => {
    if (!entry || typeof entry !== "object" || Array.isArray(entry) || typeof entry.name !== "string" || !entry.name || typeof entry.value !== "string" || !entry.value) {
 ...
 */
function validateResolvedPermutation(requested, selectedOptions) {
  ...
  for (const entry of requested) {
    ...
    if (resolved.value !== entry.value) {
      throw new Error(`Effect permutation ${entry.name} requested ${entry.value} but resolved ${resolved.value}`);
    }
  }
  return true;
}
function validateStageRecords(stages) {
  ...
  for (const [index, stage] of stages.entries()) {
    ...
    if (!stage || typeof stage.techniqueName !== "string" || !stage.techniqueName || !Number.isInteger(stage.passIndex) || stage.passIndex < 0 || stage.key !== expected) {
    ...
    if (!STAGE_NAMES.has(stage.stageName)) {
      if (KNOWN_UNSUPPORTED_STAGE_NAMES.has(stage.stageName)) {
        throw new Error(`WGSL effect stage ${expected} kind ${stage.stageName} is not supported`);
      }
      throw new Error(`Effect analysis stage ${index} is malformed`);
    }
  ...
  for (const [passKey, stageNames] of stagesByPass) {
    if (stageNames.includes("compute") && (stageNames.length !== 1 || stageNames[0] !== "compute")) {
 ...
 */
function selectEffectStages(stages, selection) {
  ...
  const selected = passIndex === null ? techniqueStages : techniqueStages.filter(stage => stage.passIndex === passIndex);
  if (!selected.length) {
  ...
  if (stageNames.length) {
    ...
    if (missing.length || unexpected.length) {
      throw new Error(`Stage assertion for ${techniqueName}.pass${passIndex} is incomplete: ` + `missing ${missing.join(",") || "none"}; unlisted ${unexpected.join(",") || "none"}`);
    }
  }
  return selected;
}
 ...
 */
function buildWgslSelectionMetadata(selection, selectedStages) {
  if (!selection) {
    return null;
  }
...
export { buildWgslSelectionMetadata, normalizeEffectPermutation, selectEffectStages, validateResolvedPermutation };
//# sourceMappingURL=packageEffectSelection.js.map
