...
function buildEffectPackage(input, options = {}) {
  ...
  if (resolved.effectRes?.m_version !== 15) {
    throw new Error("Effect package requires a version-15 compiled effect, got version " + (resolved.effectRes?.m_version ?? "unknown"));
  }
  ...
  if (isParticleClearEffectCandidate(resolved.effectDescription)) {
    ...
    effectProfileContext = preflightParticleClearEffectProfile(resolved.effectDescription, programsByKey);
  }
  ...
  const resourceTransformPlans = new Map(Array.from(programsByPass, ([key, entries]) => [key, buildResourceTransformPlan(entries.map(entry => ({
    ir: entry.ir,
  ...
  const plans = new Map(Array.from(programsByPass, ([key, entries]) => {
    const proof = entries.find(entry => entry.effectProfileProof)?.effectProfileProof ?? null;
    const resourceTransformPlan = resourceTransformPlans.get(key);
    return [key, buildWgslBindingPlan(entries.map(entry => entry.ir), {
  ...
  const bytes = buildPackage([["INFO", info], ["META", metadata], [EFFECT_PERMUTATION_GRAPH_CHUNK, permutationGraph], ...(effectReflection ? [[EFFECT_REFLECTION_CHUNK, effectReflection.reflection], [EFFECT_REFLECTION_BLOB_CHUNK, effectReflection.blobBytes]] : []), ["ANLS", analysis], ["WGSL", wgsl], ...(backendBodySet ? [[EFFECT_BACKEND_BODY_SET_CHUNK, backendBodySet]] : [])]);
...
function normalizeMode(value, allPermutations) {
  ...
  if (mode !== "selected" && mode !== "all") {
    throw new Error(`Effect package mode ${mode || "<empty>"} is not supported; ` + "supported modes are selected and all");
  }
  return mode;
}
...
function normalizeSelection(value) {
  ...
  if (!Array.isArray(stageNames) || stageNames.some(stageName => !["vertex", "pixel", "compute"].includes(stageName))) {
...
function normalizeSourceIdentity(value, source, input) {
  ...
  if (value?.sha256 !== undefined && value?.sha256 !== null && value.sha256 !== sha256) {
    throw new Error("Effect sourceIdentity.sha256 does not match the exact effect input bytes");
  }
  return Object.freeze({
...
//# sourceMappingURL=packageEffect.js.map
