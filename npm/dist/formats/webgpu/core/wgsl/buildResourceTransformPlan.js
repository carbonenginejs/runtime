...
function normalizeInput(input, transformId, index) {
  ...
  if (typeof parameter !== "string" || !parameter || !Number.isInteger(layer) || layer !== index || typeof identity !== "string" || !RESOURCE_IDENTITY.test(identity) || scoped !== `${identity}@fragment`) {
...
function normalizeTransform(transform) {
  ...
  if (typeof id !== "string" || !id || transform.version !== 1 || transform.kind !== "texture-2d-array" || typeof transform.layoutKey !== "string" || !transform.layoutKey || transform.stage !== "fragment" || !Array.isArray(transform.inputs) || transform.inputs.length < 2) {
  ...
  if (typeof output?.name !== "string" || !output.name || output.identity !== inputs[0].identity || output.scopeIdentity !== inputs[0].scopeIdentity || output.viewDimension !== "2d-array" || output.layerCount !== inputs.length || transform.representation !== "native-or-rgba8" || transform.missingLayer !== "reject") {
 ...
 */
function normalizeResourceTransformPlan(value, layoutKey = null) {
  if (value === undefined || value === null) return null;
  if (value?.format !== "CJS_WGSL_RESOURCE_TRANSFORM_PLAN" || value.formatVersion !== 1 || !Array.isArray(value.resourceTransforms) || !value.resourceTransforms.length) {
    throw new TypeError("WGSL resource transform plan must be a non-empty version-1 document");
  }
  const resourceTransforms = value.resourceTransforms.map(normalizeTransform).sort((left, right) => left.id.localeCompare(right.id));
  const ids = new Set();
...
function bindingMatchesForOperand(program, resourceKind, operand) {
  const candidates = program.bindings.filter(binding => binding.resourceKind === resourceKind);
  const rangeId = operand?.resourceReference?.rangeId;
  return Number.isInteger(rangeId) ? candidates.filter(binding => binding.range?.rangeId === rangeId) : candidates.filter(binding => bindingRegister(binding) === operand?.registerIndex);
}
function resolveBindingForOperand(program, resourceKind, operand) {
  ...
  if (operand?.typeName !== (resourceKind === "sampler" ? "sampler" : "resource") || (operand.modifierName ?? "none") !== "none" || (operand.minPrecisionName ?? "default") !== "default" || operand.nonUniform || operand.indices?.some(index => index?.relative) || operand.resourceReference?.nonUniform || absolute?.relative || absolute !== undefined && (absolute?.values?.length !== 1 || absolute.values[0] !== bindingRegister(binding))) {
    return null;
  }
  return binding;
}
 ...
 */
function buildResourceTransformPlan(entries, options = {}) {
  ...
  if (program?.format !== "CJS_SHADER_IR" || program.formatVersion !== 1 || !Array.isArray(fragments[0].semanticBindings)) {
  ...
  for (const parameter of parameters) {
    ...
    if (binding.resourceDimension !== "texture2d" || binding.structureStride !== null || binding.range?.unbounded || binding.range?.registerCount !== 1 || returns.length !== 4 || returns.some(entry => entry !== "float")) {
      return rejectCandidate();
    }
    sources.push({
      parameter,
  ...
  for (const instruction of program.instructions) {
    for (let operandIndex = 0; operandIndex < instruction.operands.length; operandIndex += 1) {
      ...
      if (!binding) {
        const unresolvedCandidate = bindingMatchesForOperand(program, "sampled-resource", operand).some(candidate => sourceByIdentity.has(bindingIdentity(candidate)));
        if (unresolvedCandidate) return rejectCandidate();
        continue;
      }
      const identity = bindingIdentity(binding);
      if (!sourceByIdentity.has(identity)) continue;
      if (!SAMPLE_OPCODES.has(instruction.opcodeName) || operandIndex !== 2 || instruction.extensions?.some(extension => extension.typeName === "sample_controls") || instruction.operands.length !== 5) {
        return rejectCandidate();
      }
      ...
      if (samplerIdentity !== null && samplerIdentity !== currentSamplerIdentity || biasFingerprint !== null && biasFingerprint !== currentBiasFingerprint) {
        return rejectCandidate();
      }
...
export { buildResourceTransformPlan, normalizeResourceTransformPlan };
//# sourceMappingURL=buildResourceTransformPlan.js.map
