...
function terminatesAllPaths(statements) {
  ...
  if (statement.kind === "if") {
    return Boolean(statement.elseStatements) && terminatesAllPaths(statement.statements) && terminatesAllPaths(statement.elseStatements);
  }
  if (statement.kind === "switch") {
    return statement.clauses.some(clause => clause.isDefault) && statement.clauses.every(clause => terminatesAllPaths(clause.statements));
  }
  return false;
}
function buildDominators(program) {
  ...
  while (changed) {
    ...
    for (const block of reachable.slice(1)) {
      const predecessors = block.predecessors.map(edge => dominators.get(edge.blockId)).filter(Boolean);
...
function reachingRef(program, blockId, register, component, dominators) {
  ...
  for (const block of program.blocks) {
    ...
    const output = (block.outputValues || []).find(entry => entry.register === register && entry.component === component);
  ...
  return best ? {
    valueId: best.ref.valueId,
    component: best.ref.component
  } : null;
}
...
function selectionConditionId(instruction) {
  ...
  const read = reads.length === 1 && reads[0].kind === "register-read" && reads[0].operandIndex === 0 && reads[0].refs.length === 1 ? reads[0] : null;
...
function undefinedPathMaskedByAnd(use, constraints) {
  ...
  if (read.kind !== "register-read" || instruction.opcodeName !== "and" || instruction.saturate || instruction.operands.length !== 3 || ![1, 2].includes(read.operandIndex)) {
    return false;
  }
  ...
  const regularReads = instruction.dataflow.reads.filter(entry => entry.kind === "register-read" && [1, 2].includes(entry.operandIndex));
  if (regularReads.length !== 2 || regularReads.filter(entry => entry.operandIndex === 1).length !== 1 || regularReads.filter(entry => entry.operandIndex === 2).length !== 1) {
    return false;
  }
  ...
  if (!operand || !siblingOperand || (operand.modifierName || "none") !== "none" || (siblingOperand.modifierName || "none") !== "none" || (operand.minPrecisionName || "default") !== "default" || (siblingOperand.minPrecisionName || "default") !== "default") {
    return false;
  }
  ...
  return Boolean(siblingRef) && constraints.get(`${siblingRef.valueId}.${siblingRef.component}`) === false;
}
function validateUndefinedMergePaths(program, live, values, plans, stage) {
  const mergePlans = new Map();
  for (const plan of plans.values()) {
    for (const merge of plan.merges) mergePlans.set(merge.id, {
      plan,
      merge
    });
    for (const merge of plan.exitMerges || []) mergePlans.set(merge.id, {
      plan,
      merge
    });
  }
  function hasUndefinedPath(valueId, constraints, visiting, use) {
    ...
    let result = false;
    if (plan.kind === "switch") {
      result = merge.perClause.some(incoming => hasUndefinedPath(incoming.valueId, constraints, visiting, use));
      ...
      result = hasUndefinedPath(merge.entryIncoming.valueId, constraints, visiting, use) || hasUndefinedPath(merge.backedgeIncoming.valueId, new Map(), visiting, use);
      ...
      result = Array.from(plan.exitEdges.values()).some(assignments => assignments.filter(assignment => assignment.id === merge.id).some(assignment => hasUndefinedPath(assignment.ref.valueId, new Map(), visiting, use)));
    } else {
      const trueRequiresNonzero = plan.testBoolean !== "zero";
      const falseConstraints = withCondition(constraints, plan.conditionId, !trueRequiresNonzero);
      const trueInputs = merge.viaSwitch ? merge.perClause : [merge.trueIncoming];
      const trueConstraints = withCondition(constraints, plan.conditionId, trueRequiresNonzero);
      result = Boolean(falseConstraints) && hasUndefinedPath(merge.falseIncoming.valueId, falseConstraints, visiting, use);
      if (!result && trueConstraints) {
        result = trueInputs.some(incoming => hasUndefinedPath(incoming.valueId, trueConstraints, visiting, use));
      }
    }
...
function buildSwitchPlan(program, region, values, live, dominators, stage, sharedJoin = false) {
  ...
  const markers = [...region.caseInstructions, ...(region.defaultInstruction !== null ? [region.defaultInstruction] : [])].sort((left, right) => left - right);
  ...
  for (let index = 0; index < markers.length;) {
    ...
    clauses.push({
      markerInstructions: group,
      bodyStart: group.at(-1) + 1,
      bodyEndMarker
    });
  }
  ...
  for (const clause of clauses) {
    ...
    clause.selectors = clause.markerInstructions.filter(marker => marker !== region.defaultInstruction).map(marker => {
  ...
  if (mergeIds.length) {
    ...
    const reachableJoinPredecessors = join.predecessors.filter(edge => program.blocks.find(block => block.id === edge.blockId)?.reachable !== false);
    ...
    merges = mergeIds.map(id => {
      ...
      if (!value || value.origin !== "control-flow-merge" || value.incoming.length > clauses.length || value.writeMask.length !== 1 || !wgslType || !zeroForType(type) || value.incoming.some(incoming => incoming.kind !== "predecessor")) {
      ...
      if (fallback) {
        ...
        if (mergeIds.includes(fallback.valueId) || fallbackValue?.origin === "undefined-register" || !definitionDominates(fallbackValue, header.id, region.startInstruction, dominators)) {
      ...
      const perClause = clauses.map(clause => {
        ...
        if (incomingValue?.origin === "undefined-register" || !definitionDominates(incomingValue, clause.tailBlockId, tailBlock.endInstruction + 1, dominators)) {
...
function buildLoopPlan(program, region, values, live, dominators, stage) {
  ...
  const merges = mergeIds.map(id => {
    ...
    const type = value?.componentTypes?.[value.writeMask];
    const wgslType = scalarTypeName(type);
    if (!value || value.origin !== "control-flow-merge" || value.incoming.length !== 2 || value.writeMask.length !== 1 || !wgslType || !zeroForType(type) || value.incoming.some(incoming => incoming.kind !== "predecessor")) {
    ...
    const entryIncoming = value.incoming.find(incoming => incoming.blockId === preheaderBlockId) || reachingRef(program, preheaderBlockId, value.register, value.writeMask, dominators);
    if (!entryIncoming) {
      throw new Error(`WGSL ${stage} merge ${id} has unsupported loop incoming edges`);
    }
    const entryValue = values.get(entryIncoming.valueId);
    const entryIsLiveMerge = entryValue?.origin === "control-flow-merge" && live.has(entryIncoming.valueId);
    if (entryValue?.origin === "undefined-register" || entryValue?.origin === "control-flow-merge" && !entryIsLiveMerge || !definitionDominates(entryValue, preheaderBlockId, program.blocks.find(block => block.id === preheaderBlockId).endInstruction + 1, dominators)) {
    ...
    if (!backedgeIncoming || !backedgeValue || backedgeValue.origin === "undefined-register" || backedgeValue.id !== id && !backedgeIsHeaderPhi && !backedgeIsLiveMerge && (backedgeValue.origin === "control-flow-merge" || !definitionDominates(backedgeValue, backedgeBlock.id, region.endInstruction + 1, dominators))) {
  ...
  if (exitMergeIds.length) {
    const breakEdges = (exitJoin.predecessors || []).filter(edge => program.blocks.find(block => block.id === edge.blockId)?.reachable !== false);
    ...
    const perMerge = exitMergeIds.map(id => {
      ...
      const type = value?.componentTypes?.[value.writeMask];
      const wgslType = scalarTypeName(type);
      if (!value || value.origin !== "control-flow-merge" || value.writeMask.length !== 1 || !wgslType || !zeroForType(type) || value.incoming.some(incoming => incoming.kind !== "predecessor")) {
    ...
    exitMerges = perMerge.map(merge => ({
      id: merge.id,
      type: merge.type,
      zeroCode: merge.zeroCode
    }));
    for (const edge of breakEdges) {
      ...
      const assignments = perMerge.map(merge => {
        ...
        if (!ref || !incomingValue || exitMergeIds.includes(ref.valueId) || incomingValue.origin === "undefined-register" || !isLoopHeaderPhi && !isLiveMerge && (incomingValue.origin === "control-flow-merge" || !definitionDominates(incomingValue, edge.blockId, block.endInstruction + 1, dominators))) {
  ...
  return {
    ...
    header,
    preheaderBlockId,
    backedgeBlockId: backedgeBlock.id,
    merges,
    exitMerges,
    exitEdges
  };
...
function buildSelectionPlans(program, stage) {
  ...
  for (const region of program.controlFlow.regions) {
    ...
    const enclosing = program.controlFlow.regions.filter(candidate => candidate.kind === "selection" && candidate.elseInstruction === null && candidate.startInstruction < region.startInstruction && candidate.endInstruction === region.endInstruction + 1).sort((left, right) => right.startInstruction - left.startInstruction)[0];
    if (enclosing) {
  ...
  for (const region of program.controlFlow.regions) {
    if (region.kind === "switch") {
      ...
      plans.set(region.startInstruction, buildSwitchPlan(program, region, values, live, dominators, stage, sharedSelectionBySwitch.has(region.startInstruction)));
      continue;
    }
    if (region.kind === "loop") {
    ...
    if (sharedSwitch) {
      ...
      let sharedMerges = [];
      if (mergeIds.length) {
        const reachablePredecessors = join.predecessors.filter(edge => program.blocks.find(block => block.id === edge.blockId)?.reachable !== false);
        if (sharedSwitch.defaultInstruction === null || switchPlan.clauses.some(clause => clause.terminator !== "break") || reachablePredecessors.length !== switchPlan.clauses.length + 1) {
        ...
        sharedMerges = mergeIds.map(id => {
          ...
          const type = value?.componentTypes?.[value.writeMask];
          const wgslType = scalarTypeName(type);
          if (!value || value.origin !== "control-flow-merge" || value.incoming.length !== switchPlan.clauses.length + 1 || value.writeMask.length !== 1 || !wgslType || !zeroForType(type) || value.incoming.some(incoming => incoming.kind !== "predecessor")) {
          ...
          if (falseValue?.origin === "undefined-register" || !definitionDominates(falseValue, header.id, region.startInstruction, dominators)) {
    ...
    const merges = mergeIds.map(id => {
      ...
      if (directTrue && !directFalse) falseIncoming = value.incoming.find(incoming => incoming !== directTrue);else if (directFalse && !directTrue) trueIncoming = value.incoming.find(incoming => incoming !== directFalse);
      if (!falseIncoming || !trueIncoming || falseIncoming === trueIncoming || [falseIncoming, trueIncoming].some(incoming => mergeIds.includes(incoming.valueId))) {
        throw new Error(`WGSL ${stage} merge ${id} has unsupported incoming edges`);
      }
      const falseValue = values.get(falseIncoming.valueId);
      ...
      if (trueValue?.origin === "undefined-register" || !armInputUsable(trueValue, trueBlockId, program.blocks.find(block => block.id === trueBlockId).endInstruction + 1, dominators, live)) {
...
export { blockForInstruction, buildSelectionPlans, cloneWritten, terminatesAllPaths, zeroForType };
//# sourceMappingURL=selectionPlans.js.map
