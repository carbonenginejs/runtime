...
import { validateIndexableTempRead, validateIndexableTempOperand, validateIndexableTempWrite } from '../ir/indexableTemps.js';
...
import { buildSelectionPlans, terminatesAllPaths, cloneWritten } from './selectionPlans.js';
...
const SUPPORTED_OPCODES = new Set(["add", "and", "atomic_iadd", "deriv_rtx", "deriv_rty", "deriv_rtx_coarse", "deriv_rty_coarse", "deriv_rtx_fine", "deriv_rty_fine", "discard", "div", "dp2", "dp3", "dp4", "eq", "exp", "f16tof32", "f32tof16", "frc", "ftoi", "ftou", "ge", "iadd", "ieq", "ige", "ilt", "imad", "imax", "imin", "imul", "ine", "ineg", "ishl", "ishr", "if", "itof", "ld", "ld_structured", "log", "lt", "mad", "max", "min", "mov", "movc", "mul", "ne", "or", "rcp", "resinfo", "round_ne", "round_ni", "round_pi", "round_z", "rsq", "sample", "sample_b", "sample_d", "sample_l", "sincos", "sqrt", "udiv", "uge", "ult", "umax", "umin", "ushr", "utof", "xor", "endif", "ret"]);
...
const DERIVATIVES = Object.freeze({
  deriv_rtx: "dpdx",
  deriv_rty: "dpdy",
  deriv_rtx_coarse: "dpdxCoarse",
  deriv_rty_coarse: "dpdyCoarse",
  deriv_rtx_fine: "dpdxFine",
  deriv_rty_fine: "dpdyFine"
});
...
const REQUIRES_UNIFORM_CONTROL_FLOW = new Set([...Object.keys(DERIVATIVES), "sample", "sample_b"]);
...
function sampleOffsetArgument(instruction, viewDimension) {
  ...
  if (![offsets?.u, offsets?.v, offsets?.w].every(value => Number.isInteger(value) && value >= -8 && value <= 7)) {
...
function interfaceField(rows, direction) {
  ...
  return {
    ...
    attribute: builtin ? {
      ...
      kind: "location",
      index: direction === "output" ? rows[0].semanticIndex : registerIndex
    }
  };
}
...
function validateInterpolation(program, input) {
  ...
  const declarations = program.declarations.filter(entry => entry.opcodeName === "dcl_input_ps" && entry.data?.registerIndex === input.registerIndex);
...
function sourceRead(instruction, operandIndex) {
  return instruction.dataflow.reads.find(entry => entry.kind !== "index-read" && entry.operandIndex === operandIndex) || null;
}
...
function bindingForOperand(bindings, resourceKind, operand) {
  ...
  if (Number.isInteger(rangeId)) {
    ...
    return resourceKind === "uniform-buffer" ? binding : validateFixedHandleBinding(operand, binding, "fragment");
  }
  const binding = bindings.find(entry => entry.resourceKind === resourceKind && entry.registerIndex === operand.registerIndex) || null;
  return resourceKind === "uniform-buffer" ? binding : validateFixedHandleBinding(operand, binding, "fragment");
}
function originalBindingForOperand(program, resourceKind, operand) {
  const rangeId = operand.resourceReference?.rangeId;
  const candidates = program.bindings.filter(entry => entry.resourceKind === resourceKind);
  const matches = Number.isInteger(rangeId) ? candidates.filter(entry => entry.range?.rangeId === rangeId) : candidates.filter(entry => (entry.range?.lowerBound ?? entry.registerIndex) === operand.registerIndex);
  if (matches.length > 1) {
  ...
  if (absoluteIndex !== undefined && (absoluteIndex?.relative || absoluteIndex?.values?.length !== 1 || absoluteIndex.values[0] !== registerIndex)) {
...
function transformedSampleForOperand(program, operand, bindings, context) {
  ...
  const textureBinding = bindings.find(binding => binding.resourceKind === "sampled-resource" && binding.identity === input.transform.output.identity && binding.transformId === input.transform.id) || null;
  if (!textureBinding) {
...
function cbufferVectorIndex(program, instruction, operandIndex, operand, inputs) {
  ...
  const read = instruction.dataflow.reads.find(entry => entry.kind === "index-read" && entry.operandIndex === operandIndex && entry.dimension === indices.length - 1);
  ...
  if (storage === "uint32" || storage === "bitpattern32") indexCode = `i32(${indexCode})`;else if (storage !== "int32") throw new Error(`WGSL fragment dynamic cbuffer index has unsupported type ${storage}`);
...
function cbufferParts(program, instruction, operandIndex, operand, destinationMask, count, bindings, expectedType, inputs, activeComponents = null) {
  ...
  const parts = rawSelectedComponents(operand, destinationMask, count, activeComponents).map(component => `${binding.generatedSymbol}[${vectorIndex}].${component}`);
...
function icbParts(program, instruction, operandIndex, operand, destinationMask, count, expectedType, inputs, activeComponents = null) {
  ...
  const vectorIndex = cbufferVectorIndex(program, instruction, operandIndex, operand, inputs);
  const parts = rawSelectedComponents(operand, destinationMask, count, activeComponents).map(component => `icb[${vectorIndex}].${component}`);
  if (expectedType === "float32") return parts;
  if (!["int32", "uint32", "bitpattern32"].includes(expectedType)) {
...
function constTableParts(program, instruction, operandIndex, operand, destinationMask, count, expectedType, inputs, activeComponents = null) {
  ...
  const vectorIndex = cbufferVectorIndex(program, instruction, operandIndex, operand, inputs);
  const parts = rawSelectedComponents(operand, destinationMask, count, activeComponents).map(component => `${table.symbol}[${vectorIndex}].${component}`);
  if (expectedType === "float32") return parts;
  if (!["int32", "uint32", "bitpattern32"].includes(expectedType)) {
...
function applyModifier(parts, operand, expected, storageTypes, instruction, operandIndex) {
  ...
  if (expected === "unknown" && storageTypes?.length === parts.length && storageTypes.every(storage => MODIFIER_STORAGE_TYPES.has(storage))) {
...
function operandLaneExpression(program, instruction, operandIndex, destinationMask, laneIndex, targetType, inputs, bindings, moverData = false) {
  ...
  if (read && operand.typeName === "indexable_temp") {
    validateIndexableTempRead(program, operand, read, rawSelectedComponents(operand, destinationMask, read.refs.length, activeComponents));
  }
  if (read) {
    ...
    const storage = valueStorageType(program, ref);
    const code = reinterpretCode(valueReference(program, ref, inputs), storage, targetType, 1, `instruction ${instruction.index} lane read`);
    return moverData ? applyLaneModifier(code, operand, targetType, instruction, operandIndex) : applyModifier([code], operand, targetType, null, instruction, operandIndex)[0];
  }
...
function operandExpression(program, instruction, operandIndex, destinationMask, count, expectedType, inputs, bindings) {
  ...
  if (read) {
    ...
    parts = read.refs.slice(0, count).map((ref, componentIndex) => {
      ...
      const bitcast = instruction.typeInfo.bitcasts.find(entry => entry.kind === "read-bitcast" && entry.operandIndex === operandIndex && entry.componentIndex === componentIndex && entry.valueId === ref.valueId && entry.component === ref.component);
      if (bitcast) {
  ...
  const storageTypes = (operand.modifierName || "none") !== "none" && expectedType === "unknown" && read ? read.refs.slice(0, count).map(ref => valueStorageType(program, ref)) : null;
...
function bitcastKey(entry) {
  if (entry.kind === "read-bitcast") {
    return [entry.kind, entry.operandIndex, entry.componentIndex, entry.valueId, entry.component, entry.from, entry.to].join(":");
  }
...
function validateRegisterBitcasts(program, instruction) {
  ...
  if (intrinsicResult !== "unknown") {
    for (const write of instruction.dataflow.writes) {
      for (const component of write.mask) {
        const storage = valueStorageType(program, {
  ...
  const actual = instruction.typeInfo.bitcasts.filter(entry => entry.kind === "read-bitcast" || entry.kind === "result-bitcast");
  const requiredKeys = required.map(bitcastKey).sort();
  const actualKeys = actual.map(bitcastKey).sort();
  if (new Set(actualKeys).size !== actualKeys.length || requiredKeys.length !== actualKeys.length || requiredKeys.some((entry, index) => entry !== actualKeys[index])) {
...
function structuredLoadExpression(program, instruction, write, type, inputs, bindings) {
  ...
  if (byteOffsetOperand?.typeName !== "immediate32" || byteOffsetOperand.immediateValues?.length !== 1 || (byteOffsetOperand.modifierName || "none") !== "none" || !Number.isInteger(byteOffset) || byteOffset % 4 !== 0) {
  ...
  const parts = selected.map(part => reinterpretCode(part, "uint32", type.scalarType, 1, `structured load ${instruction.index}`));
...
function expressionFor(program, instruction, write, inputs, bindings, context = null) {
  ...
  const source = (index, forcedCount = count) => operandExpression(program, instruction, index, mask, forcedCount, expectedType(instruction, index), inputs, bindings);
  ...
  if (op === "f16tof32" || op === "f32tof16") {
    ...
    const parts = Array.from({
      length: count
    }, (_, index) => op === "f16tof32" ? `unpack2x16float(${lane(index)}).x` : `(pack2x16float(vec2<f32>(${lane(index)}, 0.0)) & 0xffffu)`);
  ...
  if (op === "udiv") {
    ...
    if (divisor?.typeName === "immediate32" && (divisor.modifierName ?? "none") === "none" && lanes.length && lanes.every(value => Number.isInteger(value) && value !== 0)) {
  ...
  if (op === "resinfo") {
    ...
    if (mipOperand?.typeName !== "immediate32" || mipOperand.immediateValues?.length !== 1 || (mipOperand.modifierName || "none") !== "none" || (mipOperand.minPrecisionName || "default") !== "default" || !Number.isInteger(mipBits)) {
    ...
    const parts = rawSelectedComponents(resource, mask, count).map(component => {
      let value;
      let dimension = true;
      if (component === "x") value = `${dims}.x`;else if (component === "y") value = `${dims}.y`;else if (component === "z" && viewDimension === "3d") value = `${dims}.z`;else if (component === "w") {
        value = mipCount;
        dimension = false;
  ...
  if (op === "sample" || op === "sample_b" || op === "sample_l" || op === "sample_d") {
    ...
    const transformed = transformedSampleForOperand(program, resource, bindings, context);
    const textureBinding = transformed?.textureBinding || bindingForOperand(bindings, "sampled-resource", resource);
    ...
    const sampled = op === "sample_b" ? `textureSampleBias(${tex}, ${coord}${arrayArg}, ${source(4, 1)}${offsetArg})` : op === "sample_l" ? `textureSampleLevel(${tex}, ${coord}${arrayArg}, ${source(4, 1)}${offsetArg})` : op === "sample_d" ? `textureSampleGrad(${tex}, ${coord}${arrayArg}, ${source(4, gradientComponents)}, ${source(5, gradientComponents)}${offsetArg})` : `textureSample(${tex}, ${coord}${arrayArg}${offsetArg})`;
    const components = rawSelectedComponents(resource, mask, count);
...
function applyResultBitcast(instruction, write, expression, type) {
  const bitcasts = instruction.typeInfo.bitcasts.filter(entry => entry.kind === "result-bitcast" && entry.operandIndex === write.operandIndex && entry.valueId === write.valueId);
  ...
  if (bitcasts.length !== components.length || bitcasts.some(entry => entry.from !== intrinsicType || entry.to !== type.scalarType) || components.some(component => !bitcasts.some(entry => entry.component === component))) {
    throw new Error(`WGSL fragment instruction ${instruction.index} has unsupported partial result bitcasts`);
  }
  return reinterpretCode(expression, intrinsicType, type.scalarType, components.length, `instruction ${instruction.index} result`);
}
function lowerInstruction(program, instruction, inputs, outputs, bindings, written, readValueIds, nonUniform = false, context = null) {
  ...
  const unsupportedExtension = instruction.extensions?.find(extension => !METADATA_OPCODE_EXTENSIONS.has(extension.typeName) && !(extension.typeName === "sample_controls" && SAMPLE_OFFSET_OPCODES.has(instruction.opcodeName)));
  ...
  if (instruction.opcodeName === "discard") {
    ...
    const scalarShape = operand?.typeName === "immediate32" ? operand.immediateValues?.length === 1 : COMPONENTS.includes(operand?.selected);
    ...
    return {
      ...
      dxbcOffset: instruction.dxbcOffset,
      condition: {
        code: `${condition} ${projection === "zero" ? "==" : "!="} 0u`,
        type: "bool"
      },
      statements: [{
        kind: "discard"
  ...
  if (instruction.opcodeName === "atomic_iadd") {
    ...
    return {
      ...
      statements: [{
        ...
        dxbcOffset: instruction.dxbcOffset,
        expression: {
          code: `atomicAdd(&${binding.generatedSymbol}[${address}], ${value})`,
          type: "void"
        }
      }]
    };
  ...
  if (instruction.opcodeName === "sincos" || instruction.opcodeName === "udiv") {
    ...
    if (writes.length > 2 || new Set(destinationOperands).size !== writes.length || destinationOperands.some(operandIndex => ![0, 1].includes(operandIndex))) {
  ...
  const lowerWrite = write => {
    ...
    const indexableAddress = destination?.typeName === "indexable_temp" ? validateIndexableTempWrite(program, destination, write) : null;
    ...
    if (mixedTypes) {
      ...
      if (instruction.opcodeName !== "mov" || instruction.saturate || !localSsaDestination || immediateSource?.typeName !== "immediate32") {
        throw new Error(`WGSL fragment value ${write.valueId} has an unresolved or mixed result type that is not supported`);
      }
      return Array.from(write.mask).map((component, laneIndex) => ({
        kind: "let",
    ...
    if (instruction.saturate) {
      ...
      } else if (["mov", "movc"].includes(instruction.opcodeName) && ["int32", "uint32", "bitpattern32"].includes(type.scalarType)) {
    ...
    if (destination.typeName === "output") {
      ...
      const assignmentExpression = reinterpretCode(expression, type.scalarType, field.scalarType, write.mask.length, `output ${field.semanticName}${field.semanticIndex}`);
      const assignment = {
        kind: "assignment",
        instructionIndex: instruction.index,
        dxbcOffset: instruction.dxbcOffset,
      ...
      return [{
        ...
        expression: {
          code: reinterpretCode(write.valueId, type.scalarType, field.scalarType, write.mask.length, `output ${field.semanticName}${field.semanticIndex}`),
          type: outputType
        }
      }];
    }
...
function containsOutputAssignment(statements) {
  return statements.some(statement => statement.kind === "assignment" || statement.kind === "if" && (containsOutputAssignment(statement.statements) || (statement.elseStatements ? containsOutputAssignment(statement.elseStatements) : false)) || statement.kind === "switch" && statement.clauses.some(clause => containsOutputAssignment(clause.statements)) || statement.kind === "loop" && containsOutputAssignment(statement.statements));
}
...
function loopHasNonUniformExit(program, region, varying) {
  ...
  const insideNestedLoop = index => regions.some(candidate => candidate.kind === "loop" && candidate !== region && candidate.startInstruction > region.startInstruction && candidate.endInstruction < region.endInstruction && index >= candidate.startInstruction && index <= candidate.endInstruction);
  for (let index = region.startInstruction; index <= region.endInstruction; index += 1) {
    ...
    if (op === "break" || op === "continue") {
      const varyingGuard = regions.some(candidate => (candidate.kind === "selection" || candidate.kind === "switch") && candidate.startInstruction >= region.startInstruction && candidate.endInstruction <= region.endInstruction && candidate.startInstruction < index && index <= candidate.endInstruction && !conditionIsUniform(program.instructions[candidate.startInstruction], varying));
 ...
 */
function lowerFragmentProgram(program, options = {}) {
  ...
  const inputs = groupSignaturesByRegister(program.signatures.input.filter(entry => liveRegisters.has(entry.registerIndex))).map(rows => interfaceField(rows, "input"));
  ...
  const planFromBinding = options.bindingPlan?.resourceTransforms === undefined ? null : normalizeResourceTransformPlan({
  ...
  if (planFromBinding && explicitPlan && JSON.stringify(planFromBinding) !== JSON.stringify(explicitPlan)) {
  ...
  const bindings = lowerBindingLayout(program, options.bindingPlan, null, resourceTransformPlan);
  ...
  const readValueIds = new Set([...program.instructions.flatMap(instruction => instruction.dataflow.reads.flatMap(read => read.refs.map(ref => ref.valueId))), ...program.values.flatMap(value => (value.incoming || []).map(incoming => incoming.valueId))]);
  function exitMergeAssignments(loopExit, breakIndex) {
    return (loopExit?.get(breakIndex) || []).map(assign => ({
      ...
      type: assign.type,
      expression: {
        code: valueReference(program, assign.ref, inputs),
        type: assign.type
      }
    }));
  }
    ...
    for (let index = start; index < end; index += 1) {
      ...
      if (!plan) {
        ...
        if (marker === "break" || marker === "breakc") {
          ...
          if (breakInstruction.saturate || (conditionOperand?.modifierName || "none") !== "none" || !COMPONENTS.includes(conditionOperand?.selected) || conditionRead?.refs.length !== 1 || !["zero", "nonzero"].includes(breakInstruction.testBoolean)) {
          ...
          statements.push({
            ...
            dxbcOffset: breakInstruction.dxbcOffset,
            condition: {
              code: `${condition} ${breakInstruction.testBoolean === "zero" ? "==" : "!="} 0u`,
              type: "bool"
            },
            statements: [...exitMergeAssignments(loopExit, breakInstruction.index), {
              kind: "break"
            }]
          });
          continue;
        ...
        if (marker === "continue" || marker === "continuec") {
          ...
          if (continueInstruction.saturate || (conditionOperand?.modifierName || "none") !== "none" || !COMPONENTS.includes(conditionOperand?.selected) || conditionRead?.refs.length !== 1 || !["zero", "nonzero"].includes(continueInstruction.testBoolean)) {
          ...
          statements.push({
            ...
            dxbcOffset: continueInstruction.dxbcOffset,
            condition: {
              code: `${condition} ${continueInstruction.testBoolean === "zero" ? "==" : "!="} 0u`,
              type: "bool"
            },
            statements: [{
              kind: "continue"
        ...
        const lowered = lowerInstruction(program, program.instructions[index], inputs, outputs, bindings, rangeWritten, readValueIds, flowNonUniform, context);
        statements.push(...(Array.isArray(lowered) ? lowered : [lowered]));
        continue;
      }
      if (plan.kind === "loop") {
        const loopInstruction = program.instructions[index];
        if (loopInstruction.opcodeName !== "loop" || program.instructions[plan.region.endInstruction].opcodeName !== "endloop") {
        ...
        for (const merge of plan.merges) {
          statements.push({
            ...
            type: merge.type,
            expression: {
              code: valueReference(program, merge.entryIncoming, inputs),
              type: merge.type
            }
          });
        }
        ...
        const body = lowerRange(index + 1, plan.region.endInstruction, bodyWritten, true, flowNonUniform || loopNonUniform, plan.exitEdges);
        ...
        const latch = plan.merges.map(merge => ({
          kind: "value-assignment",
          name: merge.id,
          type: merge.type,
          expression: {
            code: valueReference(program, merge.backedgeIncoming, inputs),
            type: merge.type
          }
        }));
        statements.push({
          kind: "loop",
          instructionIndex: loopInstruction.index,
          dxbcOffset: loopInstruction.dxbcOffset,
          statements: body,
          continuing: latch
        });
      ...
      if (plan.kind === "switch") {
        ...
        if (switchInstruction.opcodeName !== "switch" || program.instructions[plan.region.endInstruction].opcodeName !== "endswitch") {
        ...
        if (switchInstruction.saturate || (selectorOperand?.modifierName || "none") !== "none" || !COMPONENTS.includes(selectorOperand?.selected) || selectorRead?.refs.length !== 1) {
        ...
        for (let clauseIndex = 0; clauseIndex < plan.clauses.length; clauseIndex += 1) {
          ...
          for (const merge of [...plan.merges, ...(plan.outerMerges || [])]) {
            body.push({
              ...
              type: merge.type,
              expression: {
                code: valueReference(program, merge.perClause[clauseIndex], inputs),
                type: merge.type
              }
            });
          }
      ...
      if (ifInstruction.opcodeName !== "if" || endInstruction.opcodeName !== "endif" || plan.hasElse && program.instructions[plan.region.elseInstruction].opcodeName !== "else") {
      ...
      if (ifInstruction.saturate || (conditionOperand?.modifierName || "none") !== "none" || !COMPONENTS.includes(conditionOperand?.selected) || conditionRead?.refs.length !== 1) {
        throw new Error(`WGSL fragment if instruction ${ifInstruction.index} requires one unmodified scalar condition`);
      }
      for (const merge of plan.merges) {
        const expression = plan.hasElse ? merge.zeroCode : merge.falseCode || valueReference(program, merge.falseIncoming, inputs);
        statements.push({
          kind: "var",
          name: merge.id,
          type: merge.type,
      ...
      for (const merge of plan.merges) {
        ...
        trueStatements.push({
          ...
          type: merge.type,
          expression: {
            code: valueReference(program, merge.trueIncoming, inputs),
            type: merge.type
          }
        });
      }
      ...
      if (plan.hasElse) {
        ...
        for (const merge of plan.merges) {
          falseStatements.push({
            ...
            type: merge.type,
            expression: {
              code: merge.falseCode || valueReference(program, merge.falseIncoming, inputs),
              type: merge.type
            }
          });
        }
      ...
      statements.push({
        ...
        condition: {
          code: `${condition} ${comparison} 0u`,
          type: "bool"
        },
        statements: trueStatements,
        ...(falseStatements ? {
          elseStatements: falseStatements
        } : {})
      });
      if (plan.hasElse) {
        ...
        for (const [fieldId, target] of rangeWritten) {
          ...
          if (trueReturns && !falseReturns) for (const component of fromFalse) target.add(component);else if (falseReturns && !trueReturns) for (const component of fromTrue) target.add(component);else if (!trueReturns && !falseReturns) {
...
export { lowerFragmentProgram };
//# sourceMappingURL=lowerFragmentProgram.js.map
