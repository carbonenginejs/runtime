 ...
 */
function buildWgslBindingPlan(programs, options = {}) {
  ...
  const resourceTransformPlan = normalizeResourceTransformPlan(options.resourceTransformPlan);
  ...
  for (const [index, program] of programs.entries()) {
    ...
    const layoutPolicy = particleClearSignedAtomicLayoutPolicy(program, options.effectProfileProof ?? null) ?? particleEmitSignedAtomicLayoutPolicy(program);
    for (const binding of lowerBindingLayout(program, null, layoutPolicy, resourceTransformPlan)) {
      const key = identity(binding);
      const entry = portableBinding(binding);
      if (!identities.has(key)) identities.set(key, []);
      identities.get(key).push({
        binding: entry,
        stage
      });
    }
  ...
  for (const [key, occurrences] of identities) {
    const stages = occurrences.map(entry => entry.stage).sort((left, right) => STAGE_ORDER[left] - STAGE_ORDER[right]);
  ...
  return deepFreeze({
    ...
    ...(sharedIdentities.size ? {
      sharedIdentities: Object.freeze([...sharedIdentities].sort())
    } : {}),
    ...(Object.keys(varyingInterpolation).length ? {
...
export { buildWgslBindingPlan };
//# sourceMappingURL=buildWgslBindingPlan.js.map
