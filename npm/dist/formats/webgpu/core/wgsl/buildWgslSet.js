...
function normalizeEntry(entry, index) {
  ...
  if (shader?.format !== "CJS_WGSL_SHADER" || shader.formatVersion !== 1 || typeof shader.code !== "string" || !shader.code || typeof shader.entryPoint !== "string" || !shader.entryPoint || !Array.isArray(shader.sourceMap) || shader.program?.format !== "CJS_TYPED_SHADER") {
  ...
  if (stageName === "compute") {
    ...
    if (!Array.isArray(threadGroupSize) || threadGroupSize.length !== 3 || threadGroupSize.some(value => !Number.isSafeInteger(value) || value < 1)) {
  ...
  return {
    ...
    ...(threadGroupSize ? {
      threadGroupSize: clonePlain(threadGroupSize)
    } : {}),
    key
  };
}
function portableBinding(binding, visibility) {
  if (!Number.isInteger(binding.group) || binding.group < 0 || !Number.isInteger(binding.binding) || binding.binding < 0 || !Number.isInteger(binding.registerSpace) || binding.registerSpace < 0 || !Number.isInteger(binding.registerIndex) || binding.registerIndex < 0 || typeof binding.resourceKind !== "string" || !binding.resourceKind || typeof binding.generatedSymbol !== "string" || !binding.generatedSymbol || typeof binding.type !== "string" || !binding.type) {
  ...
  if (binding.scopeIdentity !== undefined && (typeof binding.scopeIdentity !== "string" || !binding.scopeIdentity)) {
    throw new Error(`WGSL binding ${binding.generatedSymbol} has invalid scope identity ${binding.scopeIdentity || "<empty>"}`);
  }
  const scopeIdentity = binding.scopeIdentity === undefined ? `${identity}@${visibility}` : binding.scopeIdentity;
  ...
  const hasTransform = binding.transformId !== undefined || binding.arrayLayerCount !== undefined;
  if (hasTransform && (binding.resourceKind !== "sampled-resource" || typeof binding.transformId !== "string" || !binding.transformId || !Number.isInteger(binding.arrayLayerCount) || binding.arrayLayerCount < 2 || binding.type !== "texture_2d_array<f32>" || binding.texture?.viewDimension !== "2d-array")) {
  ...
  return {
    ...
    ...(Number.isInteger(binding.structureStride) ? {
      structureStride: binding.structureStride
    } : {}),
    ...(binding.buffer ? {
      buffer: clonePlain(binding.buffer)
    } : {}),
    ...(binding.texture ? {
      texture: clonePlain(binding.texture)
    } : {}),
    ...(binding.sampler ? {
      sampler: clonePlain(binding.sampler)
    } : {})
  };
}
...
function buildLayouts(entries) {
  ...
  for (const entry of entries) {
    ...
    for (const source of entry.shader.program.bindings || []) {
      ...
      if (identity === binding.identity && Array.from(baseScopes).some(scope => scope !== binding.identity) || identity !== binding.identity && baseScopes.has(binding.identity)) {
      ...
      existing.visibility = Array.from(new Set([...existing.visibility, ...binding.visibility])).sort((left, right) => VISIBILITY_ORDER.indexOf(left) - VISIBILITY_ORDER.indexOf(right));
    }
  }
...
function buildResourceTransforms(entries, layouts) {
  ...
  const transformedBindings = layouts.flatMap(layout => layout.bindGroups.flatMap(group => group.bindings.filter(binding => binding.transformId).map(binding => ({
  ...
  for (const transform of plan.resourceTransforms) {
    ...
    const links = transformedBindings.filter(({
      layoutKey,
      binding
    }) => layoutKey === transform.layoutKey && binding.transformId === transform.id);
    if (links.length !== 1) {
    ...
    const transformLayout = layouts.find(layout => layout.key === transform.layoutKey);
    const physicalScopes = new Set((transformLayout?.bindGroups || []).flatMap(group => group.bindings.map(entry => entry.scopeIdentity)));
