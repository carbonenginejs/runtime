...
function buildMorphTargets(mesh) {
  ...
  const firstVertex = targets.find(target => target.vertex)?.vertex ?? {},
...
function buildDecl(vertex) {
  const decl = [];
  let offset = 0;
  for (const channel of [["position", "Position", 3], ["normal", "Normal", 3], ["tangent", "Tangent", 3], ["binormal", "Binormal", 3], ["texcoord0", "TexCoord", 2, 0], ["texcoord1", "TexCoord", 2, 1], ["color0", "Color", 4, 0], ["blendIndice", "BoneIndices", 4, 0, "UInt16"], ["blendWeight", "BoneWeights", 4, 0], ["packedTangent", "PackedTangent", 4, 0, "Int16Norm"], ["packedTangentLegacy", "PackedTangentLegacy", 4, 0, "UInt16Norm"]]) {
...
function maxDisplacement(basePositions, targetPositions) {
  ...
  for (let i = 0; i < Math.min(basePositions.length, targetPositions.length); i += 3) {
    max = Math.max(max, Math.hypot(targetPositions[i] - basePositions[i], targetPositions[i + 1] - basePositions[i + 1], targetPositions[i + 2] - basePositions[i + 2]));
  }
  return max;
}
...
export { buildCmfFromShared, buildSharedFromCmf };
