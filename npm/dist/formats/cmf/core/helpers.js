import { OUTPUT_NATIVE, OUTPUT_GR2, CLASS_KEYS, OUTPUT_RAW, OUTPUT_SHARED, OUTPUT_CMF, OUTPUT_CMF_JSON, OUTPUT_JSON } from './constants.js';
import { readCmf, inspectCmf } from './schema.js';
import { buildSharedFromCmf, buildCmfFromShared } from './shared.js';
...
function readWithValues(owner, input, values) {
  ...
  return values.emit === OUTPUT_SHARED ? buildSharedFromCmf(raw, values.classes, {
    source: values.source
  }) : values.emit === OUTPUT_GR2 ? buildGr2FromCmf(raw, values.classes, {
    source: values.source
...
function loadSharedWithValues(input, values) {
  ...
  return values.emit === OUTPUT_SHARED ? buildSharedFromCmf(native, values.classes, {
    source: values.source
  }) : values.emit === OUTPUT_GR2 ? buildGr2FromCmf(native, values.classes, {
    source: values.source
...
function loadNativeWithValues(input, values) {
  ...
  return values.emit === OUTPUT_SHARED ? buildSharedFromCmf(input, values.classes, {
    source: values.source
  }) : values.emit === OUTPUT_GR2 ? buildGr2FromCmf(input, values.classes, {
    source: values.source
...
export { CLASS_KEYS, DEFAULT_VALUES, OUTPUT_CMF, OUTPUT_CMF_JSON, OUTPUT_GR2, OUTPUT_JSON, OUTPUT_NATIVE, OUTPUT_RAW, OUTPUT_SHARED, inspectRawCmfResult, loadNativeWithValues, loadSharedWithValues, normalizeValues, readRawInput, readRawInputAsync, readWithValues, readWithValuesAsync, toJsonValue, validateClass, validateClassKey };
