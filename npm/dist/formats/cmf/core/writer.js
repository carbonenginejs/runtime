...
import { STRUCT_SIZE, FILE_SIGNATURE, FILE_VERSION, SectionType, SectionCompression, MeshTopology, AnimationChannelTargetType, Interpolation, ElementType, Usage } from './constants.js';
...
function writeData(buffer, offset, graph, remap) {
  const writeMesh = (target, meshOffset, mesh) => {
    ...
    target.span(meshOffset + 80, mesh.morphTargets?.decl || [], STRUCT_SIZE.VertexElement, writeVertexElement, {
      dedup: true
    });
  ...
  const writeSkeleton = (target, skeletonOffset, skeleton) => {
    if ((skeleton.bones || []).some(bone => typeof bone !== "string")) {
      throw writeError("skeleton bones must be name strings; GR2-shaped skeletons need conversion (use writeShared) before writing");
    }
 ...
 */
function writeCmf(graph, options = {}) {
 ...
 */
async function writeCmfAsync(graph, options = {}) {
  await MeshoptEncoder.ready;
  encoderReady = true;
  return writeCmf(graph, {
    ...options,
    compress: options.compress !== false
  });
}

export { writeCmf, writeCmfAsync };
