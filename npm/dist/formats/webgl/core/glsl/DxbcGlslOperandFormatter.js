...
const BITCAST_FROM_FLOAT = {
  int: "floatBitsToInt",
  uint: "floatBitsToUint"
};
const BITCAST_TO_FLOAT = {
  int: "intBitsToFloat",
...
class DxbcGlslOperandFormatter {
  ...
  */
  sourceExpression(operand, options = {}) {
    ...
    if (as !== "float") {
      const integerInputKind = operand.type === 1 && this.integerInputs ? this.integerInputs.get(operand.registerIndex) : undefined;
      if (integerInputKind) {
  ...
  */
  destination(operand) {
    ...
    if (SPECIAL_OPERAND_NAMES[operand.type]) {
      return {
        ref: SPECIAL_OPERAND_NAMES[operand.type],
        mask: "",
        registerMask: operand.mask || "xyzw"
      };
    }
  ...
  */
  ...
  _registerIndex(operand) {
    ...
    if (index?.relative) {
      ...
      throw new WebglReadError("Resource operand uses a relative register index; GLSL ES 3.00 has no dynamic binding indexing", {
        operandType: operand.typeName,
        dimension
      });
    }
  ...
  */
  _indexExpression(operand, dimension) {
    ...
    const relative = index.relative ? this.sourceExpression(index.relative, {
      destMask: "x",
      as: "int"
    }) : null;
...
export { DxbcGlslOperandFormatter };
//# sourceMappingURL=DxbcGlslOperandFormatter.js.map
