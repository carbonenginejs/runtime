...
const OPTION_KEYS = new Set(["emit", "source", "packTangents", "uvHandedness", "rebuildMissingNormals", "rebuildMissingTangents", "rebuildMissingBiNormals", "classes"]);
 ...
 */
function validateClassKey(key, readerName = "CjsObjFormat") {
 ...
 * @param {string} [readerName] Format name used in thrown errors.
 */
function validateClass(key, Class, readerName = "CjsObjFormat") {
 ...
 */
function normalizeValues(base, options = {}, readerName = "CjsObjFormat") {
...
function rebuildMissingMeshData(format, json, values) {
  for (let meshIndex = 0; meshIndex < json.meshes.length; meshIndex++) {
    ...
    if (!hasVertexChannel(mesh, "normal") && shouldApplyMeshRule(format, values.rebuildMissingNormals, {
      ...
      mesh.vertex.normal = generateNormals(requireVertexChannel(mesh, meshIndex, "position", "packTangents"), triangleFaces(mesh, meshIndex, "packTangents"));
    }
    if (!hasVertexChannel(mesh, "tangent") && shouldApplyMeshRule(format, values.rebuildMissingTangents, {
      ...
      mesh.vertex.tangent = generateTangents(requireVertexChannel(mesh, meshIndex, "position", "packTangents"), requireVertexChannel(mesh, meshIndex, "normal", "packTangents"), requireVertexChannel(mesh, meshIndex, "texcoord0", "packTangents"), triangleFaces(mesh, meshIndex, "packTangents"), {
        uvHandedness: values.uvHandedness
      });
    }
    if (!hasVertexChannel(mesh, "binormal") && shouldApplyMeshRule(format, values.rebuildMissingBiNormals, {
      ...
      mesh.vertex.binormal = generateBiNormals(requireVertexChannel(mesh, meshIndex, "normal", "rebuildMissingBiNormals"), requireVertexChannel(mesh, meshIndex, "tangent", "rebuildMissingBiNormals"), {
        uvHandedness: values.uvHandedness
      });
    } else if (!hasVertexChannel(mesh, "binormal") && shouldPackTangents) {
      mesh.vertex.binormal = generateBiNormals(requireVertexChannel(mesh, meshIndex, "normal", "packTangents"), requireVertexChannel(mesh, meshIndex, "tangent", "packTangents"), {
        uvHandedness: values.uvHandedness
      });
    }
    if (shouldPackTangents) {
      mesh.vertex.tangent = packTangentFrames(requireVertexChannel(mesh, meshIndex, "normal", "packTangents"), requireVertexChannel(mesh, meshIndex, "tangent", "packTangents"), requireVertexChannel(mesh, meshIndex, "binormal", "packTangents"));
      mesh.vertex.normal = [];
 ...
 */
function readWithValues(format, input, values, readerName = "CjsObjFormat") {
  const text = toText(input);
  const json = parseObjText(text, {
    source: values.source
  });
...
function inspectWithValues(input, values) {
  const json = parseObjText(toText(input), {
    source: values.source
  });
  return {
    ...
    triangleCount: json.meshes.reduce((total, mesh) => total + mesh.indices.reduce((meshTotal, group) => meshTotal + group.faces.length / 3, 0), 0),
...
export { CLASS_KEYS, CMF_CLASS_KEYS, DEFAULT_VALUES, GR2_CLASS_KEYS, OUTPUT_CMF, OUTPUT_GR2, OUTPUT_JSON, OUTPUT_OBJ_JSON, OUTPUT_SHARED, importNodeModule, inspectWithValues, normalizeValues, readWithValues, toJsonValue, toText, validateClass, validateClassKey };
