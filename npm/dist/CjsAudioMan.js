...
class CjsAudioMan {
  ...
  #LoadWholeBank(selection, signal) {
    ...
    if (!entry) {
      entry = new CjsAudioManSharedAcquisition({
        start: operationSignal => Promise.resolve().then(() => this.#mediaProvider.Read(selection.bank, {
    ...
    if (this.#sfxEngine?.HandlesEvent(eventName)) {
      ...
      return {
        voices: selections.flatMap((selection, index) => buffers[index] ? [{
          ...
          ...(selection.sourceEffects?.some(effect => effect.rtpcCurves?.length || effect.driveRtpcCurve) ? {
            getSourceEffectRtpc: (curve, at = undefined, readControl = false) => engine.EvaluateSourceEffectRTPC(curve, controls, at, readControl)
          } : {}),
