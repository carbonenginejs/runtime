...
test("completed low-hanging classes and turret enums are absent from generated", () => {
  ...
  {
    assert.equal(
      existsSync(new URL(`../src/generated/${family}/${className}.js`, import.meta.url)),
      false,
      className
    );
  }
  assert.equal(
    existsSync(new URL("../src/generated/eve/attachment/turrets/enums.js", import.meta.url)),
    false
  );
...
test("EveSpaceObjectDecal lives in the maintained attachment subtree", () => {
  assert.equal(
    existsSync(new URL("../src/eve/attachment/decal/EveSpaceObjectDecal.js", import.meta.url)),
    true
  );
  assert.equal(
    existsSync(new URL("../src/eve/EveSpaceObjectDecal.js", import.meta.url)),
    false
  );
});
