...
class CjsJukebox {
  ...
  SetTrackLoader(loadTrack) {
    if (typeof loadTrack !== "function") {
      throw new TypeError("CjsJukebox loadTrack must be a function");
    }
  ...
  SetTrackAvailabilityChecker(isTrackAvailable) {
    if (typeof isTrackAvailable !== "function") {
      throw new TypeError("CjsJukebox isTrackAvailable must be a function");
    }
   ...
   */
  async RefreshAvailability(playlistID = null, {
    signal = null
  } = {}) {
    const library = this.#RequireLibrary();
    const playlists = playlistID === null ? library.playlists : [this.#FindPlaylist(playlistID)];
    if (!this.#isTrackAvailable) {
      return playlists.flatMap(playlist => this.GetPlaylistSongs(playlist.id, {
    ...
    try {
      await Promise.all(playlists.flatMap(playlist => playlist.songs.map(async song => {
        let available = false;
        try {
          available = Boolean(await this.#isTrackAvailable(song, {
        ...
        if (requestID === this.#availabilityRequestID) {
          this.#availability.set(AvailabilityKey(playlist.id, song.id), available);
        }
      })));
  ...
  GetPlaylistSongs(playlistID, {
    ...
    return playlist.songs.map(song => ({
      ...song,
      availability: this.GetTrackAvailability(song.id, {
        playlistID: playlist.id
      })
    })).filter(song => includeUnavailable || song.availability !== "unavailable");
  }

  /** Returns available, unavailable, or unknown for one catalog song. */
  GetTrackAvailability(songID, {
    playlistID = null
  } = {}) {
    const id = String(songID);
    const playlists = playlistID === null ? this.#RequireLibrary().playlists : [this.#FindPlaylist(playlistID)];
    for (const playlist of playlists) {
      if (!playlist.songs.some(song => song.id === id)) {
        continue;
      }
      const value = this.#availability.get(AvailabilityKey(playlist.id, id));
      return value === undefined ? "unknown" : value ? "available" : "unavailable";
    }
  ...
  SetOnChange(onChange) {
    if (onChange !== null && typeof onChange !== "function") {
      throw new TypeError("CjsJukebox onChange must be a function or null");
    }
  ...
  Attach(context, destination = context?.destination) {
    if (!context || typeof context.createGain !== "function" || typeof context.createBufferSource !== "function" || typeof context.decodeAudioData !== "function") {
      throw new TypeError("CjsJukebox requires a Web Audio compatible context");
    }
    if (!destination) {
      throw new TypeError("CjsJukebox requires an output destination");
    }
    if (this.#context === context && this.#destination === destination && this.#outputGain) {
      return this;
    }
    this.Stop();
  ...
  PlaySong(songID, {
    ...
    return Promise.reject(new RangeError(`Unknown music-library song ${id}`));
  }
  ...
  Play() {
    ...
    const index = this.#FindAvailableIndex(playlist, -1, 1, false);
    return index === -1 ? Promise.reject(new Error(`Music-library playlist ${playlist.id} has no available songs`)) : this.#SelectAndPlay(playlist, index);
  }
  ...
  Pause() {
    ...
    const elapsed = Math.max(0, Number(this.#context?.currentTime) - this.#startedAt);
    this.#offset = Math.min(Math.max(0, Number(this.#buffer?.duration) || 0), this.#offset + elapsed);
    this.#StopSource();
    this.#state = "paused";
    this.#Notify();
    return true;
  ...
  Previous() {
    if (this.#state === "playing" && Number(this.#context?.currentTime) - this.#startedAt > 3) {
      this.#StartBuffer(0);
      return Promise.resolve(this.currentSong);
    }
    return this.#Move(-1, true);
  ...
  SetVolume(value) {
    ...
    SetAudioParam(this.#outputGain?.gain, this.#volume, this.#context);
    this.#Notify();
    return this.#volume;
  }
  ...
  SetRepeat(repeat) {
    ...
    if (!REPEAT_MODES.has(value)) {
      throw new TypeError(`Unsupported CjsJukebox repeat mode ${value}`);
    }
    this.#repeat = value;
    this.#Notify();
    return value;
  ...
  GetStatus() {
    return {
      ...
      availability: this.currentSong ? this.GetTrackAvailability(this.currentSong.id, {
  ...
  #SelectAndPlay(playlist, index) {
    ...
    const available = this.#availability.get(AvailabilityKey(playlist.id, song.id));
    if (available === false) {
      return Promise.reject(new Error(`Music-library song ${song.id} is unavailable`));
    }
    this.#CancelPending();
    this.#StopSource();
  ...
  async #LoadAndPlay() {
    ...
    if (!this.#context || !this.#outputGain) {
      throw new Error("CjsJukebox must be attached before playback");
    }
    if (!this.#loadTrack) {
  ...
  #StartBuffer(offset) {
    if (!this.#context || !this.#outputGain || !this.#buffer) {
      throw new Error("CjsJukebox has no attached decoded song to play");
    }
    this.#StopSource();
  ...
  #Move(step, explicit) {
    ...
    const index = this.#FindAvailableIndex(this.#playlist, this.#songIndex, step, this.#repeat === "playlist" || explicit);
    if (index === -1) {
      this.Stop();
  ...
  #FindAvailableIndex(playlist, start, step, wrap) {
    ...
    for (let count = 0; count < length; count++) {
      ...
      const song = playlist.songs[index];
      if (this.#availability.get(AvailabilityKey(playlist.id, song.id)) !== false) {
        return index;
      }
    }
    return -1;
  ...
  #FindPlaylist(playlistID) {
    ...
    const playlist = this.#RequireLibrary().playlists.find(candidate => candidate.id === id);
    if (!playlist) {
...
async function DecodeTrack(context, loaded) {
  if (loaded && Number.isFinite(loaded.duration) && typeof loaded !== "string") {
    return loaded;
  }
  ...
  if (value instanceof ArrayBuffer) {
    ...
    throw new TypeError("CjsJukebox track loader must return audio bytes or an AudioBuffer");
  }
...
function NormalizeIndex(value, length) {
  ...
  if (!Number.isSafeInteger(index) || index < 0 || index >= length) {
    throw new RangeError(`Music-library song index ${value} is outside 0..${length - 1}`);
  }
  return index;
}
...
export { CjsJukebox };
//# sourceMappingURL=CjsJukebox.js.map
