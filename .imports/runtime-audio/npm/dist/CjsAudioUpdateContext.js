 ...
 * @returns {Object}
 */
function createAudioUpdateContext({
  ...
  return {
    ...
    Update(source = null) {
      if (source !== null && source !== undefined) {
        ...
        const time = ReadContextNumber(source, "GetTime", ["time", "currentTime"]);
        const realTime = ReadContextNumber(source, "GetRealTime", ["realTime"]);
        const deltaTime = ReadContextNumber(source, "GetDeltaT", ["deltaTime", "deltaT"]);
        const frame = ReadContextNumber(source, "GetFrame", ["frame", "frameCount", "frames"]);
        this.time = time ?? previousTime;
        this.realTime = realTime ?? time ?? previousRealTime;
        this.deltaTime = deltaTime ?? (initialized ? this.time - previousTime : 0);
      ...
      if (!Number.isFinite(current)) {
        throw new TypeError("audio update-context now must return finite seconds");
      }
      this.deltaTime = initialized ? Math.max(0, current - this.realTime) : 0;
      this.time = current;
...
export { createAudioUpdateContext };
//# sourceMappingURL=CjsAudioUpdateContext.js.map
