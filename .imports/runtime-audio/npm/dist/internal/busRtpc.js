...
function indexBusRtpcCatalog(catalog) {
...
function busRtpcCatalogUsesControl(catalog, name) {
...
function busRtpcPathUses(catalog, busPathIds, property) {
  if (!(catalog instanceof Map)) return false;
  ...
  for (const rawBusId of busPathIds ?? []) {
    ...
    seen.add(busId);
    if ((catalog.get(busId) ?? []).some(curve => CurveProperty(curve) === selected)) {
      return true;
    }
  }
  return false;
...
function evaluateBusRtpcGainDb(catalog, busPathIds, readGlobalRtpc, at = undefined) {
  return EvaluateBusRtpcPropertyGainDb(catalog, busPathIds, readGlobalRtpc, BUS_VOLUME_PROPERTY, at);
}

/** Evaluates additive Voice Volume RTPC gain across one bus ancestry. */
function evaluateBusVoiceRtpcGainDb(catalog, busPathIds, readGlobalRtpc, at = undefined) {
  return EvaluateBusRtpcPropertyGainDb(catalog, busPathIds, readGlobalRtpc, VOICE_VOLUME_PROPERTY, at);
}
function EvaluateBusRtpcPropertyGainDb(catalog, busPathIds, readGlobalRtpc, property, at) {
...
function CurveProperty(curve) {
  ...
  return curve?.property === undefined ? BUS_VOLUME_PROPERTY : String(curve.property);
}

export { busRtpcCatalogUsesControl, busRtpcPathUses, evaluateBusRtpcGainDb, evaluateBusVoiceRtpcGainDb, indexBusRtpcCatalog };
//# sourceMappingURL=busRtpc.js.map
