...
class CjsBusGraphRuntime {
  ...
  constructor(catalog) {
    ...
    this.#sfxRoutes = IndexRouteReferences(value.sfxRoutes, this.#routes, "Audio Bus graph runtime SFX routes");
    this.#musicRoutes = IndexRouteReferences(value.musicRoutes, this.#routes, "Audio Bus graph runtime music routes");
  }
  ...
  #Resolve(index, rawId, projection, kind) {
    ...
    if (handle && projection !== undefined) {
      assertBusGraphRouteProjection(handle.route, projection, `Audio Bus graph runtime ${kind} ${id}`);
    }
    return handle;
  }
}
...
export { CjsBusGraphRuntime };
//# sourceMappingURL=busGraphRuntime.js.map
