...
class CjsAudioManSharedAcquisition {
  ...
  constructor({
    ...
    this.#promise = Promise.resolve(operation).then(value => {
      const retained = typeof retain === "function" ? retain() : retain;
      if (!retained) {
        this.Evict();
      }
      return value;
  ...
  Subscribe(signal) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.#promise.then(value => finish(resolve, value), error => finish(reject, error));
    });
  }
...
function AbortReason(signal) {
  return signal?.reason instanceof Error ? signal.reason : new DOMException("Audio media load aborted", "AbortError");
}

export { CjsAudioManSharedAcquisition };
//# sourceMappingURL=CjsAudioManSharedAcquisition.js.map
