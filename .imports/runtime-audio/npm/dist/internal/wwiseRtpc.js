...
function evaluateWwiseRtpcCurve(points, value, field = "value") {
  ...
  for (let index = 1; index < points.length; index++) {
    ...
    if (input < right.x) {
      ...
      const ratio = span > 0 ? evaluateWwiseInterpolation(left.interpolation ?? 4, (input - left.x) / span) : 1;
...
function wwiseDbRtpcValueToDb(value) {
  const raw = Math.min(1, Math.max(-1, Number(value) || 0));
  return raw === -1 ? -96.3 : 20 * Math.log10(raw + 1);
}

export { evaluateWwiseRtpcCurve, wwiseDbRtpcValueToDb };
//# sourceMappingURL=wwiseRtpc.js.map
