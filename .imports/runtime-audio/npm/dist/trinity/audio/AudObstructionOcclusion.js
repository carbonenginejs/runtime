...
// Ported from CarbonEngine (MIT, (c) 2026 CCP Games) - https://github.com/carbonengine/audio
//   audio/src/AudObstructionOcclusion.h + AudObstructionOcclusion.cpp
...
class AudObstructionOcclusion {
  ...
  Update(backend) {
    if (!this.#audioManager || this.#audioManager.GetState?.() !== "enabled") {
      return;
    }
    const now = Number(this.#now());
    const deltaSeconds = this.#hasUpdated && Number.isFinite(now) ? Math.max(0, now - this.#lastUpdateTime) : 0;
    this.#lastUpdateTime = Number.isFinite(now) ? now : NowSeconds();
    this.#hasUpdated = true;
    for (const [emitterID, state] of this.#emitters) {
      let culled = false;
      const exists = this.#audioManager.WithCallbackGameObject?.(emitterID, emitter => {
      ...
      const obstructionChanged = state.obstruction.Advance(deltaSeconds, this.#fadeRate);
      const occlusionChanged = state.occlusion.Advance(deltaSeconds, this.#fadeRate);
      if (culled) {
        state.needsSend = true;
        continue;
      }
      if (obstructionChanged || occlusionChanged || state.needsSend) {
        state.needsSend = !this.#SendToBackend(backend, emitterID, state);
      }
    }
  }
  ...
  SetObstructionOcclusion(emitterID, obstruction, occlusion) {
    if (!this.#enabled || !this.#audioManager || this.#audioManager.GetState?.() !== "enabled" || emitterID === LISTENER_GAME_OBJ_ID || this.#audioManager.WithCallbackGameObject?.(emitterID, () => {}) !== true) {
      return false;
    }
  ...
  SetEmitterLineOfSightBlockage(emitterID, blockage) {
    const acousticsEnabled = this.#audioManager?.GetSpatialAudioGeometryEnabled?.() === true;
    return this.SetObstructionOcclusion(emitterID, 0, acousticsEnabled ? 0 : blockage);
  }
  ...
  #SendToBackend(backend, emitterID, state) {
    ...
    return backend.SetObjectObstructionAndOcclusion(emitterID, LISTENER_GAME_OBJ_ID, state.obstruction.currentValue, state.occlusion.currentValue) !== false;
  }
}

export { AudObstructionOcclusion };
//# sourceMappingURL=AudObstructionOcclusion.js.map
