...
import { decompressGzipIfNeeded } from '@carbonenginejs/runtime-utils/compression';
import { createAudioResourceReader, normalizeAudioResourceBytes } from '../library-builder/resourceSource.js';
...
class CjsAudioLibrary {
  ...
  static from(values) {
    return new this(values instanceof CjsAudioLibrary ? values.GetValues() : values);
  }

  /** Loads plain or gzip-compressed JSON bytes, using fetch for a path by default. */
  static async load(source, options = {}) {
    const bytes = typeof source === "string" ? await createAudioResourceReader(options)(source, {
      kind: "audioLibrary",
      signal: options.signal ?? null
    }) : await normalizeAudioResourceBytes(source, "audio library");
    const jsonBytes = await decompressGzipIfNeeded(bytes, options.compression ?? {});
    let values;
    try {
...
export { CjsAudioLibrary };
//# sourceMappingURL=CjsAudioLibrary.js.map
