...
const SUPPORTED_OPCODES = new Set(["add", "and", "div", "dp2", "dp3", "dp4", "eq", "exp", "f16tof32", "f32tof16", "frc", "ftoi", "ftou", "ge", "iadd", "ieq", "ige", "ilt", "imad", "imax", "imin", "imul", "ine", "ishl", "ishr", "itof", "ld", "ld_structured", "log", "lt", "mad", "max", "min", "mov", "movc", "mul", "ne", "or", "rcp", "round_ni", "round_pi", "round_z", "rsq", "sample_d", "sample_l", "sincos", "sqrt", "udiv", "uge", "ult", "umax", "umin", "ushr", "utof", "xor", "ret"]);
...
function liveInputRegisters(program) {
  ...
  const reachableInstructions = new Set(program.blocks.filter(block => reachableBlocks.has(block.id)).flatMap(block => block.instructionIndices));
...
function sourceComponents(operand, destinationMask, count, activeComponents = null) {
  ...
  const positions = activeComponents || (count > destinationMask.length ? COMPONENTS.slice(0, count) : Array.from(destinationMask).slice(0, count));
...
function bindingForOperand(bindings, resourceKind, operand) {
  ...
  if (Number.isInteger(rangeId)) {
    ...
    return resourceKind === "uniform-buffer" ? binding : validateFixedHandleBinding(operand, binding, "vertex");
  }
...
function cbufferVectorIndex(program, instruction, operandIndex, operand, inputs) {
  ...
  if (storage === "uint32" || storage === "bitpattern32") indexCode = `i32(${indexCode})`;else if (storage !== "int32") throw new Error(`WGSL vertex dynamic cbuffer index has unsupported type ${storage}`);
...
function cbufferParts(program, instruction, operandIndex, operand, destinationMask, count, bindings, expectedScalarType, inputs, activeComponents = null) {
  ...
  const parts = sourceComponents(operand, destinationMask, count, activeComponents).map(component => `${binding.generatedSymbol}[${vectorIndex}].${component}`);
...
function icbParts(program, instruction, operandIndex, operand, destinationMask, count, expectedScalarType, inputs, activeComponents = null) {
  ...
  const vectorIndex = cbufferVectorIndex(program, instruction, operandIndex, operand, inputs);
  const parts = sourceComponents(operand, destinationMask, count, activeComponents).map(component => `icb[${vectorIndex}].${component}`);
  if (expectedScalarType === "float32") return parts;
  if (!["int32", "uint32", "bitpattern32"].includes(expectedScalarType)) {
...
function constTableParts(program, instruction, operandIndex, operand, destinationMask, count, expectedScalarType, inputs, activeComponents = null) {
  ...
  const vectorIndex = cbufferVectorIndex(program, instruction, operandIndex, operand, inputs);
  const parts = sourceComponents(operand, destinationMask, count, activeComponents).map(component => `${table.symbol}[${vectorIndex}].${component}`);
  if (expectedScalarType === "float32") return parts;
  if (!["int32", "uint32", "bitpattern32"].includes(expectedScalarType)) {
...
function operandLaneExpression(program, instruction, operandIndex, destinationMask, laneIndex, targetType, inputs, bindings, moverData = false) {
  ...
  if (read && operand.typeName === "indexable_temp") {
    validateIndexableTempRead(program, operand, read, sourceComponents(operand, destinationMask, read.refs.length, activeComponents));
  }
  if (read) {
...
function operandExpression(program, instruction, operandIndex, destinationMask, count, inputs, bindings) {
  ...
  if (read) {
    ...
    const refs = read.refs.length === 1 && operand.selected ? Array.from({
      length: count
    }, () => read.refs[0]) : read.refs.slice(0, count);
    ...
    parts = refs.map((ref, componentIndex) => {
      ...
      const bitcast = instruction.typeInfo.bitcasts.find(entry => entry.kind === "read-bitcast" && entry.operandIndex === operandIndex && entry.componentIndex === sourceComponentIndex && entry.valueId === ref.valueId && entry.component === ref.component);
      if (bitcast) {
...
function structuredLoadExpression(program, instruction, write, type, inputs, bindings) {
  ...
  if (byteOffsetOperand?.typeName !== "immediate32" || byteOffsetOperand.immediateValues?.length !== 1 || (byteOffsetOperand.modifierName || "none") !== "none" || !Number.isInteger(byteOffset)) {
...
function expressionFor(program, instruction, write, type, inputs, bindings) {
  ...
  const source = (index, forcedCount = count) => operandExpression(program, instruction, index, mask, forcedCount, inputs, bindings);
  ...
  if (op === "sample_l" || op === "sample_d") {
    ...
    const sampled = op === "sample_l" ? `textureSampleLevel(${tex}, ${coord}${arrayArg}, ${source(4, 1)}${offsetArg})` : `textureSampleGrad(${tex}, ${coord}${arrayArg}, ${source(4, gradientComponents)}, ${source(5, gradientComponents)}${offsetArg})`;
    const components = sourceComponents(resource, write.mask, count);
...
function applyResultBitcast(program, instruction, write, expression, type) {
  ...
  const records = instruction.typeInfo.bitcasts.filter(entry => entry.kind === "result-bitcast" && entry.operandIndex === write.operandIndex && entry.valueId === write.valueId);
  if (!records.length) return expression;
  const intrinsicType = instruction.typeInfo.resultType;
  if (records.length !== components.length || records.some(entry => entry.from !== intrinsicType || entry.to !== type.scalarType) || components.some(component => !records.some(entry => entry.component === component))) {
...
function lowerInstruction(program, instruction, inputs, outputs, bindings, written, readValueIds) {
  ...
  function lowerWrite(write) {
    ...
    if (mixedTypes) {
      ...
      if (localSsaDestination && scalarTypeName(intrinsic) && instruction.opcodeName !== "mov") {
        let packedCode = expressionFor(program, instruction, write, {
          ...
          wgslType: fieldType(intrinsic, write.mask.length)
        }, inputs, bindings);
    ...
    if (destination.typeName === "output") {
      ...
      const assignmentExpression = reinterpretCode(expression, type.scalarType, field.scalarType, write.mask.length, `output r${destination.registerIndex}`);
      ...
      return [{
        ...
        ...assignment,
        expression: {
          code: reinterpretCode(write.valueId, type.scalarType, field.scalarType, write.mask.length, `output r${destination.registerIndex}`),
          type: outputType
        }
      }];
    }
 ...
 */
function lowerVertexProgram(program, options = {}) {
  ...
  const reachableInstructions = new Set(program.blocks.filter(block => block.reachable !== false).flatMap(block => block.instructionIndices));
  const readValueIds = new Set([...program.instructions.flatMap(instruction => reachableInstructions.has(instruction.index) ? instruction.dataflow.reads.flatMap(read => read.refs.map(ref => ref.valueId)) : []), ...program.values.flatMap(value => (value.incoming || []).map(incoming => incoming.valueId))]);
  ...
  function lowerRange(start, end, rangeWritten, inLoop = false, loopExit = null) {
    ...
    for (let index = start; index < end; index += 1) {
      ...
      if (!plan) {
        ...
        const lowered = lowerInstruction(program, program.instructions[index], inputs, outputs, bindings, rangeWritten, readValueIds);
...
export { lowerVertexProgram };
//# sourceMappingURL=lowerVertexProgram.js.map
