...
function encodeDescriptor(descriptor, parts, seen, transform = null) {
  ...
  parts.push("data:");
  if (transform) transform(descriptor.value, parts, seen);else encodeValue(descriptor.value, parts, seen);
}
function sortedKeys(value, omitted = null) {
  return Reflect.ownKeys(value).filter(key => !omitted?.has(key)).map(key => ({
    key,
    record: propertyKeyRecord(key)
  })).sort((left, right) => left.record < right.record ? -1 : left.record > right.record ? 1 : 0);
}
function encodeObject(value, parts, seen, omitted = null, transformProperty = null) {
  if (seen.has(value)) {
  ...
  for (const {
    ...
    const transform = transformProperty ? transformProperty(key) : null;
...
function encodeValue(value, parts, seen) {
  if (value === undefined) parts.push("undefined;");else if (value === null) parts.push("null;");else if (typeof value === "boolean") {
    parts.push(value ? "boolean:true;" : "boolean:false;");
  } else if (typeof value === "number") {
    if (Number.isNaN(value)) parts.push("number:NaN;");else if (Object.is(value, -0)) parts.push("number:-0;");else if (value === Number.POSITIVE_INFINITY) {
      parts.push("number:+Infinity;");
    } else if (value === Number.NEGATIVE_INFINITY) {
      parts.push("number:-Infinity;");
    } else parts.push("number:", `${value}`, ";");
  } else if (typeof value === "string") encodeString(value, parts);else if (typeof value === "bigint") {
    parts.push("bigint:", value.toString(), ";");
  } else if (typeof value === "symbol") {
    parts.push("symbol:", propertyKeyRecord(value), ";");
  } else if (typeof value === "function") parts.push("function;");else encodeObject(value, parts, seen);
}
function encodeInstruction(instruction, parts, seen) {
  ...
  encodeObject(instruction, parts, seen, new Set(["dataflow", "typeInfo"]));
}
function instructionTransform(key) {
  return typeof key === "string" && /^\d+$/u.test(key) ? encodeInstruction : null;
}
 ...
 */
function particleEmitSemanticDigest(program) {
  const parts = ["particle-emit-semantic-profile:1{"];
  const seen = new WeakSet();
  if (!program || typeof program !== "object") {
    encodeValue(program, parts, seen);
  } else {
    encodeObject(program, parts, seen, new Set(["source", "blocks", "controlFlow", "values"]), key => key === "instructions" ? encodeInstructions : null);
  }
  parts.push("}");
  return sha256Utf8(parts.join(""));
}

export { particleEmitSemanticDigest, sha256Utf8 };
//# sourceMappingURL=particleEmitSemanticDigest.js.map
