 ...
 */
function requireRefactoringAllowed(program, stage) {
  ...
  if (!Number.isInteger(data?.globalFlags) || data.globalFlags < 0 || typeof data.refactoringAllowed !== "boolean" || data.refactoringAllowed !== ((data.globalFlags & REFACTORING_ALLOWED) !== 0)) {
 ...
 * @param {string} stage Diagnostic stage label.
 */
function validatePreciseInstruction(instruction, stage) {
  ...
  if (!writes.length || Array.from(mask).some(component => !writes.some(write => write.mask.includes(component)))) {
    fail(stage, instruction, `mask ${mask} requires a destination write containing every precise lane`);
  }
}

export { requireRefactoringAllowed, validatePreciseInstruction };
//# sourceMappingURL=precisionControls.js.map
