...
const DECLARATION_OPCODES = Object.freeze(["dcl_global_flags", "dcl_constant_buffer", "dcl_resource", "dcl_unordered_access_view_structured", "dcl_input", "dcl_input", "dcl_temps", "dcl_thread_group"]);
const BODY_OPCODES = Object.freeze(["ld", "imad", "iadd", "and", "iadd", "ishl", "iadd", "imad", "ult", "if", "iadd", "ld_structured", "ld_structured", "lt", "if", "store_structured", "store_structured", "endif", "endif", "ret"]);
const BODY_OPERANDS_BASE_SM50 = Object.freeze(["temp:0:x:0::none:default:uniform | immediate32::::#3,#3,#3,#3:none:default:uniform | resource:0:xyzw:0::none:default:uniform", "temp:0:y:0::none:default:uniform | input_thread_group_id::x:::none:default:uniform | immediate32::::#256:none:default:uniform | input_thread_id_in_group::x:::none:default:uniform", "temp:0:z:0::none:default:uniform | constant_buffer:3:x:3,0::none:default:uniform | immediate32::::#4294967295:none:default:uniform", "temp:0:z:0::none:default:uniform | temp:0:z:0::none:default:uniform | temp:0:y:0::none:default:uniform", "temp:0:y:0::none:default:uniform | temp:0:z:0::neg:default:uniform | temp:0:y:0::none:default:uniform", "temp:0:y:0::none:default:uniform | temp:0:y:0::none:default:uniform | immediate32::::#1:none:default:uniform", "temp:0:w:0::none:default:uniform | temp:0:y:0::none:default:uniform | constant_buffer:3:y:3,0::none:default:uniform", "temp:0:w:0::none:default:uniform | constant_buffer:3:z:3,0::none:default:uniform | temp:0:z:0::none:default:uniform | temp:0:w:0::none:default:uniform", "temp:0:x:0::none:default:uniform | temp:0:w:0::none:default:uniform | temp:0:x:0::none:default:uniform", "temp:0:x:0::none:default:uniform", "temp:0:x:0::none:default:uniform | temp:0:z:0::none:default:uniform | temp:0:y:0::none:default:uniform", "temp:0:yz:0::none:default:uniform | temp:0:x:0::none:default:uniform | immediate32::::#0:none:default:uniform | uav:0:xxyx:0::none:default:uniform", "temp:1:xy:1::none:default:uniform | temp:0:w:0::none:default:uniform | immediate32::::#0:none:default:uniform | uav:0:xyxx:0::none:default:uniform", "temp:1:z:1::none:default:uniform | temp:1:y:1::none:default:uniform | temp:0:z:0::none:default:uniform", "temp:1:z:1::none:default:uniform", "uav:0:xy:0::none:default:uniform | temp:0:x:0::none:default:uniform | immediate32::::#0:none:default:uniform | temp:1:xyxx:1::none:default:uniform", "uav:0:xy:0::none:default:uniform | temp:0:w:0::none:default:uniform | immediate32::::#0:none:default:uniform | temp:0:yzyy:0::none:default:uniform", "", "", ""]);
const BODY_COMPONENT_COUNTS = Object.freeze([[4, 4, 4], [4, 4, 1, 4], [4, 4, 1], [4, 4, 4], [4, 4, 4], [4, 4, 1], [4, 4, 4], [4, 4, 4, 4], [4, 4, 4], [4], [4, 4, 4], [4, 4, 1, 4], [4, 4, 1, 4], [4, 4, 4], [4], [4, 4, 1, 4], [4, 4, 1, 4], [], [], []]);
const BODY_OPERANDS_SM50 = Object.freeze(BODY_OPERANDS_BASE_SM50.map((signature, instructionIndex) => signature ? signature.split(" | ").map((operand, operandIndex) => `${operand}:${BODY_COMPONENT_COUNTS[instructionIndex][operandIndex]}`).join(" | ") : ""));
const BODY_OPERANDS_SM51 = Object.freeze(BODY_OPERANDS_SM50.map(signature => signature.replaceAll("constant_buffer:3:", "constant_buffer:0:").replaceAll(":3,0::none:default:uniform", ":0,3,0::none:default:uniform").replaceAll("resource:0:xyzw:0::", "resource:0:xyzw:0,0::").replaceAll("uav:0:xxyx:0::", "uav:0:xxyx:0,0::").replaceAll("uav:0:xyxx:0::", "uav:0:xyxx:0,0::").replaceAll("uav:0:xy:0::", "uav:0:xy:0,0::")));
...
function validateCommonOperand(operand, context, allowedModifier = "none") {
  if (!operand || (operand.modifierName ?? "none") !== allowedModifier || (operand.minPrecisionName ?? "default") !== "default" || operand.nonUniform === true) {
...
function validateSm51Range(data, registerIndex, context) {
  ...
  if (data?.bindingModel !== "sm5.1-range" || range?.bindingModel !== "sm5.1-range" || range.rangeId !== 0 || range.lowerBound !== registerIndex || range.upperBound !== registerIndex || range.unbounded !== false || range.registerCount !== 1 || range.registerSpace !== 0) {
...
function validateSm51Reference(operand, kind, context) {
  ...
  if (reference?.bindingModel !== "sm5.1-range" || reference.rangeId !== 0 || reference.nonUniform !== false) {
    throw new Error(`WGSL sort-step ${context} has an invalid SM5.1 resource reference`);
  }
  if (kind === "constant-buffer") {
    if (reference.absoluteIndex !== null || !isCanonicalIndex(reference.bufferIndex, 1, 3) || !isCanonicalIndex(reference.vectorOffset, 2, 0)) {
      throw new Error(`WGSL sort-step ${context} has an invalid SM5.1 cb3 reference`);
    }
  } else if (!isCanonicalIndex(reference.absoluteIndex, 1, 0) || reference.bufferIndex !== null || reference.vectorOffset !== null) {
...
function validateDeclarationOperand(operand, {
  ...
  if (operand.typeName !== typeName || operand.registerIndex !== registerIndex || operand.componentCount !== componentCount || (operand.mask || operand.swizzle || operand.selected || "") !== selector || operand.indices?.length !== indices.length || indices.some((value, index) => operand.indices[index].values[0] !== value) || operand.immediateValues?.length) {
...
function validateDeclarations(program) {
  ...
  if (cb.data?.registerIndex !== 3 || cb.data.sizeInVec4 !== 1 || cb.data.accessPattern !== "immediate_indexed" || cb.operands?.length !== 1) {
  ...
  for (const [declaration, typeName, operandType] of [[groupId, "input_thread_group_id", 33], [localId, "input_thread_id_in_group", 34]]) {
    const operand = declaration.operands?.[0];
    validateCommonOperand(operand, `${typeName} declaration`);
    if (declaration.data?.registerIndex !== null || declaration.data.operandType !== operandType || declaration.data.operandTypeName !== typeName || declaration.operands?.length !== 1 || operand.typeName !== typeName || operand.registerIndex !== null || operand.componentCount !== 4 || operand.mask !== "x" || operand.swizzle || operand.selected || operand.indices?.length || operand.immediateValues?.length) {
      throw new Error(`WGSL sort-step compute requires exactly ${typeName}.x`);
    }
  }
  if (temps.data?.tempCount !== 2 || group.data?.threadGroupX !== 256 || group.data?.threadGroupY !== 1 || group.data?.threadGroupZ !== 1 || temps.operands?.length || group.operands?.length) {
  ...
  } else if (cb.data.bindingModel || cb.data.bindingRange || input.data.bindingModel || input.data.bindingRange || output.data.bindingModel || output.data.bindingRange) {
...
function validateBody(program) {
  ...
  for (const [index, instruction] of program.instructions.entries()) {
    ...
    if (instruction.index !== index || index > 0 && instruction.dxbcOffset <= program.instructions[index - 1].dxbcOffset || instruction.controlKind !== expectedControl || instruction.testBoolean !== (instruction.opcodeName === "if" ? "nonzero" : null) || instruction.saturate || instruction.preciseMask !== "" || !validExtensions(program, instruction)) {
  ...
  if (plans.size !== 2 || plans.get(9)?.kind !== "selection" || plans.get(9).hasElse || plans.get(9).merges.length || plans.get(14)?.kind !== "selection" || plans.get(14).hasElse || plans.get(14).merges.length) {
    throw new Error("WGSL sort-step compute requires the canonical nested no-else selections without live merges");
  }
}
function validateBindings(bindings) {
  const expected = [["uniform-buffer", 3, "cb3", "array<vec4<f32>, 1>", null, "uniform", 16], ["sampled-resource", 0, "t0", "array<u32>", null, "read-only-storage", 4], ["storage-resource", 0, "u0", "array<u32>", 8, "storage", 8]];
...
function lowerBody(program, bindings) {
  ...
  for (const instruction of program.instructions) {
    ...
    const statement = instruction.opcodeName === "ld" ? typedLoad(instruction, typedInput) : instruction.opcodeName === "ld_structured" ? structuredLoad(program, instruction, output) : instruction.opcodeName === "store_structured" ? structuredStore(program, instruction, output) : aluStatement(program, instruction);
    current.push(statement);
  }
  if (stack.length) {
 ...
 */
function isSortStepComputeProfile(program) {
 ...
 */
function lowerSortStepComputeProgram(program, options = {}) {
...
export { isSortStepComputeProfile, lowerSortStepComputeProgram };
//# sourceMappingURL=lowerSortStepComputeProgram.js.map
