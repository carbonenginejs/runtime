 ...
 * reader.
 */
async function readWithValuesAsync(input, values = DEFAULT_VALUES, expectedType = "") {
  const bytes = toBytes(input);
  const metadata = inspectWithValues(bytes, values, expectedType);
...
async function decodePngToRgba(bytes, metadata) {
  ...
  if (chunks.trns && metadata.colorType === 0) {
    metadata.__pngTransparentGray = metadata.bitDepth === 16 ? chunks.trns[0] << 8 | chunks.trns[1] : chunks.trns[1];
  }
...
function pngRgbaSupport(metadata) {
  if (metadata.idatChunkCount < 1 || metadata.idatBytes < 1) {
    return {
      supported: false,
      reason: "PNG has no IDAT image data for RGBA decode."
    };
  }
  if (!metadata.hasIend) {
    return {
      supported: false,
      reason: "PNG stream has no complete IEND chunk."
    };
  }
  const depths = {
    0: [1, 2, 4, 8, 16],
  ...
  if (!Object.hasOwn(depths, metadata.colorType) || !depths[metadata.colorType].includes(metadata.bitDepth)) {
    return {
      supported: false,
      reason: "PNG color type and bit depth are unsupported by the RGBA decoder."
    };
  }
  if (![0, 1].includes(metadata.interlaceMethod)) {
    return {
      supported: false,
      reason: "PNG interlace method is unsupported by the RGBA decoder."
    };
  }
  if (metadata.compressionMethod !== 0 || metadata.filterMethod !== 0) {
    return {
      supported: false,
      reason: "PNG compression/filter methods are unsupported by the RGBA decoder."
    };
  }
  if (typeof DecompressionStream !== "function" || typeof Blob !== "function" || typeof Response !== "function") {
    return {
      supported: false,
      reason: "PNG async RGBA decode requires platform DecompressionStream support."
    };
  }
  return {
    supported: true,
    reason: "Use readAsync/readWithValuesAsync for platform deflate decode."
  };
}
function readPngChunks(bytes) {
  ...
  while (offset + 12 <= bytes.byteLength) {
    ...
    if (type === "IDAT") chunks.idat.push(data);else if (type === "PLTE") chunks.palette = data;else if (type === "tRNS") chunks.trns = data;else if (type === "sRGB") chunks.srgb = true;
    offset = end + 4;
    if (type === "IEND") break;
  }
  return chunks;
...
function unfilterPngRows(inflated, height, rowBytes, bytesPerPixel) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < rowBytes; x++) {
      ...
      let value = raw;
      if (filter === 1) value = raw + left;else if (filter === 2) value = raw + up;else if (filter === 3) value = raw + Math.floor((left + up) / 2);else if (filter === 4) value = raw + paeth(left, up, upLeft);else if (filter !== 0) throw new Error(`png: unsupported filter type ${filter}`);
...
export { DEFAULT_VALUES, OUTPUT_IMAGE, OUTPUT_JSON, OUTPUT_RAW, OUTPUT_RGBA, inspectBytes, inspectWithValues, isDDS, isJPEG, isPNG, isTGA, normalizeEmit, normalizeInputType, normalizeValues, probeSupportWithValues, readWithValues, readWithValuesAsync, toBytes, toJsonValue };
