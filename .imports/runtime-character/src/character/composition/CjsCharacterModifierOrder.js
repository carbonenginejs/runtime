...
export class CjsCharacterModifierOrder
{
    ...
    static resolveCategories(rules = {})
    {
        ...
        EnsureBefore(result, rules.hidesBootShin === true
            ? [ "feet", "feettucked" ]
            : [ "feettucked", "feet" ]);
        EnsureBefore(result, rules.forcesLooseTop === true
            ? [ "bottomoutertucked", "bottomouter" ]
            : [ "bottomouter", "bottomoutertucked" ]);
        EnsureBefore(result, rules.swapTops === true
            ? [ "topmiddle", "toptight" ]
            : [ "toptight", "topmiddle" ]);
        EnsureBefore(result, rules.swapBottom === true
            ? [ "topunderweartucked", "topunderwear" ]
            : [ "topunderwear", "topunderweartucked" ]);
        EnsureBefore(result, rules.swapSocks === true
            ? [ "socks", "sockstucked" ]
            : [ "sockstucked", "socks" ]);
    ...
    } = {})
    {
        ...
        return values
            .map((value, index) => ({
                index,
                key: CjsCharacterModifierOrder.getSortKey(
                    getCategory(value),
                    getGroup(value),
                    categories
                ),
...
export default CjsCharacterModifierOrder;
