...
Status: Evolving
Scope: `@carbonenginejs/tools-browser/diagrams`, `/ship-tree`, planned `/star-map`, and optional demo UI
...
- renderer-neutral chart geometry consumed by an optional SVG view in Market
  Details;
- `@carbonenginejs/tools-browser/diagrams` for mutable models, bounds,
  transforms, selection, and a linear visible-set/picking index; and
- `@carbonenginejs/tools-browser/ship-tree` and `/ship-tree/ui` for an authored
...
tools-browser will only supply a thin adapter over an injected runtime.
